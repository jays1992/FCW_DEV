...
@section('content')    
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                  <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Service Invoice</a>
                </div>
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->SIID}}" value="{{$val->SIID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->docdate) && $val->docdate !='' && $val->docdate !='1900-01-01' ? date('d-m-Y',strtotime($val->docdate)):''}}" ></td>
              <td>{{isset($val->SI_NO) && $val->SI_NO !=''?$val->SI_NO:''}}</td>
              <td>{{isset($val->SI_DATE) && $val->SI_DATE !='' && $val->SI_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->SI_DATE)):''}}</td>
...
<div id="ReportView" class="modal" role="dialog"  data-backdrop="static"  >
  <div class="modal-dialog modal-md" style="width:50%; height:60%">
    ...
    <div class="modal-body">
	  <div class="tablename"><p>Service Invoice Print</p></div>
        <div class="row">
...
$('#btnPrint').on('click', function() {
      ...
      }else if(seletedRecord==1)
      {
            ...
            var Flag = 'H';
            var formData = 'SO='+ SONO + '&SIID='+ SONO + '&Flag='+ Flag ;
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
