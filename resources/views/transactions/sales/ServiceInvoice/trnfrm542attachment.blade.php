...
<div class="container-fluid purchase-order-view filter">     
  <form id="frm_attachment" method="post" enctype="multipart/form-data" action='{{route("transactionmodify",[$FormId,"docuploads"])}}' > 
    @CSRF     
    <div class="inner-form">
      ...
      <div class="row">
        ...
        <input type="hidden" name="VTID_REF" class="form-control" value='{{$objMstVoucherType[0]->VTID}}' />
			...
			<div class="col-lg-2 pl">
         <label> {{$objMst->SI_NO}} </label>
        <input type="hidden" name="ATTACH_DOCNO" id="ATTACH_DOCNO" value="{{$objMst->SIID}}" class="form-control" maxlength="50" >
			</div>
			
			<div class="col-lg-1 pl"><p>Document Date</p></div>
			<div class="col-lg-2 pl">
      <label> {{ date("d/m/Y", strtotime($objMstVoucherType[0]->INDATE) )}} </label> 
        <input type="hidden" name="ATTACH_DOCDT" id="ATTACH_DOCDT" value='{{date("Y-m-d", strtotime($objMstVoucherType[0]->INDATE))}}'   />
                            ...
                            @foreach($objAttachments as $row)
                              ...
                              @php
                                ...
                                if(isset($row->FILESNAME) && !empty($row->FILESNAME)){
                                  ...
                                  $viewArr = array('jpg','jpeg','png','gif','pdf','txt','bmp');
                                  if(in_array(strtolower(end($fileExt)), $viewArr) ){
                                    $viewfile = true;
                                  }else{
                                    $viewfile = false;
                                  }
                                    ...
                                    $docpath    =   "docs/company".Auth::user()->CYID_REF."/".$dirname."/".$row->FILESNAME;     
					...
					<table id="example2" class="display nowrap table table-striped table-bordered itemlist" width="100%" style="height:auto !important;">
						...
						<tbody>
							<tr  class="participantRow">
								...
								<td><input style="width: 100%"  type="text" name="REMARKS[]" id="REMARKS_0" 	class="form-control w-100" autocomplete="off"  maxlength="200" ></td>
								<td align="center" ><a class="btn add" title="add" data-toggle="tooltip"><i class="fa fa-plus"></i></a><a class="btn remove" title="Delete" data-toggle="tooltip" type="button" ><i class="fa fa-trash" ></i></a></td>
							</tr>
						</tbody>
					</table>
				</div>
...
<script>  
    ...
    function ValidateSize(file) {
                ...
                fextension = fname.substring(fname.lastIndexOf('.')+1).toLowerCase();  
                    ...
                    if ($.inArray(fextension, validExtensions) == -1){
                        ...
                        if(fsize > allowSize){/*10576-1MB(You can change the size as you want)*/
 ...
 var formCountryMst = $( "#frm_attachment" );
