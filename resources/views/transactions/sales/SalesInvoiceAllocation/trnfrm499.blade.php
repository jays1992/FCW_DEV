...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->SALINVALID}}" value="{{$val->SALINVALID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->SALINVAL_DATE) && $val->SALINVAL_DATE !=''?$val->SALINVAL_DATE:''}}" ></td>
                <td>{{isset($val->SALINVAL_NO) && $val->SALINVAL_NO !=''?$val->SALINVAL_NO:''}}</td>
                <td>{{isset($val->SALINVAL_DATE) && $val->SALINVAL_DATE !=''?$val->SALINVAL_DATE:''}}</td>
