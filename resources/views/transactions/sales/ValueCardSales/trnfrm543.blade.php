...
@section('content')    
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                  <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Value Card Sale</a>
                </div>
                ...
                <div class="col-lg-10 topnav-pd">
                        ...
                        <button class="btn topnavbt" id="btnMature"{{ (isset($objRights->APPROVAL1) || isset($objRights->APPROVAL2) || isset($objRights->APPROVAL3) || isset($objRights->APPROVAL4) || isset($objRights->APPROVAL5)) &&  ($objRights->APPROVAL1||$objRights->APPROVAL2||$objRights->APPROVAL3||$objRights->APPROVAL4||$objRights->APPROVAL5) == 1 ? '' : 'disabled'}} ><i class="fa fa-thumbs-o-up"></i> Mature</button>
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->VCS_ID}}" value="{{$val->VCS_ID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->docdate) && $val->docdate !='' && $val->docdate !='1900-01-01' ? date('d-m-Y',strtotime($val->docdate)):''}}" ></td>
              <td>{{isset($val->DOC_NO) && $val->DOC_NO !=''?$val->DOC_NO:''}}</td>
              <td>{{isset($val->DOC_DATE) && $val->DOC_DATE !='' && $val->DOC_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->DOC_DATE)):''}}</td>
              <td>{{isset($val->NAME) && $val->NAME !=''?$val->NAME:''}}</td>
              <td>{{isset($val->MOBILE_NO) && $val->MOBILE_NO !=''?$val->MOBILE_NO:''}}</td>
              <td>{{isset($val->EMAIL_ID) && $val->EMAIL_ID !=''?$val->EMAIL_ID:''}}</td>
              <td>{{isset($val->CARDNO) && $val->CARDNO !=''?$val->CARDNO:''}}</td>
              <td>{{isset($val->CURRENT_BALANCE) && $val->CURRENT_BALANCE !=''?$val->CURRENT_BALANCE:''}}</td>
              <!--<td></td>-->
              <td>{{isset($val->VALIDITY_TO) && $val->VALIDITY_TO !=''?$val->VALIDITY_TO:''}}</td>
