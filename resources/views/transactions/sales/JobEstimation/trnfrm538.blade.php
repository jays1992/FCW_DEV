...
@section('content')    
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                  <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Job Estimation</a>
                </div>
                ...
                <div class="col-lg-10 topnav-pd">
						...
						<button class="btn topnavbt" id="btnPrint_Blank" ><i class="fa fa-print"></i> Print_Blank</button>
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->JEID}}" value="{{$val->JEID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->FD_DATE) && $val->FD_DATE !='' && $val->FD_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->FD_DATE)):''}}" ></td>
              <td>{{isset($val->JOB_NO) && $val->JOB_NO !=''?$val->JOB_NO:''}}
              <input type="hidden" id="JEID_REF{{$val->JEID}}" value="{{$val->JEID_REF}}">
              </td>
              <td>{{isset($val->JOB_DATE) && $val->JOB_DATE !='' && $val->JOB_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->JOB_DATE)):''}}</td>
              
              <td>{{isset($val->CUSTOMER_NAME) && $val->CUSTOMER_NAME !=''?$val->CUSTOMER_NAME:''}}</td>
              <td>{{isset($val->MOBILE_NO) && $val->MOBILE_NO !=''?$val->MOBILE_NO:''}}</td>
              <td>{{isset($val->EMAIL_ID) && $val->EMAIL_ID !=''?$val->EMAIL_ID:''}}</td>
              <td>{{isset($val->VEHICLE_REG_NO) && $val->VEHICLE_REG_NO !=''?$val->VEHICLE_REG_NO:''}}</td>

              <td>{{$val->JEID_REF != "" ? "Invoice Done":"Invoice Not Done"}}</td>
...
<div id="ReportView" class="modal" role="dialog"  data-backdrop="static"  >
  <div class="modal-dialog modal-md" style="width:50%; height:60%">
    ...
    <div class="modal-body">
	  <div class="tablename"><p>Job Estimation Print</p></div>
        <div class="row">
...
<!-- Print -->
<div id="ReportView_Blank" class="modal" role="dialog"  data-backdrop="static"  >
  <div class="modal-dialog modal-md" style="width:50%; height:60%">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" id='ReportViewclosePopup_Blank' >&times;</button>          
      </div>
    <div class="modal-body">
	  <div class="tablename"><p>Job Estimation Print</p></div>
        <div class="row">
          <div class="frame-container col-lg-12 pl text-center" >
                <button class="btn topnavbt" id="btnReport_Blank">
                    Print
                </button>
                <button class="btn topnavbt" id="btnPdf_Blank">
                    PDF
                </button>
                <button class="btn topnavbt" id="btnExcel_Blank">
                    Excel
                </button>
          </div>
        </div>
	  ...
	  <div class="single single-select table-responsive  table-wrapper-scroll-y my-custom-scrollbar">
          <div class="inner-form">
              <div class="row">
                  <div class="frame-container col-lg-12 pl " >                      
                      <iframe id="iframe_rpt_Blank" width="100%" height="1000" >
                      </iframe>
                  </div>
              </div>
          </div>
            ...
            if(seletedRecord==0){
              ...
              var recordId = countryIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var IncoiceStatus = $("#JEID_REF"+recordId).val(); 
                    if(IncoiceStatus != ""){
                    $("#YesBtn").hide();
                    $("#NoBtn").hide();
                    $("#OkBtn").hide();
                    $("#OkBtn1").show();
                    $("#AlertMessage").text('Sorry, You can not edit as Service Invoice already approved for this Job Estimation.');
                    $("#alert").modal('show');
                    $("#OkBtn1").focus();
                    return false;
                    }else{
              ...
              var recordId = apprIdsData[0];
                ...
                if(is_approve==0){
                  ...
                  var IncoiceStatus = $("#JEID_REF"+recordId).val(); 
                    ...
                    $("#OkBtn").hide();
                    $("#OkBtn1").show();
                    $("#AlertMessage").text('Sorry, You can not approve as Service Invoice already approved for this Job Estimation.');
                    $("#alert").modal('show');
                    $("#OkBtn1").focus();
                    return false;
                    }else{
                ...
                if(seletedRecord==0){
                    ...
                    highlighFocusBtn('activeOk1');
                      ...
                      if(is_approve==1){
                        ...
                        var editURL = '{{route("transaction",[$FormId,"mature",":rcdId"]) }}';
                            editURL = editURL.replace(":rcdId",recordId);
                            window.location.href=editURL;
                      }else if(is_approve==2){
                          ...
                          $("#NoBtn").hide();
                          $("#OkBtn1").show();
                          $("#AlertMessage").text('You can not mature cancel record.');
                          $("#alert").modal('show');
                          $("#OkBtn1").focus();
                          highlighFocusBtn('activeOk1');
                      }
                      if(is_approve==0){
                        ...
                        $("#NoBtn").hide();
                        $("#OkBtn1").show();
                        $("#AlertMessage").text(' Only Approved record can be mature.');
                        $("#alert").modal('show');
                        $("#OkBtn1").focus();
                        highlighFocusBtn('activeOk1');
                      } 
...
$('#btnPrint').on('click', function() {
      ...
      }else if(seletedRecord==1)
      {
            ...
            var Flag = 'H';
            var formData = 'SO='+ SONO + '&JEID='+ SONO + '&Flag='+ Flag ;
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
...
$('#btnPrint_Blank').on('click', function() {
      var soIdsData = getSeletectedCBox();
      var seletedRecord = soIdsData.length;
      ...
      }else if(seletedRecord==1)
      {
            ...
            $.ajax({
                url:'{{route("transaction",[$FormId,"ViewReport_Blank"])}}',
                type:'POST',
                data:formData,
                success:function(data) {
                    $('#ReportView_Blank').show();
                    var localS = data;
                    document.getElementById('iframe_rpt_Blank').src = "data:text/html;charset=utf-8," + escape(localS);
                    $('#btnPdf_Blank').show();
                    $('#btnExcel_Blank').show();
                    $('#btnPrint_Blank').show();
                },
  ...
  $('#btnPdf_Blank').on('click', function() {
  var soIdsData = getSeletectedCBox();
    var seletedRecord = soIdsData.length;
      ...
      }else if(seletedRecord==1)
      {
            ...
            var Flag = 'P';
            var formData = 'SO='+ SONO + '&JEID='+ SONO + '&Flag='+ Flag ;
            var consultURL = '{{route("transaction",[$FormId,"ViewReport_Blank",":rcdId"]) }}';
            consultURL = consultURL.replace(":rcdId",formData);
            window.location.href=consultURL;
            event.preventDefault();
      }
...
$('#btnExcel_Blank').on('click', function() {
    var soIdsData = getSeletectedCBox();
    var seletedRecord = soIdsData.length;
  ...
  $('#btnReport_Blank').on('click', function() {
   var soIdsData = getSeletectedCBox();
    var seletedRecord = soIdsData.length;
  ...
  $("#ReportViewclosePopup_Blank").click(function(event){
        $("#ReportView_Blank").hide();
        event.preventDefault();
      });
