  ...
  <div class="multiple table-responsive  ">
    <table id="rpro_order_listing" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->EWID}}" value="{{$val->EWID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->EW_DT) && $val->EW_DT !='' && $val->EW_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->EW_DT)):''}}" ></td>
          <td>{{isset($val->EW_NO) && $val->EW_NO !=''?$val->EW_NO:''}}</td>
          <td>{{isset($val->EW_DT) && $val->EW_DT !='' && $val->EW_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->EW_DT)):''}}</td>
