...
<div class="container-fluid topnav">
  <div class="row">
    <div class="col-lg-2">
      <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Accessory Invoice</a>
    </div>
  ...
  <div class="multiple table-responsive  ">
    <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->ACCID}}" value="{{$val->ACCID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
          <td>{{isset($val->DOC_NO) && $val->DOC_NO !=''?$val->DOC_NO:''}}</td>
          <td>{{isset($val->DOC_DATE) && $val->DOC_DATE !=''?$val->DOC_DATE:''}}</td>
          <td>{{isset($val->CUSTOMER_TYPE) && $val->CUSTOMER_TYPE !=''?$val->CUSTOMER_TYPE:''}}</td>
    ...
    <form id="transactionForm268Print" action="{{ route('transactiongetlist',[$FormId,'printdata'])}}" method="POST" >
      @csrf
...
<div id="ReportView" class="modal" role="dialog"  data-backdrop="static"  >
  <div class="modal-dialog modal-md" style="width:50%; height:60%">
    ...
    <div class="modal-body">
	  <div class="tablename"><p>Accessory Invoice Print</p></div>
        <div class="row">
    ...
    $('#btnPrint').on('click', function() {
          ...
          if(seletedRecord==0){
              ...
              var recordsIds = resultIdsData;
               ...
               $("#transactionForm268Print").submit()
...
$('#btnPrint').on('click', function() {
      ...
      }else if(seletedRecord==1)
      {
            ...
            var Flag = 'H';
            var formData = 'SO='+ SONO + '&ACCID='+ SONO + '&Flag='+ Flag ;
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
