...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->APPROVALID}}" value="{{$val->APPROVALID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
              <td>{{isset($val->EMPCODE) && $val->EMPCODE !=''?$val->EMPCODE:''}}</td>
        ...
        <form id="transactionForm399Print" action="{{ route('transactiongetlist',[404,'printdata'])}}" method="POST" >
            @csrf
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("transaction",[404,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("transaction",[404,"view",":rcdId"]) }}';
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("transaction",[404,"attachment",":rcdId"]) }}';
...
var recordId = resultIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("transaction",[404,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("transactionmodify",[404,"cancel"])}}',
                            type:'POST',
