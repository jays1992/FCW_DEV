...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->REIMBURSEID}}" value="{{$val->REIMBURSEID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->REMB_DT) && $val->REMB_DT !=''?$val->REMB_DT:''}}" ></td>
              <td>{{isset($val->REIMBURSE_DOCNO) && $val->REIMBURSE_DOCNO !=''?$val->REIMBURSE_DOCNO:''}}</td>
        ...
        <form id="transactionForm399Print" action="{{ route('transactiongetlist',[413,'printdata'])}}" method="POST" >
            @csrf
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("transaction",[413,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("transaction",[413,"view",":rcdId"]) }}';
          ...
          var resultIdsData = getSeletectedCBox();
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  else if(checkPeriodClosing(413,$('#chkId'+recordId).data("docdate"),0) ==0){
                    $("#YesBtn").hide();
                    $("#NoBtn").hide();
                    $("#OkBtn").hide();
                    $("#OkBtn1").show();
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("transaction",[413,"attachment",":rcdId"]) }}';
...
var recordId = resultIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("transaction",[413,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("transactionmodify",[413,"cancel"])}}',
                            type:'POST',
