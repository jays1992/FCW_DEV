...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                ...
                <button class="btn topnavbt" id="btnImport" {{isset($objRights->ADD) && $objRights->ADD != 1 ? 'disabled' : ''}} > <i class="fa fa-file"></i> Import</button>
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->ATTTNDID}}" value="{{$val->ATTTNDID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
              <td>{{isset($val->DOCNO) && $val->DOCNO !=''?$val->DOCNO:''}}</td>
              <td>{{isset($val->DOCDATE) && $val->DOCDATE !='' && $val->DOCDATE !='1900-01-01' ? date('d-m-Y',strtotime($val->DOCDATE)):''}}</td>
              {{-- <td>{{isset($val->PAY_PERIOD_DESC) && $val->PAY_PERIOD_DESC !=''?$val->PAY_PERIOD_DESC:''}}</td> --}}
              <td>{{isset($val->START_TIME) && $val->START_TIME !=''? $val->START_TIME:''}} - {{isset($val->END_TIME) && $val->END_TIME !=''? $val->END_TIME:''}}</td>
              <td>{{$DataStatus}}</td>
            </tr>
            @endforeach 
            @endif
...
$('#btnImport').on('click', function(){
  window.location.href='{{route("transaction",[$FormId,"importdata"]) }}';
});
</script>
