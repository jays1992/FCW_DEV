...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->CLAIMID}}" value="{{$val->CLAIMID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->CLAIM_DOC_DT) && $val->CLAIM_DOC_DT !=''?$val->CLAIM_DOC_DT:''}}" ></td>
              <td>{{isset($val->CLAIM_DOC_NO) && $val->CLAIM_DOC_NO !=''?$val->CLAIM_DOC_NO:''}}</td>
        ...
        <form id="transactionForm399Print" action="{{ route('transactiongetlist',[412,'printdata'])}}" method="POST" >
            @csrf
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("transaction",[412,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("transaction",[412,"view",":rcdId"]) }}';
          ...
          var resultIdsData = getSeletectedCBox();
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  else if(checkPeriodClosing(412,$('#chkId'+recordId).data("docdate"),0) ==0){
                    $("#YesBtn").hide();
                    $("#NoBtn").hide();
                    $("#OkBtn").hide();
                    $("#OkBtn1").show();
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("transaction",[412,"attachment",":rcdId"]) }}';
...
var recordId = resultIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("transaction",[412,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("transactionmodify",[412,"cancel"])}}',
                            type:'POST',
