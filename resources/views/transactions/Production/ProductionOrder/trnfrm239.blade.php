  ...
  <div class="multiple table-responsive  ">
    <table id="production_order_listing" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->PROID}}" value="{{$val->PROID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->PRO_DT) && $val->PRO_DT !='' && $val->PRO_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->PRO_DT)):''}}" ></td>
          <td>{{isset($val->PRO_NO) && $val->PRO_NO !=''?$val->PRO_NO:''}}</td>
          <td>{{isset($val->PRO_DT) && $val->PRO_DT !='' && $val->PRO_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->PRO_DT)):''}}</td>
...
<div id="ReportView" class="modal" role="dialog"  data-backdrop="static"  >
  <div class="modal-dialog modal-md" style="width:90%; height:90%">
    ...
    <div class="modal-body">
	  <div class="tablename"><p>Production Order Print</p></div>
        <div class="row">
...
$(document).ready(function(){  
  ...
  var soIdsData = getSeletectedCBox();
    var seletedRecord = soIdsData.length;
      ...
      {
            var PROID = soIdsData[0];
            var Flag = 'P';
            var formData = 'SO='+ PROID + '&PROID='+ PROID + '&Flag='+ Flag ;
            var consultURL = '{{route("transaction",[239,"ViewReport",":rcdId"]) }}';
...
$('#btnPrint').on('click', function() {
      ...
      }else if(seletedRecord==1)
      {
            ...
            $.ajax({
                url:'{{route("transaction",[239,"ViewReport"])}}',
                type:'POST',
                data:formData,
                success:function(data) {
                    $('#ReportView').show();
