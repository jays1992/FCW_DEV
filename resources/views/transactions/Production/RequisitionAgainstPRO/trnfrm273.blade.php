...
<div class="container-fluid purchase-order-view">
  <div class="multiple table-responsive  ">
    <table id="rpro_order_listing" class="display nowrap table table-striped table-bordered" width="100%">
      <thead id="thead1">
        <tr>
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->RPRID}}" value="{{$val->RPRID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->RPR_DT) && $val->RPR_DT !='' && $val->RPR_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->RPR_DT)):''}}" ></td>
          <td>{{isset($val->RPR_NO) && $val->RPR_NO !=''?$val->RPR_NO:''}}</td>
          <td>{{isset($val->RPR_DT) && $val->RPR_DT !='' && $val->RPR_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->RPR_DT)):''}}</td>
          <td>{{isset($val->PRO_NO) && $val->PRO_NO !=''?$val->PRO_NO:''}}</td>
          <td>{{isset($val->PRO_TITLE) && $val->PRO_TITLE !=''?$val->PRO_TITLE:''}}</td>
...
<div id="ReportView" class="modal" role="dialog"  data-backdrop="static"  >
  <div class="modal-dialog modal-md" style="width:90%; height:90%">
    ...
    <div class="modal-body">
	  <div class="tablename"><p>Requisition Against PRO Print</p></div>
        <div class="row">
...
$(document).ready(function(){  
  ...
  var DataTable =  $('#rpro_order_listing').DataTable({}); 
  ...
  var soIdsData = getSeletectedCBox();
    var seletedRecord = soIdsData.length;
      ...
      {
            var RPRID = soIdsData[0];
            var Flag = 'P';
            var formData = 'SO='+ RPRID + '&RPRID='+ RPRID + '&Flag='+ Flag ;
            var consultURL = '{{route("transaction",[273,"ViewReport",":rcdId"]) }}';
...
$('#btnPrint').on('click', function() {
      ...
      }else if(seletedRecord==1)
      {
            ...
            $.ajax({
                url:'{{route("transaction",[273,"ViewReport"])}}',
                type:'POST',
                data:formData,
                success:function(data) {
                    $('#ReportView').show();
