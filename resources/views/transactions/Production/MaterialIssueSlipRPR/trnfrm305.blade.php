...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->MISRID}}" value="{{$val->MISRID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->MISRDT) && $val->MISRDT !='' && $val->MISRDT !='1900-01-01' ? date('d-m-Y',strtotime($val->MISRDT)):''}}"></td>
                <td>{{isset($val->MISRNO) && $val->MISRNO !=''?$val->MISRNO:''}}</td>
                <td>{{isset($val->MISRDT) && $val->MISRDT !='' && $val->MISRDT !='1900-01-01' ? date('d-m-Y',strtotime($val->MISRDT)):''}}</td>
  ...
  var soIdsData = getSeletectedCBox();
    var seletedRecord = soIdsData.length;
      ...
      }else if(seletedRecord==1)
      {
            ...
            var consultURL = '{{route("transaction",[305,"ViewReport",":rcdId"]) }}';
...
$('#btnPrint').on('click', function() {
      ...
      }else if(seletedRecord==1)
      {
            ...
            $.ajax({
                url:'{{route("transaction",[305,"ViewReport"])}}',
                type:'POST',
                data:formData,
                success:function(data) {
                    $('#ReportView').show();
