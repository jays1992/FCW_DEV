  ...
  <div class="multiple table-responsive  ">
    <table id="production_order_listing" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->PNMID}}" value="{{$val->PNMID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->PNM_DT) && $val->PNM_DT !='' && $val->PNM_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->PNM_DT)):''}}" ></td>
          <td>{{isset($val->PNM_NO) && $val->PNM_NO !=''?$val->PNM_NO:''}}</td>
          <td>{{isset($val->PNM_DT) && $val->PNM_DT !='' && $val->PNM_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->PNM_DT)):''}}</td>
