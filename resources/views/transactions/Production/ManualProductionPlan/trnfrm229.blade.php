...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Manual Production Plan - <br>Machine / Shift / Day wise</a>
                </div>
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            @endphp	
            <tr>
              <td><input type="checkbox" id="chkId{{$val->MPPID}}" value="{{$val->MPPID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->MPP_DOC_DT) && $val->MPP_DOC_DT !='' && $val->MPP_DOC_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->MPP_DOC_DT)):''}}" ></td>
              <td>{{isset($val->MPP_DOC_NO) && $val->MPP_DOC_NO !=''?$val->MPP_DOC_NO:''}}</td>
              <td>{{isset($val->MPP_DOC_DT) && $val->MPP_DOC_DT !='' && $val->MPP_DOC_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->MPP_DOC_DT)):''}}</td>
              <td>{{isset($val->MTDESCRIPTION) && $val->MTDESCRIPTION !=''?$val->MTDESCRIPTION:''}}</td>
