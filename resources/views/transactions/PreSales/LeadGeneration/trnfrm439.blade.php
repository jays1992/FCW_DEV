...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                ...
                <button class="btn topnavbt" id="btnCancel"><i class="fa fa-times"></i> Cancel</button>
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            <tr>
                ...
                <?php if($val->CUSTOMER_TYPE=='Customer'){?>
                <td>{{isset($val->CUSTNAME) && $val->CUSTNAME !=''?$val->CUSTNAME:''}}</td>
                <?php }elseif($val->CUSTOMER_TYPE=='Prospect'){?>
                <td>{{isset($val->PROSPTNAME) && $val->PROSPTNAME !=''?$val->PROSPTNAME:''}}</td>
                <?php } ?>
                <td>{{isset($val->FNAME) && $val->FNAME !=''?$val->FNAME:''}}</td>
                <td>{{isset($val->COMPANY_NAME) && $val->COMPANY_NAME !=''?$val->COMPANY_NAME:''}}</td>
                <td>{{isset($val->CONTACT_PERSON) && $val->CONTACT_PERSON !=''?$val->CONTACT_PERSON:''}}</td>
                <td>{{isset($val->MOBILE_NUMBER) && $val->MOBILE_NUMBER !=''?$val->MOBILE_NUMBER:''}}</td>
