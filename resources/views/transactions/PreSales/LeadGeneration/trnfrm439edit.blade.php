...
<!-- <form id="frm_mst_se" onsubmit="return validateForm()"  method="POST"  >     -->
    ...
    <div class="container-fluid topnav">
            <div class="row">
                ...
                {{-- <button class="btn topnavbt" onclick="submitData('fnApproveData')"  id="btnApprove" {{($objRights->APPROVAL1||$objRights->APPROVAL2||$objRights->APPROVAL3||$objRights->APPROVAL4||$objRights->APPROVAL5) == 1 ? '' : 'disabled'}}><i class="fa fa-thumbs-o-up"></i> Approved</button> --}}
    ...
    <div class="container-fluid purchase-order-view filter">     
      ...
      <div class="row">
        ...
        <div class="col-lg-2 pl"><p>Lead Date*</p></div>
          <div class="col-lg-2 pl">
            <input {{$ActionStatus}} type="date" name="LEAD_DT" id="LEAD_DT" onchange="checkPeriodClosing('{{$FormId}}',this.value,1)" value="{{isset($objResponse->LEAD_DT) && $objResponse->LEAD_DT !=''?$objResponse->LEAD_DT:''}}" class="form-control mandatory" >
        </div> 
        ...
        <div class="col-lg-2 pl">
          ...
          <div class="col-lg-2 pl">
            <select {{$ActionStatus}} name="CONVERTSTATUS" id="CONVERTSTATUS" class="form-control"  autocomplete="off">
              ...
              <option {{isset($objResponse->CONVERT_STATUS) && $objResponse->CONVERT_STATUS == 'Prospecting'?'selected="selected"':''}} value="Prospecting">Prospecting</option>
              <option {{isset($objResponse->CONVERT_STATUS) && $objResponse->CONVERT_STATUS == 'Qualifying Leads'?'selected="selected"':''}} value="Qualifying Leads">Qualifying Leads</option>
              <option {{isset($objResponse->CONVERT_STATUS) && $objResponse->CONVERT_STATUS == 'Opportunity'?'selected="selected"':''}}value="Opportunity">Opportunity</option>
            </select> 
          </div>
        </div>
...
$.ajax({
    url:'{{route("transaction",[$FormId,"getTransferLeadsCode"])}}',
    type:'POST',
    success:function(data) {
        $('#tbody_divpopp').html(data);
