...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->BANK_RECONCILEID}}" value="{{$val->BANK_RECONCILEID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->BANK_RECONCILE_DATE) && $val->BANK_RECONCILE_DATE !='' && $val->BANK_RECONCILE_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->BANK_RECONCILE_DATE)):''}}" ></td>
                <td>{{isset($val->BANK_RECONCILE_CODE) && $val->BANK_RECONCILE_CODE !=''?$val->BANK_RECONCILE_CODE:''}}</td>
                <td>{{isset($val->BANK_RECONCILE_DATE) && $val->BANK_RECONCILE_DATE !='' && $val->BANK_RECONCILE_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->BANK_RECONCILE_DATE)):''}}</td>
...
<div id="ReportView" class="modal" role="dialog"  data-backdrop="static"  >
  <div class="modal-dialog modal-md" style="width:90%; height:90%">
    ...
    <div class="modal-body">
	  <div class="tablename"><p>Additional Issue Slip Print</p></div>
        <div class="row">
  ...
  var soIdsData = getSeletectedCBox();
    var seletedRecord = soIdsData.length;
      ...
      }else if(seletedRecord==1)
      {
            ...
            var Flag = 'P';
            var formData = 'SO='+ SONO + '&MISPID='+ SONO + '&Flag='+ Flag ;
            var consultURL = '{{route("transaction",[387,"ViewReport",":rcdId"]) }}';
...
$('#btnPrint').on('click', function() {
      ...
      }else if(seletedRecord==1)
      {
            ...
            $.ajax({
                url:'{{route("transaction",[387,"ViewReport"])}}',
                type:'POST',
                data:formData,
                success:function(data) {
                    $('#ReportView').show();
