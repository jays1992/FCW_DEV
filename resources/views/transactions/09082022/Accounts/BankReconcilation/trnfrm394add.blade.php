...
<div class="container-fluid topnav">
  <div class="row">
    <div class="col-lg-2"><a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Bank Reconcilation</a></div>
  ...
  <div class="container-fluid filter">
	  <div class="inner-form">
          ...
          @if(isset($objSON->SYSTEM_GRSR) && $objSON->SYSTEM_GRSR == "1")
            <input type="text" name="DOC_NO" id="DOC_NO" value="{{ isset($objDataNo)?$objDataNo:'' }}" class="form-control" autocomplete="off" readonly style="text-transform:uppercase"  >
          @elseif(isset($objSON->MANUAL_SR) && $objSON->MANUAL_SR == "1")
            <input type="text" name="DOC_NO" id="DOC_NO" class="form-control" maxlength="{{ isset($objSON->MANUAL_MAXLENGTH)?$objSON->MANUAL_MAXLENGTH:'' }}" autocomplete="off" style="text-transform:uppercase"  >
          @else
            <input type="text" name="DOC_NO" id="DOC_NO"  class="form-control"  autocomplete="off" readonly style="text-transform:uppercase"  >
          @endif
          <span class="text-danger" id="ERROR_DOC_NO"></span>
			  ...
			  <div class="col-lg-2 pl">
				  <input type="date" name="DOC_DT" id="DOC_DT" value="{{date('Y-m-d')}}" onchange="checkPeriodClosing('{{$FormId}}',this.value,1)" class="form-control"  placeholder="dd/mm/yyyy" >
        </div>

        <div class="col-lg-1 pl"><p>Bank</p></div>
			  <div class="col-lg-2 pl">
          <input type="text" name="TEXT_BID_REF" id="TEXT_BID_REF" class="form-control" autocomplete="off" readonly onclick="showHideModal('show')" >
          <input type="hidden" name="BID_REF" id="BID_REF" class="form-control" autocomplete="off" >
          <input type="hidden" name="GLID_REF" id="GLID_REF" class="form-control" autocomplete="off" >
			  </div>

        <div class="col-lg-1 pl"><p>Mode</p></div>
			  <div class="col-lg-2 pl">
          <select name="MODE" id="MODE" class="form-control" autocomplete="off" onchange="getDataArray()" >
            <option value="ALL">ALL</option>
            <option value="NON RECONCILE" selected="selected">NON RECONCILE</option>
          </select>
			  </div>
		  </div>
...
function getDataArray(){
  ...
  $.ajax({
    ...
    type:'POST',
    data:{MODE:MODE,BID_REF:BID_REF,GLID_REF:GLID_REF,BANK_RECONCILEID:BANK_RECONCILEID,ACTION_TYPE:ACTION_TYPE},
    success:function(data) {
...
function balanceCalculation(){
  ...
  var BALANCE_OF_BANK = parseFloat(TOTAL_CREDIT_AMOUNT-TOTAL_DEBIT_AMOUNT).toFixed(2);
  var TEXT_TOTAL_DF   = parseFloat(TOTAL_DF-BALANCE_OF_BANK).toFixed(2);
