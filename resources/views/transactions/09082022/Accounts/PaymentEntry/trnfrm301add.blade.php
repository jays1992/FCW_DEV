...
@inject('helper', 'App\Helpers\Helper') 
	...
	<div class="inner-form">
        ...
        <div class="col-lg-2 pl">
              ...
              @if(isset($objPAYDOCNO->SYSTEM_GRSR) && $objPAYDOCNO->SYSTEM_GRSR == "1")
                  <input type="text" name="PAYMENT_NO" id="PAYMENT_NO" value="{{ isset($objDataNo)?$objDataNo:'' }}" class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase"  >
              @elseif(isset($objPAYDOCNO->MANUAL_SR) && $objPAYDOCNO->MANUAL_SR == "1")
                  <input type="text" name="PAYMENT_NO" id="PAYMENT_NO" value="{{ old('PAYMENT_NO') }}" class="form-control mandatory" maxlength="{{ isset($objPBDOCNO->MANUAL_MAXLENGTH)?$objPBDOCNO->MANUAL_MAXLENGTH:'' }}" autocomplete="off" style="text-transform:uppercase"  >
            @else
                <input type="text" name="PAYMENT_NO" id="PAYMENT_NO" value="{{ old('PAYMENT_NO') }}" class="form-control mandatory" maxlength="30" autocomplete="off" style="text-transform:uppercase"  >
            @endif
        </div>
        <div class="col-lg-1 pl"><p>Payment Date*</p></div>
        <div class="col-lg-2 pl">
                  <input type="date" name="PAYMENT_DT" id="PAYMENT_DT" value="{{ old('PAYMENT_DT') }}" onchange="checkPeriodClosing('{{$FormId}}',this.value,1)" class="form-control mandatory"  placeholder="dd/mm/yyyy" />
        </div>
 ...
 else if($('#chk_Account').is(':checked') == true && BANK_CASH_ID !="" && parseFloat(TotalAmount_account) > parseFloat(BALANCE_ACCOUNT) )
 {
  ...
  GetMonthlyBudget(); 
    ...
    var allblank12 = [];
        ...
        else if($("#CHECK_BUDGET_AMOUT").val() ==='' && parseFloat($("#tot_amt1").val()) > parseFloat($("#BUDGET_AMOUNT").val()) && parseFloat($("#BUDGET_AMOUNT").val()) !=""){
              $("#ProceedBtn").focus();
              $("#YesBtn").hide();
              $("#NoBtn").hide();
              $("#OkBtn1").show();
