...
@inject('helper', 'App\Helpers\Helper')     
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            if($val->PAYMENT_FOR=='Account'){
              $Name = $helper->get_gldetail_PaymentReceipt($val->PAYMENTID,'Payment');
            }else{
              $Name=$val->SLNAME;   
            }
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->PAYMENTID}}" value="{{$val->PAYMENTID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->PAYMENT_DT) && $val->PAYMENT_DT !=''?$val->PAYMENT_DT:''}}" ></td>
                <td>{{isset($val->PAYMENT_NO) && $val->PAYMENT_NO !=''?$val->PAYMENT_NO:''}}</td>
                <td>{{isset($val->PAYMENT_DT) && $val->PAYMENT_DT !=''?$val->PAYMENT_DT:''}}</td>
                <td>{{isset($val->PAYMENT_FOR) && $val->PAYMENT_FOR !=''?$val->PAYMENT_FOR:''}}</td>
                <td>{{isset($Name) && $Name !=''?$Name:''}}</td>
                <td>{{isset($val->PAYMENT_TYPE) && $val->PAYMENT_TYPE !=''?$val->PAYMENT_TYPE:''}}</td>
...
$('#btnAdd').on('click', function() {
  ...
  //   });
  //     $.ajax({
  //         url:'{{ route("transaction",[$FormId,"downloadFile"])}}',
  //         type:'POST',
  //         // dataType: 'json',
