...
<div class="container-fluid filter">
  <form id="frm_trn_pay"  method="POST"> 
	...
	<div class="inner-form">
        ...
        <div class="col-lg-2 pl">
              <input {{$ActionStatus}} type="date" name="PAYMENT_DT" id="PAYMENT_DT" value="{{isset($objPAYHDR->PAYMENT_DT)?$objPAYHDR->PAYMENT_DT:''}}" onchange="checkPeriodClosing('{{$FormId}}',this.value,1)" class="form-control mandatory"  placeholder="dd/mm/yyyy" />
        </div>
    ...
    <div class="row" id="divcust1"  style="display:none">
        ...
        <div class="col-lg-1 pl"><p>PaymentOnAccount</p></div>        
        <div class="col-lg-2 pl">
        ...
        @foreach ($objBank as $index=>$bkRow)
          ...
          <td style="text-align:center; width:6%"> <input type="checkbox"    id="bkidcode_{{$index}}" class="clsbkid" value="{{ $bkRow->BID }}" name="bankcash[]" ></td>
              <td style="width:10%;">{{ $bkRow-> BCODE }}
                <input type="hidden" id="txtbkidcode_{{ $index }}" data-desc="{{ $bkRow-> BCODE }}" data-desc2="{{ $bkRow-> NAME }}"  data-desc3="{{ $bkRow-> BANK_CASH }}" data-glid_ref="{{ $bkRow->GLID_REF }}" data-odlimit="{{ $bkRow->ODLIMIT !=''? $bkRow->ODLIMIT:'0.00' }}" value="{{ $bkRow-> BID }}"/>
              </td>
 ...
 else if($('#chk_Account').is(':checked') == true && BANK_CASH_ID !="" && parseFloat(TotalAmount_account) > parseFloat(BALANCE_ACCOUNT) )
 {
  ...
  GetMonthlyBudget(); 
    ...
    var allblank12 = [];
        ...
        else if($("#CHECK_BUDGET_AMOUT").val() ==='' && parseFloat($("#tot_amt1").val()) > parseFloat(parseFloat($("#BUDGET_AMOUNT").val())+parseFloat($("#tot_amt1").val())) && parseFloat($("#BUDGET_AMOUNT").val()) !=""){
              $("#ProceedBtn").focus();
              $("#YesBtn").hide();
              $("#NoBtn").hide();
              $("#OkBtn1").show();
...
function GetMonthlyBudget(){
  ...
  async: false,
  dataType: 'json',
  data: {GLID_REF:GLID_REF,DATE:DATE},
  done: function(response) {return response;}}).responseText;
