...
@section('content')    
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                  <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Fixed Deposit</a>
                </div><!--col-2-->
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->FDID}}" value="{{$val->FDID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->FD_DATE) && $val->FD_DATE !='' && $val->FD_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->FD_DATE)):''}}" ></td>
              <td>{{isset($val->FD_CODE) && $val->FD_CODE !=''?$val->FD_CODE:''}}</td>
              <td>{{isset($val->FD_DATE) && $val->FD_DATE !='' && $val->FD_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->FD_DATE)):''}}</td>
