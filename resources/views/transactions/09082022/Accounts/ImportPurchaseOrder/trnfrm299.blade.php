...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                ...
                <button class="btn topnavbt" id="btnAdd" {{$objRights->ADD != 1 ? 'disabled' : ''}}><i class="fa fa-plus"></i> Add</button>
                <button class="btn topnavbt" id="btnEdit" {{$objRights->EDIT != 1 ? 'disabled' : ''}} ><i class="fa fa-pencil-square-o"></i> Edit</button>
                ...
                <button class="btn topnavbt" id="btnAmendment"{{$objRights->AMENDMENT != 1 ? 'disabled' : ''}}> Amendment</button>
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            @endphp
            <tr>
            <td><input type="checkbox" id="chkId{{$val->IPO_ID}}" value="{{$val->IPO_ID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->IPO_DT) && $val->IPO_DT !='' && $val->IPO_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->IPO_DT)):''}}" ></td>
            <td>{{isset($val->IPO_NO) && $val->IPO_NO !=''?$val->IPO_NO:''}}</td>
            <td>{{isset($val->IPO_DT) && $val->IPO_DT !='' && $val->IPO_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->IPO_DT)):''}}</td>
            <td>{{isset($val->SALE_ORDER_NO) && $val->SALE_ORDER_NO !=''?$val->SALE_ORDER_NO:''}}</td>
...
$('#btnImport').on('click', function() {
  var impURL =  '{{route("transaction",[$FormId,"import"]) }}';
                ...
                if(seletedRecord==0){
                    ...
                    highlighFocusBtn('activeOk1');
                      ...
                      if(is_approve==1){
                        ...
                        var editURL = '{{route("transaction",[236,"amendment",":rcdId"]) }}';
                            editURL = editURL.replace(":rcdId",recordId);
...
function highlighFocusBtn(pclass){
	...
	$('#btnPdf').on('click', function() {
      ...
      }else if(seletedRecord==1)
      {
            ...
            var consultURL = '{{route("transaction",[299,"ViewReport",":rcdId"]) }}';
...
$('#btnPrint').on('click', function() {
      ...
      }else if(seletedRecord==1)
      {
            ...
            $.ajax({
                url:'{{route("transaction",[299,"ViewReport"])}}',
                type:'POST',
                data:formData,
                success:function(data) {
                    $('#ReportView').show();
