...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            @endphp
            <tr>
            <td><input type="checkbox" id="chkId{{$val->PII_ID}}" value="{{$val->PII_ID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->PII_DT) && $val->PII_DT !='' && $val->PII_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->PII_DT)):''}}" ></td>
            <td>{{isset($val->PII_NO) && $val->PII_NO !=''?$val->PII_NO:''}}</td>
            <td>{{isset($val->PII_DT) && $val->PII_DT !='' && $val->PII_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->PII_DT)):''}}</td>
            <td>{{isset($val->VENDOR_INVOICE_NO) && $val->VENDOR_INVOICE_NO !=''?$val->VENDOR_INVOICE_NO:''}}</td>
            <td>{{isset($val->BOE_NO) && $val->BOE_NO !=''?$val->BOE_NO:''}}</td>
