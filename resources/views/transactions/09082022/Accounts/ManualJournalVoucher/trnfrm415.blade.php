...
@inject('helper', 'App\Helpers\Helper')  
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Manual Journal Voucher (MJV)</a>
                </div>
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            <tr>
                  ...
                  <th width="15%">MJV No</th>
                  <th width="15%">MJV Date</th>
                  <th width="15%">Reverse</th>
				          <th width="15%">Source Doc Type</th>
                  <th width="15%">Vendor/Customer Name</th>
                  <th width="15%">Source Doc No</th>
                  <th width="15%">Source Doc Date</th>
            ...
            if($val->SLNAME==''){
              $Name = $helper->get_gldetail_PaymentReceipt($val->MJVID,'MJV');
            }else{
              $Name=$val->SLNAME;   
            }
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->MJVID}}" value="{{$val->MJVID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->MJV_DT) && $val->MJV_DT !=''?$val->MJV_DT:''}}" ></td>
                <td>{{isset($val->MJV_NO) && $val->MJV_NO !=''?$val->MJV_NO:''}}</td>
                <td>{{isset($val->MJV_DT) && $val->MJV_DT !=''?$val->MJV_DT:''}}</td>
                <td>{{isset($val->REVERSE_DT) && $val->REVERSE_DT !=''?$val->REVERSE_DT:''}}</td>
				        <td>{{isset($val->NARRATION) && $val->NARRATION !=''?$val->NARRATION:''}}</td>
                <td>{{isset($Name) && $Name !=''?$Name:''}}</td>
                <td>{{isset($val->SOURCE_DOCNO) && $val->SOURCE_DOCNO !=''?$val->SOURCE_DOCNO:''}}</td>
                <td>{{isset($val->SOURCE_DOCDT) && $val->SOURCE_DOCDT !='' && $val->SOURCE_DOCDT !='1900-01-01' ? date('d-m-Y',strtotime($val->SOURCE_DOCDT)):''}}</td>
                <td>{{isset($val->REFERENCE_NO) && $val->REFERENCE_NO !=''?$val->REFERENCE_NO:''}}</td>
                ...
                if(seletedRecord==0){
                    ...
                    highlighFocusBtn('activeOk1');
                      ...
                      if(is_approve==1){
                        ...
                        var editURL = '{{route("transaction",[415,"amendment",":rcdId"]) }}';
                            editURL = editURL.replace(":rcdId",recordId);
