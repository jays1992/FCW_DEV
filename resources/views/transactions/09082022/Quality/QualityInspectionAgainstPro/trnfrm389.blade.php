  ...
  <div class="multiple table-responsive  ">
    <table id="production_order_listing" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp	
        <tr>
          <td><input type="checkbox" id="chkId{{$val->QIPID}}" value="{{$val->QIPID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->QIPDT) && $val->QIPDT !='' && $val->QIPDT !='1900-01-01' ? date('d-m-Y',strtotime($val->QIPDT)):''}}" ></td>
          <td>{{isset($val->QIPNO) && $val->QIPNO !=''?$val->QIPNO:''}}</td>
          <td>{{isset($val->QIPDT) && $val->QIPDT !='' && $val->QIPDT !='1900-01-01' ? date('d-m-Y',strtotime($val->QIPDT)):''}}</td>
		      <td>{{$val->ICODE}} - {{$val->NAME}}</td>
