...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Adjustment Voucher Head</a>
                </div><!--col-2-->
    ...
    <div class="container-fluid purchase-order-view filter">     
      <form id="frm_mst_edit" method="POST" onsubmit="return validateForm()" class="needs-validation"  > 
        @CSRF
        {{isset($objResponse->HEAD_VAID) ? method_field('PUT') : '' }}
       <div class="inner-form">              
             <div class="row">
               <div class="col-lg-2 pl"><p>Doc No*</p></div>
                 <div class="col-lg-2 pl">
                  <input type="text" name="DOC_NO" id="DOC_NO" value="{{ $HDR->HEADVA_DOCNO }}" class="form-control mandatory" tabindex="1" maxlength="100" autocomplete="off" readonly style="text-transform:uppercase" autofocus >
                 <span class="text-danger" id="ERROR_DOC_NO_REF"></span>                             
                 </div>

                 <div class="col-lg-2 pl"><p>Date*</p></div>
                 <div class="col-lg-2 pl">
                   <input type="date" name="REMB_DT" id="REMB_DT" onchange="checkPeriodClosing('{{$FormId}}',this.value,1)" value="{{ $HDR->HEADVA_DOCDT }}" class="form-control"  maxlength="100" > 
                 </div>
               ...
               <div class="row">
                 ...
                 <div class="col-lg-2 pl">
                   <select name="ADJUSTMENT_TYPE" id="ADJUSTMENT_TYPE" onchange="getEarHeadName(this.id,this.value)" class="form-control mandatory">
                     <option value="" selected="">Select</option>
                     <option {{isset($MATDATA->ADJUSTMENT_TYPE) && $MATDATA->ADJUSTMENT_TYPE == 'Earning Head'?'selected="selected"':''}} value="Earning Head">Earning Head</option>
                     <option {{isset($MATDATA->ADJUSTMENT_TYPE) && $MATDATA->ADJUSTMENT_TYPE == 'Deduction Head'?'selected="selected"':''}} value="Deduction Head">Deduction Head</option>
                    </select>
                 </div>
                 ...
                 <div class="col-lg-2 pl">
                   <select name="EARNIGHEAD_REF" id="EARNIGHEAD_REF" class="form-control mandatory">
                     ...
                     <?php if(isset($MATDATA) && $MATDATA->ADJUSTMENT_TYPE =="Earning Head"){?>
                     <option {{isset($MATDATA->EARNING_HEADID_REF) && $MATDATA->EARNING_HEADID_REF == $val-> EARNING_HEADID ?'selected="selected"':''}} value="{{ $val-> EARNING_HEADID }}">{{$val->EARNING_HEADCODE}}</option>
                     <?php }else{?>
                      @foreach($objDedhedList as $val)
                      <option {{isset($MATDATA->EARNING_HEADID_REF) && $MATDATA->EARNING_HEADID_REF == $val-> DEDUCTION_HEADID ?'selected="selected"':''}} value="{{ $val-> DEDUCTION_HEADID }}">{{$val->DEDUCTION_HEADCODE}}</option>
                      @endforeach
                      <?php }?>
                     @endforeach
                   </select>                            
                           ...
                           <tbody>
                            ...
                            @foreach($MAT as $key => $row)
                             <tr  class="participantRow">
                               ...
                               {{-- <td><select name="ADJUSTMENT_TYPE[]" id="ADJUSTMENT_TYPE_0" onchange="getEarHeadName(this.id,this.value)" class="form-control mandatory">
                                 <option value="" selected="">Select</option>
                                 <option value="Earning Head">Earning Head</option>
                                 <option value="Deduction Head">Deduction Head</option>
                               </select>
                             </td> --}}
                               {{-- <td><select name="EARNIGHEAD_REF[]" id="EARNIGHEAD_REF_0" class="form-control mandatory">
                                 <option value="" selected="">Select</option>
                                 @foreach($objEarnHead as $val)
                                 <option value="{{$val->EARNING_HEADID}}">{{$val->EARNING_HEADCODE}}</option>
                                 @endforeach
                               </select>
                             </td> --}}
                             <td><input type="text" name="txtEMPID[]" id={{"txtEMPID_".$key}} value="{{$row->EMPCODE}}" class="form-control"  autocomplete="off"  readonly/></td>
                             <td hidden><input type="hidden" name="EMPID_REF[]" id={{"EMPID_REF_".$key}} value="{{$row->EMPID}}" class="form-control" autocomplete="off" /></td>
...
window.fnSaveData = function (){
     ...
     $.ajax({
        url:'{{ route("transactionmodify",[421,"update"]) }}',
         type:'POST',
         data:formData,
         success:function(data) {
...
$('#Material').on('click','[id*="txtEMPID"]',function(event){
 ...
 var fieldid = $(this).parent().parent().find('[id*="EMPID_REF"]').attr('id');
   ...
   $("#tbody_empref").html('');
     ...
     $.ajax({
         url:'{{route("transaction",[$FormId,"getglsl"])}}',
         type:'POST',
