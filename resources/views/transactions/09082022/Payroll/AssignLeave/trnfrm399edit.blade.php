...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                <a href="{{route('transaction',[399,'index'])}}" class="btn singlebt">Assign Leave</a>
                </div><!--col-2-->
       ...
       <div class="inner-form">
             ...
             <div class="row">
               <div class="col-lg-2 pl"><p>Pay Period Code*</p></div>
                 ...
                 <div class="col-lg-2 pl">
                   <input type="text" id="PAY_PERIOD_DESC" value="{{ $objLvDesList->PAY_PERIOD_DESC }}" class="form-control" readonly  maxlength="100" > 
                 </div>
               ...
               <div class="col-lg-2 pl"><p>Name</p></div>
                 <div class="col-lg-2 pl">
                   <input type="text" name="FNAME" id="FNAME" value="{{ $objEmpName->FNAME }}" class="form-control" readonly  maxlength="100" > 
                 </div>
             </div>
                   ...
                   <div class="table-responsive table-wrapper-scroll-y" style="height:500px;margin-top:10px;" >
                    ...
                    <table id="example2" class="display nowrap table table-striped table-bordered itemlist" width="100%" style="height:auto !important;">
                     <thead id="thead1"  style="position: sticky;top: 0">                      
                       ...
                       <th rowspan="2" width="3%">Action <input class="form-control" type="hidden" name="Row_Count" id ="Row_Count" /></th>
                     </tr>                      
                     ...
                     <tbody>
                      ...
                      @foreach($objDataResponse as $key => $row)
                       <tr  class="participantRow">
                         <td hidden><input type="hidden" name="ASSIGN_LDID" id="ASSIGN_LDID" value="{{ $row->ASSIGN_LDID }}" /></td>
                           <td><input  class="form-control" type="date" name={{"ASSIGN_DT_".$key}}   id ={{"ASSIGN_DT_".$key}}  value="{{ $row->ASSIGN_DT }}" autocomplete="off" style="width: 99%"></td>
                           <td><select name={{"ASSIGN_AS_".$key}}   id ={{"ASSIGN_AS_".$key}} class="form-control mandatory" tabindex="4">
                              <option value="" selected="">Select</option>
                              @foreach($objAttnceList as $val)
                              <option {{isset($row->ASSIGN_AS) && $row->ASSIGN_AS == $val-> ATTENDANCE_STID ?'selected="selected"':''}} value="{{ $val-> ATTENDANCE_STID }}">{{ $val->ATTENDANCE_CODE }}-{{ $val->ATTENDANCE_CODE_DESC }}</option>
                              @endforeach
                            </select>
                          </td>
                           <td><select name={{"LTID_REF_".$key}}   id ={{"LTID_REF_".$key}} onchange="getLeaveTyName(this.id,this.value)" class="form-control mandatory" tabindex="4">
                             <option value="" selected="">Select</option>
                             @foreach($objLeaveList as $val)
                             <option {{isset($row->LTID_REF) && $row->LTID_REF == $val-> LTID ?'selected="selected"':''}} value="{{ $val-> LTID }}">{{ $val->LEAVETYPE_CODE }}</option>
                             @endforeach
                           </select> </td>
                           <td><input type="text" id ={{"LEAVETYPE_DESC_".$key}} value="{{ $row->LEAVETYPE_DESC }}" class="form-control" readonly  maxlength="100" > </td>
                           <td><input  class="form-control" type="text" name={{"REASON_LEAVE_".$key}}   id ={{"REASON_LEAVE_".$key}}  value="{{ $row->REASON_LEAVE }}" autocomplete="off" style="width: 99%"></td>
                           <td><input  class="form-control" type="text" name={{"REMARKS_".$key}}   id ={{"REMARKS_".$key}}  value="{{ $row->REMARKS }}" autocomplete="off" style="width: 99%"></td>
                           <td align="center">
...
$('#btnAdd').on('click', function() {
   var viewURL = '{{route("transaction",[399,"add"])}}';
...
$('#Row_Count').val(rcount);
 ...
 function checkDuplicateCode(){
     ...
     $.ajax({
         url:'{{route("transaction",[399,"codeduplicate"])}}',
         type:'POST',
         data:formData,
...
window.fnSaveData = function (){
     ...
     $.ajax({
         url:'{{ route("transactionmodify",[399,"update"]) }}',
         type:'POST',
         data:formData,
         success:function(data) {
             ...
             if(data.success) {                   
               ...
               //  window.location.href='{{ route("transaction",[399,"index"])}}';
             }
...
$.ajax({
    url:'{{ route("transactionmodify",[399,"Approve"])}}',
    type:'POST',
    data:formData,
    success:function(data) {
...
window.fnUndoYes = function (){
   ...
   //reload form
   window.location.href = "{{route('transaction',[399,'add'])}}";
 ...
 $.ajaxSetup({
     ...
     $.ajax({
         url:'{{route("transaction",[399,"getPayPrName"])}}',
         type:'POST',
...
function getLeaveTyName(id,LTID){
 ...
 $.ajaxSetup({
     ...
     $.ajax({
         url:'{{route("transaction",[399,"getLeaveTyName"])}}',
         type:'POST',
         data:{LTID:LTID},
         success:function(data) {
...
function getEmpName(EMPID){
  ...
  $.ajaxSetup({
      ...
      $.ajax({
          url:'{{route("transaction",[399,"getEmpName"])}}',
          type:'POST',
          data:{EMPID:EMPID},
          success:function(data) {
             $("#FNAME").val(data);                
