...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->ASSIGN_LID}}" value="{{$val->ASSIGN_LID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" ></td>
              <td>{{isset($val->PAY_PERIOD_CODE) && $val->PAY_PERIOD_CODE !=''?$val->PAY_PERIOD_CODE:''}}</td>
              <td>{{isset($val->INDATE) && $val->INDATE !='' && $val->INDATE !='1900-01-01' ? date('d-m-Y',strtotime($val->INDATE)):''}}</td>
              <td>{{isset($val->DEACTIVATED) && $val->DEACTIVATED =='1'?'Yes':'No'}}</td>
        ...
        <form id="transactionForm399Print" action="{{ route('transactiongetlist',[399,'printdata'])}}" method="POST" >
            @csrf
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("transaction",[399,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("transaction",[399,"view",":rcdId"]) }}';
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("transaction",[399,"attachment",":rcdId"]) }}';
...
var recordId = resultIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("transaction",[399,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("transactionmodify",[399,"cancel"])}}',
                            type:'POST',
    ...
    $('#btnPrint').on('click', function() {
          ...
          if(seletedRecord==0){
              ...
              var recordsIds = resultIdsData;
               ...
               $("#transactionForm399Print").submit()
