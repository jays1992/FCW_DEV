...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->SALARYPRID}}" value="{{$val->SALARYPRID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->DOCDATE) && $val->DOCDATE !=''?$val->DOCDATE:''}}" ></td>
              <td>{{isset($val->DOCNO) && $val->DOCNO !=''?$val->DOCNO:''}}</td>
              <td>{{isset($val->DOCDATE) && $val->DOCDATE !=''?$val->DOCDATE:''}}</td>
