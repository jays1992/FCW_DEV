...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->LOAN_DISBURSEID}}" value="{{$val->LOAN_DISBURSEID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->LOAN_DISB_DOCDT) && $val->LOAN_DISB_DOCDT !=''?$val->LOAN_DISB_DOCDT:''}}" ></td>
              <td>{{isset($val->LOAN_DISB_DOCNO) && $val->LOAN_DISB_DOCNO !=''?$val->LOAN_DISB_DOCNO:''}}</td>
              <td>{{isset($val->LOAN_DISB_DOCDT) && $val->LOAN_DISB_DOCDT !=''?$val->LOAN_DISB_DOCDT:''}}</td>
        ...
        <form id="transactionForm399Print" action="{{ route('transactiongetlist',[403,'printdata'])}}" method="POST" >
            @csrf
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("transaction",[403,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("transaction",[403,"view",":rcdId"]) }}';
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("transaction",[403,"attachment",":rcdId"]) }}';
...
var recordId = resultIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("transaction",[403,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("transactionmodify",[403,"cancel"])}}',
                            type:'POST',
