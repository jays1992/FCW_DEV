...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                ...
                <button class="btn topnavbt" id="btnAdd"><i class="fa fa-plus"></i> Add</button>
                <button class="btn topnavbt" id="btnEdit"><i class="fa fa-edit"></i> Edit</button>
                ...
                <button class="btn topnavbt"  id="btnAttach"><i class="fa fa-link"></i> Attachment</button>
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            <tr>
                ...
                <th>Lead Closure No</th>
                <th>Lead Closure Date</th>
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->LEAD_ID}}" value="{{$val->LEAD_ID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->LEAD_DT) && $val->LEAD_DT !=''?$val->LEAD_DT:''}}" ></td>
                <td>{{isset($val->LEAD_NO) && $val->LEAD_NO !=''?$val->LEAD_NO:''}}</td>
                <td>{{isset($val->LEAD_DT) && $val->LEAD_DT !=''?$val->LEAD_DT:''}}</td>
        ...
        if(seletedRecord==0){
          ...
          var recordId = resultIdsData[0];
              ...
              if(is_approve==0){
                ...
                var addURL = '{{route("transaction",[$FormId,"add",":rcdId"]) }}';
                    addURL = addURL.replace(":rcdId",window.btoa(recordId));
                    check_approval_level(<?php echo json_encode($REQUEST_DATA);?>,recordId,addURL);
                  ...
                  $("#OkBtn").hide();
                  $("#OkBtn1").show();
                  $("#AlertMessage").text('You cannot add cancel record.');
                  $("#alert").modal('show');
                  $("#OkBtn1").focus();
              }else{
                ...
                $("#OkBtn").hide();
                $("#OkBtn1").show();
                $("#AlertMessage").text('You cannot add record against approved Lead.');
                $("#alert").modal('show');
                $("#OkBtn1").focus();
              } 
        }
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    $("#OkBtn").hide();
                    $("#OkBtn1").show();
                    $("#AlertMessage").text('You cannot edit record against approved Lead.');
                    $("#alert").modal('show');
                    $("#OkBtn1").focus();
                  } 
            }
      ...
      if(seletedRecord==0){
        ...
        var recordId = resultIdsData[0];
            ...
            var viewURL = '{{route("transaction",[$FormId,"view",":rcdId"]) }}';
                  viewURL = viewURL.replace(":rcdId",window.btoa(recordId));
                  check_approval_level(<?php echo json_encode($REQUEST_DATA);?>,recordId,viewURL); 
      }
            ...
            if(jQuery.inArray("false", allblank) !== -1)
            {
              ...
              var recordId = resultIdsData[0];
                ...
                if(is_approve==0){
                  ...
                  $("#OkBtn").hide();
                  $("#OkBtn1").show();
                  $("#AlertMessage").text('You cannot approve record against approved Lead.');
                  $("#alert").modal('show');
                  $("#OkBtn1").focus();
