    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive">
            ...
            <tr>
                        ...
                        <th width="15%">Open Sales Order No</th>
                        <th width="15%">Open Sales Order Date</th>
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->OSOID}}" value="{{$val->OSOID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->OSODT) && $val->OSODT !='' && $val->OSODT !='1900-01-01' ? date('d-m-Y',strtotime($val->OSODT)):''}}" ></td>
              <td>{{isset($val->OSONO) && $val->OSONO !=''?$val->OSONO:''}}</td>
              ...
              <td>{{isset($val->OVFDT) && $val->OVFDT !='' && $val->OVFDT !='1900-01-01' ? date('d-m-Y',strtotime($val->OVFDT)):''}}</td>
              <td>{{isset($val->OVTDT) && $val->OVTDT !='' && $val->OVTDT !='1900-01-01' ? date('d-m-Y',strtotime($val->OVTDT)):''}}</td>
          ...
          var osoIdsData = getSeletectedCBox();
            ...
            if(seletedRecord==0){
                ...
                highlighFocusBtn('activeOk1');
                  ...
                  var viewURL = '{{route("transaction",[40,"view",":rcdId"]) }}';
            ...
            if(seletedRecord==0){
                ...
                highlighFocusBtn('activeOk1');
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("transaction",[40,"attachment",":rcdId"]) }}';
      ...
      $('#btnCancel').on('click', function() {
            ...
            if(seletedRecord==0){
                ...
                highlighFocusBtn('activeOk1');
                  ...
                  else if(checkPeriodClosing(40,$('#chkId'+recordId).data("docdate"),0) ==0){
                    $("#YesBtn").hide();
                    $("#NoBtn").hide();
                    $("#OkBtn").hide();
                    $("#OkBtn1").show();
            ...
            if(seletedRecord==0){
                ...
                highlighFocusBtn('activeOk1');
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("transaction",[40,"edit",":rcdId"]) }}';
...
var recordId = osoIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("transaction",[40,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("transactionmodify",[40,"cancel"])}}',
                            type:'POST',
  ...
  var soIdsData = getSeletectedCBox();
    var seletedRecord = soIdsData.length;
      ...
      }else if(seletedRecord==1)
      {
            ...
            var Flag = 'P';
            var formData = 'SO='+ SONO + '&OSONO='+ SONO + '&Flag='+ Flag ;
            var consultURL = '{{route("transaction",[40,"ViewReport",":rcdId"]) }}';
...
$('#btnPrint').on('click', function() {
      ...
      }else if(seletedRecord==1)
      {
            ...
            $.ajax({
                url:'{{route("transaction",[40,"ViewReport"])}}',
                type:'POST',
                data:formData,
                success:function(data) {
                    $('#ReportView').show();
