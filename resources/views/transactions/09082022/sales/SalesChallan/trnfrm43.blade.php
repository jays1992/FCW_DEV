...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
              <table id="frm_trn_sc" class="display nowrap table table-striped table-bordered" width="100%">
            <thead id="thead1">
            <tr>
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->SCID}}" value="{{$val->SCID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->SCDT) && $val->SCDT !='' && $val->SCDT !='1900-01-01' ? date('d-m-Y',strtotime($val->SCDT)):''}}" ></td>
              ...
              <td>{{isset($val->SONO) && $val->SONO !=''?$val->SONO:''}}</td>
              <td>{{isset($val->CPNO) && $val->CPNO !=''?$val->CPNO:''}}</td>
...
<div id="ReportView" class="modal" role="dialog"  data-backdrop="static"  >
  <div class="modal-dialog modal-md" style="width:50%; height:60%">
    ...
    <div class="modal-body">
	  <div class="tablename"><p>Sales Challan Print</p></div>
        <div class="row">
    ...
    $('#btnExit').on('click', function() {
      var viewURL = '{{route('home')}}';
        ...
        // Data table for serverside
         var trnfrmscTable =  $('#frm_trn_sc').DataTable({
          ...
          var scIdsData = getSeletectedCBox();
            ...
            if(seletedRecord==0){
                ...
                highlighFocusBtn('activeOk1');
                  ...
                  var viewURL = '{{route("transaction",[43,"view",":rcdId"]) }}';
            ...
            if(seletedRecord==0){
                ...
                highlighFocusBtn('activeOk1');
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("transaction",[43,"attachment",":rcdId"]) }}';
      ...
      $('#btnCancel').on('click', function() {
            ...
            if(seletedRecord==0){
                ...
                highlighFocusBtn('activeOk1');
                  ...
                  else if(checkPeriodClosing(43,$('#chkId'+recordId).data("docdate"),0) ==0){
                    $("#YesBtn").hide();
                    $("#NoBtn").hide();
                    $("#OkBtn").hide();
                    $("#OkBtn1").show();
            ...
            if(seletedRecord==0){
                ...
                highlighFocusBtn('activeOk1');
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("transaction",[43,"edit",":rcdId"]) }}';
...
var recordId = scIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("transaction",[43,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("transactionmodify",[43,"cancel"])}}',
                            type:'POST',
  ...
  var soIdsData = getSeletectedCBox();
    var seletedRecord = soIdsData.length;
      ...
      }else if(seletedRecord==1)
      {
            ...
            var Flag = 'P';
            var formData = 'SO='+ SONO + '&SCNO='+ SONO + '&Flag='+ Flag ;
            var consultURL = '{{route("transaction",[43,"ViewReport",":rcdId"]) }}';
...
$('#btnPrint').on('click', function() {
      ...
      }else if(seletedRecord==1)
      {
            ...
            $.ajax({
                url:'{{route("transaction",[43,"ViewReport"])}}',
                type:'POST',
                data:formData,
                success:function(data) {
                    $('#ReportView').show();
