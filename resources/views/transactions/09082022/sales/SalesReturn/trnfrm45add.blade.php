      ...
      <div class="inner-form">
        ...
        <div class="row">
          ...
          @if(isset($objSON->SYSTEM_GRSR) && $objSON->SYSTEM_GRSR == "1")
              <input type="text" name="SONO" id="SONO" value="{{ isset($objSONO)?$objSONO:'' }}" class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase" autofocus >
          @elseif(isset($objSON->MANUAL_SR) && $objSON->MANUAL_SR == "1")
              <input type="text" name="SONO" id="SONO"  class="form-control mandatory" maxlength="{{  isset($objSON->MANUAL_MAXLENGTH)?$objSON->MANUAL_MAXLENGTH:'' }}" autocomplete="off" style="text-transform:uppercase" autofocus >
          @else
            <input type="text" name="SONO" id="SONO"  class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase" autofocus >
          @endif
          </div>
                              
          <div class="col-lg-2 pl"><p>Date*</p></div>
          <div class="col-lg-2 pl">
              <input type="date" name="SODT" id="SODT" onchange="checkPeriodClosing(45,this.value,1)"  class="form-control mandatory" autocomplete="off" placeholder="dd/mm/yyyy" >
          </div>
...
$(document).ready(function(e) {
    ...
    $('#btnAdd').on('click', function() {
        var viewURL = '{{route("transaction",[45,"add"])}}';
    ...
    //to check the label duplicacy
     $('#SONO').focusout(function(){
      ...
      if(SONO ===""){
        ...
        $.ajax({
            url:'{{route("transaction",[45,"checkso"])}}',
            type:'POST',
            data:formData,
            success:function(data) {
               if(data.exists) {
...
//SO Date Check
    ...
    window.fnUndoYes = function (){
      ...
      //reload form
      window.location.href = "{{route('transaction',[45,'add'])}}";
...
$("#OkBtn").click(function(){
    ...
    $("#OkBtn").hide();
    $(".text-danger").hide();
    window.location.href = '{{route("transaction",[45,"index"]) }}';
});
