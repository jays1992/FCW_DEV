...
<div class="container-fluid topnav">
  <div class="row">
    <div class="col-lg-2"><a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Dealer Commission</a></div>
  ...
  <div class="container-fluid filter">
      ...
      <div class="row">
        ...
        <div class="col-lg-2 pl">
          ...
          <input type="text" name="DC_NO" id="DC_NO" value="{{ isset($objDataNo)?$objDataNo:'' }}" class="form-control mandatory" readonly  autocomplete="off"  style="text-transform:uppercase">
          @elseif(isset($objSON->MANUAL_SR) && $objSON->MANUAL_SR == "1")
          <input type="text" name="DC_NO" id="DC_NO" class="form-control mandatory" maxlength="{{isset($objSON->MANUAL_MAXLENGTH)?$objSON->MANUAL_MAXLENGTH:''}}" autocomplete="off" style="text-transform:uppercase"  >
          @else
          <input type="text" name="DC_NO" id="DC_NO"  class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase"  >
          @endif
        </div>
              
        <div class="col-lg-1 pl"><p>Doc Date</p></div>
        <div class="col-lg-2 pl">
          <input type="date" name="DC_DATE" id="DC_DATE" onchange="checkPeriodClosing('{{$FormId}}',this.value,1)" class="form-control mandatory" autocomplete="off" >
        </div>
...
function validateForm(){
  ...
  else if(checkPeriodClosing('{{$FormId}}',$("#DC_DATE").val(),0) ==0){
    $("#YesBtn").hide();
    $("#NoBtn").hide();
    $("#OkBtn").hide();
    $("#OkBtn1").show();
...
function getCommission(){
  ...
  $.ajax({
    url:'{{route("transaction",[$FormId,"getCommission"])}}',
    type:'POST',
...
function getInvoiceDetails(id){
  ...
  $.ajax({
    url:'{{route("transaction",[$FormId,"getInvoiceDetails"])}}',
    type:'POST',
