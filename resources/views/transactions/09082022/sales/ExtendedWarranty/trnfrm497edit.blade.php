...
<div class="container-fluid topnav">
	<div class="row">
		<div class="col-lg-2"><a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Extended Warranty</a></div>

		<div class="col-lg-10 topnav-pd">
			<button class="btn topnavbt" id="btnAdd" disabled="disabled"><i class="fa fa-plus"></i> Add</button>
			<button class="btn topnavbt" id="btnEdit" disabled="disabled"><i class="fa fa-edit"></i> Edit</button>
      <button class="btn topnavbt" id="btnSave" onclick="submitData('fnSaveData')"><i class="fa fa-save"></i> Save</button>
      ...
      <button class="btn topnavbt" onclick="submitData('fnApproveData')"  id="btnApprove" {{($objRights->APPROVAL1||$objRights->APPROVAL2||$objRights->APPROVAL3||$objRights->APPROVAL4||$objRights->APPROVAL5) == 1 ? '' : 'disabled'}}><i class="fa fa-lock"></i> Approved</button>
...
<form id="frm_mst_edit" onsubmit="return validateForm(actionType)" method="POST"  class="needs-validation"> 
  @CSRF
    {{isset($objResponse->EWID) ? method_field('PUT') : '' }}
    <div class="container-fluid filter">
      <div class="inner-form">
        <div class="row">
          <div class="col-lg-2 pl"><p>Doc No*</p></div>
          <div class="col-lg-2 pl">
            <input {{$ActionStatus}} type="text" name="DOC_NO" id="DOC_NO" value="{{isset($objResponse->EW_NO) && $objResponse->EW_NO !=''?$objResponse->EW_NO:''}}" class="form-control mandatory"  autocomplete="off" readonly >
           {{-- <span class="text-danger" id="ERROR_DOC_NO"></span> --}}
          </div>
                
          <div class="col-lg-2 pl"><p>DOC Date</p></div>
          <div class="col-lg-2 pl">
            <input {{$ActionStatus}} type="date" name="DOC_DT" id="DOC_DT" onchange="checkPeriodClosing('{{$FormId}}',this.value,1)" value="{{isset($objResponse->DOC_DT) && $objResponse->DOC_DT !=''?$objResponse->DOC_DT:''}}" class="form-control mandatory" >
            <span class="text-danger" id="ERROR_DOC_DT"></span>
          </div> 
          
          <div class="col-lg-2 pl"><p>Customer*</p></div>
          <div class="col-lg-2 pl">
            <input {{$ActionStatus}} type="text" name="cust_popup" id="txtcust_popup" value="{{isset($objResponse->CCODE) && $objResponse->CCODE !=''?$objResponse->CCODE:''}}{{isset($objResponse->NAME) && $objResponse->NAME !=''?'-'.$objResponse->NAME:''}}" class="form-control mandatory"  autocomplete="off" readonly/>
            <input type="hidden" name="CUSTID_REF" id="CUSTID_REF" value="{{isset($objResponse->CUSTOMER_REF) && $objResponse->CUSTOMER_REF !=''?$objResponse->CUSTOMER_REF:''}}" class="form-control" autocomplete="off" />           
          </div>
        </div>    
      </div>
          ...
          <li class="active"><a data-toggle="tab" href="#Material">Material</a></li>
			  ...
			  <div class="tab-content">
                  ...
                  $TOTALTAX=@number_format(isset($row->EXTW_TAX) && $row->EXTW_TAX !=''?$row->EXTW_TAX:'');
                    ...
                    <td><input  type="text" name="popupInvNo[]" id="popupInvNo_{{$key}}" value="{{isset($row->SINO) && $row->SINO !=''?$row->SINO:''}}" class="form-control"  autocomplete="off"  readonly style="width: 120px"/></td>
                    <td hidden><input type="hidden" name="INVNOID_REF[]" id="INVNOID_REF_{{$key}}" value="{{isset($row->EWINVNOID_REF) && $row->EWINVNOID_REF !=''?$row->EWINVNOID_REF:''}}" class="form-control" autocomplete="off" /></td>
                    
                    <td><input type="date" name="INVDATE[]" id="INVDATE_{{$key}}" value="{{isset($row->SIDT) && $row->SIDT !=''?$row->SIDT:''}}" class="form-control"  autocomplete="off"  readonly style="width: 120px"/></td>
                    
                    <td hidden><input type="text" name="INVIDNO[]" id="INVIDNO_{{$key}}" value="{{isset($row->SIID_REF) && $row->SIID_REF !=''?$row->SIID_REF:''}}" class="form-control" autocomplete="off" /></td>
                    <td hidden><input type="text" name="SEID_REF[]" id="SEID_REF_{{$key}}" value="{{isset($row->SEID_REF) && $row->SEID_REF !=''?$row->SEID_REF:''}}" class="form-control" autocomplete="off" /></td>
                    
                    <td><input  type="text" name="popupITEMID[]" id="popupITEMID_{{$key}}" value="{{isset($row->ICODE) && $row->ICODE !=''?$row->ICODE:''}}" class="form-control"  autocomplete="off"  readonly style="width: 120px"/></td>
                    <td hidden><input type="text" name="ITEMID_REF[]" id="ITEMID_REF_{{$key}}" value="{{isset($row->ITEMID) && $row->ITEMID !=''?$row->ITEMID:''}}" class="form-control" autocomplete="off" /></td>
                    <td><input type="text" name="ItemName[]" id="ItemName_{{$key}}" value="{{isset($row->NAME) && $row->NAME !=''?$row->NAME:''}}" class="form-control"  autocomplete="off"  readonly style="width: 150px"/></td>
                    
                    <td><input type="text" name="EXTENDEDWNTMONTH[]" id="EXTENDEDWNTMONTH_{{$key}}" value="{{isset($row->EXTWA_MONTH) && $row->EXTWA_MONTH !=''?$row->EXTWA_MONTH:''}}" onkeyup="getExtnFromDate(this.id,this.value)" class="form-control"  onkeypress="return onlyNumberKey(event)" autocomplete="off" /></td>
                    
                    <td hidden><input type="hidden" name="popupMUOM[]" id="popupMUOM_{{$key}}" class="form-control"  autocomplete="off"  readonly/></td>
                    <td hidden><input type="text" name="MAIN_UOMID_REF[]" id="MAIN_UOMID_REF_{{$key}}" class="form-control"  autocomplete="off" /></td>
                    
                    <td><input type="date"   name="STARTFROM[]" id="STARTFROM_{{$key}}" value="{{isset($row->EW_STARTFROM) && $row->EW_STARTFROM !=''?$row->EW_STARTFROM:''}}" class="form-control" autocomplete="off"   /></td>
                    <td><input type="date"   name="STARTTO[]" id="STARTTO_{{$key}}" value="{{isset($row->EW_STARTTO) && $row->EW_STARTTO !=''?$row->EW_STARTTO:''}}" class="form-control" autocomplete="off" /></td>
                    <td><input type="text"   name="WARRANTYAMT[]" id="WARRANTYAMT_{{$key}}" value="{{isset($row->EXTWA_AMOUNT) && $row->EXTWA_AMOUNT !=''?$row->EXTWA_AMOUNT:''}}" class="form-control" maxlength="200" onkeyup="getWatnTaxDetails(this.id,this.value)"  onkeypress="return onlyNumberKey(event)" autocomplete="off" /></td>
                    
                    <td><input type="text"   name="TOTALTAX[]" id="TOTALTAX_{{$key}}" value="{{isset($TOTALTAX) && $TOTALTAX !=''?$TOTALTAX:''}}" class="form-control" onkeyup="getWatnTaxDetails(this.id,this.value)" autocomplete="off" readonly style="width: 120px"/></td>
                    <td><input type="text"   name="TAXAMT[]" id="TAXAMT_{{$key}}" value="{{isset($TAXAMT) && $TAXAMT !=''?$TAXAMT:''}}" class="form-control" autocomplete="off"  style="width: 120px" readonly/></td>
                    <td><input type="text"   name="TOTALAMT[]" id="TOTALAMT_{{$key}}" value="{{isset($row->EXTOTAL_AMOUNT) && $row->EXTOTAL_AMOUNT !=''?$row->EXTOTAL_AMOUNT:''}}" class="form-control" maxlength="200"  autocomplete="off" readonly style="width: 100px" /></td>
  ...
  <div class="modal-dialog modal-md column3_modal">
    ...
    <tr>
      ...
      <table id="CustTable2" class="display nowrap table  table-striped table-bordered" >
        ...
        <tbody>
        @foreach ($objCUST as $index=>$cust_Row)
        <tr> 
        <td class="ROW1"> <input type="checkbox" name="SELECT_CUSTID_REF[]" id="custcode_{{ $index }}" class="clscustcode" value="{{ $cust_Row->SLID_REF }}" ></td>
          <td class="ROW2">{{ $cust_Row->CCODE }}
          <input type="hidden" id="txtcustcode_{{ $index }}" data-desc="{{ $cust_Row->CCODE }}-{{ $cust_Row->NAME }}"  value="{{ $cust_Row->SLID_REF }}" data-title="{{ $cust_Row->NAME }}"/>
          </td>
          <td class="ROW3">{{ $cust_Row->NAME }}</td>
        </tr>
        @endforeach
        </tbody>
      </table>
