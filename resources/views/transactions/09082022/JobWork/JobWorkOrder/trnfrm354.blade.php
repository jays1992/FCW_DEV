  ...
  <div class="multiple table-responsive  ">
    <table id="production_order_listing" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->JWOID}}" value="{{$val->JWOID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->JWODT) && $val->JWODT !='' && $val->JWODT !='1900-01-01' ? date('d-m-Y',strtotime($val->JWODT)):''}}" ></td>
          <td>{{isset($val->JWONO) && $val->JWONO !=''?$val->JWONO:''}}</td>
          <td>{{isset($val->JWODT) && $val->JWODT !='' && $val->JWODT !='1900-01-01' ? date('d-m-Y',strtotime($val->JWODT)):''}}</td>
  ...
  var soIdsData = getSeletectedCBox();
    var seletedRecord = soIdsData.length;
      ...
      }else if(seletedRecord==1)
      {
            ...
            var Flag = 'P';
            var formData = 'SO='+ SONO + '&JWOID='+ SONO + '&Flag='+ Flag ;
            var consultURL = '{{route("transaction",[354,"ViewReport",":rcdId"])}}';
...
$('#btnPrint').on('click', function() {
      ...
      }else if(seletedRecord==1)
      {
            ...
            $.ajax({
                url:'{{route("transaction",[354,"ViewReport"])}}',
                type:'POST',
                data:formData,
                success:function(data) {
                    $('#ReportView').show();
