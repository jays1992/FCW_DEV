...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->JWIID}}" value="{{$val->JWIID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->JWIDT) && $val->JWIDT !='' && $val->JWIDT !='1900-01-01' ? date('d-m-Y',strtotime($val->JWIDT)):''}}" ></td>
          <td>{{isset($val->JWINO) && $val->JWINO !=''?$val->JWINO:''}}</td>
          <td>{{isset($val->JWIDT) && $val->JWIDT !='' && $val->JWIDT !='1900-01-01' ? date('d-m-Y',strtotime($val->JWIDT)):''}}</td>
