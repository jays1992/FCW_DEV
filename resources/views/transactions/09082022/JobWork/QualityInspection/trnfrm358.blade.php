  ...
  <div class="multiple table-responsive  ">
    <table id="production_order_listing" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->QIJID}}" value="{{$val->QIJID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->QIJDT) && $val->QIJDT !='' && $val->QIJDT !='1900-01-01' ? date('d-m-Y',strtotime($val->QIJDT)):''}}" ></td>
          <td>{{isset($val->QIJNO) && $val->QIJNO !=''?$val->QIJNO:''}}</td>
          <td>{{isset($val->QIJDT) && $val->QIJDT !='' && $val->QIJDT !='1900-01-01' ? date('d-m-Y',strtotime($val->QIJDT)):''}}</td>
