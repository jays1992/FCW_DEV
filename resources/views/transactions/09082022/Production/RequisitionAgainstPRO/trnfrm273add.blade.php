...
<form id="trn_form" onsubmit="return validateForm()"  method="POST" class="needs-validation"  >  
  <div class="container-fluid filter">
      ...
      <div class="row">
        ...
        <div class="col-lg-2 pl">
          ...
          <input type="text" name="RPRO_NO" id="RPRO_NO" value="{{ $objDataNo }}" class="form-control mandatory" readonly  autocomplete="off"  style="text-transform:uppercase" autofocus >
          @endif

          @if(isset($objSON->MANUAL_SR) && $objSON->MANUAL_SR == "1")
          <input type="text" name="RPRO_NO" id="RPRO_NO" value="{{ old('RPRO_NO') }}" class="form-control mandatory" maxlength="{{$objSON->MANUAL_MAXLENGTH}}" autocomplete="off" style="text-transform:uppercase" autofocus >
          @endif
        ...
        <div class="col-lg-2 pl">
          <input type="date" name="RPR_DT" id="RPR_DT" onchange="checkPeriodClosing('{{$FormId}}',this.value,1)" value="{{ old('RPR_DT') }}" class="form-control mandatory"  placeholder="dd/mm/yyyy" maxlength="10" >
      ...
      <div class="row">
        ...
        <div class="col-lg-2 pl">
          <input type="text" name="pro_popup" id="txtpro_popup" class="form-control mandatory"  autocomplete="off" readonly/>
          <input type="hidden" name="PROID_REF" id="PROID_REF" class="form-control" autocomplete="off" />           
        </div>
        <div class="col-lg-2 pl"><p>Title</p></div>
        <div class="col-lg-4 pl">
          <input type="text" name="TITLE" id="TITLE" class="form-control" maxlength="100" autocomplete="off" readonly>
					  ...
					  <div class="table-responsive table-wrapper-scroll-y" style="margin-top:10px;" >
						  <table id="example2" class="display nowrap table table-striped table-bordered itemlist w-200" width="100%" style="height:auto !important;">
							  <thead id="thead1"  style="position: sticky;top: 0">
								  <tr>
                    <th  hidden><input class="form-control" type="text" name="Row_Count1" id ="Row_Count1" value="1" style="width: 50px;"></th>
          ...
          <div id="udf" class="tab-pane fade">
              ...
              @foreach($objUdfData as $uindex=>$uRow)
                <tr  class="participantRow3">
                    ...
                    <td hidden><input type="hidden" name={{"UDF_".$uindex}} id={{"UDF_".$uindex}} class="form-control" value="{{$uRow->UDFRPRID}}" autocomplete="off"   /></td>
...
<!-- PRO NO Dropdown -->
<div id="pro_popup" class="modal" role="dialog"  data-backdrop="static">
  <div class="modal-dialog modal-md column3_modal">
    ...
    <table id="ProdNoTable" class="display nowrap table  table-striped table-bordered" >
    <thead>
    <tr>
    ...
    <tr>
        ...
        <td class="ROW2"><input type="text" id="pronosearch" class="form-control" autocomplete="off" onkeyup="ProNoFunction()"></td>
        <td class="ROW3"><input type="text" id="prodatesearch" class="form-control" autocomplete="off" onkeyup="ProDateFunction()"></td>
      </tr>

    </tbody>
    </table>
      <table id="ProdNoTable2" class="display nowrap table  table-striped table-bordered" >
        <thead id="thead2">
          
        </thead>
        <tbody>
        @foreach ($objPRO as $index=>$prono_Row)
        <tr> 
        <td class="ROW1"> <input type="checkbox" name="SELECT_PROID_REF[]" id="prono_{{ $index }}" class="clsprono" value="{{ $prono_Row->PROID }}" ></td>
          <td class="ROW2">{{ $prono_Row-> PRO_NO }}
          <input type="hidden" id="txtprono_{{ $index }}" data-desc="{{ $prono_Row->PRO_NO }}"  value="{{ $prono_Row->PROID }}" data-title="{{ $prono_Row->PRO_TITLE }}"/>
          </td>
          <td class="ROW3">{{ $prono_Row-> PRO_DT }}</td>
        </tr>
        @endforeach
        </tbody>
      </table>
...
function validateForm(){
  ...
  if(RPRO_NO ===""){
    ...
    $('#example2').find('.participantRow').each(function(){
      ...
      else if($.trim($(this).find("[id*=REQ_QTY]").val()) ==="" || parseFloat($.trim($(this).find("[id*=REQ_QTY]").val()))<=0 ){
    ...
    if(jQuery.inArray("false", allblank1) !== -1){
      ...
      $("#FocusId").val(focustext);
      $("#alert").modal('show');
      $("#AlertMessage").text('Requisition Qty must be greater than zero in material tab.');
      $("#YesBtn").hide(); 
      $("#NoBtn").hide();  
      $("#OkBtn1").show();
      $("#OkBtn1").focus();
    ...
    else if(checkPeriodClosing('{{$FormId}}',$("#RPR_DT").val(),0) ==0){
      $("#YesBtn").hide();
      $("#NoBtn").hide();
      $("#OkBtn").hide();
      $("#OkBtn1").show();
...
/*================================== PRONO POPUP FUNCTION =================================*/
      let tid = "#ProdNoTable2";
      let tid2 = "#ProdNoTable";
      let headers = document.querySelectorAll(tid2 + " th");

      // Sort the table element when clicking on the table headers
      headers.forEach(function(element, i) {
        element.addEventListener("click", function() {
          w3.sortHTML(tid, ".clsprono", "td:nth-child(" + (i + 1) + ")");
        });
      });

      function ProNoFunction() {
        var input, filter, table, tr, td, i, txtValue;
        input = document.getElementById("pronosearch");
        filter = input.value.toUpperCase();
        table = document.getElementById("ProdNoTable2");
  ...
  function ProDateFunction() {
        var input, filter, table, tr, td, i, txtValue;
        input = document.getElementById("prodatesearch");
        filter = input.value.toUpperCase();
        table = document.getElementById("ProdNoTable2");
        tr = table.getElementsByTagName("tr");
        for (i = 0; i < tr.length; i++) {
      ...
      $('#txtpro_popup').click(function(event){
...
function get_all_item(stritemarray){
    ...
    $.ajax({
        ...
        type:'POST',
        data:{'PROID_REF':PROID_REF,'item_array':stritemarray,'STID_REF':STID_REF},
        success:function(data) {
