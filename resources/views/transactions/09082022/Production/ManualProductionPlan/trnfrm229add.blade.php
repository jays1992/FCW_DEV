	...
	<div class="inner-form">
        ...
        @if(!empty($objSON))
            @if($objSON->SYSTEM_GRSR == "1")
                <input type="text" name="MPPDOCNO" id="MPPDOCNO" value="{{ $objMPPDOCNO }}" class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase" autofocus required>
            @endif
            @if($objSON->MANUAL_SR == "1")
                <input type="text" name="MPPDOCNO" id="MPPDOCNO" value="{{ old('MPPDOCNO') }}" class="form-control mandatory" maxlength="{{$objSON->MANUAL_MAXLENGTH}}" autocomplete="off" style="text-transform:uppercase" autofocus required>
            @endif
        @endif    
        <span class="text-danger" id="ERROR_MPPDOCNO"></span> 
            ...
            <input type="date" name="MPPDT" id="MPPDT" onchange="checkPeriodClosing('{{$FormId}}',this.value,1)"  class="form-control mandatory" autocomplete="off" placeholder="dd/mm/yyyy" >
...
window.onload = function(){
  var docno = <?php echo json_encode($objSON); ?>;
  if(docno==""){     
    $("#YesBtn").hide();
      $("#NoBtn").hide();
      $("#OkBtn").hide();
