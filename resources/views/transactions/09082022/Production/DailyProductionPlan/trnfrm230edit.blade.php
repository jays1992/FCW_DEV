...
<div class="inner-form">
  ...
  <div class="row">
    ...
    <div class="col-lg-2 pl">
          <input type="date" name="DPPDT" id="DPPDT" onchange="checkPeriodClosing('{{$FormId}}',this.value,1)" value="{{ $objDPP->DPP_DT }}" class="form-control mandatory DPPDT"  placeholder="dd/mm/yyyy" >         
    </div>
...
<tr  class="participantRow">
  <td style="text-align:center;">
    <input type="text" name="SubGl_popup_{{$key}}" id="txtsubgl_popup_{{$key}}" value="{{$row->SL_CODE}}" class="form-control mandatory" style="width:140px" readonly />
  </td>
  <td  hidden>
    <input type="text" name="SLID_REF_{{$key}}" id="hdnSLIDREF_{{$key}}" value="{{$row->SLID_REF}}" class="form-control"/> 
  </td> 
  <td > 
    <input type="text" name="SaleOrd_Popup_{{$key}}" id="txtSORD_popup_{{$key}}"  value="{{$row->SONO}}" readonly data-dayno="1"  class="form-control"  style="width:140px;"  />
  </td>
  <td  hidden>
  ...
  <td><input type="text" name="BAL_SOQTY_{{$key}}" id="BALSO_QTY_{{$key}}" value="{{ $row->totalppqty }}" class="form-control" readonly style="width:130px;" autocomplete="off"  /></td>
  <td><input type="text" name="PRO_PLANQTY_{{$key}}" id="PROPLAN_QTY_{{$key}}"  value="{{$row->PRO_PLAN_QTY}}"  class="form-control three-digits" style="width:130px;" maxlength="13" autocomplete="off"  /></td>
  <td>
    <select id="dpp_priority_{{$key}}" name="dpp_priority_{{$key}}" class="form-control">
...
$(document).ready(function() {
  ...
  var lastdt = <?php echo json_encode($objlast_DT[0]->DPP_DT); ?>;
  var dpp = <?php echo json_encode($objDPP); ?>;
  var today = new Date(); 
  var sodate = today.getFullYear() + "-" + ("0" + (today.getMonth() + 1)).slice(-2) + "-" + ('0' + today.getDate()).slice(-2);
  if(lastdt < dpp.DPP_DT)
  {
	$('#DPPDT').attr('min',lastdt);
  }
  else
  {
	  $('#DPPDT').attr('min',dpp.DPP_DT);
  }
  $('#DPPDT').attr('max',sodate);
...
function validateForm(actionName){
  $("#FocusId").val('');
        ...
        if(jQuery.inArray("false", allblank) !== -1){
          ...
          else if(checkPeriodClosing('{{$FormId}}',$("#DPPDT").val(),0) ==0){
            $("#YesBtn").hide();
            $("#NoBtn").hide();
            $("#OkBtn").hide();
            $("#OkBtn1").show();
