	...
	<div class="inner-form">
        ...
        @if(!empty($objDDNO))
            @if($objDDNO->SYSTEM_GRSR == "1")
                <input type="text" name="DPP_NO" id="DPP_NO" value="{{ $objDPDOCNO }}" class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase" autofocus required>
            @endif
            @if($objDDNO->MANUAL_SR == "1")
                <input type="text" name="DPP_NO" id="DPP_NO" value="{{ old('DPP_NO') }}" class="form-control mandatory" maxlength="{{$objDDNO->MANUAL_MAXLENGTH}}" autocomplete="off" style="text-transform:uppercase" autofocus required>
            @endif
        @endif    
        <span class="text-danger" id="ERROR_DPP_NO"></span> 
            ...
            <input type="date" name="DPPDT" id="DPPDT"  onchange="checkPeriodClosing('{{$FormId}}',this.value,1)" class="form-control mandatory" autocomplete="off" placeholder="dd/mm/yyyy" >
    ...
    <td>
      <select id="dpp_priority_0" name="dpp_priority_0" class="form-control">
        ...
        @foreach ($objPriority as $pindex=>$prow)
          <option value="{{ $prow->PRIORITYID }}" >{{ $prow->PRIORITYCODE }} - {{ $prow->DESCRIPTIONS }}</option> 
        @endforeach
      </select>
    </td>
...
window.fnUndoYes = function (){
    //reload form
  window.location.href = "{{route('transaction',[230,'add'])}}";
}//fnUndoYes
...
$.ajax({
    url:'{{ route("transaction",[230,"save"])}}',
    type:'POST',
    data:formData,
    success:function(data) {
      $(".buttonload").hide(); 
...
window.onload = function(){
  var docno = <?php echo json_encode($objDDNO); ?>;
  if(docno=="" || docno==null ){     
      $("#YesBtn").hide();
      $("#NoBtn").hide();
      $("#OkBtn").hide();
...
function checkDuplicateCode(){
        ...
        $.ajax({
            url:'{{route("transaction",[230,"codeduplicate"])}}',
            type:'POST',
