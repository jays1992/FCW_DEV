...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Non Returnable Gate Pass</a>
                </div>
	...
	<div class="inner-form">
		...
		<div class="row">
            ...
            @if(isset($objSON->SYSTEM_GRSR) && $objSON->SYSTEM_GRSR == "1")
                <input type="text" name="NRGP_NO" id="NRGP_NO" value="{{ isset($objDataNo)?$objDataNo:'' }}" class="form-control mandatory" readonly  autocomplete="off"  style="text-transform:uppercase"  >
            @elseif(isset($objSON->MANUAL_SR) && $objSON->MANUAL_SR == "1")
                <input type="text" name="NRGP_NO" id="NRGP_NO" value="{{ old('NRGP_NO') }}" class="form-control mandatory" maxlength="{{isset($objSON->MANUAL_MAXLENGTH)?$objSON->MANUAL_MAXLENGTH:''}}" autocomplete="off" style="text-transform:uppercase"  >
            @else
              <input type="text" name="NRGP_NO" id="NRGP_NO"  class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase"  >
            @endif

            <span class="text-danger" id="ERROR_NRGP_NO"></span>
			</div>
			
			<div class="col-lg-1 pl"><p>NRGP Date</p></div>
			<div class="col-lg-2 pl">
				    <input type="date" name="NRGP_DT" id="NRGP_DT" onchange="checkPeriodClosing('{{$FormId}}',this.value,1)" value="{{ old('NRGP_DT') }}" class="form-control mandatory"  placeholder="dd/mm/yyyy" >
      </div>
      ...
      <div class="col-lg-1 pl"><p>Purpose</p></div>
			<div class="col-lg-5 pl">
				<input type="text" name="PURPOSE" id="PURPOSE" class="form-control" autocomplete="off"  maxlength="200"  >
			</div>

      <div class="col-lg-1 pl"><p>Direct</p></div>
			<div class="col-lg-2 pl">
        <input type="checkbox" name="NRGP_STATUS" id="NRGP_STATUS" value='0' checked onchange="direct()" >
			...
			<div class="tab-content">
				...
				<div id="Material" class="tab-pane fade in active">
                  ...
                  <th>Reason for NRGP</th>
                          ...
                          <td><input type="text" name="MRS_NO_0" id="MRS_NO_0" class="form-control"  autocomplete="off"  readonly onclick="getMrsNo(this.id)" /></td>
                          ...
                          <td><input type="text" name="REMARKS_0" id="REMARKS_0" class="form-control w-100" autocomplete="off" ></td>
              ...
              @foreach($objUdfData as $uindex=>$uRow)
                <tr  class="participantRow3">
                    ...
                    <td hidden><input type="hidden" name={{"UDF_".$uindex}} id={{"UDF_".$uindex}} class="form-control" value="{{$uRow->UDFNRGPID}}" autocomplete="off"   /></td>
...
<div id="Prioritypopup" class="modal" role="dialog"  data-backdrop="static">
  <div class="modal-dialog modal-md column3_modal">
    ...
    <tr>
        <th class="ROW1"><span class="check_th">&#10004;</span></th>
        <td class="ROW2"><input type="text" id="Prioritycodesearch" autocomplete="off" class="form-control" onkeyup="PriorityCodeFunction()"></td>
        <td class="ROW3"><input type="text" id="Prioritynamesearch" autocomplete="off" class="form-control" onkeyup="PriorityNameFunction()"></td>
      </tr>
    </tbody>
    </table>
...
function loadVendor(CODE,NAME){
  ...
  $.ajax({
    url:'{{route("transaction",[91,"getVendor"])}}',
    type:'POST',
...
var lastdt = <?php echo json_encode($objlastdt[0]->NRGP_DT); ?>;
var today = new Date(); 
...
//  } 
  ...
  $('#example3').find('.participantRow3').each(function(){
    ...
    else if(checkPeriodClosing('{{$FormId}}',$("#NRGP_DT").val(),0) ==0){
      $("#YesBtn").hide();
      $("#NoBtn").hide();
      $("#OkBtn").hide();
      $("#OkBtn1").show();
...
$.ajax({
    ...
    success:function(data) {
        if(data.exists) {
            $(".text-danger").hide();
            showError('ERROR_NRGP_NO',data.msg);
            $("#NRGP_NO").focus();
        }
        else{
          $("#alert").modal('show');
...
$("#Material").on('click', '.checkstore', function() {
