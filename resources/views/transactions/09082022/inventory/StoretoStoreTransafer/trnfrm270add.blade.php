        ...
        <div class="container-fluid filter">
            <div class="inner-form">
                ...
                <div class="col-lg-2 pl"><p>Doc No</p></div>
                            ...
                            @if(isset($objSTSTN->SYSTEM_GRSR) && $objSTSTN->SYSTEM_GRSR == "1")
                                <input type="text" name="DOCNO" id="DOCNO" value="{{ isset($objSTSNO)?$objSTSNO:'' }}" class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase"  >
                            
                            @elseif(isset($objSTSTN->MANUAL_SR) && $objSTSTN->MANUAL_SR == "1")
                    ...
                    <div class="col-lg-2 pl">
                    <input type="hidden" id="objlastSTSDT"  value="{{$objlastSTSDT[0]->STTOST_DOCDT}}">
                                <input type="date" name="STSDT" id="STSDT" onchange="checkPeriodClosing(270,this.value,1)" value="{{ old('STSDT') }}" class="form-control mandatory" autocomplete="off" placeholder="dd/mm/yyyy" >
                    </div>
...
// Store Popup Ends
    ...
    $('#DOCNO').focusout(function(){
          ...
          if(DOCNO ===""){
            ...
            $.ajax({
                url:'{{route("transaction",[270,"checkdocno"])}}',
                type:'POST',
                data:formData,
                success:function(data) {
                   if(data.exists) {
...
<script>
    ...
    $.ajax({
        url:'{{ route("transaction",[270,"save"])}}',
        type:'POST',
        data:formData,
        success:function(data) {
          $(".buttonload").hide(); 
