...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->GEID}}" value="{{$val->GEID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->GE_DT) && $val->GE_DT !='' && $val->GE_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->GE_DT)):''}}" ></td>
                <td>{{isset($val->GE_NO) && $val->GE_NO !=''?$val->GE_NO:''}}</td>
                <td>{{isset($val->GE_DT) && $val->GE_DT !='' && $val->GE_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->GE_DT)):''}}</td>
                <td>{{isset($val->TYPE) && $val->TYPE !=''?$val->TYPE:''}}</td>
                <td>
                  @php
                  if(isset($val->TYPE) && $val->TYPE =="Vendor"){
                    echo $val->VENDOR_NAME;
                  }
                  else if(isset($val->TYPE) && $val->TYPE =="Customer"){
                    echo $val->CUSTOMER_NAME;
                  }
                  else if(isset($val->TYPE) && $val->TYPE =="Employee"){
                    echo $val->EMPLOYEE_NAME;
                  }
                  @endphp
                </td>
                <td>{{isset($val->VENDOR_BILLNO) && $val->VENDOR_BILLNO !=''?$val->VENDOR_BILLNO:''}}</td>
