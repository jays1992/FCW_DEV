    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            <tr>
                ...
                <th>Doc No</th>
                <th>Date</th>
                <th>Doc Type (Serial No & Barcode)</th>
                <th>Doc No (Serial No & Barcode)</th>
                <th>Created Date</th>               
                <th>Status</th>
            </tr>
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->BRCID}}" value="{{$val->BRCID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->BRC_DT) && $val->BRC_DT !='' && $val->BRC_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->BRC_DT)):''}}" ></td>
                <td>{{isset($val->BRC_NO) && $val->BRC_NO !=''?$val->BRC_NO:''}}</td>
                <td>{{isset($val->BRC_DT) && $val->BRC_DT !='' && $val->BRC_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->BRC_DT)):''}}</td>
                <td>{{isset($val->SOURCE_TYPE) && $val->SOURCE_TYPE !=''?$val->SOURCE_TYPE:''}}</td>
...
$('#btnImport').on('click', function(){
  window.location.href='{{route("transaction",[418,"importdata"]) }}';
});
