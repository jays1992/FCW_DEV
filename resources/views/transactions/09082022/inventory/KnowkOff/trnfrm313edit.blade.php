...
<div class="container-fluid topnav">
  <div class="row">
    <div class="col-lg-2"><a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Knock Off</a></div>
  ...
  <div class="container-fluid purchase-order-view"> 
    ...
    <div class="row">
        ...
        <div class="col-lg-2 pl">
          <input {{$ActionStatus}} type="text" name="KNOCKOFF_NO" id="KNOCKOFF_NO" value="{{isset($HDR->KNOCKOFF_NO) && $HDR->KNOCKOFF_NO !=''?$HDR->KNOCKOFF_NO:''}}" value class="form-control mandatory" readonly  autocomplete="off"  style="text-transform:uppercase"  >
        </div>
        
        <div class="col-lg-1 pl"><p>Knockoff Date</p></div>
        <div class="col-lg-2 pl">
          <input {{$ActionStatus}} type="date" name="KNOCKOFF_DT" id="KNOCKOFF_DT" onchange="checkPeriodClosing('{{$FormId}}',this.value,1)" value="{{isset($HDR->KNOCKOFF_DT) && $HDR->KNOCKOFF_DT !=''?$HDR->KNOCKOFF_DT:''}}" class="form-control mandatory"  placeholder="dd/mm/yyyy" >
        </div>

        <div class="col-lg-1 pl"><p>Vendor</p></div>
        <div class="col-lg-1 pl">
          <input {{$ActionStatus}} type="radio" name="VENDOR_CUSTOMER" value="VENDOR" onchange="changeVendorCustomer()" {{isset($HDR->VENDOR_CUSTOMER) && $HDR->VENDOR_CUSTOMER =='VENDOR'?'checked':''}} />
        </div>

        <div class="col-lg-1 pl"><p>Customer</p></div>
        <div class="col-lg-1 pl">
          <input {{$ActionStatus}} type="radio" name="VENDOR_CUSTOMER" value="CUSTOMER" onchange="changeVendorCustomer()" {{isset($HDR->VENDOR_CUSTOMER) && $HDR->VENDOR_CUSTOMER =='CUSTOMER'?'checked':''}}  />
        </div>
      ...
      <div class="row">
        ...
        <div class="col-lg-2 pl">
          <input {{$ActionStatus}} type="text" name="TXT_VID_CID_REF" id="TXT_VID_CID_REF" class="form-control" value="{{isset($CustomerVendor->VCODE) && $CustomerVendor->VCODE !=''?$CustomerVendor->VCODE:''}} {{isset($CustomerVendor->NAME) && $CustomerVendor->NAME !=''?'-'.$CustomerVendor->NAME:''}}"  readonly  />
          <input type="hidden" name="VID_CID_REF" id="VID_CID_REF" value="{{isset($HDR->VID_CID_REF) && $HDR->VID_CID_REF !=''?$HDR->VID_CID_REF:''}}"  class="form-control" />
          ...
          <table id="example2" class="display nowrap table table-striped table-bordered itemlist" width="100%" style="height:auto !important;">
            ...
            <tbody id="knowkoff_data">
              <tr class="participantRow">
                <td><input {{$ActionStatus}} type="checkbox"  name="DOC_ID[]"         id="DOC_ID_0"       class="clssQCPID"     disabled ></td>
                <td><input {{$ActionStatus}} type="text"      name="DOC_TYPE_0"       id="DOC_TYPE_0"     class="form-control"  autocomplete="off" readonly /></td>
                <td><input {{$ActionStatus}} type="text"      name="DOC_NO_0"         id="DOC_NO_0"       class="form-control"  autocomplete="off" readonly /></td>
                <td><input {{$ActionStatus}} type="text"      name="DOC_DT_0"         id="DOC_DT_0"       class="form-control"  autocomplete="off" readonly /></td>
                <td><input {{$ActionStatus}} type="text"      name="AMOUNT_0"         id="AMOUNT_0"       class="form-control"  autocomplete="off" readonly /></td>
                <td><input {{$ActionStatus}} type="text"      name="BAL_AMOUNT_0"     id="BAL_AMOUNT_0"   class="form-control"  autocomplete="off" readonly ></td>
                <td><input {{$ActionStatus}} type="text"      name="RECONCIL_AMT_0"   id="RECONCIL_AMT_0" class="form-control"  autocomplete="off" readonly /></td>
              </tr>
            </tbody>
          </table>
        </div>
...
function loadCustomer(CODE,NAME){
  ...
  $.ajax({
    url:'{{route("transaction",[$FormId,"getCustomerVendor"])}}',
    type:'POST',
...
function validateForm(ActionType,ActionMsg){
  ...
  else if(checkPeriodClosing('{{$FormId}}',$("#KNOCKOFF_DT").val(),0) ==0){
    $("#YesBtn").hide();
    $("#NoBtn").hide();
    $("#OkBtn").hide();
    $("#OkBtn1").show();
...
function getKnowkOffDetails(){
  ...
  $.ajax({
    url:'{{route("transaction",[$FormId,"getKnowkOffDetails"])}}',
    type:'POST',
...
$(document).ready(function(){
  ...
  $.ajax({
    url:'{{route("transaction",[$FormId,"getKnowkOffDetailsEdit"])}}',
    type:'POST',
    data:{'VID_CID_REF':VID_CID_REF,'VENDOR_CUSTOMER':VENDOR_CUSTOMER,'KNOWOFFID':'{{isset($HDR->KNOWOFFID) && $HDR->KNOWOFFID !=""?$HDR->KNOWOFFID:""}}','ActionStatus':'{{$ActionStatus}}'},
    success:function(data) {
      $("#knowkoff_data").html(data); 
      bindTotalValue();
