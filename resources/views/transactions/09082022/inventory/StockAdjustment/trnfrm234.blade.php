  ...
  <div class="row">
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->ST_ADJUSTID}}" value="{{$val->ST_ADJUSTID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->ST_ADJUST_DOCDT) && $val->ST_ADJUST_DOCDT !='' && $val->ST_ADJUST_DOCDT !='1900-01-01' ? date('d-m-Y',strtotime($val->ST_ADJUST_DOCDT)):''}}" ></td>
                <td>{{isset($val->ST_ADJUST_DOCNO) && $val->ST_ADJUST_DOCNO !=''?$val->ST_ADJUST_DOCNO:''}}</td>
                <td>{{isset($val->ST_ADJUST_DOCDT) && $val->ST_ADJUST_DOCDT !='' && $val->ST_ADJUST_DOCDT !='1900-01-01' ? date('d-m-Y',strtotime($val->ST_ADJUST_DOCDT)):''}}</td>
                <td>{{isset($val->ST_ADJUST_TYPE) && $val->ST_ADJUST_TYPE !=''?$val->ST_ADJUST_TYPE:''}}</td>
