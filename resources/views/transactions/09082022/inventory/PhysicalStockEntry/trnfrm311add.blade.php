...
<div class="container-fluid topnav">
    <div class="row">
        <div class="col-lg-2">
            <a href="{{route('transaction',[311,'index'])}}" class="btn singlebt">Physical Stock Entry </a>
        </div>
        <!--col-2-->
        ...
        <div class="container-fluid filter">
            <div class="inner-form">
                ...
                <div class="col-lg-2 pl"><p>Doc No</p></div>
                            ...
                            @if(isset($objPSENO->SYSTEM_GRSR) && $objPSENO->SYSTEM_GRSR == "1")
                                <input type="text" name="DOCNO" id="DOCNO" value="{{ isset($objPSENOS)?$objPSENOS:'' }}" class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase"  >
                            
                            @elseif(isset($objPSENO->MANUAL_SR) && $objPSENO->MANUAL_SR == "1")
                                <input type="text" name="DOCNO" id="DOCNO" value="{{ old('STSNO') }}" class="form-control mandatory" maxlength="{{isset($objSQN->MANUAL_MAXLENGTH)?$objSQN->MANUAL_MAXLENGTH:''}}" autocomplete="off" style="text-transform:uppercase"  >
                                @else
                                <input type="text" name="DOCNO" id="DOCNO"  class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase"  >
                              @endif
                            </div>
             ...
             <div class="col-lg-2 pl"><p>Date</p></div>
                <div class="col-lg-2 pl">
                    <input type="hidden" id="objlastPSEDT"  value="{{$objlastPSEDT[0]->PSEDT}}">
                    <input type="date" name="PSEDT" id="PSEDT" onchange="checkPeriodClosing(311,this.value,1)" value="{{ old('PSEDT') }}" class="form-control mandatory" autocomplete="off" placeholder="dd/mm/yyyy" >
           </div>
                                ...
                                <table id="example3" class="display nowrap table table-striped table-bordered itemlist" width="100%" style="height: auto !important;">
                                    ...
                                    <tbody>
                                        <tr class="participantRow1">
                                            ...
                                            <td {{$AlpsStatus['hidden']}}><input type="text" name="Alpspartno_0" id="Alpspartno_0" class="form-control"  autocomplete="off" value="{{ isset($row->ALPS_PART_NO)?$row->ALPS_PART_NO:''}}" readonly/></td>
                                            <td {{$AlpsStatus['hidden']}}><input type="text" name="Custpartno_0" id="Custpartno_0" class="form-control"  autocomplete="off" value="{{ isset($row->CUSTOMER_PART_NO)?$row->CUSTOMER_PART_NO:''}}" readonly/></td>
                                            <td {{$AlpsStatus['hidden']}}><input type="text" name="OEMpartno_0"  id="OEMpartno_0" class="form-control"  autocomplete="off"  value="{{ isset($row->OEM_PART_NO)?$row->OEM_PART_NO:'' }}" readonly/></td>
   ...
   <td style="width:30%;"> 
    ...
    <input type="text" id="ItemGroupnamesearch" class="form-control" autocomplete="off" onkeyup="ItemGroupNameFunction()">
      ...
      <table id="ItemGroupTable2" class="display nowrap table  table-striped table-bordered" width="100%">
        ...
        <tbody >     
        @foreach ($objItemGroup as $index=>$itemcatRow)
        <tr >
        <td style="text-align:center; width:10%"> <input type="checkbox" name="salesperson[]" id="spidcode_{{ $index }}" class="clsspid"  value="{{ $itemcatRow->ITEMGID }}" ></td>



          <td style="width:30%">{{ $itemcatRow->GROUPCODE }}
          <input type="hidden" id="txtspidcode_{{ $index }}" data-desc="{{ $itemcatRow->GROUPCODE }} - {{ $itemcatRow-> GROUPNAME }}"  value="{{ $itemcatRow->ITEMGID }}"/>
          </td>
          <td style="width:60%">{{ $itemcatRow->GROUPNAME }} </td>
        </tr>
        @endforeach  
        </tbody>
      </table>
                    ...
                    <table id="ITEMCodeTable1" class="display nowrap table table-striped table-bordered" style="width:100%;">
                        <thead>
                            ...
                            <th style="width:10%;">Select</th> 
                                ...
                                <th style="width:15%;" {{$AlpsStatus['hidden']}}>{{isset($TabSetting->FIELD9) && $TabSetting->FIELD9 !=''?$TabSetting->FIELD9:'Add. Info Customer Part No'}}</th>
                                <th style="width:15%;" {{$AlpsStatus['hidden']}}>{{isset($TabSetting->FIELD10) && $TabSetting->FIELD10 !=''?$TabSetting->FIELD10:'Add. Info OEM Part No.'}}</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                            <th style="text-align:center; width:10%;">&#10004;</th>
                                ...
                                <td style="width:15%;" {{$AlpsStatus['hidden']}}>
...
$("#TIME").val(time); 
    ...
    $("[id*='PHYSICAL']").ForceNumericOnly();
        ...
        $('#btnAdd').on('click', function() {
            var viewURL = '{{route("transaction",[311,"add"])}}';
...
$("#MaterialCustom").on('click', '.add', function() {
  ...
  event.preventDefault();
        ...
        window.fnUndoYes = function (){
          //reload form
          window.location.href = "{{route('transaction',[311,'add'])}}";
       }//fnUndoYes
...
return false;
  ...
  $("#OkBtn1").focus();
    ...
    $.ajax({
        url:'{{ route("transaction",[311,"save"])}}',
        type:'POST',
        data:formData,
        success:function(data) {
          $(".buttonload").hide(); 
    ...
    $("#OkBtn").click(function(){
        ...
        $("#OkBtn").hide();
        $(".text-danger").hide();
        window.location.href = '{{route("transaction",[311,"index"]) }}';
    });
       ...
       var STID_REF = $("#FROMSTOREID_REF").val();
         ...
         if(STID_REF==''){
                ...
                $("#Itemresult").html('');
                  ...
                  $("#item_seach").show();
                  $.ajax({
                      url:'{{route("transaction",[311,"get_items"])}}',
                      type:'POST',
