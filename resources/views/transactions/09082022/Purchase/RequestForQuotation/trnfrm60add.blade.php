...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                  <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Request for Quotation (RFQ)</a>
                </div><!--col-2-->
	...
	<div class="inner-form">
		...
		<div class="row">
            ...
            @if(isset($objSON->SYSTEM_GRSR) && $objSON->SYSTEM_GRSR == "1")
                <input type="text" name="RFQ_NO" id="RFQ_NO" value="{{ $objRFQ_NO }}" class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase"  required>
            @elseif(isset($objSON->MANUAL_SR) && $objSON->MANUAL_SR == "1")
                <input type="text" name="RFQ_NO" id="RFQ_NO" value="{{ old('RFQ_NO') }}" class="form-control mandatory" maxlength="{{$objSON->MANUAL_MAXLENGTH}}" autocomplete="off" style="text-transform:uppercase"  required>
            @else
              <input type="text" name="RFQ_NO" id="RFQ_NO"  class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase"  >
            @endif
			...
			<div class="col-lg-2 pl">
				    <input type="date" name="RFQ_DT" id="RFQ_DT" onchange="checkPeriodClosing('{{$FormId}}',this.value,1)"  class="form-control mandatory"  placeholder="dd-mm-yyyy" >
      </div>
    ...
    <div class="col-lg-2 pl"><p>Vendor*</p></div>
      ...
      <div class="col-lg-2 pl">
      <input type="checkbox" name="DIRECT_RFQ" id="DIRECT_RFQ" value="1" />
    </div>
			...
			<div class="tab-content">
				...
				<div id="Material" class="tab-pane fade in active">
					<div class="table-responsive table-wrapper-scroll-y" style="height:500px;margin-top:10px;" >
						<table id="example2" class="display nowrap table table-striped table-bordered itemlist w-200" width="100%" style="height:auto !important;">
							<thead id="thead1"  style="position: sticky;top: 0">
								  <tr>
									<th>PI No<input class="form-control" type="hidden" name="Row_Count1" id ="Row_Count1" value="1"></th>
									<th>Item Code</th>
									<th>Item Name</th>
									<th>UOM</th>
                  ...
                  <td hidden><input type="text" name="MRSNO_0" id="MRSNO_0" > </td>
                    ...
                    <td><input type="text" name="PI_popup_0" id="txtPI_popup_0" class="form-control CLS_PI"  autocomplete="off"  readonly/></td>
                    <td hidden ><input type="text" name="PIID_0" id="HDNPIID_0" class="form-control" autocomplete="off" /></td>
                    ...
                    <td ><input type="text" name="BAL_PIQTY_0" id="BAL_PIQTY_0" class="form-control three-digits" maxlength="13"  autocomplete="off"  readonly/></td>
                    <td ><input type="text" name="RFQ_QTY_0" id="RFQ_QTY_0" class="form-control three-digits" maxlength="13"  autocomplete="off"  /></td>
   ...
   $('#Material').on('click','[id*="txtPI_popup"]',function(event){
        ...
        var fieldid = $(this).parent().parent().find('[id*="HDNPIID"]').attr('id');
              if(customid!=''){
                  ...
                  $.ajax({
                      url:'{{route("transaction",[$FormId,"getPI"])}}',
                      type:'POST',
...
$(document).ready(function(e) {
  ...
  var lastRFQ_DT = <?php echo json_encode($objlastRFQ_DT[0]->RFQ_DT); ?>;
  var today = new Date(); 
  var mrsdate = today.getFullYear() + "-" + ("0" + (today.getMonth() + 1)).slice(-2) + "-" + ('0' + today.getDate()).slice(-2) ;
  $('#RFQ_DT').attr('min',lastRFQ_DT);
  $('#RFQ_DT').attr('max',mrsdate);
...
$('#Material').on('focusout',"[id*='RFQ_QTY']",function()
{
  if(intRegex.test($(this).val())){
    ...
    // var temp_MRS_QTY       = parseFloat( $(this).parent().parent().find('[id*="MRS_QTY"]').val()).toFixed(3);
    // var  temp_bal_MRS_QTY  = parseFloat( $(this).parent().parent().find('[id*="BAL_SOQTY"]').val()).toFixed(3); 
    // var temp_exp_dis_qty = parseFloat( $(this).parent().parent().find('[id*="EXP_DIS_QTY"]').val() ).toFixed(3);

    // var tot_qty = parseFloat(parseFloat(temp_bal_MRS_QTY) + parseFloat(temp_exp_dis_qty)).toFixed(3);

    // if(parseFloat(temp_exp_dis_qty)>parseFloat(temp_bal_MRS_QTY)){
    //         $(this).val('');
    //         $("#alert").modal('show');
  ...
  if(formItemMst.valid()){
          ...
          event.preventDefault();
              ...
              if(jQuery.inArray("true", allblank1) !== -1 && $("#DIRECT_RFQ").prop("checked")==false){
                ...
                else if(checkPeriodClosing('{{$FormId}}',$("#RFQ_DT").val(),0) ==0){
                  $("#YesBtn").hide();
                  $("#NoBtn").hide();
                  $("#OkBtn").hide();
                  $("#OkBtn1").show();
