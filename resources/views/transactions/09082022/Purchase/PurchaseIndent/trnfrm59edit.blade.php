...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                  <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Purchase Indent (PI)</a>
                </div><!--col-2-->
    ...
    <form id="form_data"  method="POST"  > 
      @CSRF 
      {{isset($objMstResponse->PIID) ? method_field('PUT') : '' }}
	...
	<div class="inner-form">
        ...
        <input {{$ActionStatus}} type="text" name="PI_NO" id="PI_NO" value="{{ isset($objMstResponse->PI_NO)?$objMstResponse->PI_NO:'' }}" class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase"  >
			</div>
			
			<div class="col-lg-2 pl"><p>PI Date*</p></div>
			<div class="col-lg-2 pl">
				    <input {{$ActionStatus}} type="date" name="PI_DT" id="PI_DT" onchange="checkPeriodClosing('{{$FormId}}',this.value,1)"  class="form-control mandatory" value="{{ isset($objMstResponse->PI_DT)?$objMstResponse->PI_DT:'' }}"  placeholder="dd-mm-yyyy" >
      </div>
			
			<div class="col-lg-2 pl"><p>From Store*</p></div>
			<div class="col-lg-2 pl">
                <input {{$ActionStatus}} type="text" name="store_popup" id="txtstore_popup" class="form-control mandatory" value="{{ isset($objstorecode2->STCODE)?$objstorecode2->STCODE:'' }} {{ isset($objstorecode2->NAME)?'-'.$objstorecode2->NAME:'' }}" autocomplete="off" readonly/>
                <input type="hidden" name="STID_REF" id="STID_REF"  value="{{ isset($objMstResponse->STID_REF)?$objMstResponse->STID_REF:'' }}"  class="form-control" autocomplete="off" />
			</div>
    ...
    <div class="col-lg-2 pl"><p>To Department*</p></div>
			<div class="col-lg-2 pl">
                <input {{$ActionStatus}} type="text" name="dept_popup" id="txtdept_popup" value="{{ isset($objdeptcode2->DCODE)?$objdeptcode2->DCODE:'' }}  {{ isset($objdeptcode2->NAME)?'-'.$objdeptcode2->NAME:'' }}" class="form-control mandatory"  autocomplete="off" readonly/>
                <input type="hidden" name="DEPID_REF" id="DEPID_REF" value="{{ isset($objMstResponse->DEPID_REF)?$objMstResponse->DEPID_REF:'' }}" class="form-control" autocomplete="off" />
      ...
      <div class="col-lg-2 pl"><p>Direct PI</p></div>
        <div class="col-lg-2 pl">
        <input {{$ActionStatus}} type="checkbox" name="DIRECT_PI" id="DIRECT_PI" value='{{ isset($objMstResponse->DIRECT_PI) && $objMstResponse->DIRECT_PI == 1 ? 1 : 0}}' {{isset($objMstResponse->DIRECT_PI) && $objMstResponse->DIRECT_PI == 1 ? "checked" : "" }} />
      </div>
      <div class="col-lg-2 pl"><p>Remarks</p></div>
			<div class="col-lg-2 pl">
         <input {{$ActionStatus}} type="text" name="REMARKS" id="txtREMARKS" value="{{ isset($objMstResponse->REMARKS)?$objMstResponse->REMARKS:'' }}"class="form-control"  autocomplete="off" />
			</div>
    </div>  
			...
			<div class="tab-content">
				...
				<div id="Material" class="tab-pane fade in active">
					<div class="table-responsive table-wrapper-scroll-y" style="height:500px;margin-top:10px;" >
						<table id="example2" class="display nowrap table table-striped table-bordered itemlist w-200" width="100%" style="height:auto !important;">
							<thead id="thead1"  style="position: sticky;top: 0">
								  <tr>
									<th>MRS No<input class="form-control" type="hidden" name="Row_Count1" id ="Row_Count1" value="{{ $objList1Count }}"></th>
									<th>Item Code</th>
									<th>Item Name</th>
									<th>UOM</th>
                  ...
                  <th {{$AlpsStatus['hidden']}} >{{isset($TabSetting->FIELD10) && $TabSetting->FIELD10 !=''?$TabSetting->FIELD10:'Add. Info OEM Part No.'}}</th>
							...
							<tbody>
                @if(!empty($ObjItem2))
                  @foreach($ObjItem2 as $key => $row)
                  @php

                      $tmpMRSID_REF = ($row->MRSNO==0 || is_null($row->MRSNO))? '' : $row->MRSNO;
                      $tmpITEMID_REF = $row->ITEMID_REF;
                      $tmpMAIN_UOMID_REF = $row->UOMID_REF;

                      if(trim($tmpMRSID_REF)==""){
                        $CustomId = $tmpITEMID_REF; 
                      }else{
                        $CustomId = '0'.$tmpMRSID_REF.$tmpITEMID_REF.$tmpMAIN_UOMID_REF;  //custom id for unique row/item
                      }
                    

                  @endphp
								  <tr  class="participantRow">
                    <td hidden><input type="text" name="recordId_{{ $key }}" id="txtrecordId_{{ $key }}" class="form-control" value="{{ $CustomId }}"   autocomplete="off"  readonly/></td>
                    
                    <td hidden><input type="text" name="MrsID_{{ $key }}" id="HDNMrsID_{{ $key }}" value="{{ ( is_null($row->MRSNO) || $row->MRSNO==0 ) ? '' : $row->MRSNO }}"  class="form-control" autocomplete="off" /></td>
                    <td><input {{$ActionStatus}} type="text" name="MRS_popup_{{ $key }}" id="txtMRS_popup_{{ $key }}" value="{{ $row->MRS_NO }}" class="form-control"  autocomplete="off" {{ ( is_null($row->MRSNO) || $row->MRSNO==0 ) ? 'disabled' : '' }}  readonly/></td>
                    ...
                    <td><input {{$ActionStatus}} type="text" name="ItemName_{{ $key }}" id="ItemName_{{ $key }}" value="{{ $row->NAME }}"  class="form-control"  autocomplete="off"  readonly/></td>
                    <td><input {{$ActionStatus}} type="text" name="popupMUOM_{{ $key }}" id="popupMUOM_{{ $key }}" value="{{ $row->UOMCODE }} - {{ $row->DESCRIPTIONS }}" class="form-control"  autocomplete="off"  readonly/></td>
                    ...
                    <td><input {{$ActionStatus}} type="text" name="Itemspec_{{ $key }}" id="Itemspec_{{ $key }}" value="{{ $row->ITEMSPECI }}" class="form-control"  autocomplete="off"  /></td>
                    <td {{$AlpsStatus['hidden']}} ><input {{$ActionStatus}} type="text" name="Alpspartno_{{ $key }}" id="Alpspartno_{{ $key }}" value="{{ $row->ALPS_PART_NO }}" class="form-control"  autocomplete="off" readonly /></td>
                    <td {{$AlpsStatus['hidden']}} ><input {{$ActionStatus}} type="text" name="Custpartno_{{ $key }}" id="Custpartno_{{ $key }}" value="{{ $row->CUSTOMER_PART_NO }}" class="form-control"  autocomplete="off" readonly /></td>
                    <td {{$AlpsStatus['hidden']}} ><input {{$ActionStatus}} type="text" name="OEMpartno_{{ $key }}" id="OEMpartno_{{ $key }}" value="{{ $row->OEM_PART_NO }}" class="form-control"  autocomplete="off" readonly /></td>
                    <td><input {{$ActionStatus}} type="text" name="STOCK_QTY_{{ $key }}" id="txtSTOCK_QTY_{{ $key }}" value="{{ $row->STOCK_QTY }}" class="form-control three-digits" maxlength="13"  autocomplete="off"  readonly /></td>
                    <td ><input {{$ActionStatus}} type="text" name="BAL_MRSQTY_{{ $key }}" id="BAL_MRSQTY_{{ $key }}" value="{{ $row->BAL_MRSQTY }}" class="form-control three-digits" maxlength="13"  autocomplete="off"  readonly/></td>

                    <td ><input {{$ActionStatus}} type="text" name="INDENT_QTY_{{ $key }}" id="INDENT_QTY_{{ $key }}" value="{{ $row->INDENT_QTY }}"  class="form-control three-digits" maxlength="13"  autocomplete="off"  /></td>

                    <td ><input {{$ActionStatus}} type="date" name="EDA_{{ $key }}" id="EDA_{{ $key }}" value="{{ $row->EDA }}"  class="form-control" placeholder="dd-mm-yyyy" /></td>

                    <td ><input {{$ActionStatus}} type="text" name="APPROX_VAL_{{ $key }}" id="APPROX_VAL_{{ $key }}" value="{{ $row->APPROX_VL }}"  class="form-control three-digits" maxlength="13"  autocomplete="off"  /></td>
  ...
  <div class="modal-dialog modal-md column3_modal">
    ...
    <tr>
      ...
      <table id="StoreCodeTable2" class="display nowrap table  table-striped table-bordered" >
        ...
        <tbody>
        @foreach ($objStore as $index=>$glRow)
        <tr>
        <td class="ROW1"> <input type="checkbox" name="SELECT_STID_REF[]" id="storeidcode_{{ $index }}" class="clsstoreid" value="{{ $glRow-> STID }}" ></td>
          <td class="ROW2">{{ $glRow-> STCODE }} <input type="hidden" id="txtstoreidcode_{{ $index }}" data-desc="{{ $glRow-> STCODE }}-{{ $glRow-> NAME }}"  value="{{ $glRow-> STID }}"/></td>
...
<div id="ITEMIDpopup" class="modal" role="dialog"  data-backdrop="static">
  <div class="modal-dialog modal-md" style="width:90%;">
   ...
   <td style="width:9%;">
    ...
    <td style="width:9%;" {{$AlpsStatus['hidden']}} ><input type="text" id="ItemAPNsearch" class="form-control" autocomplete="off" onkeyup="ItemAPNFunction()"></td>
    <td style="width:9%;" {{$AlpsStatus['hidden']}} ><input type="text" id="ItemCPNsearch" class="form-control" autocomplete="off" onkeyup="ItemCPNFunction()"></td>
    <td style="width:9%;" {{$AlpsStatus['hidden']}} ><input type="text" id="ItemOEMPNsearch" class="form-control" autocomplete="off" onkeyup="ItemOEMPNFunction()"></td>
      ...
      $(".clsstoreid").click(function(){
        ...
        $('[id*="txtMRS_popup"]').each(function(){
          ...
          if($.trim(mrs_val)!=""){
            ...
            $("#"+$(this).parent().parent().find('[id*="APPROX_VAL"]').attr('id')).val('');
...
$("#txtdept_popup").click(function(event){
     ...
     $("#deptpopup").show();
          ...
          $('#tbody_deptacct').html('<tr><td colspan="2">Please wait..</td></tr>');
            ...
            $.ajax({
                url:'{{route("transaction",[$FormId,"getdepartment"])}}',
                type:'POST',
   ...
   $('#Material').on('click','[id*="txtMRS_popup"]',function(event){
        ...
        var fieldid = $(this).parent().parent().find('[id*="HDNMrsID"]').attr('id');
              if(customid!=''){
                  ...
                  $.ajax({
                      url:'{{route("transaction",[$FormId,"getMRS"])}}',
                      type:'POST',
...
function loadItem(taxstate,CODE,NAME,MUOM,GROUP,CTGRY,BUNIT,APART,CPART,OPART){
		...
		$.ajax({
      url:'{{route("transaction",[$FormId,"getMstItems"])}}',
			type:'POST',
...
$("#btnSaveFormData").click(function() {
    ...
    if(formItemMst.valid()){
      ...
      if(commonvalidation()){
        ...
        if(checkPeriodClosing('{{$FormId}}',$("#PI_DT").val(),0) ==0){
          $("#YesBtn").hide();
          $("#NoBtn").hide();
          $("#OkBtn").hide();
          $("#OkBtn1").show();
