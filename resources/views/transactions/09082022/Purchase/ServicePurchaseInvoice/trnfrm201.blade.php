...
@section('content')    
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                  <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">@if($checkCompany =='')Service Purchase Invoice @else Service GRN @endif</a>
                </div><!--col-2-->
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->SPIID}}" value="{{$val->SPIID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->SPI_DT) && $val->SPI_DT !='' && $val->SPI_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->SPI_DT)):''}}" ></td>
              <td>{{isset($val->SPI_NO) && $val->SPI_NO !=''?$val->SPI_NO:''}}</td>
