  ...
  <div class="multiple table-responsive  ">
    <table id="production_order_listing" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp	
        <tr>
          <td><input type="checkbox" id="chkId{{$val->PMSL_ID}}" value="{{$val->PMSL_ID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->PMSL_DATE) && $val->PMSL_DATE !='' && $val->PMSL_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->PMSL_DATE)):''}}" ></td>
          <td>{{isset($val->PMSL_NO) && $val->PMSL_NO !=''?$val->PMSL_NO:''}}</td>
          <td>{{isset($val->PMSL_DATE) && $val->PMSL_DATE !='' && $val->PMSL_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->PMSL_DATE)):''}}</td>
