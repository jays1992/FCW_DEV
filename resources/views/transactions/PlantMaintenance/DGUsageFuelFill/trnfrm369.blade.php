  ...
  <div class="multiple table-responsive  ">
    <table id="production_order_listing" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      @if(!empty($objDataList))           
            ...
            @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->DGUFF_ID}}" value="{{$val->DGUFF_ID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->DGUFF_DATE) && $val->DGUFF_DATE !='' && $val->DGUFF_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->DGUFF_DATE)):''}}" ></td>
          <td>{{isset($val->DGUFF_NO) && $val->DGUFF_NO !=''?$val->DGUFF_NO:''}}</td>
          <td>{{isset($val->DGUFF_DATE) && $val->DGUFF_DATE !='' && $val->DGUFF_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->DGUFF_DATE)):''}}</td>
