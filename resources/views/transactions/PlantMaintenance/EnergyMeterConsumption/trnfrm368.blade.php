  ...
  <div class="multiple table-responsive  ">
    <table id="production_order_listing" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      @if(!empty($objDataList))           
            ...
            @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->EMC_ID}}" value="{{$val->EMC_ID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->EMC_DATE) && $val->EMC_DATE !='' && $val->EMC_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->EMC_DATE)):''}}" ></td>
          <td>{{isset($val->EMC_NO) && $val->EMC_NO !=''?$val->EMC_NO:''}}</td>
          <td>{{isset($val->EMC_DATE) && $val->EMC_DATE !='' && $val->EMC_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->EMC_DATE)):''}}</td>
          <td>{{isset($val->METER_CODE) && $val->METER_CODE !=''?$val->METER_CODE:''}}</td>
          <td>{{isset($val->METER_DESC) && $val->METER_DESC !=''?$val->METER_DESC:''}}</td>
