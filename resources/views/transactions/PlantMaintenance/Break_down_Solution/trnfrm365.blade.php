    ...
    <table id="production_order_listing" class="display nowrap table table-striped table-bordered" width="100%">
      <thead id="thead1">
        <tr>
          ...
          <th>Solution No</th>
          <th>Solution Date</th>
          <th>Breakdown Complaint No</th>
          <th>Breakdown Complaint Date</th>
          <th>Breakdown Complaint Status</th>
          <th>Complaint By</th>
          <th>Created Date</th>
          <th>Created By</th>
          <th>Status</th>
      ...
      @if(!empty($objDataList))           
            ...
            @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->BDSL_ID}}" value="{{$val->BDSL_ID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->BDSL_DATE) && $val->BDSL_DATE !='' && $val->BDSL_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->BDSL_DATE)):''}}" ></td>
          <td>{{isset($val->BDSL_NO) && $val->BDSL_NO !=''?$val->BDSL_NO:''}}</td>
          <td>{{isset($val->BDSL_DATE) && $val->BDSL_DATE !='' && $val->BDSL_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->BDSL_DATE)):''}}</td>
          <td>{{isset($val->BDCL_NO) && $val->BDCL_NO !=''?$val->BDCL_NO:''}}</td>
          <td>{{isset($val->BDCL_DATE) && $val->BDCL_DATE !='' && $val->BDCL_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->BDCL_DATE)):''}}</td>
          <td>{{isset($val->BREAKDOWN_STATUS) && $val->BREAKDOWN_STATUS !=''?$val->BREAKDOWN_STATUS:''}}</td>
          <td>{{isset($val->COMPLAINT_BY) && $val->COMPLAINT_BY !=''?$val->COMPLAINT_BY:''}}</td>
