  ...
  <div class="multiple table-responsive  ">
    <table id="production_order_listing" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->PMAL_ID}}" value="{{$val->PMAL_ID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->PMAL_DATE) && $val->PMAL_DATE !='' && $val->PMAL_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->PMAL_DATE)):''}}" ></td>
          <td>{{isset($val->PMAL_NO) && $val->PMAL_NO !=''?$val->PMAL_NO:''}}</td>
          <td>{{isset($val->PMAL_DATE) && $val->PMAL_DATE !='' && $val->PMAL_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->PMAL_DATE)):''}}</td>
