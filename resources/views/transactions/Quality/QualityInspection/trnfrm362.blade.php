  ...
  <div class="multiple table-responsive  ">
    <table id="production_order_listing" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp	
        <tr>
          <td><input type="checkbox" id="chkId{{$val->QIGID}}" value="{{$val->QIGID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->QIGDT) && $val->QIGDT !='' && $val->QIGDT !='1900-01-01' ? date('d-m-Y',strtotime($val->QIGDT)):''}}" ></td>
          <td>{{isset($val->QIGNO) && $val->QIGNO !=''?$val->QIGNO:''}}</td>
          <td>{{isset($val->QIGDT) && $val->QIGDT !='' && $val->QIGDT !='1900-01-01' ? date('d-m-Y',strtotime($val->QIGDT)):''}}</td>
		      <td>{{isset($val->VENDOR_NAME) ? $val->VENDOR_NAME :""}}</td>
