...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">AR Debit / Credit Note</a>
                </div>
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            <tr>
                  ...
                  <th width="10%">AR Debit Note No.</th>
                  <th width="10%">AR Debit Note Date</th>
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->ARDRCRID}}" value="{{$val->ARDRCRID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->AR_DOC_DT) && $val->AR_DOC_DT !=''?$val->AR_DOC_DT:''}}" ></td>
                <td>{{isset($val->AR_DOC_NO) && $val->AR_DOC_NO !=''?$val->AR_DOC_NO:''}}</td>
                <td>{{isset($val->AR_DOC_DT) && $val->AR_DOC_DT !=''?$val->AR_DOC_DT:''}}</td>
                <td>{{isset($val->SGLCODE) && $val->SGLCODE !==''?$val->SGLCODE:''}}</td>
                <td>{{isset($val->SLNAME) && $val->SLNAME !==''?$val->SLNAME:''}}</td>
                <td>{{isset($val->AR_TYPE) && $val->AR_TYPE !=''?$val->AR_TYPE:''}}</td>
                <td>{{isset($val->REASON_DRCR_NOTE) && $val->REASON_DRCR_NOTE !=''?$val->REASON_DRCR_NOTE:''}}</td></td>
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  if(is_approve==2){
                    ...
                    $("#OkBtn").hide();
                    $("#OkBtn1").show();
                    $("#AlertMessage").text('Due to some technical issue, this functionality not avaiable.');
                    $("#alert").modal('show');
                    $("#OkBtn1").focus();
                    // $("#YesBtn").show();
                    // $("#NoBtn").show();
