...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">AP Debit / Credit Note</a>
                </div>
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            <tr>
                  ...
                  <th >AP Debit Note No.</th>
                  <th >AP Debit Note Date</th>
                  <th >Customer Code</th>
                  <th >Customer Name</th>
                  <th >Type</th>
                  <th>Reason</th>
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->APDRCRID}}" value="{{$val->APDRCRID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->AP_DOC_DT) && $val->AP_DOC_DT !=''?$val->AP_DOC_DT:''}}" ></td>
                <td>{{isset($val->AP_DOC_NO) && $val->AP_DOC_NO !=''?$val->AP_DOC_NO:''}}</td>
                <td>{{isset($val->AP_DOC_DT) && $val->AP_DOC_DT !=''?$val->AP_DOC_DT:''}}</td>
                <td>{{isset($val->SGLCODE) && $val->SGLCODE !==''?$val->SGLCODE:''}}</td>
                <td>{{isset($val->SLNAME) && $val->SLNAME !==''?$val->SLNAME:''}}</td>
                <td>{{isset($val->AP_TYPE) && $val->AP_TYPE !=''?$val->AP_TYPE:''}}</td>
