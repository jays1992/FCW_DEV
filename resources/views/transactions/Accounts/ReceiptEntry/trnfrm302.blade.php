...
@inject('helper', 'App\Helpers\Helper')        
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            if($val->RECEIPT_FOR=='Account'){
              $Name = $helper->get_gldetail_PaymentReceipt($val->RECEIPTID,'Receipt');
            }else{
              $Name=$val->SLNAME;   
            }
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->RECEIPTID}}" value="{{$val->RECEIPTID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->RECEIPT_DT) && $val->RECEIPT_DT !=''?$val->RECEIPT_DT:''}}" ></td>
                <td>{{isset($val->RECEIPT_NO) && $val->RECEIPT_NO !=''?$val->RECEIPT_NO:''}}</td>
                <td>{{isset($val->RECEIPT_DT) && $val->RECEIPT_DT !=''?$val->RECEIPT_DT:''}}</td>
                <td>{{isset($val->RECEIPT_FOR) && $val->RECEIPT_FOR !=''?$val->RECEIPT_FOR:''}}</td>                
                <td>{{isset($Name) && $Name !=''?$Name:''}}</td>
                <td>{{isset($val->RECEIPT_TYPE) && $val->RECEIPT_TYPE !=''?$val->RECEIPT_TYPE:''}}</td>
                <td>{{isset($val->NARRATION) && $val->NARRATION !=''?$val->NARRATION:''}}</td></td>
...
$('#btnPdf').on('click', function() {
      ...
      {
            var POID = soIdsData[0];
            var Flag = 'P';
            var formData = 'PO='+ POID + '&RECEIPTID='+ POID + '&Flag='+ Flag ;
