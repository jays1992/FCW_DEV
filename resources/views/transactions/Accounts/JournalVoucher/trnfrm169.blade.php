...
@inject('helper', 'App\Helpers\Helper')  
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Journal Voucher(JV)</a>
                </div>
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            <tr>
                  ...
                  <th width="15%">JV No</th>
                  <th width="15%">JV Date</th>
            ...
            if($val->SLNAME==''){
              $Name = $helper->get_gldetail_PaymentReceipt($val->JVID,'JV');
            }else{
              $Name=$val->SLNAME;   
            }
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->JVID}}" value="{{$val->JVID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->JV_DT) && $val->JV_DT !=''?$val->JV_DT:''}}" ></td>
                <td>{{isset($val->JV_NO) && $val->JV_NO !=''?$val->JV_NO:''}}</td>
                <td>{{isset($val->JV_DT) && $val->JV_DT !=''?$val->JV_DT:''}}</td>
                <td>{{isset($val->REVERSE_DT) && $val->REVERSE_DT !=''?$val->REVERSE_DT:''}}</td>
				<td>{{isset($val->SOURCE_DOCTYPE) && $val->SOURCE_DOCTYPE !=''?$val->SOURCE_DOCTYPE:''}}</td>
                ...
                if(seletedRecord==0){
                    ...
                    highlighFocusBtn('activeOk1');
                      ...
                      if(is_approve==1){
                        ...
                        var editURL = '{{route("transaction",[169,"amendment",":rcdId"]) }}';
                            editURL = editURL.replace(":rcdId",recordId);
