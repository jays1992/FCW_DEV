...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                ...
                <!--<button class="btn topnavbt" id="btnAmendment" {{isset($objRights->AMENDMENT) && $objRights->AMENDMENT != 1 ? 'disabled' : ''}} > Amendment</button>-->
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            <tr>
                  ...
                  <th width="15%">Purchase Bill Doc No.</th>
                  <th width="15%">Purchase Bill Doc Date</th>
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->PBID}}" value="{{$val->PBID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->PB_DOCDT) && $val->PB_DOCDT !=''?$val->PB_DOCDT:''}}" ></td>
                <td>{{isset($val->PB_DOCNO) && $val->PB_DOCNO !=''?$val->PB_DOCNO:''}}</td>
                <td>{{isset($val->PB_DOCDT) && $val->PB_DOCDT !=''?$val->PB_DOCDT:''}}</td>
                <td>{{isset($val->DCODE) && $val->DCODE !=''?$val->DCODE:''}}</td>
                <td>{{isset($val->SLNAME) && $val->SLNAME !=''?$val->SLNAME:''}}</td>
                <td>{{isset($val->VENDOR_INNO) && $val->VENDOR_INNO !=''?$val->VENDOR_INNO:''}}</td>
                <td>{{isset($val->GRN_NO) && $val->GRN_NO !=''?$val->GRN_NO:''}}</td>
                <td>{{isset($val->REMARKS) && $val->REMARKS !=''?$val->REMARKS:''}}</td></td>
