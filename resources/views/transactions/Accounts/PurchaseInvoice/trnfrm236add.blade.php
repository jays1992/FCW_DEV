...
@section('content')
	...
	<div class="inner-form">
        ...
        <div class="col-lg-2 pl">
              <input type="text" name="PB_DOCNO" id="PB_DOCNO" value="{{$docarray['DOC_NO']}}" {{$docarray['READONLY']}} class="form-control" maxlength="{{$docarray['MAXLENGTH']}}" autocomplete="off" style="text-transform:uppercase" >
              <script>docMissing(@json($docarray['FY_FLAG']));</script>
        </div>
        <div class="col-lg-1 pl"><p>PB Doc Date</p></div>
        <div class="col-lg-2 pl">
                  <input type="date" name="PB_DOCDT" id="PB_DOCDT" value="{{ old('PB_DOCDT') }}" onchange='checkPeriodClosing("{{$FormId}}",this.value,1),getDocNoByEvent("PB_DOCNO",this,@json($doc_req))' class="form-control mandatory"  placeholder="dd/mm/yyyy" />
        </div>
