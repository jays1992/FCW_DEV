...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->GEJWOID}}" value="{{$val->GEJWOID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->GEDT) && $val->GEDT !='' && $val->GEDT !='1900-01-01' ? date('d-m-Y',strtotime($val->GEDT)):''}}" ></td>
                <td>{{isset($val->GENO) && $val->GENO !=''?$val->GENO:''}}</td>
                <td>{{isset($val->GEDT) && $val->GEDT !='' && $val->GEDT !='1900-01-01' ? date('d-m-Y',strtotime($val->GEDT)):''}}</td>
