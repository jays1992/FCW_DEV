...
<div class="container-fluid topnav">
  <div class="row">
    <div class="col-lg-2"><a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Job Work GRN</a></div>
  ...
  <div class="multiple table-responsive  ">
    <table id="production_order_listing" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->GRJID}}" value="{{$val->GRJID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->GRNDT) && $val->GRNDT !='' && $val->GRNDT !='1900-01-01' ? date('d-m-Y',strtotime($val->GRNDT)):''}}" ></td>
          <td>{{isset($val->GRNNO) && $val->GRNNO !=''?$val->GRNNO:''}}</td>
          <td>{{isset($val->GRNDT) && $val->GRNDT !='' && $val->GRNDT !='1900-01-01' ? date('d-m-Y',strtotime($val->GRNDT)):''}}</td>
...
<div id="ReportView" class="modal" role="dialog"  data-backdrop="static"  >
  <div class="modal-dialog modal-md" style="width:90%; height:90%">
    ...
    <div class="modal-body">
	  <div class="tablename"><p>Job Work GRN Print</p></div>
        <div class="row">
  ...
  var soIdsData = getSeletectedCBox();
    var seletedRecord = soIdsData.length;
      ...
      }else if(seletedRecord==1)
      {
            ...
            var Flag = 'P';
            var formData = 'SO='+ SONO + '&GRJID='+ SONO + '&Flag='+ Flag ;
            var consultURL = '{{route("transaction",[357,"ViewReport",":rcdId"])}}';
...
$('#btnPrint').on('click', function() {
      ...
      }else if(seletedRecord==1)
      {
            ...
            $.ajax({
                url:'{{route("transaction",[357,"ViewReport"])}}',
                type:'POST',
                data:formData,
                success:function(data) {
                    $('#ReportView').show();
