...
<div class="container-fluid topnav">
  <div class="row">
    <div class="col-lg-2"><a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Job Work Return (JWR)</a></div>
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->JWRID}}" value="{{$val->JWRID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->JWRDT) && $val->JWRDT !='' && $val->JWRDT !='1900-01-01' ? date('d-m-Y',strtotime($val->JWRDT)):''}}" ></td>
                <td>{{isset($val->JWRNO) && $val->JWRNO !=''?$val->JWRNO:''}}</td>
                <td>{{isset($val->JWRDT) && $val->JWRDT !='' && $val->JWRDT !='1900-01-01' ? date('d-m-Y',strtotime($val->JWRDT)):''}}</td>
