  ...
  <div class="multiple table-responsive  ">
    <table id="production_order_listing" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->JWCID}}" value="{{$val->JWCID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->JWCDT) && $val->JWCDT !='' && $val->JWCDT !='1900-01-01' ? date('d-m-Y',strtotime($val->JWCDT)):''}}" ></td>
          <td>{{isset($val->JWCNO) && $val->JWCNO !=''?$val->JWCNO:''}}</td>
          <td>{{isset($val->JWCDT) && $val->JWCDT !='' && $val->JWCDT !='1900-01-01' ? date('d-m-Y',strtotime($val->JWCDT)):''}}</td>
...
<div id="ReportView" class="modal" role="dialog"  data-backdrop="static"  >
  <div class="modal-dialog modal-md" style="width:90%; height:90%">
    ...
    <div class="modal-body">
	  <div class="tablename"><p>Job Work Challan Print</p></div>
        <div class="row">
  ...
  var soIdsData = getSeletectedCBox();
    var seletedRecord = soIdsData.length;
      ...
      }else if(seletedRecord==1)
      {
            ...
            var Flag = 'P';
            var formData = 'SO='+ SONO + '&JWCID='+ SONO + '&Flag='+ Flag ;
            var consultURL = '{{route("transaction",[355,"ViewReport",":rcdId"])}}';
...
$('#btnPrint').on('click', function() {
      ...
      }else if(seletedRecord==1)
      {
            ...
            $.ajax({
                url:'{{route("transaction",[355,"ViewReport"])}}',
                type:'POST',
                data:formData,
                success:function(data) {
                    $('#ReportView').show();
