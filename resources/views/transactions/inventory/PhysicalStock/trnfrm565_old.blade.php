...
@section('content')    
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                  <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Dispatch Goods</a>
                </div>
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->DGID}}" value="{{$val->DGID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->docdate) && $val->docdate !='' && $val->docdate !='1900-01-01' ? date('d-m-Y',strtotime($val->docdate)):''}}" ></td>
              <td>{{isset($val->DOCNO) && $val->DOCNO !=''?$val->DOCNO:''}}</td>
              <td>{{isset($val->DOCDT) && $val->DOCDT !='' && $val->DOCDT !='1900-01-01' ? date('d-m-Y',strtotime($val->DOCDT)):''}}</td>
