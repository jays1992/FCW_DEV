...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->NRGPID}}" value="{{$val->NRGPID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->NRGP_DT) && $val->NRGP_DT !='' && $val->NRGP_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->NRGP_DT)):''}}" ></td>
                <td>{{isset($val->NRGP_NO) && $val->NRGP_NO !=''?$val->NRGP_NO:''}}</td>
                <td>{{isset($val->NRGP_DT) && $val->NRGP_DT !='' && $val->NRGP_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->NRGP_DT)):''}}</td>
