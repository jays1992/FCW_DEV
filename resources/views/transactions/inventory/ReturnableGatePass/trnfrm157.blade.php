...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->RGPID}}" value="{{$val->RGPID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->RGP_DT) && $val->RGP_DT !='' && $val->RGP_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->RGP_DT)):''}}" ></td>
                <td>{{isset($val->RGP_NO) && $val->RGP_NO !=''?$val->RGP_NO:''}}</td>
                <td>{{isset($val->RGP_DT) && $val->RGP_DT !='' && $val->RGP_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->RGP_DT)):''}}</td>
