...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Annual Forecast Purchase(AFP)</a>
                </div>
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            <tr>
                ...
                <th width="15%">AFP No</th>
                        <th width="15%">AFP Date</th>
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->AFPID}}" value="{{$val->AFPID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->AFP_DT) && $val->AFP_DT !=''?$val->AFP_DT:''}}" ></td>
                <td>{{isset($val->AFP_NO) && $val->AFP_NO !=''?$val->AFP_NO:''}}</td>
                <td>{{isset($val->AFP_DT) && $val->AFP_DT !=''?$val->AFP_DT:''}}</td>
                <td>{{isset($val->NAME) && $val->NAME !=''?$val->NAME:''}}</td>
                <td>{{isset($val->FYDESCRIPTION) && $val->FYDESCRIPTION !=''?$val->FYDESCRIPTION:''}}</td>
                ...
                if(seletedRecord==0){
                    ...
                    highlighFocusBtn('activeOk1');
                      ...
                      if(is_approve==1){
                        ...
                        var editURL = '{{route("transaction",[220,"amendment",":rcdId"]) }}';
                            editURL = editURL.replace(":rcdId",recordId);
                            window.location.href=editURL;
                      }else if(is_approve==2){
