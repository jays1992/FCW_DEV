...
@section('content')    
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->VQID}}" value="{{$val->VQID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->VQ_DT) && $val->VQ_DT !='' && $val->VQ_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->VQ_DT)):''}}" ></td>
              <td>{{isset($val->VQ_NO) && $val->VQ_NO !=''?$val->VQ_NO:''}}</td>
              <td>{{isset($val->VQ_DT) && $val->VQ_DT !='' && $val->VQ_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->VQ_DT)):''}}</td>
