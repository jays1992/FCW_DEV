...
@section('content')    
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                  <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Service Purchase Order</a>
                </div><!--col-2-->
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->SPOID}}" value="{{$val->SPOID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->SPO_DT) && $val->SPO_DT !='' && $val->SPO_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->SPO_DT)):''}}" ></td>
              <td>{{isset($val->SPO_NO) && $val->SPO_NO !=''?$val->SPO_NO:''}}</td>
              ...
              <td>{{isset($val->VALID_FR) && $val->VALID_FR !='' && $val->VALID_FR !='1900-01-01' ? date('d-m-Y',strtotime($val->VALID_FR)):''}}</td>
              <td>{{isset($val->VALID_TO) && $val->VALID_TO !='' && $val->VALID_TO !='1900-01-01' ? date('d-m-Y',strtotime($val->VALID_TO)):''}}</td>
...
<div id="ReportView" class="modal" role="dialog"  data-backdrop="static"  >
  <div class="modal-dialog modal-md" style="width:50%; height:60%">
    ...
    <div class="modal-body">
	  <div class="tablename"><p>Service Purchase Order Print</p></div>
        <div class="row">
...
function highlighFocusBtn(pclass){
    ...
    $('#btnPdf').on('click', function() {
      ...
      }else if(seletedRecord==1)
      {
            ...
            var Flag = 'P';
            var formData = 'PO='+ POID + '&SPOID='+ POID + '&Flag='+ Flag ;
            var consultURL = '{{route("transaction",[69,"ViewReport",":rcdId"]) }}';
...
$('#btnPrint').on('click', function() {
      ...
      }else if(seletedRecord==1)
      {
            ...
            $.ajax({
                url:'{{route("transaction",[69,"ViewReport"])}}',
                type:'POST',
                data:formData,
                success:function(data) {
                    $('#ReportView').show();
