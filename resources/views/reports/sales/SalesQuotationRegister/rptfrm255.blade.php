...
@extends('layouts.app')
@section('content')
<!-- <form id="frm_rpt_sislw" onsubmit="return validateForm()"  method="POST" class="needs-validation"  >     -->

    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-3">
                <a href="{{route('report',[255,'index'])}}" class="btn singlebt">Sales Quotation Register</a>
                </div><!--col-2-->
    ...
    <div class="container-fluid purchase-order-view">
        <form id="frm_rpt_sqs"  method="POST">   
            @csrf
                    ...
                    <div class="inner-form">
                        ...
                        <div class="row"> 
                            ...
                            <div class="col-lg-3 pl" id="div_cust">
                                ...
                                @foreach($objSubGL as $cindex=>$cRow)
                                        <option value="{{$cRow->SGLID}}" selected>{{$cRow->SGLCODE}}-{{$cRow->SLNAME}}</option>
                                @endforeach
                                </select>
                            </div>                            
                            <div class="col-lg-3 pl"><p>Sales Quotation</p></div>
                            <div class="col-lg-3 pl" id="div_sqno">
                                <select name="SQNO[]" id="SQNO" data-hide-disabled="hide" multiple data-actions-box="true" class="form-control selectpicker" multiple data-live-search="true"  >
                                @foreach($objQuotation as $Pindex=>$PRow)
                                        <option value="{{$PRow->SQNO}}" selected>{{$PRow->SQNO}}</option>
                                @endforeach
                                </select>
                            </div> 
                        </div>
...
$('#btnPdf').on('click', function() {
    ...
    var consultURL = '{{route("report",[255,"ViewReport",":rcdId"]) }}';
...
$('#btnView').on('click', function() {
        ...
        var SQNO  = [];
        $("select[name='SQNO[]']").each(function() {
            var value4 = $(this).val();
            if (value4) {
                SQNO.push(value4);
            }
        ...
        else if(SQNO  == '')
        {
            ...
            $("#OkBtn").hide();
            $("#OkBtn1").show();
            $("#AlertMessage").text('Please Select Sales Quotation.');
            $("#alert").modal('show');
            $("#OkBtn1").focus();
            return false;
        }
        else if(ITEMID  == '')
        {
            ...
            $.ajaxSetup({
                ...
                $(".buttonload").show();
                $.ajax({
                    url:'{{route("report",[255,"ViewReport"])}}',
                    type:'POST',
                    data:formData,
