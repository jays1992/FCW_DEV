...
@section('content')
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-4">
                <a href="{{route('report',[256,'index'])}}" class="btn singlebt">Sales Order Register</a>
                </div><!--col-2-->
                    ...
                    <div class="inner-form">
                        ...
                        <div class="row"> 
                            ...
                            <div class="col-lg-3 pl" id="div_sperson">
                                <select name="EMPID[]" id="EMPID" data-hide-disabled="hide" multiple data-actions-box="true" class="form-control selectpicker" multiple data-live-search="true"  >
                        ...
                        <div class="row">   
                        <div class="col-lg-3 pl"><p>Sales Order No</p></div>
                            <div class="col-lg-3 pl" id="div_itemgrp">
                                <select name="SONO[]" id="SONO" data-hide-disabled="hide" multiple data-actions-box="true" class="form-control selectpicker" multiple data-live-search="true"  >
                                @foreach($ObjSONO as $Gindex=>$GRow)
                                        <option value="{{$GRow->SOID}}" selected>{{$GRow->SONO}}</option>
                                @endforeach
                                </select>
                            </div>   
                            </div>   
...
$('#btnPdf').on('click', function() {
    ...
    var consultURL = '{{route("report",[256,"ViewReport",":rcdId"]) }}';
...
$('#btnView').on('click', function() {
        ...
        var SONO = [];
        $("select[name='SONO[]']").each(function() {
            var value5 = $(this).val();
            if (value5) {
                SONO.push(value5);
            }
        ...
        else if(SONO  == '')
        {
            ...
            var formData = trnsoForm.serialize();
                ...
                $(".buttonload").show();
                $.ajax({
                    url:'{{route("report",[256,"ViewReport"])}}',
                    type:'POST',
                    data:formData,
