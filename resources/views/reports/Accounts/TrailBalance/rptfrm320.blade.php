...
<!-- <form id="frm_rpt_sis" onsubmit="return validateForm()"  method="POST" class="needs-validation"  >     -->
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-4">
                <a href="{{route('report',[320,'index'])}}" class="btn singlebt">General Ledger Trail Balance</a>
                </div><!--col-2-->
                    ...
                    <div class="inner-form">
                        ...
                        <div class="row"> 
                            ...
                            <div class="col-lg-3 pl" id="div_gl">
                                ...
                                @foreach($ObjGeneralLedger as $cindex=>$cRow)
                                        <option value="{{$cRow->GLID}}" selected>{{$cRow->GLCODE}}-{{$cRow->GLNAME}}</option>
                                @endforeach
                                </select>
                            </div>
                            <div class="col-lg-3 pl"><p>Account Group</p></div>
                            <div class="col-lg-3 pl" id="div_ag">
                                <select name="AGID[]" data-hide-disabled="hide" multiple data-actions-box="true" id="AGID" class="form-control selectpicker" multiple data-live-search="true"  >
                                @foreach($ObjAccountGroup as $cindex=>$cRow)
                                        <option value="{{$cRow->AGID}}" selected>{{$cRow->AGCODE}}-{{$cRow->AGNAME}}</option>
                                @endforeach
                                </select>
                            </div>
                        </div>
						<div class="row"> 
                            <div class="col-lg-3 pl"><p>Report Level</p></div>
                            <div class="col-lg-3 pl" id="div_rl">
                                <select name="LEVELID" data-hide-disabled="hide"  data-actions-box="true" id="LEVELID" class="form-control selectpicker"  data-live-search="true"  >
...
$('#btnPdf').on('click', function() {
    ...
    var consultURL = '{{route("report",[320,"ViewReport",":rcdId"]) }}';
...
$('#btnView').on('click', function() {
        ...
        var AGID = [];
        $("select[name='AGID[]']").each(function() {
            var value3 = $(this).val();
            if (value3) {
                AGID.push(value3);
            }
        ...
        else if(AGID  == '')
        {
            ...
            $("#OkBtn").hide();
            $("#OkBtn1").show();
            $("#AlertMessage").text('Please Select Account Group.');
            $("#alert").modal('show');
            $("#OkBtn1").focus();
            return false;
        }
		else if(LEVELID ==="")
        {
            ...
            $("#OkBtn").hide();
            $("#OkBtn1").show();
            $("#AlertMessage").text('Please Select Level');
            $("#alert").modal('show');
            $("#OkBtn1").focus();
            return false;
        }
            ...
            $.ajaxSetup({
                ...
                $(".buttonload").show();
                $.ajax({
                    url:'{{route("report",[320,"ViewReport"])}}',
                    type:'POST',
                    data:formData,
