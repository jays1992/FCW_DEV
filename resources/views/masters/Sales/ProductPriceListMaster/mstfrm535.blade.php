...
<div class="container-fluid topnav">
  <div class="row">
    <div class="col-lg-2">
      <a href="{{route('master',[$FormId,'index'])}}" class="btn singlebt">Product Price List Master</a>
    </div>
  ...
  <div class="multiple table-responsive  ">
    <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->PPLM_ID}}" value="{{$val->PPLM_ID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
          <td>{{isset($val->PPLM_NO) && $val->PPLM_NO !=''?$val->PPLM_NO:''}}</td>
          <td>{{isset($val->PPLM_DATE) && $val->PPLM_DATE !=''?$val->PPLM_DATE:''}}</td>
