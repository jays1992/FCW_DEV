      ...
      <div class="inner-form">         
        <div class="row">
          ...
          <div class="col-lg-2 pl">
            <input {{$ActionStatus}} type="hidden"  name="DOC_ID"     id="DOC_ID" value="{{isset($HDR->VCADD_ID)?$HDR->VCADD_ID:''}}" >
            <input {{$ActionStatus}} type="text"    name="DOC_NO"  id="DOC_NO"  value="{{isset($HDR->VCADD_NO)?$HDR->VCADD_NO:''}}"  class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase"  >
          </div>

          <div class="col-lg-2 pl"><p>Document Date*</p></div>
          <div class="col-lg-2 pl">
            <input {{$ActionStatus}} type="date" name="DOC_DATE" id="DOC_DATE" value="{{isset($HDR->VCADD_DATE)?$HDR->VCADD_DATE:''}}"  class="form-control" autocomplete="off" placeholder="dd/mm/yyyy" readonly >
          </div>
                ...
                <table id="example2" class="display nowrap table table-striped table-bordered itemlist w-200" width="100%" style="height:auto !important;">
                  ...
                  <tbody>
                    ...
                    <tr class="participantRow">
                      <td hidden><input type="text" name="DETAIL_ID[]" id="DETAIL_ID_{{$key}}" value="{{isset($row->DETAIL_ID_REF)?$row->DETAIL_ID_REF:''}}"     class="form-control"  autocomplete="off" /></td>
                      <td hidden><input type="text" name="AMOUNT_ID[]" id="AMOUNT_ID_{{$key}}" value="{{isset($row->AMOUNT_ID)?$row->AMOUNT_ID:''}}"    class="form-control"  autocomplete="off" /></td>
                      <td><input {{$ActionStatus}} type="text" name="CARD_NO[]" id="CARD_NO_{{$key}}" value="{{isset($row->CARD_NO)?$row->CARD_NO:''}}"      class="form-control"  autocomplete="off" maxlength="15" onclick="getCardMaster(this.id)" readonly  /></td>
                      <td><input {{$ActionStatus}} type="text" name="AMOUNT[]" id="AMOUNT_{{$key}}"   value="{{isset($row->AMOUNT)?$row->AMOUNT:''}}"     class="form-control"  autocomplete="off"   readonly/></td>
                      <td><input {{$ActionStatus}} type="text" name="BALANCE_AMT[]" id="BALANCE_AMT_{{$key}}" value="{{isset($row->BALANCE_AMT)?floatval($row->BALANCE_AMT):''}}"     class="form-control"  autocomplete="off"  readonly/></td>
                      <td><input {{$ActionStatus}} type="text" name="NET_AMOUNT[]"  id="NET_AMOUNT_{{$key}}" value="{{isset($row->NET_AMOUNT)?$row->NET_AMOUNT:''}}"  class="form-control"  autocomplete="off"  readonly/></td>
                      <td><input {{$ActionStatus}} type="text" name="VALIDITY_MON[]" id="VALIDITY_MON_{{$key}}" value="{{isset($row->VALIDITY_MON)?$row->VALIDITY_MON:''}}"  class="form-control"  autocomplete="off" onkeyup="getValidityDate()" /></td>
                      <td><input {{$ActionStatus}} type="date" name="VALIDITY_TILL[]" id="VALIDITY_TILL_{{$key}}" value="{{isset($row->VALIDITY_TILL)?$row->VALIDITY_TILL:''}}" class="form-control"  autocomplete="off"  readonly/></td>
                      <td><input {{$ActionStatus}} type="text" name="ADD_VALUE[]" id="ADD_VALUE_{{$key}}"  value="{{isset($row->ADD_AMOUNT)?$row->ADD_AMOUNT:''}}"    class="form-control"  autocomplete="off" /></td>
                      <td align="center" >
