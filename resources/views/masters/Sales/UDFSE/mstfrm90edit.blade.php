...
<!-- <form id="frm_mst_se" onsubmit="return validateForm()"  method="POST"  >     -->
    ...
    @CSRF
          ...
          {{isset($objUdfForSE->UDFSEID[0]) ? method_field('PUT') : '' }}
            <table id="example2" class="display nowrap table table-striped table-bordered itemlist" width="100%" style="height:auto !important;">
                ...
                <tbody>
                @if(!empty($objUdfForSE))
                @foreach($objUdfForSE as $key => $row)
                    <tr  class="participantRow">
                        <td hidden>
                        <input  class="form-control" type="hidden" name={{"UDFSEID_".$key}} id ={{"txtID_".$key}} maxlength="100" value="{{ $row->UDFSEID }}" autocomplete="off"   >
                        </td>
                        <td>
                        ...
                        <i class="fa fa-plus"></i></button><button class="btn remove" title="Delete" data-toggle="tooltip" >
