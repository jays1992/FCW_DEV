...
<div class="container-fluid topnav">
  <div class="row">
    <div class="col-lg-2">
      <a href="{{route('master',[$FormId,'index'])}}" class="btn singlebt">Vehicle Make Master</a>
    </div>
  ...
  <div class="multiple table-responsive  ">
    <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->VM_ID}}" value="{{$val->VM_ID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
          <td>{{isset($val->VM_NO) && $val->VM_NO !=''?$val->VM_NO:''}}</td>
          <td>{{isset($val->VM_DATE) && $val->VM_DATE !=''?$val->VM_DATE:''}}</td>
          <td>{{isset($val->VM_NAME) && $val->VM_NAME !=''?$val->VM_NAME:''}}</td> 
