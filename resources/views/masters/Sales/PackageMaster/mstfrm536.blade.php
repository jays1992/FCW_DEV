  ...
  <div class="multiple table-responsive  ">
    <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->PKMID}}" value="{{$val->PKMID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
          <td>{{isset($val->PKMCODE) && $val->PKMCODE !=''?$val->PKMCODE:''}}</td>
          <td>{{isset($val->PKMDATE) && $val->PKMDATE !=''?$val->PKMDATE:''}}</td>
          <td>{{isset($val->PKMNAME) && $val->PKMNAME !=''?$val->PKMNAME:''}}</td>
