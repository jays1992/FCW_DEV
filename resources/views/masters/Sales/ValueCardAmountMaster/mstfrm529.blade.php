...
<div class="container-fluid topnav">
  <div class="row">
    <div class="col-lg-2">
      <a href="{{route('master',[$FormId,'index'])}}" class="btn singlebt">Value Card Amount Master</a>
    </div>
  ...
  <div class="multiple table-responsive  ">
    <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->DOC_ID}}" value="{{$val->DOC_ID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
          <td>{{isset($val->VCA_AMT) && $val->VCA_AMT !=''?$val->VCA_AMT:''}}</td>
          <td>{{isset($val->DIS_PERCENT) && $val->DIS_PERCENT !=''?$val->DIS_PERCENT:''}}</td>
          <td>{{isset($val->PERCENTAGE_AMT) && $val->PERCENTAGE_AMT !=''?$val->PERCENTAGE_AMT:''}}</td>
          <td>{{isset($val->CARDVALIDITY_MON) && $val->CARDVALIDITY_MON !=''?$val->CARDVALIDITY_MON:''}}</td>
          <td>{{isset($val->TOTAL_AMT) && $val->TOTAL_AMT !=''?$val->TOTAL_AMT:''}}</td>
          <td>{{isset($val->DEACTIVATED) && $val->DEACTIVATED !='' && $val->DEACTIVATED =='1'?'No':'Yes'}}</td>
