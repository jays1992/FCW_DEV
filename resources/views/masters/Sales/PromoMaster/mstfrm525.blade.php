...
<div class="container-fluid topnav">
  <div class="row">
    <div class="col-lg-2">
      <a href="{{route('master',[$FormId,'index'])}}" class="btn singlebt">Promo Master</a>
    </div>
  ...
  <div class="multiple table-responsive  ">
    <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->PROMOID}}" value="{{$val->PROMOID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
          <td>{{isset($val->PROMOCODE) && $val->PROMOCODE !=''?$val->PROMOCODE:''}}</td>
          <td>{{isset($val->PROMODATE) && $val->PROMODATE !='' && $val->PROMODATE !='1900-01-01' ? date('d-m-Y',strtotime($val->PROMODATE)):''}}</td>
          <td>{{isset($val->VALID_DATE) && $val->VALID_DATE !='' && $val->VALID_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->VALID_DATE)):''}}</td>
