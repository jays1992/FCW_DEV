...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->PLID}}" value="{{$val->PLID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
              <td>{{isset($val->PLCODE) && $val->PLCODE !=''?$val->PLCODE:''}}</td>
              <td>{{isset($val->INDATE) && $val->INDATE !='' && $val->INDATE !='1900-01-01' ? date('d-m-Y',strtotime($val->INDATE)):''}}</td>
              <td>{{isset($val->PLNAME) && $val->PLNAME !=''?$val->PLNAME:''}}</td>
              <!-- <td>{{isset($val->MRP_APPLICABLE) && $val->MRP_APPLICABLE =='1'?'Yes':'No'}}</td> -->
              <td>{{isset($val->CREATED_DATE) && $val->CREATED_DATE !='' && $val->CREATED_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->CREATED_DATE)):''}}</td>
            ...
            if(seletedRecord==0){
              ...
              var recordId = countryIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("master",[526,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("master",[526,"view",":rcdId"]) }}';
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("master",[526,"attachment",":rcdId"]) }}';
...
var recordId = forApprIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("master",[526,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("mastermodify",[526,"cancel"])}}',
                            type:'POST',
