...
<div class="container-fluid topnav">
  <div class="row">
    <div class="col-lg-2"><a href="{{route('master',[$FormId,'index'])}}" class="btn singlebt">Value Card Master</a></div>
                ...
                <table id="example2" class="display nowrap table table-striped table-bordered itemlist w-200" width="100%" style="height:auto !important;">
                  ...
                  <tbody>
                    ...
                    <tr class="participantRow">
                      <td><input {{$ActionStatus}} type="text" name="CARD_NO[]" id="CARD_NO_{{$key}}" value="{{isset($row->CARD_NO)?$row->CARD_NO:''}}"      class="form-control"  autocomplete="off" maxlength="15" /></td>
                      <td><input {{$ActionStatus}} type="text" name="AMOUNT[]" id="AMOUNT_{{$key}}" value="{{isset($row->AMOUNT)?$row->AMOUNT:''}}"        class="form-control"  autocomplete="off" onclick="getAmountMaster(this.id)"  readonly/></td>
                      ...
                      <td><input {{$ActionStatus}} type="text" name="DISCOUNT_PERCENT[]" id="DISCOUNT_PERCENT_{{$key}}" value="{{isset($row->DISCOUNT_PERCENT)?$row->DISCOUNT_PERCENT:''}}"      class="form-control"  autocomplete="off"  readonly/></td>
                      <td><input {{$ActionStatus}} type="text" name="DISCOUNT_AMT[]" id="DISCOUNT_AMT_{{$key}}" value="{{isset($row->DISCOUNT_AMT)?$row->DISCOUNT_AMT:''}}"      class="form-control"  autocomplete="off"  readonly/></td>
                      ...
                      <td><input {{$ActionStatus}} type="text" name="VALIDITY_MON[]" id="VALIDITY_MON_{{$key}}" value="{{isset($row->VALIDITY_MON)?$row->VALIDITY_MON:''}}"  class="form-control"  autocomplete="off"  readonly/></td>
...
function getAmountMaster(textid){
  ...
  $.ajax({
    url:'{{route("master",[$FormId,"getAmountMaster"])}}',
    type:'POST',
    success:function(data) {
      var html = '';
