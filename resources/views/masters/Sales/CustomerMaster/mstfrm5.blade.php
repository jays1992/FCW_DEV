...
<div class="container-fluid topnav">
  <div class="row">
    <div class="col-lg-2">
      <a href="{{route('master',[5,'index'])}}" class="btn singlebt">Customer / Dealer Master</a>
    </div>

    <div class="col-lg-10 topnav-pd">
      <a href="{{route('master',[5,'add'])}}" id="btnSelectedRows" class="btn topnavbt" {{$objRights->ADD != 1 ? 'disabled' : ''}}><i class="fa fa-plus"></i> Add</a>
      <button class="btn topnavbt" id="btnEdit" {{$objRights->EDIT != 1 ? 'disabled' : ''}}><i class="fa fa-edit"></i> Edit</button>
      ...
      <button class="btn topnavbt" id="btnCancel" {{$objRights->CANCEL != 1 ? 'disabled' : ''}}><i class="fa fa-times"></i> Cancel</button>
      ...
      <button class="btn topnavbt"  id="btnAttach" {{$objRights->ATTECHMENT != 1 ? 'disabled' : ''}}><i class="fa fa-link"></i> Attachment</button>
      ...
      <button class="btn topnavbt" id="btnImport" {{$objRights->EDIT != 1 ? 'disabled' : ''}}> <i class="fa fa-file"></i> Import</button>
...
<div class="container-fluid purchase-order-view">
  ...
  <div class="multiple table-responsive" id="load_scroll" >
    <table id="custlist" class="display nowrap table table-striped table-bordered" width="100%">
      <thead id="thead1"> 
        ...
        <tr>
          ...
          <th>
            <input type="text" id="CCODE" name='CCODE' class="form-control" autocomplete="off" onkeyup="searchData(event,this.value)" >
...
function load_country_data(limit, start){
  ...
  $.ajax({
    url:'{{route("master",[5,"getListingData"])}}',
    method:"POST",
    data:{TYPE:TYPE,CCODE:CCODE,NAME:NAME,STATE:STATE,GSTIN:GSTIN,INDATE:INDATE,STATUS:STATUS,limit:limit, start:start},
    cache:false,
...
function getListingData(){
  ...
  $.ajax({
    url:'{{route("master",[5,"getListingData"])}}',
    type:'POST',
    data:{TYPE:TYPE,CCODE:CCODE,NAME:NAME,STATE:STATE,GSTIN:GSTIN,INDATE:INDATE,STATUS:STATUS,limit:'50',start:'0'},
    success:function(data) {
      $('#load_data').html(data);
      $("#BindRow").text($("#custlist .participantRow").length);
      $(".process_alert").modal('hide'); 
...
$(document).ready(function(){
  ...
  $('.js-selectall').on('change', function() {
            ...
            if(seletedRecord==0){
              ...
              var recordId = countryIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("master",[5,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("master",[5,"view",":rcdId"]) }}';
                ...
                if(seletedRecord==0){
                    ...
                    highlighFocusBtn('activeOk1');
                      ...
                      if(is_approve==1){
                        ...
                        var editURL = '{{route("master",[5,"amendment",":rcdId"]) }}';
                            editURL = editURL.replace(":rcdId",recordId);
                            window.location.href=editURL;
                      }else if(is_approve==2){
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("master",[5,"attachment",":rcdId"]) }}';
    ...
    var impURL =  '{{route("master",[5,"importdata"]) }}';
...
var recordId = forApprIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("master",[5,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
      ...
      window.fnCancelData = function (){
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("mastermodify",[5,"cancel"])}}',
                            type:'POST',
