  ...
  <div class="multiple table-responsive  ">
    <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->DISID}}" value="{{$val->DISID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
          <td>{{isset($val->DISCODE) && $val->DISCODE !=''?$val->DISCODE:''}}</td>
          <td>{{isset($val->DOC_DATE) && $val->DOC_DATE !='' && $val->DOC_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->DOC_DATE)):''}}</td>
          <td>{{isset($val->DESCRIPTION) && $val->DESCRIPTION !=''?$val->DESCRIPTION:''}}</td>
