...
@section('content')    
    <div class="container-fluid topnav">
      <div class="row">
          <div class="col-lg-2">
            <a href="{{route('master',[$FormId,'index'])}}" class="btn singlebt">Period Closing</a>
          </div>
          <div class="col-lg-10 topnav-pd">
            <a href="{{route('master',[$FormId,'add'])}}" id="btnSelectedRows" class="btn topnavbt" {{isset($objRights->ADD) && $objRights->ADD != 1 ? 'disabled' : ''}} ><i class="fa fa-plus"></i> Add</a>
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->PERIODCLID}}" value="{{$val->PERIODCLID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
              <td>{{isset($val->PERIODCL_NO) && $val->PERIODCL_NO !=''?$val->PERIODCL_NO:''}}</td>
              <td>{{isset($val->PERIODCL_DATE) && $val->PERIODCL_DATE !=''?$val->PERIODCL_DATE:''}}</td>
