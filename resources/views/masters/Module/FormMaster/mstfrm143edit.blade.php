...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                <a href="{{route('master',[143,'index'])}}" class="btn singlebt">Form Master</a>
                </div><!--col-2-->
    ...
    <div class="container-fluid purchase-order-view filter">     
         <form id="frm_mst_edit" method="POST"  > 
          @CSRF
          {{isset($objResponse->FORMID) ? method_field('PUT') : '' }}
          <div class="inner-form">
          
              
              <div class="row">
                  <div class="col-lg-2 pl"><p>Form Code</p></div>
                  <div class="col-lg-2 pl">
                  
                    <label> {{$objResponse->FORMCODE}} </label>
                    <input type="hidden" name="FORMID" id="FORMID" value="{{ $objResponse->FORMID }}" />
                    <input type="hidden" name="FORMCODE" id="FORMCODE" value="{{ $objResponse->FORMCODE }}" autocomplete="off"  maxlength="20"   />
                ...
                <div class="row">
                  ...
                  <div class="col-lg-5 pl">
                    <input type="text" name="FORMNAME" id="FORMNAME" class="form-control mandatory" value="{{ old('FORMNAME',$objResponse->FORMNAME) }}" maxlength="200" tabindex="1"  />
                    <span class="text-danger" id="ERROR_FORMNAME"></span> 
                  </div>
                </div>
...
$('#btnAdd').on('click', function() {
      var viewURL = '{{route("master",[143,"add"])}}';
 ...
 var formDataMst = $( "#frm_mst_edit" );
    ...
    $("#FORMNAME").blur(function(){
        ...
        $("#ERROR_FORMNAME").hide();
        validateSingleElemnet("FORMNAME");
    ...
    window.fnSaveData = function (){
        ...
        $.ajax({
            url:'{{route("mastermodify",[143,"update"])}}',
            type:'POST',
            data:formData,
    ...
    window.fnApproveData = function (){
        ...
        $.ajax({
            url:'{{route("mastermodify",[143,"singleapprove"])}}',
            type:'POST',
            data:formData,
            success:function(data) {
    ...
    $("#OkBtn").click(function(){
        ...
        $(".text-danger").hide();
        window.location.href = '{{route("master",[143,"index"]) }}';
