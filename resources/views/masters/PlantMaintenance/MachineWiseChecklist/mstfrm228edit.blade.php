...
@section('content')
    ...
    <div class="container-fluid purchase-order-view filter">     
         <form id="frm_mst_edit" method="POST" onsubmit="return validateForm()" class="needs-validation"  > 
          @CSRF
          {{isset($objMstHeader->MCKLISTID) ? method_field('PUT') : '' }}
              ...
              <div class="inner-form">
                <div class="row">
                  ...
                  <div class="col-lg-2 pl"> 
                    ...
                    <input type="hidden" name="MCKLISTID" id="MCKLISTID"  value="{{$objMstHeader->MCKLISTID}}"/>
                    <input type="hidden" name="MACHINEID_REF" id="MACHINEID_REF"  value="{{$objMstHeader->MACHINEID_REF}}" class="form-control" autocomplete="off" />
                  ...
                  <div class="col-lg-2 pl">
                     <label> {{$objMachineNo->MACHINE_DESC}} </label>
                  </div>
                </div>    
                ...
                <div class="row">
                  ...
                  <div class="col-lg-2 pl">
                    <label> {{$objChecklistNo->CHECKLIST_NO}} </label>
                    <input type="hidden" name="CHECKLIST_REF" id="CHECKLIST_REF"  value="{{$objMstHeader->CKLISTID_REF}}" class="form-control" autocomplete="off" />
                  </div>              
                  <div class="col-lg-2 pl"><p>CheckList Description</p></div>
                  <div class="col-lg-2 pl">
                    <label> {{$objChecklistNo->CHECKLIST_DESC}} </label>
                  </div>
                </div>    
              ...
              <div class="row">
                ...
                <input type="checkbox"   name="HDR_DEACTIVATED"  id="deactive-checkbox_0" {{$objMstHeader->DEACTIVATED == 1 ? "checked" : ""}}
                value='{{$objMstHeader->DEACTIVATED == 1 ? 1 : 0}}'  >
                </div>
                
                <div class="col-lg-2 pl"><p>Date of De-Activated</p></div>
                <div class="col-lg-2 pl">
                  <input type="date" name="HDR_DODEACTIVATED" class="form-control" id="HDR_DODEACTIVATED" {{$objMstHeader->DEACTIVATED == 1 ? "" : "disabled"}} value="{{isset($objMstHeader->DODEACTIVATED) && $objMstHeader->DODEACTIVATED !="" && $objMstHeader->DODEACTIVATED !="1900-01-01" ? $objMstHeader->DODEACTIVATED:''}}" placeholder="dd/mm/yyyy"  />
                </div>
              </div>
            {{-- deactive row end --}}
                      ...
                      <table id="example2" class="display nowrap table table-striped table-bordered itemlist" width="100%" style="height:auto !important;">
                        ...
                        @foreach($objResponse as $key => $row)
                          ...
                          <tr  class="participantRow">
                              <td  hidden><input  class="form-control w-100" type="text" name={{"MCKLIST_DID_".$key}}   id ={{"HDNMCKLIST_DID_".$key}} value="{{ $row->MCKLIST_DID}}" ></td>
                              
                              <td><input type="text" name={{"txtFORM_popup_".$key}} id={{"txtFORM_popup_".$key}} class="form-control" value="{{ $row->MP_CODE }}"  autocomplete="off"  readonly style="width:100%;"  /></td>
                              <td  hidden><input type="text" name={{"FORMID_".$key}} id={{"hdnFORMID_".$key}} class="form-control" value="{{ $row->MPID_REF }}"  autocomplete="off" /></td>
                              <td><input  class="form-control" style="width: 100%" type="text" name={{"FORMNAME_".$key}} id ={{"FORMNAME_".$key}} value="{{ $row->MP_DESC }}"   autocomplete="off" readonly></td>
                              ...
                              <td><input type="text" name={{"txtLISTPOP1_popup_".$key}} id={{"txtLISTPOP1_popup_".$key}}  value="{{ $row->MSP_CODE }}" class="form-control "  autocomplete="off"  readonly style="width:100%;" /></td>
                              <td><input  class="form-control w-100" type="text" name={{"DESC2_".$key}}       id ={{"DESC2_".$key}}  value="{{ $row->MSP_DESC }}"  style="width:100%;"  autocomplete="off" readonly></td>
                              <td><input  class="form-control" style="width: 100%" type="text" name={{"STANDARD_VAL_".$key}} id ={{"STANDARDID_VAL_".$key}} value="{{ $row->VALUE }}" maxlength="200" autocomplete="off" ></td>                            
  ...
  <div class="modal-dialog modal-md">
    ...
    <tr>
      ...
      <td class="ROW3"  style="width: 40%">
          <input type="text" class="form-control"  id="LISTPOP1namesearch" id="LISTPOP1namesearch" onkeyup="LISTPOP1NameFunction()" />
      </td>
    </tr>
    </tbody>
    </table>
...
$(document).ready(function(e) {
 ...
 var formDataMst = $( "#frm_mst_edit" );
    ...
    window.fnSaveData = function (){
        ...
        $.ajax({
            url:'{{route("mastermodify",[228,"update"])}}',
            type:'POST',
            data:formData,
            success:function(data) {
    ...
    window.fnApproveData = function (){
        ...
        $.ajax({
            url:'{{route("mastermodify",[228,"singleapprove"])}}',
            type:'POST',
            data:formData,
            success:function(data) {
