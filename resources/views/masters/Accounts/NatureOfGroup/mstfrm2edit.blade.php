...
@section('content')
    ...
    <div class="container-fluid purchase-order-view filter">     
         <form id="frm_mst_edit" method="POST"  > 
          @CSRF
          {{isset($objResponse->NOGID) ? method_field('PUT') : '' }}
          <div class="inner-form">
              ...
              <div class="row">
                  ...
                  <div class="col-lg-2 pl">
                    ...
                    <input type="hidden" name="NOGID" id="NOGID" value="{{ $objResponse->NOGID }}" />
                    <input type="hidden" name="NOGCODE" id="NOGCODE" value="{{ $objResponse->NOGCODE }}" autocomplete="off"  maxlength="20"   />
                ...
                <div class="row">
                  ...
                  <div class="col-lg-5 pl">
                    <input type="text" name="NOGNAME" id="NOGNAME" class="form-control mandatory" value="{{ old('NOGNAME',$objResponse->NOGNAME) }}" maxlength="200" tabindex="1"  />
                    <span class="text-danger" id="ERROR_NOGNAME"></span> 
                  </div>
                </div>
                ...
                <div class="row">
                  ...
                  <div class="col-lg-2 pl">
                    <div class="col-lg-11 pl">
                        <select name="NOG_TYPE" id="NOG_TYPE" class="form-control mandatory" autocomplete="off"tabindex="2" >
                          <option value="">Select</option>
                          @if(isset($getNatureOfGroupType) && !empty($getNatureOfGroupType))
                          @foreach($getNatureOfGroupType as $val)
                          <option {{isset($objResponse) && $objResponse->NOG_TYPE==$val->NOGTID?"selected='selected'":''}} value="{{$val->NOGTID}}">{{$val->NOG_TYPE}}</option>
                          @endforeach
                          @endif
                        </select>
                        <span class="text-danger" id="ERROR_NOG_TYPE"></span> 
                    </div>
                  </div>
                </div>
...
$('#btnAdd').on('click', function() {
 ...
 var formDataMst = $( "#frm_mst_edit" );
    ...
    $("#NOGNAME").blur(function(){
        $(this).val($.trim( $(this).val() ));
        $("#ERROR_NOGNAME").hide();
        validateSingleElemnet("NOGNAME");
    ...
    $("#NOG_TYPE").blur(function(){
        $(this).val($.trim( $(this).val() ));
        $("#ERROR_NOG_TYPE").hide();
        validateSingleElemnet("NOG_TYPE");
    });
    ...
    window.fnSaveData = function (){
        ...
        $.ajax({
            url:'{{route("mastermodify",[2,"update"])}}',
            type:'POST',
            data:formData,
            success:function(data) {
                ...
                if(data.errors) {
                    ...
                    if(data.errors.NOGNAME){
                        showError('ERROR_NOGNAME',data.errors.NOGNAME);
                    }
    ...
    window.fnApproveData = function (){
        ...
        $.ajax({
            url:'{{route("mastermodify",[2,"singleapprove"])}}',
            type:'POST',
            data:formData,
            success:function(data) {
    ...
    $("#OkBtn").click(function(){
        ...
        $(".text-danger").hide();
        window.location.href = '{{route("master",[2,"index"]) }}';
