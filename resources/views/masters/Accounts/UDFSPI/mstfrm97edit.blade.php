...
<!-- <form id="frm_mst_edit" onsubmit="return validateForm()"  method="POST"  >     -->
    ...
    @CSRF
          ...
          {{isset($objUdfResponse->UDFSPIID[0]) ? method_field('PUT') : '' }}
            <table id="example2" class="display nowrap table table-striped table-bordered itemlist" width="100%" style="height:auto !important;">
                ...
                @foreach($objUdfResponse as $key => $row)
                    <tr  class="participantRow">
                        <td hidden>
                        <input  class="form-control" type="hidden" name={{"UDFSPIID_".$key}} id ={{"txtID_".$key}} maxlength="100" value="{{ $row->UDFSPIID }}" autocomplete="off"   >
                        </td>
                        <td>
...
window.fnSaveData = function (){
            ...
            event.preventDefault();
                            ...
                            $.ajax({
                                url:'{{ route("mastermodify",[97,"update"]) }}',
                                type:'POST',
                                data:formData,
...
$.ajax({
    url:'{{ route("mastermodify",[97,"Approve"]) }}',
    type:'POST',
    data:formData,
    success:function(data) {
