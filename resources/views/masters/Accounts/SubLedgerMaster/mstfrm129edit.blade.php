...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                <a href="{{route('master',[129,'index'])}}" class="btn singlebt">Sub Ledger Master</a>
                </div><!--col-2-->
    ...
    <div class="container-fluid purchase-order-view filter">     
         <form id="frm_mst_edit" method="POST"  > 
          @CSRF
          {{isset($objResponse->SGLID) ? method_field('PUT') : '' }}
          <div class="inner-form">
            ...
            <div class="col-lg-2 pl"><p>GL Code</p></div>
                ...
                <input type="text" name="GLID_REF_POPUP" id="GLID_REF_POPUP" class="form-control mandatory" readonly tabindex="1" value="{{$objGlName->GLCODE}} - {{$objGlName->GLNAME}}" />
              ...
              <div class="row">
                  ...
                  <div class="col-lg-2 pl">
                    ...
                    <label> {{$objResponse->SGLCODE}} </label>
                    <input type="hidden" name="SGLID" id="SGLID" value="{{ $objResponse->SGLID }}" />
                    <input type="hidden" name="SGLCODE" id="SGLCODE" value="{{ $objResponse->SGLCODE }}" autocomplete="off"  maxlength="20"   />
                ...
                <div class="row">
                  ...
                  <div class="col-lg-5 pl">
                    <input type="text" name="SLNAME" id="SLNAME" class="form-control mandatory" value="{{ old('SLNAME',$objResponse->SLNAME) }}" maxlength="200" tabindex="2"  />
                    <span class="text-danger" id="ERROR_SLNAME"></span> 
                  </div>
                </div>
                ...
                <div class="row">
                  ...
                  <div class="col-lg-5 pl">
                    <input type="text" name="SALIAS" id="SALIAS" class="form-control" value="{{ old('SALIAS',$objResponse->SALIAS) }}" maxlength="100" tabindex="3"  />
                    <span class="text-danger" id="ERROR_SALIAS"></span> 
                  </div>
                </div>
                ...
                <div class="row">
                  ...
                  <div class="col-lg-1 pl">
                    <div class="col-lg-11 pl">
                      <select name="CC" id="CC" class="form-control" tabindex="4">
                          <option {{isset($objResponse->CC) && $objResponse->CC=='0'?'selected="selected"':''}} value="0">No</option>
                          <option {{isset($objResponse->CC) && $objResponse->CC=='1'?'selected="selected"':''}} value="1">Yes</option>
                      </select>
                  </div> 
                  </div>
                </div>
              ...
              <div class="row">
                ...
                <input type="checkbox"   name="DEACTIVATED"  id="deactive-checkbox_0" {{$objResponse->DEACTIVATED == 1 ? "checked" : ""}}
                 value='{{$objResponse->DEACTIVATED == 1 ? 1 : 0}}' tabindex="5"  >
                </div>
                
                <div class="col-lg-2 pl"><p>Date of De-Activated</p></div>
                <div class="col-lg-2 pl">
                  <input type="date" name="DODEACTIVATED" class="form-control" id="DODEACTIVATED" {{$objResponse->DEACTIVATED == 1 ? "" : "disabled"}} value="{{isset($objResponse->DODEACTIVATED) && $objResponse->DODEACTIVATED !="" && $objResponse->DODEACTIVATED !="1900-01-01" ? $objResponse->DODEACTIVATED:''}}" tabindex="6" placeholder="dd/mm/yyyy"  />
                </div>
             </div>
...
$('#btnAdd').on('click', function() {
      var viewURL = '{{route("master",[129,"add"])}}';
 ...
 var formDataMst = $( "#frm_mst_edit" );
    ...
    $("#SLNAME").blur(function(){
        $(this).val($.trim( $(this).val() ));
        $("#ERROR_SLNAME").hide();
        validateSingleElemnet("SLNAME");
    ...
    window.fnSaveData = function (){
        ...
        $.ajax({
            url:'{{route("mastermodify",[129,"update"])}}',
            type:'POST',
            data:formData,
            success:function(data) {
                ...
                if(data.errors) {
                    ...
                    if(data.errors.SLNAME){
                        showError('ERROR_SLNAME',data.errors.SLNAME);
                    }
    ...
    window.fnApproveData = function (){
        ...
        $.ajax({
            url:'{{route("mastermodify",[129,"singleapprove"])}}',
            type:'POST',
            data:formData,
            success:function(data) {
    ...
    $("#OkBtn").click(function(){
        ...
        $(".text-danger").hide();
        window.location.href = '{{route("master",[129,"index"]) }}';
