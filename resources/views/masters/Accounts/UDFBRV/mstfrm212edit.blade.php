...
<!-- <form id="frm_mst_edit" onsubmit="return validateForm()"  method="POST"  >     -->
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                <a href="{{route('master',[212,'index'])}}" class="btn singlebt">UDF for BRV</a>
                </div><!--col-2-->
    ...
    @CSRF
          ...
          {{isset($objUdfResponse->UDFBRVID[0]) ? method_field('PUT') : '' }}
            <table id="example2" class="display nowrap table table-striped table-bordered itemlist" width="100%" style="height:auto !important;">
                ...
                @foreach($objUdfResponse as $key => $row)
                    <tr  class="participantRow">
                        <td hidden>
                        <input  class="form-control" type="hidden" name={{"UDFBRVID_".$key}} id ={{"txtID_".$key}} maxlength="100" value="{{ $row->UDFBRVID }}" autocomplete="off"   >
                        </td>
                        <td>
...
$('#btnAdd').on('click', function() {
      var viewURL = '{{route("master",[212,"add"])}}';
            ...
            else
            {
                ...
                var texid10    = "";
                    ...
                    $("[id*=txtlabel]").each(function(){
                        ...
                        //check duplicate  row
                        var record_data = $.trim($(this).val().toUpperCase());
                        if(ExistArray.indexOf(record_data) > -1) {
                            allblank10.push('true');
                            texid10 =  $(this).attr('id');  
                        }
                        else{
                    ...
                    if(jQuery.inArray("false", allblank3) !== -1){
                        ...
                        highlighFocusBtn('activeOk1');
                            ...
                            $("#alert").modal('show');
                            $("#AlertMessage").text('Duplicate label. Please check.');
                            $("#focusid").val(texid10);
                            $("#YesBtn").hide(); 
                            $("#NoBtn").hide();  
                            $("#OkBtn").hide();  
...
window.fnSaveData = function (){
            ...
            event.preventDefault();
                            ...
                            $.ajax({
                                url:'{{ route("mastermodify",[212,"update"]) }}',
                                type:'POST',
                                data:formData,
                                success:function(data) {
                                    ...
                                    if(data.success) {                   
                                        ...
                                        // window.location.href="{{ route('master',[212,'index']) }}";
                                    }
                                    else if(data.cancel) {                   
...
$.ajax({
    url:'{{ route("mastermodify",[212,"Approve"]) }}',
    type:'POST',
    data:formData,
    success:function(data) {
