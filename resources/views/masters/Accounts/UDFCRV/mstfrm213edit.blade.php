...
<!-- <form id="frm_mst_edit" onsubmit="return validateForm()"  method="POST"  >     -->
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                <a href="{{route('master',[213,'index'])}}" class="btn singlebt">UDF for CRV</a>
                </div><!--col-2-->
    ...
    @CSRF
          ...
          {{isset($objUdfResponse->UDFCRVID[0]) ? method_field('PUT') : '' }}
            <table id="example2" class="display nowrap table table-striped table-bordered itemlist" width="100%" style="height:auto !important;">
                ...
                @foreach($objUdfResponse as $key => $row)
                    <tr  class="participantRow">
                        <td hidden>
                        <input  class="form-control" type="hidden" name={{"UDFCRVID_".$key}} id ={{"txtID_".$key}} maxlength="100" value="{{ $row->UDFCRVID }}" autocomplete="off"   >
                        </td>
                        <td>
...
$('#btnAdd').on('click', function() {
      var viewURL = '{{route("master",[213,"add"])}}';
...
window.fnSaveData = function (){
            ...
            event.preventDefault();
                            ...
                            $.ajax({
                                url:'{{ route("mastermodify",[213,"update"]) }}',
                                type:'POST',
                                data:formData,
                                success:function(data) {
                                    ...
                                    if(data.success) {                   
                                        ...
                                        // window.location.href="{{ route('master',[213,'index']) }}";
                                    }
                                    else if(data.cancel) {                   
...
$.ajax({
    url:'{{ route("mastermodify",[213,"Approve"]) }}',
    type:'POST',
    data:formData,
    success:function(data) {
