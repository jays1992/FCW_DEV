...
<div class="container-fluid filter">
    ...
    @CSRF  
    {{isset($objMstResponse->HSNID) ? method_field('PUT') : '' }}
		<div class="inner-form">
          ...
          <label>{{ $objMstResponse->HSNCODE }}</label>
					<input type="hidden" name="HSNCODE" id="HSNCODE" class="form-control mandatory" value="{{ $objMstResponse->HSNCODE }}"  maxlength="20" required tabindex="1" autocomplete="off" >
			...
			<div class="row">
				...
				<div class="col-lg-4 pl">
					<input type="text" name="HSNDESCRIPTION" id="HSNDESCRIPTION" value="{{ $objMstResponse->HSNDESCRIPTION }}"  class="form-control mandatory"  maxlength="200" required tabindex="2" autocomplete="off">
              ...
              <table id="table3" class="display nowrap table table-striped table-bordered itemlist itemlistscroll w-200" style="height:auto !important;">
                ...
                <tbody>
                  ...
                  <tr  class="participantRow">
                    <td>
                      <input  class="form-control w-100" type="text" name="NOR_TAXID_REF_{{ $key }}" id="TXTNOR_TAXID_REF_POPUP_{{ $key }}" value="{{ $row->TTCODE }}" maxlength="100" readonly>
                    </td>
                    <td hidden>
                      <input  class="form-control w-100" type="text" name="HDNNOR_TAXID_REF_{{ $key }}" id="HDNNOR_TAXID_REF_POPUP_{{ $key }}"  value="{{ $row->TAXID_REF }}" maxlength="100" >
                    </td>
                    <td>
                      <input  class="form-control w-100" type="text" name="NOR_TTDESCRIPTION_{{ $key }}" id="NOR_TTDESCRIPTION_{{ $key }}" value="{{ $row->TTDESCRIPTION }}" maxlength="20" autocomplete="off" readonly >
                    </td>
                    <td>
                      <input  class="form-control rightalign two-digits" type="text" name="NOR_RATE_{{ $key }}" id="NOR_RATE_{{ $key }}"  value="{{ $row->NRATE }}" maxlength="7" autocomplete="off"  >
                    </td>
                    <td style="text-align:center;" >
                      <input type="checkbox" name="NOR_DEACTIVATED_{{ $key }}" id="NOR_DEACTIVATED_{{ $key }}" {{ ($row->DEACTIVATED == 1 || $row->DEACTIVATED=='on')?'checked':'' }} class="filter-none"  value="{{ ($row->DEACTIVATED == 1 || $row->DEACTIVATED=='on')? 1 : 0 }}" >
                    </td>
                    <td style="text-align:center;" >
                      <input type="date"  name="NOR_DODEACTIVATED_{{ $key }}" id="NOR_DODEACTIVATED_{{ $key }}" class="form-control" value="{{ (!is_null($row->DODEACTIVATED) && $row->DODEACTIVATED!='1900-01-01')? $row->DODEACTIVATED : '' }}"   autocomplete="off" >
                    </td>
                    <td align="center" >
                      <a class="btn add ainvoice" title="add" data-toggle="tooltip"><i class="fa fa-plus"></i></a>
                      <button class="btn remove dinvoice" title="Delete" data-toggle="tooltip" type="button"  @if($loop->iteration-1==0) disabled @endif><i class="fa fa-trash" @if($loop->iteration-1==0) disabled @endif></i></button>
                    </td>
                  </tr>
                  @endforeach
                  @else 
          ...
          <div class="table-responsive table-wrapper-scroll-y my-custom-scrollbar" style="height:250px;margin-top:10px;" >					 
            <table id="table4" class="display nowrap table table-striped table-bordered itemlist itemlistscroll w-200" style="height:auto !important;">
              ...
              @foreach($objList2 as $key => $row)
                <tr  class="participantRow">
                  <td>
                    <input  class="form-control w-100" type="text" name="EXC_TAXID_REF_{{ $key }}" id="TXTEXC_TAXID_REF_POPUP_{{ $key }}" value="{{ $row->TTCODE }}" maxlength="100" readonly>
                  </td>
                  <td hidden >
                    <input  class="form-control w-100" type="text" name="HDNEXC_TAXID_REF_{{ $key }}" id="HDNEXC_TAXID_REF_POPUP_{{ $key }}" value="{{ $row->TAXID_REF }}"  maxlength="100" >
                  </td>
                  <td>
                    <input  class="form-control w-100" type="text" name="EXC_TTDESCRIPTION_{{ $key }}" id="EXC_TTDESCRIPTION_{{ $key }}" value="{{ $row->TTDESCRIPTION }}"  autocomplete="off" readonly >
                  </td>
                  <td>
                    <input  class="form-control two-digits rightalign" type="text" name="EXC_RATE_{{ $key }}" id="EXC_RATE_{{ $key }}" value="{{ $row->ERATE }}" maxlength="6" autocomplete="off"  >
                  </td>
                  <td style="text-align:center;" >
                    <input type="checkbox" name="EXC_DEACTIVATED_{{ $key }}" id="EXC_DEACTIVATED_{{ $key }}" {{ ($row->DEACTIVATED == 1 || $row->DEACTIVATED=='on')?'checked':'' }}  class="filter-none"  value="{{ ($row->DEACTIVATED == 1 || $row->DEACTIVATED=='on')? 1 : 0 }}" >
                  </td>
                  <td style="text-align:center;" >
                    <input type="date" name="EXC_DODEACTIVATED_{{ $key }}" id="EXC_DODEACTIVATED_{{ $key }}" class="form-control"  autocomplete="off" value="{{ (!is_null($row->DODEACTIVATED) && $row->DODEACTIVATED!='1900-01-01')? $row->DODEACTIVATED : '' }}" >
                  </td>
                  <td align="center" >
                    <a class="btn add" title="add" data-toggle="tooltip"><i class="fa fa-plus"></i></a>
      ...
      <table id="nor_tax_tab2" class="display nowrap table  table-striped table-bordered" width="100%">
        ...
        @foreach ($objTaxTypeList as $index=>$TTList)
          <tr class="clsinvoiceid">
            <td class="ROW1" style="width: 12%" align="center"> <input type="checkbox" name="SELECT_TAXID_REF[]"  id="nor_taxidref_{{ $TTList->TAXID }}" class="js-selectall1 cls_nor_taxidref" value="{{ $TTList->TAXID }}" /></td>
...
window.fnApproveData = function (){
  ...
  $.ajax({
      url:'{{route("master",[144,"singleapprove"])}}',
      type:'POST',
      data:formData,
      success:function(data) {
...
$("#OkBtn").click(function(){
    ...
    $("#OkBtn1").hide();
    $(".text-danger").hide();
    window.location.href = "{{route('master',[144,'index'])}}";
