...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                <a href="{{route('master',[127,'index'])}}" class="btn singlebt">Account Group</a>
                </div><!--col-2-->
    ...
    <div class="container-fluid purchase-order-view filter">     
         <form id="frm_mst_edit" method="POST"  > 
          @CSRF
          {{isset($objResponse->AGID) ? method_field('PUT') : '' }}
          <div class="inner-form">
          
              
              <div class="row">
                  <div class="col-lg-2 pl"><p>Account Group Code</p></div>
                  <div class="col-lg-2 pl">
                  
                    <label> {{$objResponse->AGCODE}} </label>
                    <input type="hidden" name="AGID" id="AGID" value="{{ $objResponse->AGID }}" />
                    <input type="hidden" name="AGCODE" id="AGCODE" value="{{ $objResponse->AGCODE }}" autocomplete="off"  maxlength="20"   />
                ...
                <div class="row">
                  ...
                  <div class="col-lg-2 pl"><p>Account Group Name</p></div>
                  <div class="col-lg-5 pl">
                    <input type="text" name="AGNAME" id="AGNAME" class="form-control mandatory" value="{{ old('AGNAME',$objResponse->AGNAME) }}" maxlength="200" tabindex="1"  />
                    <span class="text-danger" id="ERROR_AGNAME"></span> 
                  </div>
                </div>
                  ...
                  <div class="col-lg-2 pl">
                    <div class="col-lg-11 pl">
                      <select name="NOGID_REF" id="NOGID_REF" class="form-control mandatory" >
                        ...
                        @foreach($objNogList as $Nog)
                        <option {{isset($objResponse->NOGID_REF) && $objResponse->NOGID_REF==$Nog->NOGID?'selected="selected"':''}} value="{{$Nog->NOGID}}">{{$Nog->NOGCODE.' - '.$Nog->NOGNAME}}</option>
                        @endforeach
                      </select>
...
$('#btnAdd').on('click', function() {
      var viewURL = '{{route("master",[127,"add"])}}';
 ...
 var formDataMst = $( "#frm_mst_edit" );
    ...
    $("#AGNAME").blur(function(){
        $(this).val($.trim( $(this).val() ));
        $("#ERROR_AGNAME").hide();
        validateSingleElemnet("AGNAME");
    ...
    window.fnSaveData = function (){
        ...
        $.ajax({
            url:'{{route("mastermodify",[127,"update"])}}',
            type:'POST',
            data:formData,
            success:function(data) {
                ...
                if(data.errors) {
                    ...
                    if(data.errors.AGNAME){
                        showError('ERROR_AGNAME',data.errors.AGNAME);
                    }
    ...
    window.fnApproveData = function (){
        ...
        $.ajax({
            url:'{{route("mastermodify",[127,"singleapprove"])}}',
            type:'POST',
            data:formData,
            success:function(data) {
    ...
    $("#OkBtn").click(function(){
        ...
        $(".text-danger").hide();
        window.location.href = '{{route("master",[127,"index"]) }}';
