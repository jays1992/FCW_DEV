...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->TEAMMSTID}}" value="{{$val->TEAMMSTID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
                <td>{{isset($val->TEAMMST_NO) && $val->TEAMMST_NO !=''?$val->TEAMMST_NO:''}}</td>
                ...
                <td>{{isset($val->TEAMMST_DATE) && $val->TEAMMST_DATE !='' && $val->TEAMMST_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->TEAMMST_DATE)):''}}</td>
