          ...
          <div class="inner-form">
              <div class="row">
                  ...
                  <div class="col-lg-2 pl">
                    <label> {{$objResponse->TASK_TYPECODE}} </label>
                    <input type="hidden" name="TASK_TYPECODE" id="TASK_TYPECODE" value="{{ $objResponse->ID }}" />
                    <input type="hidden" name="TASK_TYPECODE" id="TASK_TYPECODE" value="{{ $objResponse->TASK_TYPECODE }}" autocomplete="off"  maxlength="20"   />
                </div>
                </div>
                ...
                <div class="row">
                  ...
                  <div class="col-lg-5 pl">
                    <input {{$ActionStatus}} type="text" name="TASK_TYPENAME" id="TASK_TYPENAME" class="form-control mandatory" value="{{ old('TASK_TYPENAME',$objResponse->TASK_TYPENAME) }}" maxlength="200" tabindex="1"  />
                  </div>
                </div>
  ...
  $('#btnExit').on('click', function() {
    ...
    var formDataMst = $( "#frm_mst_edit" );
     formDataMst.validate();
      $("#TASK_TYPENAME").blur(function(){
        $(this).val($.trim( $(this).val() ));
        $("#ERROR_DESCRIPTIONS").hide();
        validateSingleElemnet("TASK_TYPENAME");
      });

    $("#TASK_TYPENAME").keydown(function(){
      $("#ERROR_DESCRIPTIONS").hide();
      validateSingleElemnet("TASK_TYPENAME");
      });
...
function submitForm(requestType){
  ...
  $.ajax({
      ...
      success:function(data) {
        ...
        if(data.errors) {
        $(".text-danger").hide();
        if(data.errors.TASK_TYPENAME){
        showError('ERROR_ACTIVITYNAME',data.errors.TASK_TYPENAME);
        }
      }
        if(data.success) {                   
