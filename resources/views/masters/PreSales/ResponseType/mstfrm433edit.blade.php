          ...
          <div class="inner-form">
              <div class="row">
                  ...
                  <div class="col-lg-2 pl">
                    <label> {{$objResponse->RESPONSECODE}} </label>
                    <input type="hidden" name="RESPONSECODE" id="RESPONSECODE" value="{{ $objResponse->ID }}" />
                    <input type="hidden" name="RESPONSECODE" id="RESPONSECODE" value="{{ $objResponse->RESPONSECODE }}" autocomplete="off"  maxlength="20"   />
                </div>
                </div>
                ...
                <div class="row">
                  ...
                  <div class="col-lg-5 pl">
                    <input {{$ActionStatus}} type="text" name="RESPONSENAME" id="RESPONSENAME" class="form-control mandatory" value="{{ old('RESPONSENAME',$objResponse->RESPONSENAME) }}" maxlength="200" tabindex="1"  />
                  </div>
                </div>
  ...
  $('#btnExit').on('click', function() {
    ...
    var formDataMst = $( "#frm_mst_edit" );
     formDataMst.validate();
      $("#RESPONSENAME").blur(function(){
        $(this).val($.trim( $(this).val() ));
        $("#ERROR_DESCRIPTIONS").hide();
        validateSingleElemnet("RESPONSENAME");
      });

    $("#RESPONSENAME").keydown(function(){
      $("#ERROR_DESCRIPTIONS").hide();
      validateSingleElemnet("RESPONSENAME");
      });
...
function submitForm(requestType){
  ...
  $.ajax({
      ...
      success:function(data) {
        ...
        if(data.errors) {
        $(".text-danger").hide();
        if(data.errors.RESPONSENAME){
        showError('ERROR_ACTIVITYNAME',data.errors.RESPONSENAME);
        }
      }
        if(data.success) {                   
