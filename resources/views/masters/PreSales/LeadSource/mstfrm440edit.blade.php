          ...
          <div class="inner-form">
              <div class="row">
                  ...
                  <div class="col-lg-2 pl">
                    <label> {{$objResponse->LEAD_SOURCECODE}} </label>
                    <input type="hidden" name="LEAD_SOURCECODE" id="LEAD_SOURCECODE" value="{{ $objResponse->ID }}" />
                    <input type="hidden" name="LEAD_SOURCECODE" id="LEAD_SOURCECODE" value="{{ $objResponse->LEAD_SOURCECODE }}" autocomplete="off"  maxlength="20"   />
                </div>
                </div>
                ...
                <div class="row">
                  ...
                  <div class="col-lg-5 pl">
                    <input {{$ActionStatus}} type="text" name="LEAD_SOURCENAME" id="LEAD_SOURCENAME" class="form-control mandatory" value="{{ old('LEAD_SOURCENAME',$objResponse->LEAD_SOURCENAME) }}" maxlength="200" tabindex="1"  />
                  </div>
                </div>
  ...
  $('#btnExit').on('click', function() {
    ...
    var formDataMst = $( "#frm_mst_edit" );
     formDataMst.validate();
      $("#LEAD_SOURCENAME").blur(function(){
        $(this).val($.trim( $(this).val() ));
        $("#ERROR_DESCRIPTIONS").hide();
        validateSingleElemnet("LEAD_SOURCENAME");
      });

    $("#LEAD_SOURCENAME").keydown(function(){
      $("#ERROR_DESCRIPTIONS").hide();
      validateSingleElemnet("LEAD_SOURCENAME");
      });
...
function submitForm(requestType){
  ...
  $.ajax({
      ...
      success:function(data) {
        ...
        if(data.errors) {
        $(".text-danger").hide();
        if(data.errors.LEAD_SOURCENAME){
        showError('ERROR_ACTIVITYNAME',data.errors.LEAD_SOURCENAME);
        }
      }
        if(data.success) {                   
