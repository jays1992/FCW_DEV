    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            <tr>
                ...
                <th>Company Holiday Code</th>
                <th>Company Holiday Date</th>
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->CP_HOLIDAYID}}" value="{{$val->CP_HOLIDAYID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
                <td>{{isset($val->COMPANY_HOLIDAY_CODE) && $val->COMPANY_HOLIDAY_CODE !=''?$val->COMPANY_HOLIDAY_CODE:''}}</td>
                <td>{{isset($val->COMPANY_HOLIDAY_DATE) && $val->COMPANY_HOLIDAY_DATE !=''?$val->COMPANY_HOLIDAY_DATE:''}}</td>
        ...
        <form id="masterForm268Print" action="{{ route('mastergetlist',[268,'printdata'])}}" method="POST" >
            @csrf
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("master",[268,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("master",[268,"view",":rcdId"]) }}';
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("master",[268,"attachment",":rcdId"]) }}';
...
var recordId = resultIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("master",[268,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("mastermodify",[268,"cancel"])}}',
                            type:'POST',
