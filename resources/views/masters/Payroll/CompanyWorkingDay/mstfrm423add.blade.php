...
@section('content')
  ...
  <div class="container-fluid purchase-order-view filter">     
    ...
    <div class="row">
        ...
        <input type="text" name="COMPANY_WORK_CODE" id="COMPANY_WORK_CODE" value="{{ isset($objAutoGenNo)?$objAutoGenNo:'' }}" class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase"  >
      @elseif(isset($objSON->MANUAL_SR) && $objSON->MANUAL_SR == "1")
        <input type="text" name="COMPANY_WORK_CODE" id="COMPANY_WORK_CODE" value="{{ old('COMPANY_WORK_CODE') }}" class="form-control mandatory" maxlength="{{isset($objSON->MANUAL_MAXLENGTH)?$objSON->MANUAL_MAXLENGTH:''}}" autocomplete="off" style="text-transform:uppercase"  >
      @else
        <input type="text" name="COMPANY_WORK_CODE" id="COMPANY_WORK_CODE" value="{{ isset($objAutoGenNo)?$objAutoGenNo:'' }}"  class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase"  >
        @endif
      </div>
      <div class="col-lg-2 pl"><p>Company Working Date*</p></div>
        <div class="col-lg-2 pl">
        <input type="date" name="COMPANY_WORK_DATE"  id="COMPANY_WORK_DATE" value="{{ old('COMPANY_WORK_DATE') }}" class="form-control mandatory" autocomplete="off" placeholder="dd/mm/yyyy" >
        </div> 
        
        <div class="col-lg-2 pl"><p>Year*</p></div>
        <div class="col-lg-2 pl">
          <input type="text" name="YRIDREF" id="YRIDREF" class="form-control mandatory"  autocomplete="off" readonly/>
          <input type="hidden" name="YRID_REF" id="YRID_REF" class="form-control" autocomplete="off" />
        ...
        <div class="col-lg-2 pl">
          <input type="text" name="MTID_popup" id="MTID_popup" class="form-control mandatory"  autocomplete="off" readonly/>
          <input type="hidden" name="MTID_REF" id="MTID_REF" class="form-control" autocomplete="off" />
    ...
    <div class="row">
            ...
            <tr class="participantRow">
              <td><input type="text" name="popupMENU[]" id="popupMENU_0" class="form-control"  autocomplete="off"  readonly/></td>
              <td><input type="text" name="Weeklyoff[]" id="Weeklyoff_0" class="form-control"  autocomplete="off"  readonly/></td>
              </tr>
              <tr>
            </tr>
        </tbody>
...
<div id="yearpopup" class="modal" role="dialog"  data-backdrop="static">
  <div class="modal-dialog modal-md" style="width: 600px;">
    ...
    <div class="modal-body">
    <div class="tablename"><p id="tablename"></p></div>
  ...
  $('#btnAdd').on('click', function() {
    ...
    $('#btnExit').on('click', function() {
      ...
      window.location.href=viewURL;
        ...
        $("#COMPANY_WORK_CODE").blur(function(){
              $(this).val($.trim( $(this).val() ));
              $("#ERROR_COMPANY_WORK_CODE").hide();
              validateSingleElemnet("COMPANY_WORK_CODE");
                
            });

            $( "#COMPANY_WORK_CODE" ).rules( "add", {
                required: true,
                //nowhitespace: true,
    ...
    function validateSingleElemnet(element_id){
      var validator =$("#frm_mst_condition" ).validate();
         if(validator.element( "#"+element_id+"" )){
          ...
          if(element_id=="LOBCOMPANY_HOLIDAY_CODE_NO" || element_id=="COMPANY_WORK_CODE" ) {
            checkDuplicateCode();
          }
    ...
    function checkDuplicateCode(){
        ...
        $.ajax({
            ...
            success:function(data) {
                if(data.exists) {
                    $(".text-danger").hide();
                    showError('COMPANY_WORK_CODE',data.msg);
                    $("#COMPANY_WORK_CODE").focus();
                }                                
            },
            error:function(data){
    ...
    $( "#btnSave" ).click(function() {
        ...
        if(formConditionMst.valid()){
                $("#FocusId").val('');
                var COMPANY_WORK_CODE          =   $.trim($("[id*=COMPANY_WORK_CODE]").val());
                var YRID_REF                   =   $.trim($("[id*=YRID_REF]").val());
                var MTID_REF                   =   $.trim($("[id*=MTID_REF]").val());
                
                if(COMPANY_WORK_CODE ===""){
                    $("#FocusId").val('COMPANY_WORK_CODE');
                    $("#ProceedBtn").focus();
                    $("#YesBtn").hide();
                    $("#NoBtn").hide();
                    $("#OkBtn1").show();
...
$("#YRIDREF").click(function(event){
  $('#tablename').html('Year Details');
  $.ajaxSetup({
      headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      }
...
$(document).ready(function(e) {
  ...
  $('#COMPANY_WORK_DATE').val(currentdate);
});
