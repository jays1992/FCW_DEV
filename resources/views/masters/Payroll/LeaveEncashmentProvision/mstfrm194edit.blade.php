...
@section('content')
    ...
    <div class="container-fluid purchase-order-view filter">     
         <form id="frm_mst_edit" method="POST"  > 
          @CSRF
          {{isset($objResponse->LEPID) ? method_field('PUT') : '' }}
          <div class="inner-form">
                ...
                <div class="row">
                  ...
                  <label style="margin-left: 17px;"> {{$objResponse->LEAVE_TYPE_CODE}} </label>
                    <input type="hidden" name="LEPID" id="LEPID" value="{{ $objResponse->LEPID }}" />
                    <input type="hidden" name="LEAVE_TYPE_CODE" id="LEAVE_TYPE_CODE" value="{{ $objResponse->LEAVE_TYPE_CODE }}" autocomplete="off"  maxlength="20"   />
                  ...
                  <div class="col-lg-5 pl">
                    <input type="text" name="LEAVE_TYPE_DESC" id="LEAVE_TYPE_DESC" class="form-control mandatory" value="{{ $objResponse->LEAVE_TYPE_DESC }}" maxlength="200" tabindex="2"  />
            ...
            <div class="col-lg-1">
            <input type="radio" name="PROVISION" value="Quater" {{$objResponse->PROVISION == 'Quater' ? 'checked' : ''}} >
            </div>

            <div class="col-lg-1"><p>Half Yearly</p></div>
            <div class="col-lg-1">
              <input type="radio" name="PROVISION" value="Half Yearly" {{$objResponse->PROVISION == 'Half Yearly' ? 'checked' : ''}}  >
            </div>

            <div class="col-lg-1"><p>Yearly</p></div>
            <div class="col-lg-1">
              <input type="radio" name="PROVISION" value="Yearly" {{$objResponse->PROVISION == 'Yearly' ? 'checked' : ''}}  >
            </div>
              ...
              <div class="row">
                ...
                <input type="checkbox"   name="DEACTIVATED" style="margin-left: 17px; id="deactive-checkbox_0" {{$objResponse->DEACTIVATED == 1 ? "checked" : ""}}
...
$('#btnAdd').on('click', function() {
 ...
 var formDataMst = $( "#frm_mst_edit" );
    ...
    window.fnSaveData = function (){
        ...
        $.ajax({
            url:'{{route("mastermodify",[194,"update"])}}',
            type:'POST',
            data:formData,
    ...
    window.fnApproveData = function (){
        ...
        $.ajax({
            url:'{{route("mastermodify",[194,"singleapprove"])}}',
            type:'POST',
            data:formData,
            success:function(data) {
