...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                <a href="{{route('master',[294,'index'])}}" class="btn singlebt">ESI Rules</a>
                </div><!--col-2-->
               ...
               <input type="hidden" id="focusid" >
                 <div class=" table-responsive table-wrapper-scroll-y my-custom-scrollbar" style="height:300px;" >
                   ...
                   <tr>                          
                       ...
                       <th rowspan="2" width="3%">De-Activated</th>
                       <th rowspan="2" width="3%">Date of De-Activated</th>                       
                     ...
                     <tbody>
                       <tr  class="participantRow">
                           <td>
                             @php
                               $EMP_ESI_RATE = $objResponse->EMP_ESI_RATE; 
                               $EMPR_ESI_RATE = $objResponse->EMPR_ESI_RATE;
                               $total =$EMP_ESI_RATE+ $EMPR_ESI_RATE;
                             @endphp
                             ...
                             @foreach($objYearList as $val)
                             <option {{isset($objResponse->FYID_REF) && $objResponse->FYID_REF == $val-> YRID ?'selected="selected"':''}} value="{{ $val-> YRID }}">{{ $val->YRCODE }}</option>
                             @endforeach
                           </select>
                         </td>
                           <td><input type="text" id="YRDESCRIPTION_0" value="{{ $objFyDesList->YRDESCRIPTION }}" class="form-control"  maxlength="100" readonly ></td>
                           <td>
                             <select name="FR_PAYPID_REF[]" id="FR_PAYPID_REF_0" class="form-control mandatory" tabindex="4">
                               <option value="" selected="">Select</option>
                               @foreach($obPeriodList as $val)                               
                               <option {{isset($objResponse->FR_PAYPID_REF) && $objResponse->FR_PAYPID_REF == $val-> PAYPERIODID ?'selected="selected"':''}} value="{{ $val-> PAYPERIODID }}">{{ $val->PAY_PERIOD_CODE }}-{{ $val->PAY_PERIOD_DESC }}</option>
                               @endforeach
                             </select>                           
                           <td>                                
                             <select name="TO_PAYPID_REF[]" id="TO_PAYPID_REF_0" class="form-control mandatory" tabindex="4">
                               <option value="" selected="">Select</option>
                               @foreach($obPeriodList as $val)
                               <option {{isset($objResponse->TO_PAYPID_REF) && $objResponse->TO_PAYPID_REF == $val-> PAYPERIODID ?'selected="selected"':''}} value="{{ $val-> PAYPERIODID }}">{{ $val->PAY_PERIOD_CODE }}-{{ $val->PAY_PERIOD_DESC }}</option>
                               @endforeach
                             </select> 
                           <td><input  class="form-control" type="text" name="CAP_LIMIT[]" id ="CAP_LIMIT_0" value="{{ $objResponse->CAP_LIMIT }}"  autocomplete="off" onkeypress="return onlyNumberKey(event)" style="width: 99%" ondrop="return false;oncopy="return false" oncut="return false" onpaste="return false"></td>
                           <td><input  class="form-control txtCal" type="text" name="EMP_ESI_RATE[]" id ="EMP_ESI_RATE_0" value="{{ $objResponse->EMP_ESI_RATE }}" onkeyup="totalVlaue(this.id,this.value)"  autocomplete="off" onkeypress="return onlyNumberKey(event)" style="width: 99%" ondrop="return false;oncopy="return false" oncut="return false" onpaste="return false"></td>
                           <td><input  class="form-control txtCal" type="text" name="EMPR_ESI_RATE[]" id ="EMPR_ESI_RATE_0" value="{{ $objResponse->EMPR_ESI_RATE }}" onkeyup="totalVlaue(this.id,this.value)"  autocomplete="off" onkeypress="return onlyNumberKey(event)" style="width: 99%" ondrop="return false;"oncopy="return false" oncut="return false" onpaste="return false"></td>
                           <td><input type="text" id="total_sum_value_0" value="{{ $total }}%" class="form-control"  maxlength="100" readonly ></td>
                           <td><input type="checkbox"   name="DEACTIVATED[]"  id="CHKDEACTIVATED" {{$objResponse->DEACTIVATED == 1 ? "checked" : ""}} value='{{$objResponse->DEACTIVATED == 1 ? 1 : 0}}' tabindex="2"  ></td>
                           <td><input type="date" name="DODEACTIVATED[]" class="form-control" id="DODEACTIVATED" {{$objResponse->DEACTIVATED == 1 }} value="{{isset($objResponse->DODEACTIVATED) && $objResponse->DODEACTIVATED !="" && $objResponse->DODEACTIVATED !="1900-01-01" ? $objResponse->DODEACTIVATED:''}}" tabindex="3" placeholder="dd/mm/yyyy"  /></td>
...
$('#btnAdd').on('click', function() {
   var viewURL = '{{route("master",[294,"add"])}}';
...
var formResponseMst = $( "#frm_mst_edit" );
 ...
 function checkDuplicateCode(){
     ...
     $.ajax({
         url:'{{route("master",[294,"codeduplicate"])}}',
         type:'POST',
         data:formData,
...
window.fnSaveData = function (){
     ...
     $.ajax({
         url:'{{ route("mastermodify",[294,"update"]) }}',
         type:'POST',
         data:formData,
         success:function(data) {
             ...
             if(data.success) {                   
               ...
               //  window.location.href='{{ route("master",[294,"index"])}}';
             }
    ...
    window.fnApproveData = function (){
        ...
        $.ajax({
            url:'{{route("mastermodify",[294,"Approve"])}}',
            type:'POST',
            data:formData,
            success:function(data) {
...
window.fnUndoYes = function (){
   ...
   //reload form
   window.location.href = "{{route('master',[294,'add'])}}";
 ...
 $.ajaxSetup({
     ...
     $.ajax({
         url:'{{route("master",[294,"getFYearName"])}}',
         type:'POST',
         data:{YRID:value},
         success:function(data) {
...
<script type="text/javascript">
  $(function () {
    ...
    $('input[type=checkbox][id=DEACTIVATED]').change(function() {
