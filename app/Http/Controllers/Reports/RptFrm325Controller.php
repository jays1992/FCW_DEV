...
class RptFrm325Controller extends Controller
{
    protected $form_id = 325;
    protected $vtid_ref   = 413;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        $ObjVendor = DB::table('TBL_TRN_IRGP01_HDR')
                        ->leftJoin('TBL_MST_SUBLEDGER', 'TBL_MST_SUBLEDGER.SGLID','=','TBL_TRN_IRGP01_HDR.VID_REF')
                        ->where('TBL_TRN_IRGP01_HDR.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_TRN_IRGP01_HDR.BRID_REF','=',Auth::user()->BRID_REF)
        ...
        $ObjVendorGrp = DB::table('TBL_MST_VENDORGROUP')
                        ...
                        ->where('TBL_MST_VENDORGROUP.BRID_REF','=',Auth::user()->BRID_REF)
        ...
        $ObjItem = DB::table('TBL_TRN_IRGP01_HDR')
                        ->leftJoin('TBL_TRN_IRGP01_MAT', 'TBL_TRN_IRGP01_MAT.RGPID_REF','=','TBL_TRN_IRGP01_HDR.RGPID')
                        ->leftJoin('TBL_MST_ITEM', 'TBL_MST_ITEM.ITEMID','=','TBL_TRN_IRGP01_MAT.ITEMID_REF')
        ...
        return view('reports.inventory.RGPReport.rptfrm325',compact(['objRights','objBranchGroup','objBranch','ObjVendor',
                    'ObjVendorGrp','ObjItem','company_check']));        
    }  
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        $CYID_REF          = Auth::user()->CYID_REF;
            ...
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/RGP');
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            {
                return Excel::download(new RGPDetail($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMID,$STATUS,$CYID_REF), 'RGPDetail.xlsx');
            }
