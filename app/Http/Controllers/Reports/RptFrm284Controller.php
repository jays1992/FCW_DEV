...
use App\Exports\SPIDetail;
use App\Exports\SPISummary;
use Maatwebsite\Excel\Facades\Excel;

class RptFrm284Controller extends Controller
{
    protected $form_id = 284;
    protected $vtid_ref   = 374;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        $ObjVendor = DB::table('TBL_TRN_PRPB02_HDR')
                    ...
                    ->where('TBL_TRN_PRPB02_HDR.BRID_REF','=',Auth::user()->BRID_REF)
        ...
        return view('reports.purchase.SPIRpt.rptfrm284',compact(['objRights','objBranchGroup','objBranch','ObjVendor',
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        {
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/ServicePurchaseInvoice');
        }
        else
        {
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/ServicePurchaseInvoiceSummary');
        }
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            else if($Flag == 'E')
            {
                if($ReportType == "Detail")
                {
                return Excel::download(new SPIDetail($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$STATUS,$CYID_REF), 'SPIDetail.xlsx');
    
                }
                else
                {
                    return Excel::download(new SPISummary($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$STATUS,$CYID_REF), 'SPISummary.xlsx');
                }
