...
use Chartblocks;
use App\Exports\PurchaseOrderDetail;
use App\Exports\PurchaseOrderSummary;
use Maatwebsite\Excel\Facades\Excel;

class RptFrm265Controller extends Controller
{
    protected $form_id = 265;
    protected $vtid_ref   = 355;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){   
        ...
        return view('reports.purchase.PurchaseOrderRpt.rptfrm265',compact(['objRights','objBranchGroup','objBranch','ObjVendor',
   ...
   public function ViewReport($request) {
    ...
    $ssrs = new \SSRS\Report(Session::get('ssrs_config')['REPORT_URL'], array('username' => Session::get('ssrs_config')['username'], 'password' => Session::get('ssrs_config')['password'])); 
        ...
        {
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/PurchaseOrderDetail');

        }
        else
        {
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/PurchaseOrderSummary');
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            {         
                if($ReportType == "Detail")
                {
                return Excel::download(new PurchaseOrderDetail($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$STATUS,$CYID_REF), 'PurchaseOrderDetail.xlsx');
    
                }
                else
                {
                return Excel::download(new PurchaseOrderSummary($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$STATUS,$CYID_REF), 'PurchaseOrderSummary.xlsx');
                }
            }
