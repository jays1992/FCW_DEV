...
use App\Exports\RFQDetail;
use App\Exports\RFQSummary;
use Maatwebsite\Excel\Facades\Excel;

class RptFrm280Controller extends Controller
{
    protected $form_id = 280;
    protected $vtid_ref   = 370;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        ->get(); 
        $ObjVendor = DB::table('TBL_TRN_RQFQ01_HDR')
        ->leftJoin('TBL_MST_SUBLEDGER', 'TBL_MST_SUBLEDGER.SGLID','=','TBL_TRN_RQFQ01_HDR.VID_REF')
        ->where('TBL_TRN_RQFQ01_HDR.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_TRN_RQFQ01_HDR.BRID_REF','=',Auth::user()->BRID_REF)
        ...
        ->get();
        $ObjItem = DB::table('TBL_TRN_RQFQ01_HDR')
        ->leftJoin('TBL_TRN_RQFQ01_MAT', 'TBL_TRN_RQFQ01_MAT.RFQID_REF','=','TBL_TRN_RQFQ01_HDR.RFQID')
        ->leftJoin('TBL_MST_ITEM', 'TBL_MST_ITEM.ITEMID','=','TBL_TRN_RQFQ01_MAT.ITEMID_REF')
        ...
        return view('reports.purchase.RFQRpt.rptfrm280',compact(['objRights','objBranchGroup','objBranch','ObjVendor',
   ...
   public function ViewReport($request) {
    ...
    {  
        ...
        {
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/RFQDetail');
        }
        else
        {
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/RFQSummary');
        }
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            if($ReportType == "Detail")
            {
            return Excel::download(new RFQDetail($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$STATUS,$CYID_REF), 'RFQDetail.xlsx');

            }
            else
            {
                return Excel::download(new RFQSummary($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$STATUS,$CYID_REF), 'RFQSummary.xlsx');
            }
            }
