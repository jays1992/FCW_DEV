...
class RptFrm259Controller extends Controller
{
    protected $form_id = 259;
    protected $vtid_ref   = 349;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        $ObjSINO = DB::table('TBL_TRN_SLSI01_HDR')
        ->where('TBL_TRN_SLSI01_HDR.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_TRN_SLSI01_HDR.BRID_REF','=',Auth::user()->BRID_REF)
        ->select('TBL_TRN_SLSI01_HDR.SIID','TBL_TRN_SLSI01_HDR.SINO','TBL_TRN_SLSI01_HDR.SIDT')
        ->distinct('TBL_TRN_SLSI01_HDR.SIID')
        ->get();
        ...
        return view('reports.sales.SalesInvoiceRegister.rptfrm259',compact(['objRights','objBranchGroup','objBranch','ObjCustomer',
                    'ObjSINO','ObjItem','company_check']));        
    }  
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/Sales_Invoice_Register');
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            {
                return Excel::download(new SalesInvoiceRegister($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMID,$STATUS,$CYID_REF,$SINO), 'SalesInvoiceRegister.xlsx');
            }
