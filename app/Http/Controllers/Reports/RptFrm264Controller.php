...
class RptFrm264Controller extends Controller
{
    protected $form_id = 264;
    protected $vtid_ref   = 354;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        $ObjItem = DB::table('TBL_TRN_PRAF01_HDR')
                        ->leftJoin('TBL_TRN_PRAF01_MAT', 'TBL_TRN_PRAF01_MAT.AFPID_REF','=','TBL_TRN_PRAF01_HDR.AFPID')
                        ->leftJoin('TBL_MST_ITEM', 'TBL_MST_ITEM.ITEMID','=','TBL_TRN_PRAF01_MAT.ITEMID_REF')
                        ->leftJoin('TBL_MST_BUSINESSUNIT', 'TBL_MST_BUSINESSUNIT.BUID','=','TBL_MST_ITEM.BUID_REF')
                        ->where('TBL_TRN_PRAF01_HDR.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_TRN_PRAF01_HDR.BRID_REF','=',Auth::user()->BRID_REF)
                        ->where('TBL_TRN_PRAF01_HDR.STATUS','=','A')
        ...
        $objAFPNo =  DB::table('TBL_TRN_PRAF01_HDR')
        ...
        return view('reports.purchase.Annual_Purchase_Forecast_Register.rptfrm264',compact(['objRights','objBranchGroup','objBranch',
                    'ObjItemGrp','ObjItem','objAFPNo','company_check']));        
    }  
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        $Flag            = $myValue['Flag'];
            ...
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/AnnualForecastPurchaseRegister');
