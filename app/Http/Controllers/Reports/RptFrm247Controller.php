...
use Chartblocks;
use App\Exports\SalesServiceOrderDetail;
use App\Exports\SalesServiceOrderSummary;
use Maatwebsite\Excel\Facades\Excel;


class RptFrm247Controller extends Controller
{
    protected $form_id = 247;
    protected $vtid_ref   = 337;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        ->get(); 
        $ObjCustomer = DB::table('TBL_TRN_SLSO04_HDR')
        ->Join('TBL_MST_SUBLEDGER', 'TBL_MST_SUBLEDGER.SGLID','=','TBL_TRN_SLSO04_HDR.SLID_REF')
        ...
        ->where('TBL_MST_EMPLOYEE.BRID_REF','=',Auth::user()->BRID_REF)
        ...
        ->get();
        $ObjItem = DB::table('TBL_TRN_SLSO04_HDR')
        ->Join('TBL_TRN_SLSO04_MAT', 'TBL_TRN_SLSO04_MAT.SSOID_REF','=','TBL_TRN_SLSO04_HDR.SSOID')
        ->Join('TBL_MST_ITEM', 'TBL_MST_ITEM.ITEMID','=','TBL_TRN_SLSO04_MAT.ITEMID_REF')
		->Join('TBL_MST_BUSINESSUNIT', 'TBL_MST_ITEM.BUID_REF','=','TBL_MST_BUSINESSUNIT.BUID')
        ->where('TBL_TRN_SLSO04_HDR.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_TRN_SLSO04_HDR.BRID_REF','=',Auth::user()->BRID_REF)
        ...
        return view('reports.sales.SalesServiceOrderRpt.rptfrm247',compact(['objRights','objBranchGroup','objBranch','ObjCustomer',
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        {
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/SalesServiceOrderDetails');

        }
        else
        {
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/SalesServiceOrderSummary');
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            else if($Flag == 'E')
            {
                if($ReportType == "Detail")
                {
                return Excel::download(new SalesServiceOrderDetail($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$EMPID,$ITEMGID,$ITEMID,$STATUS,$CYID_REF), 'SalesServiceOrderDetail.xlsx');
      
                }
                else
                {
                    return Excel::download(new SalesServiceOrderSummary($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$EMPID,$ITEMGID,$ITEMID,$STATUS,$CYID_REF), 'SalesServiceOrderSummary.xlsx');
                }
