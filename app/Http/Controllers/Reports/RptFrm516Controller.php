...
class RptFrm516Controller extends Controller{

    protected $form_id  =   516;
    protected $vtid_ref =   586;  
    protected $view_ref =   "PendingKnockReport";
    protected $repo_nam =   "Pending Knock Off Report";
    ...
    public function index(){ 
        ...
        $Dealer      =   DB::select("SELECT * FROM TBL_MST_SUBLEDGER
                                WHERE CYID_REF=$CYID_REF AND STATUS='A' AND (DEACTIVATED IS NULL  OR DEACTIVATED = 0)
                                ");


        return view('reports.Accounts.'.$this->view_ref.'.rptfrm'.$FORM_ID,compact(['FORM_ID','REPO_NAM','objRights','objBranchGroup','objBranch',
		'Dealer',]));        
    }  
