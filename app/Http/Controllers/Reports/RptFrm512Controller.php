...
class RptFrm512Controller extends Controller{

    protected $form_id  =   512;
    protected $vtid_ref =   582;  
    protected $view_ref =   "GrossProfit";
    protected $repo_nam =   "Item Wise Gross Profit Report";
    ...
    public function index(){ 
		...
		$ItemList      =   DB::select("SELECT ITEMID,ICODE,NAME FROM TBL_MST_ITEM
                                WHERE CYID_REF=$CYID_REF AND STATUS='A' AND (DEACTIVATED IS NULL  OR DEACTIVATED = 0) AND BRID_REF IN($BRID_REF)
                                ");
