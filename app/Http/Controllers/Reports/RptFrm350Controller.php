...
class RptFrm350Controller extends Controller
{
    protected $form_id = 350;
    protected $vtid_ref   = 162;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        ->get(); 
        $ObjCustomer = DB::table('TBL_TRN_CRSV01_HDR')
        ->leftJoin('TBL_MST_SUBLEDGER', 'TBL_MST_SUBLEDGER.SGLID','=','TBL_TRN_CRSV01_HDR.SGLID_REF')
        ->where('TBL_TRN_CRSV01_HDR.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_TRN_CRSV01_HDR.BRID_REF','=',Auth::user()->BRID_REF)
        ->where('TBL_TRN_CRSV01_HDR.STATUS','=','A')
        ...
        $ObjItem = DB::table('TBL_TRN_CRSV01_HDR')
        ->leftJoin('TBL_TRN_CRSV01_MAT', 'TBL_TRN_CRSV01_MAT.CSVID_REF','=','TBL_TRN_CRSV01_HDR.CSVID')
        ->leftJoin('TBL_MST_ITEM', 'TBL_MST_ITEM.ITEMID','=','TBL_TRN_CRSV01_MAT.ITEMID_REF')
        ...
        $objCSVNo =  DB::table('TBL_TRN_CRSV01_HDR')
        ...
        return view('reports.Accounts.Credit_Note_Register.rptfrm350',compact(['objRights','objBranchGroup','objBranch','ObjCustomer',
                    'ObjItemGrp','ObjItem','objCSVNo','company_check']));        
    }  
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        $Flag            = $myValue['Flag'];
            ...
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/Credit_Note_Register');
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            {
                return Excel::download(new CreditNoteRegister($SGLID,$CSVID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$CYID_REF), 'CreditNoteRegister.xlsx');
