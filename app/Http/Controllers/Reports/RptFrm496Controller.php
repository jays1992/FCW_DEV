...
class RptFrm496Controller extends Controller{

    protected $form_id  =   496;
    protected $vtid_ref =   566;  
    protected $view_ref =   "PurchaseStatusReport";
    protected $repo_nam =   "Purchase Status Report";
    ...
    public function index(){ 
        ...
        $VendorList          =  DB::select("SELECT SLID_REF,VCODE,NAME FROM TBL_MST_VENDOR
                                WHERE SLID_REF IS NOT NULL AND CYID_REF=$CYID_REF AND STATUS='A' AND (DEACTIVATED IS NULL  OR DEACTIVATED = 0) AND BRID_REF IN($BRID_REF)
                                ");




        return view('reports.purchase.'.$this->view_ref.'.rptfrm'.$FORM_ID,compact(['FORM_ID','REPO_NAM','objRights','objBranchGroup','objBranch','VendorList']));        
    }  
