...
class RptFrm344Controller extends Controller
{
    protected $form_id = 344;
    protected $vtid_ref   = 432;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        ->get(); 
        $ObjCustomer = DB::table('TBL_TRN_PDPRO_HDR')
        ->leftJoin('TBL_TRN_PDPRO_MAT', 'TBL_TRN_PDPRO_MAT.PROID_REF','=','TBL_TRN_PDPRO_HDR.PROID')
        ->leftJoin('TBL_MST_SUBLEDGER', 'TBL_MST_SUBLEDGER.SGLID','=','TBL_TRN_PDPRO_MAT.SLID_REF')
        ->where('TBL_TRN_PDPRO_HDR.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_TRN_PDPRO_HDR.BRID_REF','=',Auth::user()->BRID_REF)
     ...
     //  dd($ObjCustomer); 
        ...
        ->get();
        $ObjItem = DB::table('TBL_TRN_PDPRO_HDR')
        ->leftJoin('TBL_TRN_PDPRO_MAT', 'TBL_TRN_PDPRO_MAT.PROID_REF','=','TBL_TRN_PDPRO_HDR.PROID')
        ->leftJoin('TBL_MST_ITEM', 'TBL_MST_ITEM.ITEMID','=','TBL_TRN_PDPRO_MAT.ITEMID_REF')
        ...
        $objPRONo =  DB::table('TBL_TRN_PDPRO_HDR')
        ...
        $objSONo =  DB::table('TBL_TRN_SLSO01_HDR')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
        ->where('BRID_REF','=',Auth::user()->BRID_REF)
        ->where('STATUS','=','A')
        ->select('SOID','SONO')
        ...
        return view('reports.production.Production_Order_Register.rptfrm344',compact(['objRights','objBranchGroup','objBranch','ObjCustomer',
                    'ObjItemGrp','ObjItem','objPRONo','objSONo','company_check']));        
    }  
   ...
   public function ViewReport($request) {
    ...
    if($myValue['Flag'] == 'H')
    {
        ...
        $SALE_ORDER_ID              = $myValue['SALE_ORDER_ID'];
        $PRODUCTION_ORDER_ID        = $myValue['PRODUCTION_ORDER_ID'];
    ...
    else
    {
        ...
        $PRODUCTION_ORDER_ID    = Session::get('PRODUCTION_ORDER_ID');
        ...
        $Flag            = $myValue['Flag'];
           ...
           $ssrs = new \SSRS\Report(Session::get('ssrs_config')['REPORT_URL'], array('username' => Session::get('ssrs_config')['username'], 'password' => Session::get('ssrs_config')['password'])); 
            ...
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/ProductionOrderRegister');
        ...
        $reportParameters = array(
            ...
            'ProductionOrderNo'         => $PRODUCTION_ORDER_ID,         
        );
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            if($Flag == 'H')
            {
                ...
                Session::put('SALE_ORDER_ID', $SALE_ORDER_ID);
                Session::put('PRODUCTION_ORDER_ID', $PRODUCTION_ORDER_ID);
