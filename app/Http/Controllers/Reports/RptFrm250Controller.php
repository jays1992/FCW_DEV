...
use App\Exports\SalesServiceOrderRegister;
use Maatwebsite\Excel\Facades\Excel;

class RptFrm250Controller extends Controller
{
    protected $form_id = 250;
    protected $vtid_ref   = 340;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        ->leftJoin('TBL_MST_SUBLEDGER', 'TBL_MST_SUBLEDGER.SGLID','=','TBL_TRN_SLSO04_HDR.SLID_REF')
        ...
        ->get(); 
        $ObjSSONO = DB::table('TBL_TRN_SLSO04_HDR')
        ->where('TBL_TRN_SLSO04_HDR.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_TRN_SLSO04_HDR.BRID_REF','=',Auth::user()->BRID_REF)
        ->select('TBL_TRN_SLSO04_HDR.SSO_NO','TBL_TRN_SLSO04_HDR.SSOID','TBL_TRN_SLSO04_HDR.SSO_DT')
        ->distinct('TBL_TRN_SLSO04_HDR.SSOID')
        ->get();    
        $ObjSalesPerson = DB::table('TBL_TRN_SLSO04_HDR')
        ->leftJoin('TBL_MST_EMPLOYEE', 'TBL_MST_EMPLOYEE.EMPID','=','TBL_TRN_SLSO04_HDR.SPID_REF')
        ->where('TBL_TRN_SLSO04_HDR.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_TRN_SLSO04_HDR.BRID_REF','=',Auth::user()->BRID_REF)
        ...
        ->leftJoin('TBL_TRN_SLSO04_MAT', 'TBL_TRN_SLSO04_MAT.SSOID_REF','=','TBL_TRN_SLSO04_HDR.SSOID')
        ->leftJoin('TBL_MST_ITEM', 'TBL_MST_ITEM.ITEMID','=','TBL_TRN_SLSO04_MAT.ITEMID_REF')
        ...
        return view('reports.sales.SalesServiceOrderRegister.rptfrm250',compact(['objRights','objBranchGroup','objBranch','ObjCustomer',
                    'ObjSalesPerson','ObjItem','ObjSSONO','company_check']));        
    }  
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/Sales_Service_Order_Register');
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            {
                return Excel::download(new SalesServiceOrderRegister($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$EMPID,$ITEMID,$STATUS,$CYID_REF,$SSOID), 'SalesServiceOrderRegister.xlsx');
            }
