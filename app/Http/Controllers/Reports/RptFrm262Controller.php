...
class RptFrm262Controller extends Controller
{
    protected $form_id = 262;
    protected $vtid_ref   = 352;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        $ObjItem = DB::table('TBL_TRN_SLAF01_HDR')
                        ->leftJoin('TBL_TRN_SLAF01_MAT', 'TBL_TRN_SLAF01_MAT.AFSID_REF','=','TBL_TRN_SLAF01_HDR.AFSID')
                        ->leftJoin('TBL_MST_ITEM', 'TBL_MST_ITEM.ITEMID','=','TBL_TRN_SLAF01_MAT.ITEMID_REF')
                         ->leftJoin('TBL_MST_BUSINESSUNIT', 'TBL_MST_BUSINESSUNIT.BUID','=','TBL_MST_ITEM.BUID_REF')
                        ->where('TBL_TRN_SLAF01_HDR.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_TRN_SLAF01_HDR.BRID_REF','=',Auth::user()->BRID_REF)
        ...
        $objAFSNo =  DB::table('TBL_TRN_SLAF01_HDR')
      ...
      $company_check=$this->AlpsStatus();
        ...
        return view('reports.sales.Annual_Sales_Forecast_Register.rptfrm262',compact(['objRights','objBranchGroup','objBranch',
                    'ObjItemGrp','ObjItem','objAFSNo','company_check']));        
    }  
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        $Flag            = $myValue['Flag'];
            ...
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/Sales_Forecast_Register');
      
        
        $reportParameters = array(
            'pr_cy'                      => Auth::user()->CYID_REF,
            'p_userid'                       => Auth::user()->USERID,
            'FromDate'                       => $From_Date,
            'ToDate'                         => $To_Date,
            'p_branchgroup'                  => $BranchGroup,
            'pr_branch'                      => $BranchName,
            'pr_itemgroup'                   => $ITEMGID,
            'pr_item'                        => $ITEMID,
            'AFSNO'                         => $AFSID,
            'STATUS'                        => $STATUS,
