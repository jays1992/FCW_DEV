...
class RptFrm317Controller extends Controller
{
    protected $form_id = 317;
    ...
    public function index(){    
        ...
        ->where('TBL_MST_BANK.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_BANK.BRID_REF','=',Auth::user()->BRID_REF)
        ->where('TBL_MST_BANK.STATUS','=','A')
        ->where('TBL_MST_BANK.BANK_CASH','=','C')
        ->select('TBL_MST_BANK.BID','TBL_MST_BANK.BCODE','TBL_MST_BANK.NAME')
        ->distinct('TBL_MST_BANK.BID')
        ->get(); 

        return view('reports.Accounts.CashBookRpt.rptfrm317',compact(['objRights','objBranchGroup','objBranch','ObjBank']));        
    }  
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/CashBook');
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            {
                return Excel::download(new CashBook($BANK,$From_Date,$To_Date,$BranchGroup,$BranchName,$CYID_REF), 'CashBook.xlsx');
            }
