...
class RptFrm266Controller extends Controller
{
    protected $form_id = 266;
    protected $vtid_ref   = 356;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        return view('reports.purchase.PurchaseBillRpt.rptfrm266',compact(['objRights','objBranchGroup','objBranch','ObjVendor',
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        {
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/PurchaseBillDetails');

        }
        else
        {
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/PurchaseBillSummary');
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            else if($Flag == 'E')
            {
                if($ReportType == "Detail")
                {
                return Excel::download(new PurchaseBillDetail($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$STATUS,$CYID_REF), 'PurchaseBillDetail.xlsx');
    
                }
                else
                {
                    return Excel::download(new PurchaseBillSummary($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$STATUS,$CYID_REF), 'PurchaseBillSummary.xlsx');
                }
            }
