...
class RptFrm352Controller extends Controller
{
    protected $form_id = 352;
    protected $vtid_ref   = 438;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        $ObjItem = DB::table('TBL_MST_BOM_HDR')
                        ->leftJoin('TBL_MST_BOM_MAT', 'TBL_MST_BOM_MAT.BOMID_REF','=','TBL_MST_BOM_HDR.BOMID')
                        ->leftJoin('TBL_MST_ITEM', 'TBL_MST_ITEM.ITEMID','=','TBL_MST_BOM_MAT.ITEMID_REF')
                        ->where('TBL_MST_BOM_HDR.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_MST_BOM_HDR.BRID_REF','=',Auth::user()->BRID_REF)
                        ->where('TBL_MST_BOM_HDR.STATUS','=','A')
        ...
        $objBOMNo =  DB::table('TBL_MST_BOM_HDR')
        ...
        return view('reports.production.BOM_Checklist.rptfrm352',compact(['objRights','objBranchGroup','objBranch',
                    'ObjItemGrp','ObjItem','objBOMNo']));        
    }  
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        $Flag            = $myValue['Flag'];
            ...
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/BOM_Checklist');
