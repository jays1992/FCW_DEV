...
class RptFrm314Controller extends Controller
{
    protected $form_id = 314;
    protected $vtid_ref   = 402;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        ->where('TBL_MST_BANK.BANK_CASH','=','B')
        ->select('TBL_MST_BANK.BID','TBL_MST_BANK.BCODE','TBL_MST_BANK.NAME')
        ->distinct('TBL_MST_BANK.BID') 
        ->get(); 

        return view('reports.Accounts.BankBookRpt.rptfrm314',compact(['objRights','objBranchGroup','objBranch','ObjBank']));        
    }  
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/BankBook');
        ...
        //return Excel::download(new BankBook($BANK,$From_Date,$To_Date,$BranchGroup,$BranchName,$CYID_REF), 'BankBook.xlsx');
            }
