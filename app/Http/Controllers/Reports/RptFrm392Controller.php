...
class RptFrm392Controller extends Controller
{
    protected $form_id = 392;
    protected $vtid_ref   = 477;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        $ObjGeneralLedger = DB::select("SELECT DISTINCT G.GLID, G.GLCODE, G.GLNAME FROM TBL_MST_GENERALLEDGER G
                           WHERE   (G.STATUS = 'a') AND
                           G.CYID_REF='$CYID_REF'");
        $ObjAccountGroup = DB::select("SELECT       DISTINCT SG.ASGID AS AGID, AG.AGCODE, AG.AGNAME FROM    TBL_MST_ACCOUNTGROUP AG JOIN TBL_MST_ACCOUNTSUBGROUP SG
                            ON SG.AGID_REF=AG.AGID JOIN TBL_MST_GENERALLEDGER G ON SG.ASGID=G.ASGID_REF
                            WHERE        (AG.STATUS = 'a') AND
                            AG.CYID_REF='$CYID_REF' ");
                    //dd($ObjAccountGroup); 
        


        return view('reports.Accounts.PNL.rptfrm392',compact(['objRights','objBranchGroup','objBranch','ObjGeneralLedger','ObjAccountGroup']));        
    }  
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/PNL');
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            {
                return Excel::download(new PNL($GLID,$AGID,$From_Date,$To_Date,$BranchName,$CYID_REF), 'PNL.xlsx');
            }
