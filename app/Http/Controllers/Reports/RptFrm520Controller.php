...
class RptFrm520Controller extends Controller{

    protected $form_id  =   520;
    protected $vtid_ref =   590;  
    protected $view_ref =   "BudgetActualReport";
    protected $repo_nam =   "Budget Actual Report";
    ...
    public function index(){ 
        ...
        $GLList   =   DB::select("SELECT
        DISTINCT
        T3.GLID,CONCAT(T3.GLCODE,' ',T3.GLNAME) AS GLCODE_NAME
        FROM TBL_MST_BUDGET_HDR T1
        LEFT JOIN TBL_MST_BUDGET_MAT T2 ON T2.BUGID_REF=T1.BUGID 
        LEFT JOIN TBL_MST_GENERALLEDGER T3 ON T3.GLID=T2.GLID_REF
        WHERE T1.CYID_REF='$CYID_REF' AND T1.STATUS='A'
        ");

        return view('reports.Accounts.'.$this->view_ref.'.rptfrm'.$FORM_ID,compact(['FORM_ID','REPO_NAM','objRights','objBranchGroup','objBranch','GLList']));        
    }  
