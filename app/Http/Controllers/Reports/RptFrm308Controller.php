...
use Chartblocks;
use App\Exports\VendorQuotationDetail;
use App\Exports\VendorQuotationSummary;
use Maatwebsite\Excel\Facades\Excel;

class RptFrm308Controller extends Controller
{
    protected $form_id = 308;
    protected $vtid_ref   = 371;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        return view('reports.purchase.VQRpt.rptfrm308',compact(['objRights','objBranchGroup','objBranch','ObjVendor',
   ...
   public function ViewReport($request) {
    ...
    {  
        ...
        {
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/VendorQuotation');
        }
        else
        {
             $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/VendorQuotationSummary');
        }
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            else if($Flag == 'E')
            {
                if($ReportType == "Detail")
                {
                return Excel::download(new VendorQuotationDetail($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$STATUS,$CYID_REF), 'VendorQuotationDetail.xlsx');
    
                }
                else
                {
                return Excel::download(new VendorQuotationSummary($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$STATUS,$CYID_REF), 'VendorQuotationSummary.xlsx');
                }
            }
