...
use App\Exports\PurchaseReturnDetail;
use App\Exports\PurchaseReturnSummary;
use Maatwebsite\Excel\Facades\Excel;


class RptFrm267Controller extends Controller
{
    protected $form_id = 267;
    protected $vtid_ref   = 357;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        return view('reports.purchase.PurchaseReturnRpt.rptfrm267',compact(['objRights','objBranchGroup','objBranch','ObjVendor',
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        {
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/PurchaseReturnDetail');

        }
        else
        {
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/PurchaseReturnSummary');
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            else if($Flag == 'E')
            {
                if($ReportType == "Detail")
                {
                return Excel::download(new PurchaseReturnDetail($From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$STATUS,$CYID_REF,$SGLID), 'PurchaseReturnDetail.xlsx');
    
                }
                else
                {
                    return Excel::download(new PurchaseReturnSummary($From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$STATUS,$CYID_REF,$SGLID), 'PurchaseReturnSummary.xlsx');
                }
