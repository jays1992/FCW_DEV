...
class RptFrm515Controller extends Controller{

    protected $form_id  =   515;
    protected $vtid_ref =   585;  
    protected $view_ref =   "SalesIncentive";
    protected $repo_nam =   "Sales Incentive Report";
    ...
    public function index(){ 
        ...
        $Dealer      =   DB::select("SELECT CID,NAME FROM TBL_MST_CUSTOMER
                                WHERE CYID_REF=$CYID_REF AND STATUS='A' AND (DEACTIVATED IS NULL  OR DEACTIVATED = 0) AND TYPE = 'DEALER'
                                ");
		$SPID    	 =   DB::select("SELECT  DISTINCT EMPID, FNAME+'  '+ISNULL(MNAME,'')+' '+ISNULL(LNAME,'') AS EMPNAME FROM TBL_MST_EMPLOYEE
                                WHERE CYID_REF=$CYID_REF AND STATUS='A' AND (DEACTIVATED IS NULL  OR DEACTIVATED = 0) AND SALES_PERSON = 1
                                ");
