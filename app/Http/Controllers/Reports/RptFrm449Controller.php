...
class RptFrm449Controller extends Controller
{
    protected $form_id    = 449;
    protected $vtid_ref   = 519;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        $ObjEarningHead         =       DB::select("SELECT EARNING_HEADID,EARNING_HEADCODE,EARNING_HEAD_DESC FROM TBL_MST_EARNING_HEAD
                                        WHERE CYID_REF=$CYID_REF AND STATUS='A' AND BRID_REF=$BRID_REF AND 
                                        (DEACTIVATED IS NULL  OR DEACTIVATED = 0)
                                        "); 
        ...
        return view('reports.Payroll.PFMonthWiseHeadWiseSummary.rptfrm449',compact([
        'objRights',
        'objBranchGroup',
        'objBranch',
        'ObjPayPeriod',
   ...
   public function ViewReport($request) {
    ...
    if($myValue['Flag'] == 'H')
    {
        ...
        $EARNING_HEADID  = $myValue['EARNING_HEADID'];   
    ...
    else
    {
        ...
        $EARNING_HEADID  = Session::get('EARNING_HEADID');
        $Flag            = $myValue['Flag'];
        $CYID_REF          = Auth::user()->CYID_REF;
    }
          ...
          $ssrs = new \SSRS\Report(Session::get('ssrs_config')['REPORT_URL'], array('username' => Session::get('ssrs_config')['username'], 'password' => Session::get('ssrs_config')['password'])); 
            ...
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/TDSDeductionSummary');
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            if($Flag == 'H')
            {
                ...
                Session::put('EARNING_HEADID', $EARNING_HEADID);           
