...
class RptFrm381Controller extends Controller
{
    protected $form_id = 381;
    protected $vtid_ref   = 467;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
		...
		$BRID_REF = Auth::user()->BRID_REF;
         ...
         $ObjCustomerGroup = DB::select("SELECT * FROM TBL_MST_CUSTOMERGROUP WHERE CYID_REF= $CYID_REF AND BRID_REF=$BRID_REF AND STATUS = 'A' AND (DEACTIVATED = 0 OR
         DEACTIVATED IS NULL)");
        ...
        return view('reports.Accounts.CustomerLedgerDocWise.rptfrm381',compact(['objRights','objBranchGroup','objBranch','ObjCustomerGroup','ObjCustomer']));        
    }  
   ...
   public function ViewReport($request) {
    ...
    $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/Cust_Ledger_Doc_Wise');
        ...
        // dd($ssrs); 
            ...
            {
                return Excel::download(new CustomerLedgerDocWise($CUSTOMER,$CDG,$From_Date,$To_Date,$BranchGroup,$BranchName,$CYID_REF), 'CustomerLedgerDocWise.xlsx');
            }
