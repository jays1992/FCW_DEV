...
class RptFrm256Controller extends Controller
{
    protected $form_id = 256;
    protected $vtid_ref   = 346;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
       ...
       //dd($ObjSalesPerson); 
        ...
        ->where('TBL_TRN_SLSO01_HDR.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_TRN_SLSO01_HDR.BRID_REF','=',Session::get('BRID_REF'))
        ->select('TBL_TRN_SLSO01_HDR.SOID','TBL_TRN_SLSO01_HDR.SONO','TBL_TRN_SLSO01_HDR.SODT')
        ...
        return view('reports.sales.SalesOrderRegister.rptfrm256',compact(['objRights','objBranchGroup','objBranch','ObjCustomer',
                    'ObjSalesPerson','ObjSONO','ObjItem','ObjItemGrp','company_check']));        
    }  
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/Sales_Order_Register');
       ...
       // dd($reportParameters); 
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            {
            return Excel::download(new SalesOrderRegister($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$EMPID,$ITEMGID,$ITEMID,$STATUS,$CYID_REF,$SONO), 'SalesOrderRegister.xlsx');
            }
