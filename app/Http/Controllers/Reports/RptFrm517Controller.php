...
class RptFrm517Controller extends Controller{

    protected $form_id  =   517;
    protected $vtid_ref =   587;  
    protected $view_ref =   "FixedDepositMaturityReport";
    protected $repo_nam =   "Fixed Deposit Maturity Report";
    ...
    public function index(){ 
        ...
        $BankList   =   DB::select("SELECT 
        BID,BCODE,NAME AS BNAME
        FROM TBL_MST_BANK 
        WHERE CYID_REF='$CYID_REF' AND STATUS='A' AND ACTYPE !='FD' AND (DEACTIVATED=0 OR DEACTIVATED IS NULL)
        ");

        $FdBankList   =   DB::select("SELECT 
        BID,BCODE,NAME AS BNAME
        FROM TBL_MST_BANK 
        WHERE CYID_REF='$CYID_REF' AND STATUS='A' AND ACTYPE ='FD' AND (DEACTIVATED=0 OR DEACTIVATED IS NULL) 
        ");

        return view('reports.Accounts.'.$this->view_ref.'.rptfrm'.$FORM_ID,compact(['FORM_ID','REPO_NAM','objRights','objBranchGroup','objBranch','BankList','FdBankList']));        
    }  
