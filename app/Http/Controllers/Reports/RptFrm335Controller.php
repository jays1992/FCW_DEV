...
class RptFrm335Controller extends Controller
{
    protected $form_id = 335;
    protected $vtid_ref   = 423;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        $ObjItem = DB::table('TBL_TRN_PII_HDR')
                        ->leftJoin('TBL_TRN_PII_MAT', 'TBL_TRN_PII_MAT.PII_ID_REF','=','TBL_TRN_PII_HDR.PII_ID')
                        ->leftJoin('TBL_MST_ITEM', 'TBL_MST_ITEM.ITEMID','=','TBL_TRN_PII_MAT.ITEMID_REF')
                        ...
                        ->where('TBL_TRN_PII_HDR.BRID_REF','=',Auth::user()->BRID_REF)
        ...
        $objVendor =  DB::table('TBL_TRN_PII_HDR')
                    ->leftJoin('TBL_MST_SUBLEDGER', 'TBL_MST_SUBLEDGER.SGLID','=','TBL_TRN_PII_HDR.VID_REF')
                    ->where('TBL_TRN_PII_HDR.CYID_REF','=',Auth::user()->CYID_REF)
                    ->where('TBL_TRN_PII_HDR.BRID_REF','=',Auth::user()->BRID_REF)
        ...
        return view('reports.purchase.IPI_Register.rptfrm335',compact(['objRights','objBranchGroup','objBranch',
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        $CYID_REF          = Auth::user()->CYID_REF;
            ...
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/IPI_register');
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            {
                return Excel::download(new IPIRegister($VID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$STATUS,$CYID_REF), 'IPIRegister.xlsx');
            }
