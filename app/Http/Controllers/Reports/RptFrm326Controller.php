...
class RptFrm326Controller extends Controller
{
    protected $form_id = 326;
    protected $vtid_ref   = 414;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        $objRights = DB::table('TBL_MST_USERROLMAP')
            ...
            $ObjVQ = DB::table('TBL_TRN_VDQT01_HDR')
            ...
            ->OrderBy('TBL_TRN_VDQT01_HDR.VQID')
            ->get();

        return view('reports.purchase.VendorQuotationPrint.rptfrm326', compact(['objRights','ObjVQ']));        
    }
   ...
   public function ViewReport($request) {
        ...
        $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/VQPrint');
        
        $reportParameters = array(
            'VQNo' => $objSalesOrder->VQ_NO,
        );
