...
class RptFrm470Controller extends Controller{

    protected $form_id  =   470;
    protected $vtid_ref =   540;  
    protected $view_ref =   "OpportunityRegisterSourceOfEnquiryWise";
    protected $repo_nam =   "Opportunity Register Source Of Enquiry Wise";
    ...
    public function index(){ 
        ...
        $SourceOfEnq    =   DB::select("SELECT 
                            ID,
                            LEAD_SOURCENAME AS SOURCE_OF_ENQ 
                            FROM TBL_MST_LEAD_SOURCE
                            WHERE STATUS='A' AND (DEACTIVATED IS NULL  OR DEACTIVATED = 0)
                            ");


        return view('reports.PreSales.'.$this->view_ref.'.rptfrm'.$FORM_ID,compact(['FORM_ID','REPO_NAM','objRights','objBranchGroup','objBranch','SourceOfEnq']));        
    }  
    ...
    public function ViewReport($request) {
        ...
        if($myValue['Flag'] == 'H'){
            ...
            $SOURCE_OF_ENQUIRY  =   $myValue['SOURCE_OF_ENQUIRY'];
            $Flag               =   $myValue['Flag']; 
        }
        else{
            ...
            $SOURCE_OF_ENQUIRY  =   Session::get('SOURCE_OF_ENQUIRY');
            $Flag               =   $myValue['Flag'];
        }
        ...
        if($Flag == 'H'){
            ...
            Session::put('SOURCE_OF_ENQUIRY', $SOURCE_OF_ENQUIRY);
