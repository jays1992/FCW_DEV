...
class RptFrm245Controller extends Controller
{
    protected $form_id = 245;
    protected $vtid_ref   = 335;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        ->get(); 
        $ObjCustomer = DB::table('TBL_TRN_SLQT01_HDR')
        ->leftJoin('TBL_MST_SUBLEDGER', 'TBL_MST_SUBLEDGER.SGLID','=','TBL_TRN_SLQT01_HDR.SLID_REF')
        ->where('TBL_TRN_SLQT01_HDR.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_TRN_SLQT01_HDR.BRID_REF','=',Auth::user()->BRID_REF)
        ->select('TBL_MST_SUBLEDGER.SGLID','TBL_MST_SUBLEDGER.SGLCODE','TBL_MST_SUBLEDGER.SLNAME')
        ->distinct('TBL_MST_SUBLEDGER.SGLID')
        ->get();     
        $ObjSalesPerson = DB::table('TBL_TRN_SLQT01_HDR')
        ->leftJoin('TBL_MST_EMPLOYEE', 'TBL_MST_EMPLOYEE.EMPID','=','TBL_TRN_SLQT01_HDR.SPID_REF')
        ->where('TBL_TRN_SLQT01_HDR.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_TRN_SLQT01_HDR.BRID_REF','=',Auth::user()->BRID_REF)
        ...
        ->get();
        $ObjItem = DB::table('TBL_TRN_SLQT01_HDR')
        ->leftJoin('TBL_TRN_SLQT01_MAT', 'TBL_TRN_SLQT01_MAT.SQID_REF','=','TBL_TRN_SLQT01_HDR.SQID')
        ->leftJoin('TBL_MST_ITEM', 'TBL_MST_ITEM.ITEMID','=','TBL_TRN_SLQT01_MAT.ITEMID_REF')
        ...
        return view('reports.sales.SalesQuotationRpt.rptfrm245',compact(['objRights','objBranchGroup','objBranch','ObjCustomer',
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        {
        $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/SalesQuotationDetails');

        }
        else
        {
        $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/SalesQuotationSummary');
