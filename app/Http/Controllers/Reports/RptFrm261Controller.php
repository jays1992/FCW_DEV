...
class RptFrm261Controller extends Controller
{
    protected $form_id = 261;
    protected $vtid_ref   = 351;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        ->get(); 
        $ObjSRNO = DB::table('TBL_TRN_SLSR01_HDR')
        ->where('TBL_TRN_SLSR01_HDR.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_TRN_SLSR01_HDR.BRID_REF','=',Auth::user()->BRID_REF)
        ->select('TBL_TRN_SLSR01_HDR.SRNO','TBL_TRN_SLSR01_HDR.SRID','TBL_TRN_SLSR01_HDR.SRDT')
        ->distinct('TBL_TRN_SLSR01_HDR.SRID')
        ->get(); 
        ...
        return view('reports.sales.SalesReturnRegister.rptfrm261',compact(['objRights','objBranchGroup','objBranch','ObjCustomer',
                    'ObjItem','ObjSRNO','company_check']));        
    }  
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/Sales_Return_Register');
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            else if($Flag == 'E')
            {
                ...
                return Excel::download(new SalesReturnRegister($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMID,$STATUS,$CYID_REF,$SRID), 'SalesReturnRegister.xlsx');
            }
