...
use Chartblocks;
use App\Exports\SalesChallanDetail;
use App\Exports\SalesChallanSummary;
use Maatwebsite\Excel\Facades\Excel;

class RptFrm242Controller extends Controller
{
    protected $form_id = 242;
    protected $vtid_ref   = 332;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        return view('reports.sales.SalesChallanRpt.rptfrm242',compact(['objRights','objBranchGroup','objBranch','ObjCustomer',
   ...
   public function ViewReport($request) {
    ...
    //dd($Flag.$ReportType);
        ...
        {
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/SalesChallanDetails');
        }
        else
        {
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/SalesChallanSummary');
       ...
       //dd($reportParameters); 
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            else if($Flag == 'E')
            {
                ...
                if($ReportType == "Detail")
                {
                return Excel::download(new SalesChallanDetail($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$STATUS,$CYID_REF), 'SalesChallanDetail.xlsx');
      
                }
                else
                {                     
                    return Excel::download(new SalesChallanSummary($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$STATUS,$CYID_REF), 'SalesChallanSummary.xlsx');
                }
            }
