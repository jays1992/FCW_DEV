...
class RptFrm255Controller extends Controller
{
    protected $form_id = 255;
    protected $vtid_ref   = 345;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        $objQuotation = DB::table('TBL_TRN_SLQT01_HDR')
        ...
        ->distinct('TBL_TRN_SLQT01_HDR.SQID')
        ->get();
        ...
        $objSubGL = DB::table('TBL_TRN_SLQT01_HDR')
        ->where('TBL_TRN_SLQT01_HDR.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_TRN_SLQT01_HDR.BRID_REF','=',Auth::user()->BRID_REF)
        ...
        return view('reports.sales.SalesQuotationRegister.rptfrm255',compact(['objRights','objBranchGroup','objBranch','objQuotation',
        'objItem','objSubGL','company_check']));        
    }
    ...
    public function ViewReport($request) {
        ...
        else
        {
            ...
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/Sales_Quotation');
