...
class RptFrm330Controller extends Controller
{
    protected $form_id = 330;
    protected $vtid_ref   = 418;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        $ObjVendor = DB::table('TBL_TRN_PROR03_HDR')
                        ->leftJoin('TBL_MST_SUBLEDGER', 'TBL_MST_SUBLEDGER.SGLID','=','TBL_TRN_PROR03_HDR.VID_REF')
                        ->where('TBL_TRN_PROR03_HDR.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_TRN_PROR03_HDR.BRID_REF','=',Auth::user()->BRID_REF)
        ...
        $ObjItem = DB::table('TBL_TRN_PROR03_HDR')
                        ->leftJoin('TBL_TRN_PROR03_MAT', 'TBL_TRN_PROR03_MAT.BPOID_REF','=','TBL_TRN_PROR03_HDR.BPOID')
                        ->leftJoin('TBL_MST_ITEM', 'TBL_MST_ITEM.ITEMID','=','TBL_TRN_PROR03_MAT.ITEMID_REF')
        ...
        return view('reports.purchase.BPO_Register.rptfrm330',compact(['objRights','objBranchGroup','objBranch',
   ...
   public function ViewReport($request) {
    ...
    else
    {
       ...
       // dd($QUANTITY); 
            ...
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/BPO_register');
       ...
       // dd($reportParameters); 
            ...
            {
                return Excel::download(new BPORegister($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$STATUS,$CYID_REF), 'BPORegister.xlsx');
            }
