...
class RptFrm349Controller extends Controller
{
    protected $form_id = 349;
    protected $vtid_ref   = 161;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        ->get(); 
        $ObjVendor = DB::table('TBL_TRN_DRSV01_HDR')
        ->leftJoin('TBL_MST_SUBLEDGER', 'TBL_MST_SUBLEDGER.SGLID','=','TBL_TRN_DRSV01_HDR.SGLID_REF')
        ->where('TBL_TRN_DRSV01_HDR.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_TRN_DRSV01_HDR.BRID_REF','=',Auth::user()->BRID_REF)
        ->where('TBL_TRN_DRSV01_HDR.STATUS','=','A')
        ...
        ->get();
        $ObjItem = DB::table('TBL_TRN_DRSV01_HDR')
        ->leftJoin('TBL_TRN_DRSV01_MAT', 'TBL_TRN_DRSV01_MAT.DSVID_REF','=','TBL_TRN_DRSV01_HDR.DSVID')
        ->leftJoin('TBL_MST_ITEM', 'TBL_MST_ITEM.ITEMID','=','TBL_TRN_DRSV01_MAT.ITEMID_REF')
        ...
        $objDSVNo =  DB::table('TBL_TRN_DRSV01_HDR')
        ...
        return view('reports.Accounts.Debit_Note_Register.rptfrm349',compact(['objRights','objBranchGroup','objBranch','ObjVendor',
                    'ObjItemGrp','ObjItem','objDSVNo','company_check']));        
    }  
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        $CYID_REF        = Auth::user()->CYID_REF;
            ...
            $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/Debit_Note_Register');
        ...
        $ssrs->setSessionId($result->executionInfo->ExecutionID)
            ...
            {
                return Excel::download(new DebitNoteRegister($SGLID,$DSVID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$CYID_REF), 'DebitNoteRegister.xlsx');
            }
