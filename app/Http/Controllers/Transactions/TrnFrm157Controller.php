...
class TrnFrm157Controller extends Controller{

    protected $form_id  = 157;
    protected $vtid_ref = 260;
    protected $view     = "transactions.inventory.ReturnableGatePass.trnfrm";
    ...
    public function index(){    
        ...
        $objDataList	=	DB::select("select hdr.RGPID,hdr.RGP_NO,hdr.RGP_DT,hdr.INDATE,hdr.TYPE,
                            (
                            SELECT TOP 1 USR.DESCRIPTIONS FROM TBL_TRN_AUDITTRAIL AUD 
                            LEFT JOIN TBL_MST_USER USR ON AUD.USERID=USR.USERID 
                            WHERE  AUD.VID=hdr.RGPID AND  AUD.CYID_REF=hdr.CYID_REF AND  AUD.BRID_REF=hdr.BRID_REF AND  
                            AUD.FYID_REF=hdr.FYID_REF AND  AUD.VTID_REF=hdr.VTID_REF AND AUD.ACTIONNAME='ADD'       
                            ) AS CREATED_BY,
                            hdr.STATUS, sl.SLNAME AS VENDOR_NAME,cu.SLNAME AS CUSTOMER_NAME,EMP.FNAME AS EMPLOYEE_NAME,
                            ...
                            inner join TBL_TRN_IRGP01_HDR hdr
                            on a.VID = hdr.RGPID 
    ...
    public function add(){  
        ...
        $ObjUnionUDF = DB::table("TBL_MST_UDFFOR_RGP")->select('*')
                    ->whereIn('PARENTID',function($query) use ($CYID_REF)
                                {       
                                $query->select('UDFRGPID')->from('TBL_MST_UDFFOR_RGP')
                                                ->where('STATUS','=','A')
                                                ->where('PARENTID','=',0)
                                                ->where('DEACTIVATED','=',0)
        ...
        $objUdfData = DB::table('TBL_MST_UDFFOR_RGP')
            ->where('STATUS','=','A')
            ->where('PARENTID','=',0)
            ->where('DEACTIVATED','=',0)
            ->where('CYID_REF','=',$CYID_REF)
        ...
        return view($this->view.$FormId.'add',compact(['AlpsStatus','FormId','objUdfData','objCountUDF','objlastdt','objPriority','TabSetting','MrsNo','Employee','doc_req','docarray']));       
    }
    ...
    public function save(Request $request) {
        ...
        $RGP_DT         = $request['RGP_DT'];
        $VID_REF        = $request['VID_REF'];
        $PRIORITYID_REF = $request['PRIORITYID_REF'];
        $EDA            = $request['EDA'];
        $PURPOSE        = $request['PURPOSE'];
        $RGP_STATUS     = (isset($request['RGP_STATUS'])!="true" ? 1 : 0);
        $TYPE           = $request['TYPE'];
        $ISSUE_TYPE     = $request['ISSUE_TYPE'];

        $log_data = [ 
            $RGP_NO,$RGP_DT,$VID_REF,$PRIORITYID_REF,$EDA,
            $PURPOSE,$CYID_REF,$BRID_REF,$FYID_REF,$VTID_REF,
            $XMLMAT, $XMLUDF,$XMLSTORE,$USERID, Date('Y-m-d'), 
            Date('h:i:s.u'),$ACTIONNAME,$IPADDRESS,$RGP_STATUS,$TYPE,
            $ISSUE_TYPE
        ];

        $sp_result = DB::select('EXEC SP_RGP_IN ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?', $log_data);     
    ...
    public function edit($id=NULL){
        ...
        if(!is_null($id)){
            ...
            $objMAT = DB::table('TBL_TRN_IRGP01_MAT')                    
            ->where('RGPID_REF','=',$id)
            ->orderBy('RGP_MATID','ASC')
            ->get()->toArray();
            ...
            $objMAT =   DB::select("SELECT 
                        ...
                        WHERE T1.RGPID_REF='$id' ORDER BY T1.RGP_MATID ASC
                        ");
            ...
            $objUDF = DB::table('TBL_TRN_IRGP01_UDF')                    
            ->where('RGPID_REF','=',$id)
            ->orderBy('RGP_UDFID','ASC')
            ->get()->toArray();
            ...
            $ObjUnionUDF2 = DB::table("TBL_MST_UDFFOR_RGP")->select('*')
            ->whereIn('PARENTID',function($query) use ($CYID_REF)
                        {       
                        $query->select('UDFRGPID')->from('TBL_MST_UDFFOR_RGP')
                                        ->where('PARENTID','=',0)
            ...
            $objUdfData2 = DB::table('TBL_MST_UDFFOR_RGP')
            ->where('PARENTID','=',0)
            ->where('DEACTIVATED','=',0)
            ->where('CYID_REF','=',$CYID_REF)
            ->union($ObjUnionUDF2)
    ...
    public function update(Request $request){
        ...
        $sp_result = DB::select('EXEC SP_RGP_UP ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?', $log_data);
        
        $contains = Str::contains($sp_result[0]->RESULT, 'SUCCESS');
    
        if($contains){
            return Response::json(['success' =>true,'msg' => $RGP_NO. ' Sucessfully Updated.']);
   ...
   public function Approve(Request $request){
        ...
        if($contains){
            return Response::json(['success' =>true,'msg' => $RGP_NO. ' Sucessfully Approved.']);
    ...
    public function MultiApprove(Request $request){
        ...
        $sp_result = DB::select('EXEC SP_TRN_MULTIAPPROVAL_RGP ?,?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
    ...
    public function cancel(Request $request){
        ...
        $req_data[0]=[
         'NT'  => 'TBL_TRN_IRGP01_MAT',
        ];
        $req_data[1]=[
        'NT'  => 'TBL_TRN_IRGP01_UDF',
        ];
        $req_data[2]=[
        'NT'  => 'TBL_TRN_IRGP01_MULTISTORE',
        ];
        ...
        $sp_result = DB::select('EXEC SP_TRN_CANCEL_RGP  ?,?,?,?, ?,?,?,?, ?,?,?,?', $salesorder_cancel_data);
    ...
    public function attachment($id){
        ...
        if(!is_null($id)){
            ...
            $objResponse = DB::table('TBL_TRN_IRGP01_HDR')->where('RGPID','=',$id)->first();
    ...
    public function docuploads(Request $request){
		...
		$image_path         =   "docs/company".$CYID_REF."/ReturnableGatePass";     
    ...
    public function codeduplicate(Request $request){
        ...
        $RGP_NO  =   trim($request['RGP_NO']);
        $objLabel = DB::table('TBL_TRN_IRGP01_HDR')
    ...
    public function getLastdt(){
        ...
        return  DB::select('SELECT MAX(RGP_DT) RGP_DT FROM TBL_TRN_IRGP01_HDR  
