...
class TrnFrm307Controller extends Controller{
    protected $form_id  =   307;
    protected $vtid_ref =   397;
    protected $view     =   "transactions.Accounts.DebitNoteDsv.trnfrm";
    ...
    public function index(){  
        ...
        $objDataList	=	DB::select("select hdr.DSVID,hdr.DSV_NO,hdr.DSV_DT,hdr.REASON_DR_NOTE,hdr.NARRATION,hdr.INDATE,
                            (
                            SELECT TOP 1 USR.DESCRIPTIONS FROM TBL_TRN_AUDITTRAIL AUD 
                            LEFT JOIN TBL_MST_USER USR ON AUD.USERID=USR.USERID 
                            WHERE  AUD.VID=hdr.DSVID AND  AUD.CYID_REF=hdr.CYID_REF AND  AUD.BRID_REF=hdr.BRID_REF AND  
                            ...
                            inner join TBL_TRN_DRSV01_HDR hdr
                            on a.VID = hdr.DSVID 
    ...
    public function add(){  
        ...
        $ObjUnionUDF = DB::table("TBL_MST_UDFFOR_DRV")->select('*')
                    ->whereIn('PARENTID',function($query) use ($CYID_REF,$BRID_REF,$FYID_REF)
                                {       
                                $query->select('UDFDRVID')->from('TBL_MST_UDFFOR_DRV')
                                                ->where('STATUS','=','A')
                                                ->where('PARENTID','=',0)
                                                ->where('DEACTIVATED','=',0)
        ...
        $objUdfSOData = DB::table('TBL_MST_UDFFOR_DRV')
            ->where('STATUS','=','A')
            ->where('PARENTID','=',0)
            ->where('DEACTIVATED','=',0)
            ->where('CYID_REF','=',$CYID_REF)
        ...
        return view($this->view.$FormId.'add',compact(['AlpsStatus','FormId','objCalculationHeader','objUdfSOData','objCountUDF','TabSetting','doc_req','docarray','objothcurrency']));       
   }
   ...
   public function gettncdetails(Request $request){
    ...
    public function getsubledger(Request $request){
        ...
        $ObjData    =   DB::select("SELECT VID,VCODE,NAME 
                        FROM TBL_MST_VENDOR 
        ...
        if(!empty($ObjData)){
            ...
            foreach ($ObjData as $index=>$dataRow){
                ...
                $row = '';
                $row = $row.'<tr id="subgl_'.$dataRow->VID .'"  class="clssubgl"><td width="50%">'.$dataRow->VCODE;
                $row = $row.'<input type="hidden" id="txtsubgl_'.$dataRow->VID.'" data-desc="'.$dataRow->VCODE .' - ';
                $row = $row.$dataRow->NAME. '" value="'.$dataRow->VID.'"/></td><td>'.$dataRow->NAME.'</td></tr>';
    ...
    public function getCodeNo(Request $request){
        ...
        if(!empty($ObjData)){
            foreach ($ObjData as $index=>$dataRow){
                $row = '';
                $row = $row.'<tr>
                <td class="ROW1"> <input type="checkbox" name="SELECT_'.$fieldid.'[]" id="sqcode_'.$dataRow->PBID .'"  class="clssqid" value="'.$dataRow->PBID.'" ></td>
                <td class="ROW2">'.$dataRow->PB_DOCNO;
                $row = $row.'<input type="hidden" id="txtsqcode_'.$dataRow->PBID.'" data-desc="'.$dataRow->PB_DOCNO.'"  data-descdate="'.$dataRow->PB_DOCDT.'" value="'.$dataRow->PBID.'"/></td>
                <td class="ROW3" >'.$dataRow->PB_DOCDT.'</td></tr>';
                echo $row;
            }
        }else{
            echo '<tr><td colspan="2">Record not found.</td></tr>';
    ...
    public function getItemList(Request $request){
        ...
        T1.ITEMID,T1.ICODE,T1.NAME,T1.ITEM_SPECI,T1.ITEMGID_REF,T1.ICID_REF,
        T2.* 
        ...
        AND ( T1.DEACTIVATED IS NULL OR T1.DEACTIVATED = 0 ) AND T1.STATUS ='$Status' AND T2.PBID_REF='$PBID_REF'");
                ...
                if(!empty($ObjItem)){
                        ...
                        ->first();
                            ...
                            if(($ObjLIST)){
                                ...
                                else
                                {
                                    ...
                                    $StdCost = isset($dataRow->STDCOST) && $dataRow->STDCOST !=""?$dataRow->STDCOST:0;
                    ...
                    $OEM_PART_NO        =   $ItemRowData[0]->OEM_PART_NO;
                        ...
                        $row = $row.'<input type="hidden" id="txtitem_'.$dataRow->ITEMID.'" data-desc="'.$dataRow->ICODE.'" data-desc1="'.$FROMQTY.'" data-desc2="'.$dataRow->BILL_RATEPUOM.'" data-desc3="'.$dataRow->DISCOUNT.'" data-desc4="'.$dataRow->DISC_AMT.'" data-desc5="'.$PBID_REF.'-'.$dataRow->ITEMID.'" data-desc22="'.$ALPS_PART_NO.'" data-desc23="'.$CUSTOMER_PART_NO.'" data-desc24="'.$OEM_PART_NO.'"  value="'.$dataRow->ITEMID.'"/></td> <td style="width:10%;" id="itemname_'.$dataRow->ITEMID.'" >'.$dataRow->NAME;
            ...
            if(!empty($ObjItem)){
                ...
                foreach ($ObjItem as $index=>$dataRow){
                    ...
                    */
                    ...
                    $desc6  =   $PBID_REF.'-'.$ITEMID.'-'.$MRSID_REF.'-'.$PIID_REF.'-'.$RFQID_REF.'-'.$VQID_REF.'-'.$POID_REF.'-'.$GEID_REF.'-'.$GRN_NO.'-'.$IPOID_REF;
    ...
    public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count1; $i++){
            ...
            if(isset($request['ITEMID_REF_'.$i]) && !is_null($request['ITEMID_REF_'.$i])){
                ...
                $req_data[$i] = [
                    ...
                    'DR_NOTE_QTY' => isset($request['SO_QTY_'.$i]) && $request['SO_QTY_'.$i] !=''?$request['SO_QTY_'.$i]:0,
                    'DR_NOTE_RATE' => isset($request['RATEPUOM_'.$i]) && $request['RATEPUOM_'.$i] !=''?$request['RATEPUOM_'.$i]:0,
                    'DR_NOTE_AMT' => isset($request['DISAFTT_AMT_'.$i]) && $request['DISAFTT_AMT_'.$i] !=''?$request['DISAFTT_AMT_'.$i]:0,
                    ...
                    'PBID_REF' => isset($request['SQA_'.$i]) && $request['SQA_'.$i] !=''?$request['SQA_'.$i]:'' ,
                    'PB_QTY' => isset($request['SQMUOMQTY_'.$i]) && $request['SQMUOMQTY_'.$i] !=''?$request['SQMUOMQTY_'.$i]:0,
                    'PB_RATE' => isset($request['SI_RATE_'.$i]) && $request['SI_RATE_'.$i] !=''?$request['SI_RATE_'.$i]:0,
                    'STID' => isset($STID_REF) && $STID_REF !=''?$STID_REF:'',
                    'BATCH_QTY'   => isset($request['HiddenRowId_'.$i]) && $request['HiddenRowId_'.$i] !=''?$request['HiddenRowId_'.$i]:'',
                    'POID_REF' => isset($POID_REF) && $POID_REF !=''?$POID_REF:'',
                    'VQID_REF' => isset($VQID_REF) && $VQID_REF !=''?$VQID_REF:'',
                    'RFQID_REF' => isset($RFQID_REF) && $RFQID_REF !=''?$RFQID_REF:'',
                    'PIID_REF' => isset($PIID_REF) && $PIID_REF !=''?$PIID_REF:'',
                    'GRNID_REF' => isset($GRN_NO) && $GRN_NO !=''?$GRN_NO:'',
                    'GEID_REF' => isset($GEID_REF) && $GEID_REF !=''?$GEID_REF:'',
                    'MRSID_REF' => isset($MRSID_REF) && $MRSID_REF !=''?$MRSID_REF:'',
                    'IPOID_REF' => isset($IPOID_REF) && $IPOID_REF !=''?$IPOID_REF:'',
                ];
            }
        }
        ...
        for ($i=0; $i<=$r_count3; $i++)
        {
                ...
                {
                    $reqdata3[$i] = [
                        'UDFDRVID_REF'   => $request['UDFSOID_REF_'.$i],
        ...
        $DSV_NO     = $request['DSV_NO'];
        $DSV_DT     = $request['DSV_DT'];
        ...
        $log_data = [
            $DSV_NO,$DSV_DT,$SLID_REF, $REASONOFSR,$COMMONNRT,$BILLTO,$SHIPTO,$CYID_REF, $BRID_REF,$FYID_REF,
        ...
        $sp_result = DB::select('EXEC SP_DSV_IN ?,?,?,?,?,?,?,?,?,?, ?,?,?,?,?,?,?,?,?,?,?, ?, ?,?,?,?,?, ?,?,?,?,?', $log_data); 
    ...
    public function edit($id=NULL){
        ...
        if(!is_null($id)){
            ...
            $objSO = DB::table('TBL_TRN_DRSV01_HDR')
            ->where('TBL_TRN_DRSV01_HDR.FYID_REF','=',Session::get('FYID_REF'))
            ->where('TBL_TRN_DRSV01_HDR.CYID_REF','=',Auth::user()->CYID_REF)
            ->where('TBL_TRN_DRSV01_HDR.BRID_REF','=',Session::get('BRID_REF'))
            ->where('TBL_TRN_DRSV01_HDR.DSVID','=',$id)
            ->leftJoin('TBL_MST_CURRENCY', 'TBL_TRN_DRSV01_HDR.CRID_REF','=','TBL_MST_CURRENCY.CRID')
            ->select('TBL_TRN_DRSV01_HDR.*','TBL_MST_CURRENCY.CRDESCRIPTION','TBL_MST_CURRENCY.CRCODE')
            ->first();
            ...
            $objSOMAT = DB::table('TBL_TRN_DRSV01_MAT')
            ->leftJoin('TBL_TRN_PRPB01_HDR', 'TBL_TRN_DRSV01_MAT.PBID_REF','=','TBL_TRN_PRPB01_HDR.PBID')  
            ->leftJoin('TBL_MST_ITEM', 'TBL_TRN_DRSV01_MAT.ITEMID_REF','=','TBL_MST_ITEM.ITEMID')
            ->leftJoin('TBL_MST_UOM', 'TBL_TRN_DRSV01_MAT.UOMID_REF','=','TBL_MST_UOM.UOMID')  
            ->where('TBL_TRN_DRSV01_MAT.DSVID_REF','=',$id)
            ->select(
                'TBL_TRN_DRSV01_MAT.*',
            ...
            ->orderBy('TBL_TRN_DRSV01_MAT.DSV_MATID','ASC')
            ->get()->toArray();

            $objCount1 = count($objSOMAT);

            $objSOCAL = DB::table('TBL_TRN_DRSV01_CAL')                    
            ->where('TBL_TRN_DRSV01_CAL.DSVID_REF','=',$id)
            ->select('TBL_TRN_DRSV01_CAL.*')
            ->orderBy('TBL_TRN_DRSV01_CAL.DSVCALID','ASC')
            ->get()->toArray();
            $objCount4 = count($objSOCAL);

            $objSOUDF = DB::table('TBL_TRN_DRSV01_UDF')                    
            ->where('TBL_TRN_DRSV01_UDF.DSVID_REF','=',$id)
            ->select('TBL_TRN_DRSV01_UDF.*')
            ->orderBy('TBL_TRN_DRSV01_UDF.DSV_UDFID','ASC')
            ->get()->toArray();
            ...
            $ObjUnionUDF2 = DB::table("TBL_MST_UDFFOR_DRV")->select('*')
            ->whereIn('PARENTID',function($query) use ($CYID_REF)
                        {       
                        $query->select('UDFDRVID')->from('TBL_MST_UDFFOR_DRV')
                                        ->where('PARENTID','=',0)
            ...
            $objUdfSOData2 = DB::table('TBL_MST_UDFFOR_DRV')
                ->where('PARENTID','=',0)
                ->where('DEACTIVATED','=',0)
                ->where('CYID_REF','=',$CYID_REF)
                ->union($ObjUnionUDF)
            ...
            return view($this->view.$FormId.'edit',compact([
                'AlpsStatus','FormId','objSO','objRights','objCount1','objCount3','objSOMAT','objSOUDF','objUdfSOData','objSOCAL','objCount4',
                'objShpAddress','objBillAddress','objSQMAT','objUdfSOData2','TAXSTATE','objvendorcode2','ActionStatus','TabSetting',
                'objCalHeader','objCalculationHeader','objCalDetails','objothcurrency'
            ]));
    ...
    public function update(Request $request){
        ...
        $log_data = [ 
            $DSV_NO,            $DSV_DT,                $SLID_REF,                  $REASONOFSR,
            $COMMONNRT,         $BILLTO,                $SHIPTO,                    $CYID_REF, 
            $BRID_REF,          $FYID_REF,              $VTID_REF,                  $XMLMAT,
            $XMLSTORE,          $XMLUDF,                $USERID,                    Date('Y-m-d'), 
            Date('h:i:s.u'),    $ACTIONNAME,            $IPADDRESS,                 $Direct,
            $XMLCAL,            $TRANSACTION_TYPE ,     $FC,                        $CRID_REF,
            $CONVFACT,          $GROSS_TOTAL,           $NET_TOTAL,                 $CGSTAMT,
            $SGSTAMT,           $IGSTAMT,               $DISCOUNT,                  $OTHER_CHARGES
        ];

       

        $sp_result = DB::select('EXEC SP_DSV_UP ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?', $log_data);
        ...
        if($contains){
            return Response::json(['success' =>true,'msg' => $DSV_NO. ' Sucessfully Updated.']);
   ...
   public function Approve(Request $request){
        ...
        if($contains){
            return Response::json(['success' =>true,'msg' => $DSV_NO. ' Sucessfully Approved.']);
    ...
    public function MultiApprove(Request $request){
            ...
            $sp_result = DB::select('EXEC SP_TRN_MULTIAPPROVAL_DSV ?,?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
    ...
    public function cancel(Request $request){
        ...
        $req_data[0]=[
            'NT'  => 'TBL_TRN_DRSV01_MAT',
        ];
        $req_data[1]=[
            'NT'  => 'TBL_TRN_DRSV01_STORE',
        ];
        $req_data[2]=[
            'NT'  => 'TBL_TRN_DRSV01_UDF',
        ];
        ...
        $sp_result = DB::select('EXEC SP_TRN_CANCEL_DSV  ?,?,?,?, ?,?,?,?, ?,?,?,?', $salesorder_cancel_data);
    ...
    public function attachment($id){
        ...
        if(!is_null($id)){
            ...
            $objResponse = DB::table('TBL_TRN_DRSV01_HDR')->where('DSVID','=',$id)->first();
    ...
    public function docuploads(Request $request){
		...
		//$destinationPath = storage_path()."/docs/company".$CYID_REF."/DebitNoteDsv";
        $image_path         =   "docs/company".$CYID_REF."/DebitNoteDsv";     
    ...
    public function getDSVNo(Request $request){
    ...
    public function checkso(Request $request){
        ...
        ->where('TBL_TRN_DRSV01_HDR.BRID_REF','=',Session::get('BRID_REF'))
        ->where('TBL_TRN_DRSV01_HDR.FYID_REF','=',Session::get('FYID_REF'))
        ->where('TBL_TRN_DRSV01_HDR.DSV_NO','=',$DSV_NO)
        ->select('TBL_TRN_DRSV01_HDR.DSV_NO')
        ->first();
    ...
    public function getStoreDetails(Request $request){
        ...
        /*
        $WHERE_GRN_NO   =   $GRN_NO !=""?" AND ST.GRNID_REF='$GRN_NO'":'';
        $WHERE_GEID_REF =   $GEID_REF !=""?" AND ST.GEID_REF='$GEID_REF'":'';
        $WHERE_POID_REF =   $POID_REF !=""?" AND ST.POID_REF='$POID_REF'":'';
        ...
        WHERE ST.ITEMID_REF='$ITEMID_REF' AND ST.MAIN_UOMID_REF='$MAIN_UOMID_REF' $WHERE_GRN_NO $WHERE_GEID_REF $WHERE_POID_REF
        ");
        ...
        echo '<th width="15%">Debit Return Qty (MU)</th>';
        echo '<th hidden>Alt UOM (AU)</th>';
        echo '<th width="15%" hidden>Return Qty (AU)</th>';
        echo '</tr>';
        echo '</thead>';
        echo '<tbody>';
        foreach($objBatch as $key=>$val){
            ...
            echo '<td hidden>'.$val->AltUom.'</td>';
            echo '<td style="width:10%" hidden><input '.$ACTION_TYPE.' type="text" name="AltUserQty_'.$key.'" id="AltUserQty_'.$key.'" value="'.$AluQty.'" readonly class="qtytext" autocomplete="off"  ></td>';
            echo '<td hidden><input type="hidden" name="BATCHID_'.$key.'" id="BATCHID_'.$key.'" value="'.$desc6.'" class="qtytext" ></td>';
            echo '</tr>';
        }*/
