...
class TrnFrm543Controller extends Controller{
    ...
    public function searchCard(Request $request){
        ...
        $data   =   DB::select("SELECT D.DETAIL_ID AS DOC_ID,D.CARD_NO AS DOC_NO,D.NET_AMOUNT AS DOC_DATE,D.NET_AMOUNT,D.VALIDITY_MON,D.DISCOUNT_AMT,D.AMOUNT 
        FROM TBL_MST_V_MASTER_DETAILS D 
        LEFT JOIN TBL_MST_V_MASTER H ON H.DOC_ID=D.DOC_ID_REF
        WHERE H.CYID_REF='$CYID_REF' AND H.FRANCHISE_ID='$BRID_REF' AND H.STATUS='A' AND D.DETAIL_ID NOT IN(SELECT CARDID_REF FROM TBL_TRN_VALUECARD_SALE_HDR WHERE BRID_REF='$BRID_REF') AND (H.DEACTIVATED=0 OR H.DEACTIVATED IS NULL)
        "); 
