   ...
   public function save(Request $request){
    ...
    public function importexcelindb(Request $request){
        ...
        foreach($excelAlldata as $eIndex=>$eRowData){
            ...
            $exist_data =   $doc_no.'###'.$shift.'###'.$pay_period.'###'.$employee_code;
            ...
            if(is_null($this->get_paypreriod_id($pay_period)['PAYPERIODID_REF'])){
            ...
            if(is_null($this->get_attendance_status($attendance_status)['ATTENDANCE_CODEID'])){
                $this->appendLogData($logfile,"Invalid: Material/Attendance Status is not exist in database. check row no ".$eIndex);
                $validationErr=true;
            } 
            ...
            if($validationErr ==false){
                ...
                if($hkey!=""){
                    ...
                    if(!array_key_exists($hkey, $headerArr)) {
                        ...
                        $headerArr[$eRowData["doc_no"]]["material"][$eIndex]["ATTENDANCE_CODEID"]      =   $attendance_status;
        ...
        foreach($headerArr as $hIndex=>$hRowData){
            ...
            $PAYPERIOD_REFARRAY  =   $this->get_paypreriod_id($hRowData["header"]["PAYPERIODID_REF"]);
            $PAYPERIODID_REF         =   $PAYPERIOD_REFARRAY["PAYPERIODID_REF"];
            ...
            if($sp_result){
                ...
                foreach($hRowData["material"] as $pindex=>$prow){
                    ...
                    $ATTENDANCE_CODEID_ARRAY      =   $this->get_attendance_status($prow["ATTENDANCE_CODEID"]);
                    $ATTENDANCEID_REF              =   $ATTENDANCE_CODEID_ARRAY["ATTENDANCE_CODEID"];
                    $ATTENDANCE_CODE                  =   $ATTENDANCE_CODEID_ARRAY["ATTENDANCE_CODE"];
                    $ATTENDANCE_CODE_DESC                  =   $ATTENDANCE_CODEID_ARRAY["ATTENDANCE_CODE_DESC"];
                    ...
                    $strTime = ($FromTime > 0) ? ( $FromTime - 25569 ) * 86400 + $time_format * 86400 :    $time_format * 86401;
                    $FromTimeFormat = date("H:i:s", $strTime);

                    $ToTime = floor($TIME_TO);
                    $totime_format = $TIME_TO;
                    $strToTime = ($ToTime > 0) ? ( $ToTime - 25569 ) * 86400 + $totime_format * 86400 :    $totime_format * 86401;
                    ...
                    $mat_data[]= [
                        ...
                        'ATTENDANCE_STATUS' =>  $ATTENDANCEID_REF,
                        'REMARKS'           =>  $REMARKS,
                    ];
                }
            ...
            if(isset($mat_data) && !empty($mat_data)){
    ...
    public function get_employee($EMPID_REF){
        ...
        $CYID_REF   =   Auth::user()->CYID_REF;        
        $data       =   DB::select("SELECT EMPID,FNAME FROM TBL_MST_EMPLOYEE where EMPCODE='$EMPID_REF' AND CYID_REF='$CYID_REF'");
    ...
    public function get_shift($SHIFTID_REF){
        ...
        $CYID_REF   =   Auth::user()->CYID_REF;        
        $data       =   DB::select("SELECT SHIFTID,SHIFT_NAME,START_TIME,END_TIME FROM TBL_MST_SHIFT where SHIFT_NAME='$SHIFTID_REF' AND CYID_REF='$CYID_REF'");
        $item_array =   array();
        if(!empty($data)){           
            $item_array=array(
    ...
    public function get_paypreriod_id($pay_period){
        ...
        $CYID_REF   =   Auth::user()->CYID_REF;
        $data       =   DB::select("SELECT PAYPERIODID,PAY_PERIOD_DESC FROM TBL_MST_PAY_PERIOD where PAY_PERIOD_DESC = '$pay_period' AND CYID_REF='$CYID_REF'");
        
        $item_array =   array();
        if(!empty($data)){           
            $item_array=array(
                'PAYPERIODID_REF'    =>$data[0]->PAYPERIODID,
            );
        }
        else{
            $item_array=array(
                'PAYPERIODID_REF'    =>NULL,
            );
        }
        return $item_array;
    }
    ...
    public function get_attendance_status($ATTENDANCE_CODEID){

        $CYID_REF   =   Auth::user()->CYID_REF;        
        $data       =   DB::select("SELECT ATTENDANCE_CODEID,ATTENDANCE_CODE,ATTENDANCE_CODE_DESC FROM TBL_MST_ATTENDANCE_CODE where ATTENDANCE_CODE='$ATTENDANCE_CODEID' AND CYID_REF='$CYID_REF'");
        
        $item_array =   array();
        if(!empty($data)){           
            $item_array=array(
                'ATTENDANCE_CODEID'    =>$data[0]->ATTENDANCE_CODEID,
                'ATTENDANCE_CODE'=>$data[0]->ATTENDANCE_CODE,
                'ATTENDANCE_CODE_DESC'=>$data[0]->ATTENDANCE_CODE_DESC,
            );
        }
        else{
            $item_array=array(
                'ATTENDANCE_CODEID'    =>NULL,
                'ATTENDANCE_CODE'=>NULL,
                'ATTENDANCE_CODE_DESC'=>NULL,
            );
        }
