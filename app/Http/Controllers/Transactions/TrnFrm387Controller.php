...
class TrnFrm387Controller extends Controller{

    protected $form_id  = 387;
    protected $vtid_ref = 473;
    protected $view     = "transactions.Production.AdditionalMaterialIssue.trnfrm";
    ...
    public function index(){    
        ...
        $objDataList	=	DB::select("select hdr.MISPID,hdr.MISP_NO,hdr.MISP_DT,hdr.INDATE,
                            (
                            SELECT TOP 1 USR.DESCRIPTIONS FROM TBL_TRN_AUDITTRAIL AUD 
                            LEFT JOIN TBL_MST_USER USR ON AUD.USERID=USR.USERID 
                            WHERE  AUD.VID=hdr.MISPID AND  AUD.CYID_REF=hdr.CYID_REF AND  AUD.BRID_REF=hdr.BRID_REF AND  
                            ...
                            inner join TBL_TRN_MISP01_HDR hdr
                            on a.VID = hdr.MISPID 
    ...
    public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count1; $i++){
            if(isset($request['ITEMID_REF_'.$i])){
                ...
                $req_data[$i] = [
                    'MRSPID_REF'        => $request['RGP_NO_'.$i],
                    'ITEMID_REF'       => $request['ITEMID_REF_'.$i],
                    'ITEM_SPECI'       => $request['Itemspec_'.$i],
                    'STID'             =>  $STID_REF,
                    'MRSP_QTY_BL'       => $request['SE_QTY_'.$i],
        ...
        */
        ...
        $sp_result = DB::select('EXEC SP_MISP_IN ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?', $log_data);  
    ...
    public function edit($id=NULL){
        ...
        if(!is_null($id)){
            ...
            $objResponse =  DB::table('TBL_TRN_MISP01_HDR')
            ...
            $objMAT = DB::select("SELECT 
                    ...
                    LEFT JOIN TBL_TRN_PDPRO_HDR T7 ON T6.PROID_REF=T7.PROID
                    WHERE T1.MISPID_REF='$id' ORDER BY T1.MISP_MATID ASC
                    "); 

            $objCount1 = count($objMAT);  
 
            $objUDF = DB::table('TBL_TRN_MISP01_UDF')                    
            ->where('MISPID_REF','=',$id)
            ->orderBy('MISP_UDFID','ASC')
            ->get()->toArray();
    ...
    public function update(Request $request){
        ...
        */
        ...
        $sp_result = DB::select('EXEC SP_MISP_UP ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?', $log_data);  
   ...
   public function Approve(Request $request){
    ...
    public function cancel(Request $request){
        ...
        $req_data[0]=[
         'NT'  => 'TBL_TRN_MISP01_MAT',
        ];
        $req_data[1]=[
        'NT'  => 'TBL_TRN_MISP01_MULTISTORE',
        ];
        $req_data[2]=[
        'NT'  => 'TBL_TRN_MISP01_UDF',
        ];
        ...
        $sp_result = DB::select('EXEC SP_TRN_CANCEL_MISP  ?,?,?,?, ?,?,?,?, ?,?,?,?', $salesorder_cancel_data);
    ...
    public function attachment($id){
        ...
        if(!is_null($id)){
            ...
            $objResponse = DB::table('TBL_TRN_MISP01_HDR')->where('MISPID','=',$id)->first();
    ...
    public function docuploads(Request $request){
		...
		$image_path         =   "docs/company".$CYID_REF."/AdditionalMaterialIssue";     
    ...
    public function codeduplicate(Request $request){
        ...
        $MISP_NO  =   trim($request['MIS_NO']);
        $objLabel = DB::table('TBL_TRN_MISP01_HDR')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
        ->where('BRID_REF','=',Session::get('BRID_REF'))
        ->where('FYID_REF','=',Session::get('FYID_REF'))
        ->where('MISP_NO','=',$MISP_NO)
        ->select('MISPID')->first();
    ...
    public function getLastdt(){
        ...
        return  DB::select('SELECT MAX(MISP_DT) MISP_DT FROM TBL_TRN_MISP01_HDR  
    ...
    public function getItemDetails(Request $request){
        ...
        INNER JOIN TBL_TRN_MRSP01_MAT T2 ON T1.ITEMID=T2.ITEMID_REF
        WHERE T1.CYID_REF = '$CYID_REF' 
        AND ( T1.DEACTIVATED IS NULL OR T1.DEACTIVATED = 0 ) AND T1.STATUS ='$Status' AND T2.MRSPID_REF='$CodeNoId'");
    ...
    public function getStoreDetails(Request $request){
        ...
        WHERE T1.STATUS='A' AND T1.CYID_REF ='$CYID_REF' AND T1.BRID_REF ='$BRID_REF'  AND T1.ITEMID_REF ='$ITEMID_REF' AND T1.UOMID_REF ='$MAIN_UOMID_REF' AND T1.CURRENT_QTY > 0
        ");
    ...
    public function getCodeNo(Request $request){
        ...
        $ObjData =  DB::select("SELECT MRSPID,MRSP_NO,MRSP_DT,PROID_REF
        FROM TBL_TRN_MRSP01_HDR 
        WHERE CYID_REF='$CYID_REF' AND BRID_REF='$BRID_REF' AND FYID_REF='$FYID_REF' AND DEPID_REF='$DEPID_REF' AND STATUS='A'
        AND MRSPID IN (SELECT MRSPID_REF FROM TBL_TRN_MRSP01_MAT WHERE PENDING_QTY > '0.000')");


        if(!empty($ObjData)){
            foreach ($ObjData as $index=>$dataRow){

                $PRO_DATA =  DB::table('TBL_TRN_PDPRO_HDR')->where('PROID','=',$dataRow->PROID_REF)->select('PROID','PRO_NO')->first();
  
                $row = '';
                $row = $row.'<tr >
                <td class="ROW1"> <input type="checkbox" name="SELECT_'.$fieldid.'[]" id="rgpcode_'.$dataRow->MRSPID .'"  class="clssqid" value="'.$dataRow->MRSPID.'" ></td>
                <td class="ROW2">'.$dataRow->MRSP_NO;
                $row = $row.'<input type="hidden" id="txtrgpcode_'.$dataRow->MRSPID.'" data-desc="'.$dataRow->MRSP_NO.'" data-desc1="'.$PRO_DATA->PRO_NO.'"  value="'.$dataRow->MRSPID.'"/></td>
                <td class="ROW3">'.$dataRow->MRSP_DT.'</td></tr>';
                echo $row;
