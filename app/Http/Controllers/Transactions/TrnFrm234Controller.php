...
class TrnFrm234Controller extends Controller{

    protected $form_id  = 234;
    protected $vtid_ref = 324;
    protected $view     = "transactions.inventory.StockAdjustment.trnfrm";
    ...
    public function index(){    
        ...
        $objDataList	=	DB::select("select hdr.ST_ADJUSTID,hdr.ST_ADJUST_DOCNO,hdr.ST_ADJUST_DOCDT,hdr.ST_ADJUST_TYPE,hdr.INDATE,
                            (
                            SELECT TOP 1 USR.DESCRIPTIONS FROM TBL_TRN_AUDITTRAIL AUD 
                            LEFT JOIN TBL_MST_USER USR ON AUD.USERID=USR.USERID 
                            WHERE  AUD.VID=hdr.ST_ADJUSTID AND  AUD.CYID_REF=hdr.CYID_REF AND  AUD.BRID_REF=hdr.BRID_REF AND  
                            ...
                            inner join TBL_TRN_STOCK_ADJUST_HDR hdr
                            on a.VID = hdr.ST_ADJUSTID 
    ...
    public function add(){       
        ...
        $doc_req    =   array(
            'VTID_REF'=>$this->vtid_ref,
            'HDR_TABLE'=>'TBL_TRN_STOCK_ADJUST_HDR',
            'HDR_ID'=>'ST_ADJUSTID',
            'HDR_DOC_NO'=>'ST_ADJUST_DOCNO',
            'HDR_DOC_DT'=>'ST_ADJUST_DOCDT'
        );
    ...
    public function save(Request $request) {
        ...
        $log_data = [ 
            $ST_ADJUST_DOCNO,$ST_ADJUST_DOCDT,$ST_ADJUST_TYPE,$CYID_REF,$BRID_REF,
            $FYID_REF,$VTID_REF,$XMLMAT,$XMLSTORE,$USERID, 
            Date('Y-m-d'),Date('h:i:s.u'),$ACTIONNAME,$IPADDRESS
        ];  

        $sp_result = DB::select('EXEC SP_STOCK_ADJUSTMENT_IN ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?', $log_data);  
    ...
    public function edit($id=NULL){
        ...
        if(!is_null($id)){
            ...
            $objResponse =  DB::table('TBL_TRN_STOCK_ADJUST_HDR')
            ->where('CYID_REF','=',Auth::user()->CYID_REF)
            ->where('BRID_REF','=',Session::get('BRID_REF'))
            ->where('ST_ADJUSTID','=',$id)
            ->first();
            ...
            WHERE T1.ST_ADJUSTID_REF='$id' ORDER BY T1.ST_ADJUST_MATID ASC");             
    ...
    public function update(Request $request){
        ...
        $sp_result = DB::select('EXEC SP_STOCK_ADJUSTMENT_UP ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?', $log_data);  


        $contains = Str::contains($sp_result[0]->RESULT, 'SUCCESS');
    
        if($contains){
            return Response::json(['success' =>true,'msg' => $ST_ADJUST_DOCNO. ' Sucessfully Updated.']);
   ...
   public function Approve(Request $request){
    ...
    public function MultiApprove(Request $request){
        ...
        $BRID_REF   =   Session::get('BRID_REF');
        $FYID_REF   =   Session::get('FYID_REF');       
        $TABLE      =   "TBL_TRN_STOCK_ADJUST_HDR";
        $FIELD      =   "ST_ADJUSTID";
        $ACTIONNAME     = $Approvallevel;
        $UPDATE     =   Date('Y-m-d');
        $UPTIME     =   Date('h:i:s.u');
    ...
    public function cancel(Request $request){
        ...
        $req_data[0]=[
         'NT'  => 'TBL_TRN_STOCK_ADJUST_MAT',
        ];
        $req_data[1]=[
        'NT'  => 'TBL_TRN_STOCK_ADJUST_STORE',
        ];
        ...
        $sp_result = DB::select('EXEC SP_TRN_CANCEL_STOCK_ADJUST  ?,?,?,?, ?,?,?,?, ?,?,?,?', $salesorder_cancel_data);
    ...
    public function attachment($id){
        ...
        if(!is_null($id)){
            ...
            $objResponse = DB::table('TBL_TRN_STOCK_ADJUST_HDR')->where('ST_ADJUSTID','=',$id)->first();
    ...
    public function docuploads(Request $request){
		...
		$image_path         =   "docs/company".$CYID_REF."/StockAdjustment";     
    ...
    public function getLastdt(){
        ...
        return  DB::select('SELECT MAX(ST_ADJUST_DOCDT) ST_ADJUST_DOCDT FROM TBL_TRN_STOCK_ADJUST_HDR  
