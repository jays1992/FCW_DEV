...
class TrnFrm63Controller extends Controller
{
	...
	public function ViewReport($request) {
		...
		$result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/POPrint');
   ...
   public function gettncdetails(Request $request){
    ...
    public function getVendor(Request $request){
        ...
        $ObjData = DB::select('EXEC sp_get_vendor_popup_enquiry_FRANCHISE ?,?,?,?', $sp_popup);
    ...
    public function getItemDetails(Request $request){        
        ...
        if(!empty($ObjItem)){
            ...
            foreach ($ObjItem as $index=>$dataRow){
                ...
                $objPROR =  DB::select("SELECT TOP 1 PPLM_DET_ID,PURCHASE_PRICE FROM TBL_MST_PPLM_DETAILS  where ITEMID_REF=$dataRow->ITEMID order by PPLM_DET_ID desc");

                if(!empty($objPROR))
				{
					$lstPrice = isset($objPROR[0]->PURCHASE_PRICE) && $objPROR[0]->PURCHASE_PRICE !='' ? $objPROR[0]->PURCHASE_PRICE:NULL ;
                    $ratepuom = isset($objPROR[0]->PURCHASE_PRICE) && $objPROR[0]->PURCHASE_PRICE !='' ? $objPROR[0]->PURCHASE_PRICE:NULL ;
                    $pplmId   = isset($objPROR[0]->PPLM_DET_ID) && $objPROR[0]->PPLM_DET_ID !='' ? $objPROR[0]->PPLM_DET_ID:NULL ;
				}
				else
				{
					$lstPrice = NULL;
                ...
                $row = $row.'<td hidden id="ratepuom_'.$ITEMID.'"><input type="hidden" id="txtratepuom_'.$ITEMID.'" data-desc="'.$ratepuom.'"
                value="'.$ratepuom.'"/></td>';

                $row = $row.'<td hidden id="lppid_'.$ITEMID.'"><input type="hidden" id="txtlppid_'.$ITEMID.'" data-desc="'.$pplmId.'"
                value="'.$pplmId.'"/></td>';
        ...
        for ($i=0; $i<=$r_count1; $i++)
        {
            if(isset($request['ITEMID_REF_'.$i]) && !is_null($request['ITEMID_REF_'.$i]))
            {
                ...
                $req_data[$i] = [
                    ...
                    'PPLM_DET_ID_REF' => $request['PPLM_DET_ID_'.$i],
                ];
            }
        }
    ...
    public function edit($id=NULL){
        ...
        if(!is_null($id)){
            ...
            $objPOMAT = DB::select("SELECT 
                        ...
                        T5.FROM_QTY,T5.TO_QTY,0.00000 AS RATEP_UOM1,T6.PURCHASE_PRICE
                        ...
                        LEFT JOIN TBL_MST_PPLM_DETAILS T6 ON T1.PPLM_DET_ID_REF=T6.PPLM_DET_ID
            ...
            foreach ($objPOMAT as $index => $value) {
              ...
              $objtempUOM =   DB::select('SELECT top 1 * FROM TBL_MST_UOM  
                ...
                $objtempPR1 =  DB::select("SELECT TOP 1 PURCHASE_PRICE FROM TBL_MST_PPLM_DETAILS  where ITEMID_REF=$value->ITEMID_REF order by PPLM_DET_ID desc");

                if(!empty($objtempPR1)){
                    $objPOMAT[$index]->PURCHASE_PRICE = $objtempPR1[0]->PURCHASE_PRICE;
                }
