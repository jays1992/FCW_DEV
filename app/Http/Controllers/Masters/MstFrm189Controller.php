...
use App\Models\Master\TblMstFrm189;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm189Controller extends Controller
{
   
    protected $form_id = 189;
    protected $vtid_ref   = 216;  //voucher type id

    //validation messages
    protected   $messages = [
                    'ATTENDANCE_CODE.required' => 'Required field',
                    'ATTENDANCE_CODE.unique' => 'Duplicate Code',
                    'ATTENDANCE_NAME.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_ATTENDANCE_STATUS')
        ...
        return view('masters.Payroll.AttendanceStatus.mstfrm189',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        $objAttendace = DB::table('TBL_MST_ATTENDANCE_CODE')
        ->where('STATUS','=',"A")
        ...
        return view('masters.Payroll.AttendanceStatus.mstfrm189add',compact(['objAttendace','objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_ATTENDANCE_STATUS')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
   ...
   public function save(Request $request){
        ...
        $ATTENDANCE_CODE       =   strtoupper(trim($request['ATTENDANCE_CODE']) );
        $ATTENDANCE_NAME   =   trim($request['ATTENDANCE_NAME']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_ATTENDANCE_STATUS_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = DB::table('TBL_MST_ATTENDANCE_STATUS')
            ->where('TBL_MST_ATTENDANCE_STATUS.CYID_REF','=',Auth::user()->CYID_REF)
            ->where('TBL_MST_ATTENDANCE_STATUS.ATTENDANCE_STID','=',$id)
            ->leftJoin('TBL_MST_ATTENDANCE_CODE', 'TBL_MST_ATTENDANCE_STATUS.ATTENDANCE_NAME','=','TBL_MST_ATTENDANCE_CODE.ATTENDANCE_CODEID')
            ->select('TBL_MST_ATTENDANCE_STATUS.*', 'TBL_MST_ATTENDANCE_CODE.ATTENDANCE_CODE as ATTENDANCE_CODES','TBL_MST_ATTENDANCE_CODE.ATTENDANCE_CODE_DESC')
            ->first();
            ...
            return view('masters.Payroll.AttendanceStatus.mstfrm189edit',compact(['objResponse','user_approval_level','objRights','objAttendace']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_ATTENDANCE_STATUS_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/AttendanceStatus";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[189,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[189,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[189,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[189,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[189,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.AttendanceStatus.mstfrm189view',compact(['objResponse','objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm189::whereIn('ATTENDANCE_STID',$ids_data)->get();
        
        return view('masters.Payroll.AttendanceStatus.mstfrm189print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm189::where('ATTENDANCE_STID','=',$id)->first();
            ...
            return view('masters.Payroll.AttendanceStatus.mstfrm189attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
    ...
    public function MultiApprove(Request $request){
        ...
        if(!empty($sp_listing_result))
            ...
            $BRID_REF   =   Session::get('BRID_REF');
            $FYID_REF   =   Session::get('FYID_REF');       
            $TABLE      =   "TBL_MST_ATTENDANCE_STATUS";
            $FIELD      =   "ATTENDANCE_STID";
