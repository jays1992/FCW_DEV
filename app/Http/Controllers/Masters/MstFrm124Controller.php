...
use App\Models\Master\TblMstFrm124;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm124Controller extends Controller
{
   
    protected $form_id = 124;
    protected $vtid_ref   = 110;  //voucher type id

    //validation messages
    protected   $messages = [
                    'VCODE.required' => 'Required field',
                    'VCODE.unique' => 'Duplicate Code',
    ...
    public function index(){
        ...
        $objDataList=DB::select("SELECT * FROM TBL_MST_VOUCHERTYPE 
        WHERE CYID_REF='$CYID_REF' ORDER BY VCODE ");

        return view('masters.Common.VoucherTypeMaster.mstfrm124',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        return view('masters.Common.VoucherTypeMaster.mstfrm124add');
    }
   ...
   public function save(Request $request){
        ...
        $rules = [
            'VCODE' => 'required|unique:TBL_MST_VOUCHERTYPE',
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_VOUCHERTYPE_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm124::where('VTID','=',$id)->first();
            ...
            return view('masters.Common.VoucherTypeMaster.mstfrm124edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_VOUCHERTYPE_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/VoucherTypeMst";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[124,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[124,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[124,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[124,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[124,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[124,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm124::where('VTID','=',$id)->first();
            return view('masters.Common.VoucherTypeMaster.mstfrm124view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm124::whereIn('VTID',$ids_data)->get();
        
        return view('masters.Common.VoucherTypeMaster.mstfrm124print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Common.VoucherTypeMaster.mstfrm124attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
