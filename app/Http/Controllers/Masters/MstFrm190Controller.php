...
use App\Models\Master\TblMstFrm190;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm190Controller extends Controller
{
   
    protected $form_id = 190;
    protected $vtid_ref   = 217;  //voucher type id

    //validation messages
    protected   $messages = [
                    'HOLIDAY_TYPE.required' => 'Required field',
                    'HOLIDAY_TYPE.unique' => 'Duplicate Code',
                    'TYPE_DESC.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $objDataList    =   DB::table('TBL_MST_HOLIDAY_TYPE')
            ->where('CYID_REF','=',Auth::user()->CYID_REF)
            ->get();


        return view('masters.Payroll.HolidayType.mstfrm190',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        return view('masters.Payroll.HolidayType.mstfrm190add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_HOLIDAY_TYPE')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
   ...
   public function save(Request $request){
        ...
        $HOLIDAY_TYPE       =   strtoupper(trim($request['HOLIDAY_TYPE']) );
        $TYPE_DESC   =   trim($request['TYPE_DESC']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_HOLIDAY_TYPE_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm190::where('HOLIDAYTYPEID','=',$id)->first();
            ...
            return view('masters.Payroll.HolidayType.mstfrm190edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_HOLIDAY_TYPE_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
        //dd($array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/HolidayType";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[190,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[190,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[190,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[190,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[190,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm190::where('HOLIDAYTYPEID','=',$id)->first();
            return view('masters.Payroll.HolidayType.mstfrm190view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm190::whereIn('HOLIDAYTYPEID',$ids_data)->get();
        
        return view('masters.Payroll.HolidayType.mstfrm190print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.HolidayType.mstfrm190attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
