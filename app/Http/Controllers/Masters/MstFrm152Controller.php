...
use App\Models\Master\TblMstFrm152;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm152Controller extends Controller{
   
    protected $form_id  = 152;
    protected $vtid_ref = 119;
    protected $view     = "masters.Common.ItemCodingDefinition.mstfrm";
    ...
    public function index(){
        ...
        $objDataList    =   DB::table('TBL_MST_ITEMCODE')
    ...
    public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_ITEMCODE')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
        ->where('STATUS','!=','C')
        ->where('DEACTIVATE','!=',1)
        ->count();   
        ...
        if($objLabel > 0){  
            ...
            return Response::json(['exists' =>true,'msg' => 'Item coding definition already created.']);
    ...
    public function save(Request $request){
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_ITEMCODE_DEFINITION_IN ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id)){
            ...
            $objResponse = DB::table('TBL_MST_ITEMCODE')->where('ITEMCODEDEFIID','=',$id)->first();
    ...
    public function update(Request $request){
        ...
        $ITEMCODEDEFIID        =   trim($request['ITEMCODEDEFIID']); 
        ...
        $objLabel           =   DB::table('TBL_MST_ITEMCODE')
                                ->where('ITEMCODEDEFIID','!=',$ITEMCODEDEFIID)
                                ->where('CYID_REF','=',Auth::user()->CYID_REF)
                                ->where('STATUS','!=','C')
                                ->where('DEACTIVATE','=',0)
                                ->first(); 
        ...
        $array_data   = [
            $ITEMCODEDEFIID,$MANUAL_SR, $SYSTEM_GRSR, $MANUAL_MAXLENGTH,$MAX_DIGIT, $NO_START,
            $DEACTIVATED, $DODEACTIVATED, $CYID_REF, $BRID_REF,$FYID_REF, 
            $VTID, $USERID, $UPDATE,$UPTIME, $ACTION, 
            $IPADDRESS,$PREFIX
        ];
        ...
        $sp_result = DB::select('EXEC SP_ITEMCODE_DEFINITION_UP ?,?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?', $array_data);
    ...
    public function docuploads(Request $request){
		...
		$destinationPath = storage_path()."/docs/company".$CYID_REF."/ItemCodingDefinition";
    ...
    public function MultiApprove(Request $request){
        ...
        if(!empty($sp_listing_result))
            ...
            $BRID_REF   =   Session::get('BRID_REF');
            $FYID_REF   =   Session::get('FYID_REF');       
            $TABLE      =   "TBL_MST_ITEMCODE";
            $FIELD      =   "ITEMCODEDEFIID";
