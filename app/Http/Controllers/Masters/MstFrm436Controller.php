...
use App\Models\Master\TblMstFrm436;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm436Controller extends Controller{

    protected $form_id    = 436;
    protected $vtid_ref   = 506;
    protected $view       = "masters.PreSales.OpportunityType.mstfrm";
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_OPPORTUNITY_TYPE')
        ->orderBy('ID','desc')
        ->get();
    ...
    public function save(Request $request){
        ...
        $OPPORTUNITY_TYPECODE   =   trim($request['OPPORTUNITY_TYPECODE']);
        $OPPORTUNITY_TYPENAME   =   trim($request['OPPORTUNITY_TYPENAME']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        $array_data     = [$OPPORTUNITY_TYPECODE, $OPPORTUNITY_TYPENAME, $DEACTIVATED, $DODEACTIVATED,$CYID_REF, $BRID_REF, $FYID_REF, $VTID,  $USERID, $UPDATE,$UPTIME, $ACTION, $IPADDRESS  ];
        
        try {

            $sp_result = DB::select('EXEC SP_OPPORTUNITY_TYPE_IN ?,?,?,?,?,?, ?,?,?,?,?,?, ?', $array_data);
    ...
    public function showRecord($id,$type,$ActionStatus){
        ...
        if(!is_null($id)){
            ...
            $objResponse = DB::table('TBL_MST_OPPORTUNITY_TYPE')
            ->where('ID','=',$id)
            ->first();
    ...
    public function updateRecord($request,$type){
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_OPPORTUNITY_TYPE_UP ?,?,?,?,?,?, ?,?,?,?,?,?,?', $array_data);
    ...
    public function cancel(Request $request){
        ...
        $BRID_REF   =   Session::get('BRID_REF');
        $FYID_REF   =   Session::get('FYID_REF');       
        $TABLE      =   "TBL_MST_OPPORTUNITY_TYPE";
        $FIELD      =   "ID";
        $ID         =   $id;
        $UPDATE     =   Date('Y-m-d');
        $UPTIME     =   Date('h:i:s.u');
