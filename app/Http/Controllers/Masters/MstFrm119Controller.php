...
use App\Models\Master\TblMstFrm119;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm119Controller extends Controller
{
   
    protected $form_id = 119;
    protected $vtid_ref   = 271;  //voucher type id

    //validation messages
    protected   $messages = [
                    'CGROUP.required' => 'Required field',
                    'CGROUP.unique' => 'Duplicate Code',
    ...
    public function index(){
        ...
        $objDataList=DB::select("SELECT * FROM TBL_MST_CUSTOMERGROUP WHERE CYID_REF='$CYID_REF' ORDER BY CGID DESC");

        return view('masters.Sales.CustomerGroup.mstfrm119',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.Sales.CustomerGroup.mstfrm119add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_CUSTOMERGROUP')
   ...
   public function save(Request $request){
        ...
        $CGROUP       =   strtoupper(trim($request['CGROUP']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_CUSTOMERGROUP_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm119::where('CGID','=',$id)->first();
            ...
            return view('masters.Sales.CustomerGroup.mstfrm119edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_CUSTOMERGROUP_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/customergroupmst";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[119,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[119,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[119,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[119,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[119,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[119,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm119::where('CGID','=',$id)->first();
            return view('masters.Sales.CustomerGroup.mstfrm119view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm119::whereIn('CGID',$ids_data)->get();
        
        return view('masters.Sales.CustomerGroup.mstfrm119print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Sales.CustomerGroup.mstfrm119attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
