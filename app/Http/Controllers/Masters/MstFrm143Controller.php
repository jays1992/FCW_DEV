...
use App\Models\Master\TblMstFrm143;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm143Controller extends Controller
{
   
    protected $form_id = 143;
    protected $vtid_ref   = 2;  //voucher type id

    //validation messages
    protected   $messages = [
                    'FORMCODE.required' => 'Required field',
                    'FORMCODE.unique' => 'Duplicate Code',
                    'FORMNAME.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $objDataList=DB::select("SELECT * FROM TBL_MST_FORM ORDER BY FORMNAME");

        return view('masters.Module.FormMaster.mstfrm143',compact(['objRights','objDataList']));

    }

    public function add(){ 
        return view('masters.Module.FormMaster.mstfrm143add');
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_FORM')
   ...
   public function save(Request $request){
        ...
        $rules = [
            'FORMCODE' => 'required|unique:TBL_MST_FORM',
            'FORMNAME' => 'required',          
        ];
        ...
        $FORMCODE       =   strtoupper(trim($request['FORMCODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_FORM_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm143::where('FORMID','=',$id)->first();
            ...
            return view('masters.Module.FormMaster.mstfrm143edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_FORM_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[143,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[143,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[143,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[143,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[143,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[143,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm143::where('FORMID','=',$id)->first();
            return view('masters.Module.FormMaster.mstfrm143view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm143::whereIn('FORMID',$ids_data)->get();
        
        return view('masters.Module.FormMaster.mstfrm143print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Module.FormMaster.mstfrm143attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
