...
use App\Models\Master\TblMstFrm133;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm133Controller extends Controller
{
   
    protected $form_id = 133;
    protected $vtid_ref   = 150;  //voucher type id

    //validation messages
    protected   $messages = [
                    'CRCODE.required' => 'Required field',
                    'CRCODE.unique' => 'Duplicate Code',
                    'CRDESCRIPTION.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $objDataList	=	DB::select("SELECT * FROM TBL_MST_CURRENCY");

        return view('masters.Accounts.CurrencyMaster.mstfrm133',compact(['objRights','objDataList']));

    }

    
    public function add(){ 
        return view('masters.Accounts.CurrencyMaster.mstfrm133add');
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_CURRENCY')
        ->where('CRCODE','=',$CRCODE)
        ->select('CRCODE')
        ->first();
   ...
   public function save(Request $request){
        ...
        $rules = [
            'CRCODE' => 'required|unique:TBL_MST_CURRENCY',
        ...
        $req_data = [
            ...
            'CRCODE'     =>    $request['CRCODE'],
            'CRDESCRIPTION' =>   $request['CRDESCRIPTION']
        ]; 
        ...
        $CRCODE       =   strtoupper(trim($request['CRCODE']) );
        $CRDESCRIPTION   =   trim($request['CRDESCRIPTION']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_CR_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm133::where('CRID','=',$id)->first();
            ...
            return view('masters.Accounts.CurrencyMaster.mstfrm133edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_CR_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/currencymaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[133,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[133,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[133,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[133,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[133,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[133,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm133::where('CRID','=',$id)->first();
            return view('masters.Accounts.CurrencyMaster.mstfrm133view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm133::whereIn('CRID',$ids_data)->get();
        
        return view('masters.Accounts.CurrencyMaster.mstfrm133print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Accounts.CurrencyMaster.mstfrm133attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
