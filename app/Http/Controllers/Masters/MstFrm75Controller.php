...
use App\Models\Master\TblMstFrm75;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm75Controller extends Controller
{
   
    protected $form_id = 75;
    protected $vtid_ref   = 75;  //voucher type id

    //validation messages
    protected   $messages = [
                    'ICCODE.required' => 'Required field',
                    'ICCODE.unique' => 'Duplicate Code',
    ...
    public function index(){
        ...
        $objDataList	=	DB::select("SELECT * FROM TBL_MST_ITEMCATEGORY WHERE CYID_REF='$CYID_REF'  ORDER BY ICID DESC");

        return view('masters.inventory.ItemCategory.mstfrm75',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.inventory.ItemCategory.mstfrm75add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_ITEMCATEGORY')
   ...
   public function save(Request $request){
        ...
        $ICCODE       =   strtoupper(trim($request['ICCODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_ICATEGORY_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm75::where('ICID','=',$id)->first();
            ...
            return view('masters.inventory.ItemCategory.mstfrm75edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_ICATEGORY_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/ItemCategoryMaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[75,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[75,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[75,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[75,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[75,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[75,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm75::where('ICID','=',$id)->first();
            return view('masters.inventory.ItemCategory.mstfrm75view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm75::whereIn('ICID',$ids_data)->get();
        
        return view('masters.inventory.ItemCategory.mstfrm75print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.inventory.ItemCategory.mstfrm75attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
