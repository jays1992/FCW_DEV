...
//use App\Models\Master\TblMstFrm214;
...
class MstFrm214Controller extends Controller
{
    protected $form_id = 214;
    protected $vtid_ref   = 314;  //voucher type id
    // //validation messages
    protected   $messages = [
    ...
    public function index(){    
        $objRights = DB::table('TBL_MST_USERROLMAP')
                        ...
                        ->first();
        $objCount = DB::table('TBL_MST_UDFFOR_BPV')
                        ->where('TBL_MST_UDFFOR_BPV.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_MST_UDFFOR_BPV.BRID_REF','=',Session::get('BRID_REF'))
                        ->where('TBL_MST_UDFFOR_BPV.FYID_REF','=',Session::get('FYID_REF'))
                        ->where('TBL_MST_UDFFOR_BPV.STATUS', '!=', 'C')
                        ->where('TBL_MST_UDFFOR_BPV.PARENTID','=', 0)
                        ->select('TBL_MST_UDFFOR_BPV.*')
                        ->count();

       

        return view('masters.Accounts.UDFBPV.mstfrm214',compact(['objRights','objCount']));        
    }

    public function add(){       
        return view('masters.Accounts.UDFBPV.mstfrm214add');       
   }
   ...
   public function attachment($id){
    ...
    if(!is_null($id))
    {
        $objResponse = DB::table("TBL_MST_UDFFOR_BPV")->where('UDFBPVID','=',$id)->first();
        ...
        $objMstVoucherType = DB::table("TBL_MST_VOUCHERTYPE")
            ...
            return view('masters.Accounts.UDFBPV.mstfrm214attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
    }
   ...
   public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            ...
            {
                $req_data[$i] = [
                    'UDFBPVID' => (isset($request['UDFBPVID_'.$i]) ? $request['UDFBPVID_'.$i] : "0") ,
            ...
            $sp_result = DB::select('EXEC SP_UDFFOR_BPV_INUPDE ?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
     ...
     public function getListing(Request $request){
        ...
        $COL_APP_STATUS =   'STATUS';  //never change value, value must be 'APPROVED_STATUS' as per stored procedure;
            ...
            $BRID_REF      =   Session::get('BRID_REF');
            $FYID_REF      =   Session::get('FYID_REF');       
            $TABLE1        =   "TBL_MST_UDFFOR_BPV";
            $PK_COL        =   "UDFBPVID";
            $SELECT_COL    =   "UDFBPVID,LABEL,VALUETYPE,DESCRIPTIONS,ISMANDATORY,DEACTIVATED,DODEACTIVATED";    
            ...
            if(!empty($request->input('search.value')))
            {
                ...
                if($filtercolumn =='ALL'){
                    ...
                    $WHERE_COL =  " WHERE PARENTID = 0 AND UDFBPVID LIKE  ". $search_text;
            ...
            if(!empty($sp_listing_result))
            {
                foreach ($sp_listing_result as $key=>$valueitem)
                {
                    ...
                    $nestedData['NO']           = '<input type="checkbox" id="chkId'.$valueitem->UDFBPVID.'"  value="'.$valueitem->UDFBPVID.'" class="js-selectall1" data-rcdstatus="'.$app_status.'">';
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            $objUdfResponse = DB::table('TBL_MST_UDFFOR_BPV')                    
                             ->where('TBL_MST_UDFFOR_BPV.UDFBPVID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_BPV.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_BPV.*')
                             ->orderBy('TBL_MST_UDFFOR_BPV.UDFBPVID','ASC')
                             ->get()->toArray();
            ...
            $objRights = DB::table('TBL_MST_USERROLMAP')
                             ...
                             ->select('TBL_MST_USERROLMAP.*', 'TBL_MST_ROLEDETAILS.*')
                             ->first();
           return view('masters.Accounts.UDFBPV.mstfrm214edit',compact(['objUdfResponse','objRights','objCount']));
        }
       ...
       public function view($id){
            ...
            if(!is_null($id))
            {
                // $objUdfResponse = TblMstFrm214::where('UDFBPVID','=',$id)->first();
                $objUdfResponse = DB::table('TBL_MST_UDFFOR_BPV')                    
                                ->where('TBL_MST_UDFFOR_BPV.UDFBPVID','=',$id)
                                ->orwhere('TBL_MST_UDFFOR_BPV.PARENTID','=',$id)
                                ->select('TBL_MST_UDFFOR_BPV.*')
                                ->orderBy('TBL_MST_UDFFOR_BPV.UDFBPVID','ASC')
                                ->get()->toArray();
                return view('masters.Accounts.UDFBPV.mstfrm214view',compact(['objUdfResponse']));
            }
   ...
   public function update(Request $request){
    ...
    // dd($request->VALUETYPE);
        $data = [
            'UDFBPVID' => (isset($request->UDFBPVID_0) ? $request->UDFBPVID_0 : "0"),
            'LABEL'    => strtoupper($request->LABEL_0),
            'VALUETYPE' => $request->VALUETYPE_0,
            'DESCRIPTIONS' => (isset($request->DESCRIPTIONS_0)? $request->DESCRIPTIONS_0 : ""),
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            if((isset($request['UDFBPVID_'.$i])) && (isset($request['LABEL_'.$i])))
            {
                $req_data[$i] = [
                    'UDFBPVID' => (isset($request['UDFBPVID_'.$i]) ? $request['UDFBPVID_'.$i] : "0") ,
                    'LABEL'    => strtoupper($request['LABEL_'.$i]),
   ...
   //save data
        ...
        $cancelData[0]= ['NT' =>'TBL_MST_UDFFOR_BPV'];
   ...
   public function docuploads(Request $request){
    ...
    $destinationPath = storage_path()."/docs/company".$CYID_REF."/udbpv"; 
        $image_path         =   "docs/company".$CYID_REF."/udbpv";     
    ...
    if(empty($uploaded_data)){
        return redirect()->route("master",[214,"attachment",$ATTACH_DOCNO])->with("success","Already exists. No file uploaded");
    }
  ...
  //  }
    ...
    if($sp_result[0]->RESULT=="SUCCESS"){
        ...
        return redirect()->route("master",[214,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


    }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
   
        return redirect()->route("master",[214,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);

    }else{

        return redirect()->route("master",[214,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
    }
    ...
    public function checkLabel(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_UDFFOR_BPV')
        ->where('TBL_MST_UDFFOR_BPV.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_UDFFOR_BPV.BRID_REF','=',Session::get('BRID_REF'))
        ->where('TBL_MST_UDFFOR_BPV.FYID_REF','=',Session::get('FYID_REF'))
        ->where('TBL_MST_UDFFOR_BPV.LABEL','=',$LABEL)
        ->select('TBL_MST_UDFFOR_BPV.UDFBPVID')
        ->first();
