...
use App\Models\Master\TblMstFrm16;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm16Controller extends Controller
{
   
    protected $form_id = 16;
    protected $vtid_ref   = 16;  //voucher type id

    //validation messages
    protected   $messages = [
                    'PRIORITYCODE.required' => 'Required field',
                    'PRIORITYCODE.unique' => 'Duplicate Code',
    ...
    public function index(){
        ...
        $objDataList=DB::select("SELECT * FROM TBL_MST_PRIORITY ORDER BY PRIORITYID DESC");


        return view('masters.Sales.Priority.mstfrm16',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.Sales.Priority.mstfrm16add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_PRIORITY')
   ...
   public function save(Request $request){
        ...
        $PRIORITYCODE       =   strtoupper(trim($request['PRIORITYCODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_PRIORITY_in ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm16::where('PRIORITYID','=',$id)->first();
            ...
            return view('masters.Sales.Priority.mstfrm16edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_PRIORITY_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/prioritymst";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[16,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[16,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[16,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[16,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[16,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[16,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm16::where('PRIORITYID','=',$id)->first();
            return view('masters.Sales.Priority.mstfrm16view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm16::whereIn('PRIORITYID',$ids_data)->get();
        
        return view('masters.Sales.Priority.mstfrm16print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Sales.Priority.mstfrm16attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
   ...
   //save data
        ...
        $mstcountry_cancel_data = [ $USERID_REF, $VTID_REF, $TABLE, $FIELD, $ID, $CYID_REF, $BRID_REF,$FYID_REF,$UPDATE,$UPTIME, $IPADDRESS ];
