...
use App\Models\Master\TblMstFrm437;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm437Controller extends Controller{

    protected $form_id    = 437;
    protected $vtid_ref   = 507;
    protected $view       = "masters.PreSales.OpportunityStage.mstfrm";
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_OPPORTUNITY_STAGE')
        ->orderBy('ID','desc')
        ->get();
    ...
    public function save(Request $request){
        ...
        $OPPORTUNITY_STAGECODE   =   trim($request['OPPORTUNITY_STAGECODE']);
        $OPPORTUNITY_STAGENAME   =   trim($request['OPPORTUNITY_STAGENAME']);
        $COMPLETE_PERCENT        =   trim($request['COMPLETE_PERCENT']);
        ...
        $array_data     = [$OPPORTUNITY_STAGECODE, $OPPORTUNITY_STAGENAME, $DEACTIVATED, $DODEACTIVATED,$CYID_REF, $BRID_REF, $FYID_REF, $VTID,  $USERID, $UPDATE,$UPTIME, $ACTION, $IPADDRESS, $COMPLETE_PERCENT  ];
        
        try {

            $sp_result = DB::select('EXEC SP_OPPORTUNITY_STAGE_IN ?,?,?,?,?,?, ?,?,?,?,?,?, ?,?', $array_data);
    ...
    public function showRecord($id,$type,$ActionStatus){
        ...
        if(!is_null($id)){
            ...
            $objResponse = DB::table('TBL_MST_OPPORTUNITY_STAGE')
            ->where('ID','=',$id)
            ->first();
    ...
    public function updateRecord($request,$type){
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_OPPORTUNITY_STAGE_UP ?,?,?,?,?,?, ?,?,?,?,?,?,?,?', $array_data);
    ...
    public function cancel(Request $request){
        ...
        $BRID_REF   =   Session::get('BRID_REF');
        $FYID_REF   =   Session::get('FYID_REF');       
        $TABLE      =   "TBL_MST_OPPORTUNITY_STAGE";
        $FIELD      =   "ID";
        $ID         =   $id;
        $UPDATE     =   Date('Y-m-d');
        $UPTIME     =   Date('h:i:s.u');
