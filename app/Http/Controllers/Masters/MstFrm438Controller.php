...
use App\Models\Master\TblMstFrm438;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm438Controller extends Controller{

    protected $form_id    = 438;
    protected $vtid_ref   = 508;
    protected $view       = "masters.PreSales.TaskStatus.mstfrm";
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_TASK_STATUS')
        ->orderBy('ID','desc')
        ->get();
    ...
    public function save(Request $request){
        ...
        $TASK_STATUSCODE   =   trim($request['TASK_STATUSCODE']);
        $TASK_STATUSNAME   =   trim($request['TASK_STATUSNAME']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        $array_data     = [$TASK_STATUSCODE, $TASK_STATUSNAME, $DEACTIVATED, $DODEACTIVATED,$CYID_REF, $BRID_REF, $FYID_REF, $VTID,  $USERID, $UPDATE,$UPTIME, $ACTION, $IPADDRESS  ];
        
        try {

            $sp_result = DB::select('EXEC SP_TASK_STATUS_IN ?,?,?,?,?,?, ?,?,?,?,?,?, ?', $array_data);
    ...
    public function showRecord($id,$type,$ActionStatus){
        ...
        if(!is_null($id)){
            ...
            $objResponse = DB::table('TBL_MST_TASK_STATUS')
            ->where('ID','=',$id)
            ->first();
    ...
    public function updateRecord($request,$type){
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_TASK_STATUS_UP ?,?,?,?,?,?, ?,?,?,?,?,?,?', $array_data);
