...
use App\Models\Master\TblMstFrm429;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm429Controller extends Controller{

    protected $form_id    = 429;
    protected $vtid_ref   = 499;
    protected $view       = "masters.PreSales.ActivityType.mstfrm";
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_ACTIVITY_TYPE')->get();
    ...
    public function save(Request $request){
        ...
        $ACTIVITYCODE       =   trim($request['ACTIVITYCODE']);
        $ACTIVITYNAME       =   trim($request['ACTIVITYNAME']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        $array_data   = [$ACTIVITYCODE, $ACTIVITYNAME, $DEACTIVATED, $DODEACTIVATED,$CYID_REF, $BRID_REF, $FYID_REF, $VTID,  $USERID, $UPDATE,$UPTIME, $ACTION, $IPADDRESS  ];
        
        try {

            $sp_result = DB::select('EXEC SP_ACTIVITY_TYPE_IN ?,?,?,?,?,?, ?,?,?,?,?,?, ?', $array_data);
    ...
    public function showRecord($id,$type,$ActionStatus){
        ...
        if(!is_null($id)){
            ...
            $objResponse = DB::table('TBL_MST_ACTIVITY_TYPE')
            ->where('ID','=',$id)
            ->first();
    ...
    public function updateRecord($request,$type){
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_ACTIVITY_TYPE_UP ?,?,?,?,?,?, ?,?,?,?,?,?,?', $array_data);
