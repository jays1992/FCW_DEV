...
use App\Models\Master\TblMstFrm52;
...
class MstFrm52Controller extends Controller
{
    protected $form_id = 52;
    protected $vtid_ref   = 52;  //voucher type id
    // //validation messages
    protected   $messages = [
    ...
    public function index(){    
        $objRights = DB::table('TBL_MST_USERROLMAP')
                        ...
                        ->first();
        $objCount = DB::table('TBL_MST_UDFFORPO')
                        ->where('TBL_MST_UDFFORPO.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_MST_UDFFORPO.STATUS', '!=', 'C')
                        ->where('TBL_MST_UDFFORPO.PARENTID','=', 0)
                        ->select('TBL_MST_UDFFORPO.*')
                        ->count();

       

        return view('masters.Purchase.UDFPO.mstfrm52',compact(['objRights','objCount']));        
    }

    public function add(){       
        return view('masters.Purchase.UDFPO.mstfrm52add');       
   }
   ...
   public function attachment($id){
    ...
    if(!is_null($id))
    {
        $objResponse = TblMstFrm52::where('UDFID','=',$id)->first();
        ...
        $objMstVoucherType = DB::table("TBL_MST_VOUCHERTYPE")
            ...
            return view('masters.Purchase.UDFPO.mstfrm52attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
    }
   ...
   public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            ...
            $sp_result = DB::select('EXEC SP_UDFFOR_PO_INUPDE ?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
        ...
        $COL_APP_STATUS =   'STATUS';  //never change value, value must be 'APPROVED_STATUS' as per stored procedure;
            ...
            if(!empty($sp_listing_result))
            {
                foreach ($sp_listing_result as $key=>$valueitem)
                {
                    ...
                    $nestedData['NO']           = '<input type="checkbox" id="chkId'.$valueitem->UDFID.'"  value="'.$valueitem->UDFID.'" class="js-selectall1" data-rcdstatus="'.$app_status.'">';
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            $objUdfResponse = DB::table('TBL_MST_UDFFORPO')                    
                             ->where('TBL_MST_UDFFORPO.UDFID','=',$id)
                             ->orwhere('TBL_MST_UDFFORPO.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFORPO.*')
                             ->orderBy('TBL_MST_UDFFORPO.UDFID','ASC')
                             ->get()->toArray();
            ...
            $objRights = DB::table('TBL_MST_USERROLMAP')
                             ...
                             ->select('TBL_MST_USERROLMAP.*', 'TBL_MST_ROLEDETAILS.*')
                             ->first();
           return view('masters.Purchase.UDFPO.mstfrm52edit',compact(['objUdfResponse','objRights','objCount']));
        }
       ...
       public function view($id){
         ...
         if(!is_null($id))
         {
             // $objUdfResponse = TblMstFrm52::where('UDFID','=',$id)->first();
             $objUdfResponse = DB::table('TBL_MST_UDFFORPO')                    
                             ->where('TBL_MST_UDFFORPO.UDFID','=',$id)
                             ->orwhere('TBL_MST_UDFFORPO.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFORPO.*')
                             ->orderBy('TBL_MST_UDFFORPO.UDFID','ASC')
                             ->get()->toArray();
             return view('masters.Purchase.UDFPO.mstfrm52view',compact(['objUdfResponse']));
         }
   ...
   public function docuploads(Request $request){
    ...
    $destinationPath = storage_path()."/docs/company".$CYID_REF."/udfpomst";
    ...
    if(empty($uploaded_data)){
        return redirect()->route("master",[52,"attachment",$ATTACH_DOCNO])->with("success","Already exists. No file uploaded");
    }
  ...
  //  }
    ...
    if($sp_result[0]->RESULT=="SUCCESS"){
        ...
        return redirect()->route("master",[52,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


    }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
   
        return redirect()->route("master",[52,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);

    }else{

        return redirect()->route("master",[52,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
    }
    ...
    public function checkLabel(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_UDFFORPO')
        ->where('TBL_MST_UDFFORPO.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_UDFFORPO.LABEL','=',$LABEL)
        ->select('TBL_MST_UDFFORPO.UDFID')
        ->first();
