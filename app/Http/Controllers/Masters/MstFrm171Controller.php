...
use App\Models\Master\TblMstFrm171;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm171Controller extends Controller
{
   
    protected $form_id = 171;
    protected $vtid_ref   = 197;  //voucher type id

    //validation messages
    protected   $messages = [
                    'DESGCODE.required' => 'Required field',
                    'DESGCODE.unique' => 'Duplicate Code',
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
            ...
            $objDataList = DB::table('TBL_MST_DESIGNATION')
            ->where('TBL_MST_DESIGNATION.CYID_REF','=',Auth::user()->CYID_REF)
             ->leftJoin('TBL_MST_POSTLEVEL', 'TBL_MST_DESIGNATION.PLID_REF','=','TBL_MST_POSTLEVEL.PLID')   
            ->select('TBL_MST_DESIGNATION.*','TBL_MST_POSTLEVEL.DESCRIPTIONS as PDESCRCRIPTIONS'
                )
            ->get();
  

        return view('masters.Payroll.DesignationMaster.mstfrm171',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        $PostLevelList    =   DB::table('TBL_MST_POSTLEVEL')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
        ->where('STATUS','=','A')
        ->select('TBL_MST_POSTLEVEL.*')
        ->get();
        ...
        if(!empty($objDD)){
            if($objDD->SYSTEM_GRSR == "1")
            {
              ...
              return view('masters.Payroll.DesignationMaster.mstfrm171add',compact(['PostLevelList','objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_DESIGNATION')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
   ...
   public function save(Request $request){
        ...
        $DESGCODE       =   strtoupper(trim($request['DESGCODE']) );
        $DESCRIPTIONS   =   trim($request['DESCRIPTIONS']);  
        $PLID_REF   =   trim($request['PLID_REF']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        $array_data   = [
                        $DESGCODE, $DESCRIPTIONS, $PLID_REF,
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_DESIGNATION_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
         ...
         //   $objResponse = TblMstFrm171::where('DESGID','=',$id)->first();


            $objResponse = DB::table('TBL_MST_DESIGNATION')
            ->where('TBL_MST_DESIGNATION.CYID_REF','=',Auth::user()->CYID_REF)
            ->where('TBL_MST_DESIGNATION.DESGID','=',$id)
             ->leftJoin('TBL_MST_POSTLEVEL', 'TBL_MST_DESIGNATION.PLID_REF','=','TBL_MST_POSTLEVEL.PLID')   
            ->select('TBL_MST_DESIGNATION.*','TBL_MST_POSTLEVEL.DESCRIPTIONS as PDESCRCRIPTIONS')
            ->first();
            ...
            return view('masters.Payroll.DesignationMaster.mstfrm171edit',compact(['objResponse','user_approval_level','objRights','PostLevelList','PostLevelList']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_DESIGNATION_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?', $array_data);
        //dd($array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/DesignationMaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[171,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[171,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[171,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[171,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[171,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.DesignationMaster.mstfrm171view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm171::whereIn('PLID',$ids_data)->get();
        
        return view('masters.Payroll.DesignationMaster.mstfrm171print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.DesignationMaster.mstfrm171attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
