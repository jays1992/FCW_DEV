...
use App\Models\Master\TblMstFrm14;
...
class MstFrm14Controller extends Controller
{
    protected $form_id = 14;
    protected $vtid_ref   = 14;  //voucher type id
    // //validation messages
    protected   $messages = [
    ...
    public function index(){    
        $objRights = DB::table('TBL_MST_USERROLMAP')
                        ...
                        ->first();
        $objCount = DB::table('TBL_MST_UDFFORSO')
                        ->where('TBL_MST_UDFFORSO.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_MST_UDFFORSO.STATUS', '!=', 'C')
                        ->where('TBL_MST_UDFFORSO.PARENTID','=', 0)
                        ->select('TBL_MST_UDFFORSO.*')
                        ->count();

       

        return view('masters.Sales.UDFSO.mstfrm14',compact(['objRights','objCount']));        
    }

    public function add(){       
        return view('masters.Sales.UDFSO.mstfrm14add');       
   }
   ...
   public function attachment($id){
    ...
    if(!is_null($id))
    {
        $objResponse = TblMstFrm14::where('UDFID','=',$id)->first();
        ...
        $objMstVoucherType = DB::table("TBL_MST_VOUCHERTYPE")
            ...
            return view('masters.Sales.UDFSO.mstfrm14attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
    }
   ...
   public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            ...
            $sp_result = DB::select('EXEC SP_UDFFORSO_INUPDE ?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            $objUdfResponse = DB::table('TBL_MST_UDFFORSO')                    
                             ->where('TBL_MST_UDFFORSO.UDFID','=',$id)
                             ->orwhere('TBL_MST_UDFFORSO.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFORSO.*')
                             ->orderBy('TBL_MST_UDFFORSO.UDFID','ASC')
                             ->get()->toArray();
            ...
            $objRights = DB::table('TBL_MST_USERROLMAP')
                             ...
                             ->select('TBL_MST_USERROLMAP.*', 'TBL_MST_ROLEDETAILS.*')
                             ->first();
           return view('masters.Sales.UDFSO.mstfrm14edit',compact(['objUdfResponse','objRights','objCount']));
        }
       ...
       public function view($id){
         ...
         if(!is_null($id))
         {
             // $objUdfResponse = TblMstFrm14::where('UDFID','=',$id)->first();
             $objUdfResponse = DB::table('TBL_MST_UDFFORSO')                    
                             ->where('TBL_MST_UDFFORSO.UDFID','=',$id)
                             ->orwhere('TBL_MST_UDFFORSO.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFORSO.*')
                             ->orderBy('TBL_MST_UDFFORSO.UDFID','ASC')
                             ->get()->toArray();
             return view('masters.Sales.UDFSO.mstfrm14view',compact(['objUdfResponse']));
         }
   ...
   public function docuploads(Request $request){
    ...
    if(empty($uploaded_data)){
        return redirect()->route("master",[14,"attachment",$ATTACH_DOCNO])->with("success","Already exists. No file uploaded");
    }
  ...
  //  }
    ...
    if($sp_result[0]->RESULT=="SUCCESS"){
        ...
        return redirect()->route("master",[14,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


    }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
   
        return redirect()->route("master",[14,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);

    }else{

        return redirect()->route("master",[14,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
    }
    ...
    public function checkLabel(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_UDFFORSO')
        ->where('TBL_MST_UDFFORSO.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_UDFFORSO.LABEL','=',$LABEL)
        ->select('TBL_MST_UDFFORSO.UDFID')
        ->first();
