...
use App\Models\Master\TblMstFrm141;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm141Controller extends Controller
{
   
    protected $form_id = 141;
    protected $vtid_ref   = 149;  //voucher type id

    //validation messages
    protected   $messages = [
                    'BCODE.required' => 'Required field',
                    'BCODE.unique' => 'Duplicate Code',
                    'NAME.required' => 'Required field',
                    'BRANCH.required' => 'Required field',
                    'IFSC.required' => 'Required field',
                    'ACTYPE.required' => 'Required field',
                    'ACNO.required' => 'Required field',
    ...
    public function index(){
        ...
        $objDataList=DB::select("SELECT BID,BCODE,NAME,BRANCH,IFSC,ACTYPE,ACNO,DEACTIVATED,DODEACTIVATED,INDATE,STATUS 
        FROM TBL_MST_BANK WHERE CYID_REF='$CYID_REF' AND BRID_REF='$BRID_REF'");
 
        return view('masters.Accounts.BankMaster.mstfrm141',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.Accounts.BankMaster.mstfrm141add',compact(['objCountryList','objGlList','objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_BANK')
   ...
   public function save(Request $request){
        ...
        $BCODE          =   strtoupper(trim($request['BCODE']) );
        $NAME           =   trim($request['NAME']); 
        $BRANCH         =   trim($request['BRANCH']);
        $IFSC           =   trim($request['IFSC']);
        $ACTYPE         =   trim($request['ACTYPE']);
        $BANK_CASH         =   trim($request['BANK_CASH']);
        $ACNO           =   trim($request['ACNO']);
       
        $ODLIMIT        =   (isset($request['ODLIMIT']) && trim($request['ODLIMIT']) !="" )? trim($request['ODLIMIT']) : NULL ;
        $BRANCHTIME     =   (isset($request['BRANCHTIME']) && trim($request['BRANCHTIME']) !="" )? trim($request['BRANCHTIME']) : NULL ;
        $ADD1           =   (isset($request['ADD1']) && trim($request['ADD1']) !="" )? trim($request['ADD1']) : NULL ;
        $ADD2           =   (isset($request['ADD2']) && trim($request['ADD2']) !="" )? trim($request['ADD2']) : NULL ;
        $CTRYID_REF     =   trim($request['CTRYID_REF']);
        $STID_REF       =   trim($request['STID_REF']);
        $CITYID_REF     =   trim($request['CITYID_REF']);
        $PIN            =   (isset($request['PIN']) && trim($request['PIN']) !="" )? trim($request['PIN']) : NULL ;
        $LANDMARK       =   (isset($request['LANDMARK']) && trim($request['LANDMARK']) !="" )? trim($request['LANDMARK']) : NULL ;
        
        $newDOO         =   (isset($request['DOO']) && trim($request['DOO']) !="" )? trim($request['DOO']) : NULL ;

        $newDOOString   = NULL;

        if(!is_null($newDOO) ){
            
            $newDOO = str_replace( "/", "-",  $newDOO ) ;

            $newDOOString = Carbon::parse($newDOO)->format('Y-m-d');        
        }
        
        $DOO            =   $newDOOString;

        $SIGNAUTH       =   (isset($request['SIGNAUTH']) && trim($request['SIGNAUTH']) !="" )? trim($request['SIGNAUTH']) : NULL ;
        $GLID_REF     =   trim($request['GLID_REF']);
		$BANK_GLID_REF     =   trim($request['BANK_GLID_REF']);
        ...
        $array_data   = [
                        $BCODE, $NAME,
                        $BRANCH, $IFSC, $ACTYPE, $ACNO,
                        $ODLIMIT, $BRANCHTIME, $ADD1, $ADD2,
                        $CITYID_REF, $STID_REF, $CTRYID_REF, $PIN,
                        $LANDMARK, $DOO, $SIGNAUTH,$GLID_REF,
                        $DEACTIVATED, $DODEACTIVATED, $CYID_REF, $BRID_REF,
                        $FYID_REF, $VTID, $USERID, $UPDATE,
                        $UPTIME, $ACTION, $IPADDRESS,$BANK_CASH ,$BANK_GLID_REF
                    ];

       // dd($array_data );
        try {

            $sp_result = DB::select('EXEC SP_BANK_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm141::where('BID','=',$id)->first();
            ...
            $objGlList=$this->objGlList();
            $objGlName=$this->objGlName($objResponse->GLID_REF);
			$objBankGlName=$this->objBankGlName($objResponse->BANK_GLID_REF);
            
            return view('masters.Accounts.BankMaster.mstfrm141edit',compact(['objResponse','user_approval_level','objRights','objCountryList','objCityName','objStateName','objCountryName','objGlList','objGlName','objBankGlName']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_BANK_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/BankMaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[141,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[141,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[141,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[141,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[141,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[141,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        if(!is_null($id)){
            ...
            return view('masters.Accounts.BankMaster.mstfrm141view',compact(['objResponse','objCityName','objStateName','objCountryName','objGlName',
				'objBankGlName']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm141::whereIn('BID',$ids_data)->get();
        
        return view('masters.Accounts.BankMaster.mstfrm141print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Accounts.BankMaster.mstfrm141attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
   ...
   //save data
    ...
    public function objGlName($GLID_REF){
        return DB::table('TBL_MST_GENERALLEDGER')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
        ->where('STATUS','=','A')
        ->where('GLID','=',$GLID_REF)
	...
	public function objBankGlName($GLID_REF){
        return DB::table('TBL_MST_GENERALLEDGER')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
        ->where('STATUS','=','A')
        ->where('GLID','=',$GLID_REF)
