...
use App\Models\Master\TblMstFrm129;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm129Controller extends Controller
{
   
    protected $form_id = 129;
    protected $vtid_ref   = 138;  //voucher type id

    //validation messages
    protected   $messages = [
                    'GLID_REF.required' => 'Required field',
                    'SGLCODE.required' => 'Required field',
                    'SGLCODE.unique' => 'Duplicate Code',
                    'SLNAME.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $objDataList=DB::select("SELECT  SGLID,SGLCODE,SLNAME,SALIAS,INDATE,DEACTIVATED,DODEACTIVATED,STATUS FROM TBL_MST_SUBLEDGER WHERE CYID_REF='$CYID_REF'");

        return view('masters.Accounts.SubLedgerMaster.mstfrm129',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.Accounts.SubLedgerMaster.mstfrm129add',compact(['objGlList']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $BRID_REF = Session::get('BRID_REF');
        $FYID_REF = Session::get('FYID_REF');
        $SGLCODE =   $request['SGLCODE'];
        
        $objLabel = DB::table('TBL_MST_SUBLEDGER')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
        ->where('SGLCODE','=',$SGLCODE)
        ->select('SGLCODE')
        ->first();
   ...
   public function save(Request $request){
        ...
        $rules = [
            'GLID_REF' => 'required', 
            'SGLCODE' => 'required|unique:TBL_MST_SUBLEDGER',
            'SLNAME' => 'required',          
        ];
        ...
        $SGLCODE        =   strtoupper(trim($request['SGLCODE']) );
        $SLNAME         =   trim($request['SLNAME']);  
        $SALIAS         =   (isset($request['SALIAS']) && trim($request['SALIAS']) !="" )? trim($request['SALIAS']) : NULL ; 
        $CC             =   trim($request['CC']);  
        ...
        $array_data   = [
                        $SGLCODE, $SLNAME,$SALIAS,$GLID_REF,
                        $DEACTIVATED, $DODEACTIVATED,$CC, $CYID_REF, $BRID_REF,
                        $FYID_REF, $VTID, $USERID, $UPDATE,
                        $UPTIME, $ACTION, $IPADDRESS
                    ];
        try {

            $sp_result = DB::select('EXEC SP_SUBLEDGER_IN ?,?,?,?, ?,?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm129::where('SGLID','=',$id)->first();
            ...
            return view('masters.Accounts.SubLedgerMaster.mstfrm129edit',compact(['objResponse','user_approval_level','objRights','objGlList','objGlName']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_SUBLEDGER_UP ?,?,?,?, ?,?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/SubLedgerMaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[129,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[129,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[129,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[129,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[129,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[129,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Accounts.SubLedgerMaster.mstfrm129view',compact(['objResponse','objGlName']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm129::whereIn('SGLID',$ids_data)->get();
        
        return view('masters.Accounts.SubLedgerMaster.mstfrm129print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Accounts.SubLedgerMaster.mstfrm129attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
