...
//use App\Models\Master\TblMstFrm213;
...
class MstFrm213Controller extends Controller
{
    protected $form_id = 213;
    protected $vtid_ref   = 313;  //voucher type id
    // //validation messages
    protected   $messages = [
    ...
    public function index(){    
        $objRights = DB::table('TBL_MST_USERROLMAP')
                        ...
                        ->first();
        $objCount = DB::table('TBL_MST_UDFFOR_CRV')
                        ->where('TBL_MST_UDFFOR_CRV.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_MST_UDFFOR_CRV.BRID_REF','=',Session::get('BRID_REF'))
                        ->where('TBL_MST_UDFFOR_CRV.FYID_REF','=',Session::get('FYID_REF'))
                        ->where('TBL_MST_UDFFOR_CRV.STATUS', '!=', 'C')
                        ->where('TBL_MST_UDFFOR_CRV.PARENTID','=', 0)
                        ->select('TBL_MST_UDFFOR_CRV.*')
                        ->count();

       

        return view('masters.Accounts.UDFCRV.mstfrm213',compact(['objRights','objCount']));        
    }

    public function add(){       
        return view('masters.Accounts.UDFCRV.mstfrm213add');       
   }
   ...
   public function attachment($id){
    ...
    if(!is_null($id))
    {
        $objResponse = DB::table("TBL_MST_UDFFOR_CRV")->where('UDFCRVID','=',$id)->first();
        ...
        $objMstVoucherType = DB::table("TBL_MST_VOUCHERTYPE")
            ...
            return view('masters.Accounts.UDFCRV.mstfrm213attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
    }
   ...
   public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            ...
            {
                $req_data[$i] = [
                    'UDFCRVID' => (isset($request['UDFCRVID_'.$i]) ? $request['UDFCRVID_'.$i] : "0") ,
            ...
            $sp_result = DB::select('EXEC SP_UDFFOR_CRV_INUPDE ?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
     ...
     public function getListing(Request $request){
        ...
        $COL_APP_STATUS =   'STATUS';  //never change value, value must be 'APPROVED_STATUS' as per stored procedure;
            ...
            $BRID_REF      =   Session::get('BRID_REF');
            $FYID_REF      =   Session::get('FYID_REF');       
            $TABLE1        =   "TBL_MST_UDFFOR_CRV";
            $PK_COL        =   "UDFCRVID";
            $SELECT_COL    =   "UDFCRVID,LABEL,VALUETYPE,DESCRIPTIONS,ISMANDATORY,DEACTIVATED,DODEACTIVATED";    
            ...
            if(!empty($request->input('search.value')))
            {
                ...
                if($filtercolumn =='ALL'){
                    ...
                    $WHERE_COL =  " WHERE PARENTID = 0 AND UDFCRVID LIKE  ". $search_text;
            ...
            if(!empty($sp_listing_result))
            {
                foreach ($sp_listing_result as $key=>$valueitem)
                {
                    ...
                    $nestedData['NO']           = '<input type="checkbox" id="chkId'.$valueitem->UDFCRVID.'"  value="'.$valueitem->UDFCRVID.'" class="js-selectall1" data-rcdstatus="'.$app_status.'">';
                    ...
                    $nestedData['DODEACTIVATED']      = ($valueitem->DODEACTIVATED ==NULL || $valueitem->DODEACTIVATED =="1900-01-01")?"":$valueitem->DODEACTIVATED;
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            $objUdfResponse = DB::table('TBL_MST_UDFFOR_CRV')                    
                             ->where('TBL_MST_UDFFOR_CRV.UDFCRVID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_CRV.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_CRV.*')
                             ->orderBy('TBL_MST_UDFFOR_CRV.UDFCRVID','ASC')
                             ->get()->toArray();
            ...
            $objRights = DB::table('TBL_MST_USERROLMAP')
                             ...
                             ->select('TBL_MST_USERROLMAP.*', 'TBL_MST_ROLEDETAILS.*')
                             ->first();
           return view('masters.Accounts.UDFCRV.mstfrm213edit',compact(['objUdfResponse','objRights','objCount']));
        }
       ...
       public function view($id){
            ...
            if(!is_null($id))
            {
                // $objUdfResponse = TblMstFrm213::where('UDFCRVID','=',$id)->first();
                $objUdfResponse = DB::table('TBL_MST_UDFFOR_CRV')                    
                                ->where('TBL_MST_UDFFOR_CRV.UDFCRVID','=',$id)
                                ->orwhere('TBL_MST_UDFFOR_CRV.PARENTID','=',$id)
                                ->select('TBL_MST_UDFFOR_CRV.*')
                                ->orderBy('TBL_MST_UDFFOR_CRV.UDFCRVID','ASC')
                                ->get()->toArray();
                return view('masters.Accounts.UDFCRV.mstfrm213view',compact(['objUdfResponse']));
            }
   ...
   public function update(Request $request){
    ...
    // dd($request->VALUETYPE);
        $data = [
            'UDFCRVID' => (isset($request->UDFCRVID_0) ? $request->UDFCRVID_0 : "0"),
            'LABEL'    => strtoupper($request->LABEL_0),
            'VALUETYPE' => $request->VALUETYPE_0,
            'DESCRIPTIONS' => (isset($request->DESCRIPTIONS_0)? $request->DESCRIPTIONS_0 : ""),
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            if((isset($request['UDFCRVID_'.$i])) && (isset($request['LABEL_'.$i])))
            {
                $req_data[$i] = [
                    'UDFCRVID' => (isset($request['UDFCRVID_'.$i]) ? $request['UDFCRVID_'.$i] : "0") ,
                    'LABEL'    => strtoupper($request['LABEL_'.$i]),
   ...
   //save data
        ...
        $cancelData[0]= ['NT' =>'TBL_MST_UDFFOR_CRV'];
   ...
   public function docuploads(Request $request){
    ...
    $destinationPath = storage_path()."/docs/company".$CYID_REF."/udfcrv"; 
	$image_path         =   "docs/company".$CYID_REF."/udfcrv";     
    ...
    if(empty($uploaded_data)){
        return redirect()->route("master",[213,"attachment",$ATTACH_DOCNO])->with("success","Already exists. No file uploaded");
    }
  ...
  //  }
    ...
    if($sp_result[0]->RESULT=="SUCCESS"){
        ...
        return redirect()->route("master",[213,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


    }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
   
        return redirect()->route("master",[213,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);

    }else{

        return redirect()->route("master",[213,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
    }
    ...
    public function checkLabel(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_UDFFOR_CRV')
        ->where('TBL_MST_UDFFOR_CRV.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_UDFFOR_CRV.BRID_REF','=',Session::get('BRID_REF'))
        ->where('TBL_MST_UDFFOR_CRV.FYID_REF','=',Session::get('FYID_REF'))
        ->where('TBL_MST_UDFFOR_CRV.LABEL','=',$LABEL)
        ->select('TBL_MST_UDFFOR_CRV.UDFCRVID')
        ->first();
