...
use App\Models\Master\TblMstFrm187;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm187Controller extends Controller
{
   
    protected $form_id = 187;
    protected $vtid_ref   = 211;  //voucher type id

    //validation messages
    protected   $messages = [
                    'DEDUCTION_TYPECODE.required' => 'Required field',
                    'DEDUCTION_TYPECODE.unique' => 'Duplicate Code',
                    'DEDUCTION_TYPE_DESC.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_DEDUCTION_HEAD_TYPE')
        ->where('TBL_MST_DEDUCTION_HEAD_TYPE.CYID_REF','=',Auth::user()->CYID_REF)
            
            ->get();


        return view('masters.Payroll.DeductionHeadType.mstfrm187',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        return view('masters.Payroll.DeductionHeadType.mstfrm187add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $DEDUCTION_TYPECODE =   $request['DEDUCTION_TYPECODE'];
        
        $objLabel = DB::table('TBL_MST_DEDUCTION_HEAD_TYPE')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
        ->where('BRID_REF','=',Session::get('BRID_REF'))
        ->where('FYID_REF','=',Session::get('FYID_REF'))
        ->where('DEDUCTION_TYPECODE','=',$DEDUCTION_TYPECODE)
        ->select('DEDUCTION_TYPECODE')
        ->first();
   ...
   public function save(Request $request){
        ...
        $rules = [
            'DEDUCTION_TYPECODE' => 'required',
            'DEDUCTION_TYPE_DESC' => 'required',          
        ];

        $req_data = [

            'DEDUCTION_TYPECODE'     =>    $request['DEDUCTION_TYPECODE'],
            'DEDUCTION_TYPE_DESC' =>   $request['DEDUCTION_TYPE_DESC']
        ]; 
        ...
        $DEDUCTION_TYPECODE       =   strtoupper(trim($request['DEDUCTION_TYPECODE']) );
        $DEDUCTION_TYPE_DESC   =   trim($request['DEDUCTION_TYPE_DESC']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        $array_data   = [
                        $DEDUCTION_TYPECODE, $DEDUCTION_TYPE_DESC,
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_DEDUCTION_HEAD_TYPE_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm187::where('DEDUCTION_TYPEID','=',$id)->first();
            ...
            return view('masters.Payroll.DeductionHeadType.mstfrm187edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_DEDUCTION_HEAD_TYPE_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
        //dd($array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/DeductionHeadType";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[187,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[187,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[187,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[187,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[187,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm187::where('DEDUCTION_TYPEID','=',$id)->first();
            return view('masters.Payroll.DeductionHeadType.mstfrm187view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm187::whereIn('DEDUCTION_TYPEID',$ids_data)->get();
        
        return view('masters.Payroll.DeductionHeadType.mstfrm187print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.DeductionHeadType.mstfrm187attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
    ...
    public function MultiApprove(Request $request){
        ...
        if(!empty($sp_listing_result))
            ...
            $BRID_REF   =   Session::get('BRID_REF');
            $FYID_REF   =   Session::get('FYID_REF');       
            $TABLE      =   "TBL_MST_DEDUCTION_HEAD_TYPE";
            $FIELD      =   "DEDUCTION_TYPEID";
   ...
   public function cancel(Request $request){
    ...
    $id = $request->{0};    
        ...
        $req_data[0]=[
            'NT'  => 'TBL_MST_DEDUCTION_HEAD_TYPE',
        ];
