...
use App\Models\Master\TblMstFrm95;
...
class MstFrm95Controller extends Controller
{
    protected $form_id = 95;
    protected $vtid_ref   = 272;  //voucher type id
    // //validation messages
    protected   $messages = [
    ...
    public function index(){    
        $objRights = DB::table('TBL_MST_USERROLMAP')
                        ...
                        ->first();
        $objCount = DB::table('TBL_MST_UDFFOR_SSO')
                        ->where('TBL_MST_UDFFOR_SSO.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_MST_UDFFOR_SSO.STATUS', '!=', 'C')
                        ->where('TBL_MST_UDFFOR_SSO.PARENTID','=', 0)
                        ->select('TBL_MST_UDFFOR_SSO.*')
                        ->count();

       

        return view('masters.Sales.UDFSSO.mstfrm95',compact(['objRights','objCount']));        
    }

    public function add(){       
        return view('masters.Sales.UDFSSO.mstfrm95add');       
   }
   ...
   public function attachment($id){
    ...
    if(!is_null($id))
    {
        $objResponse = TblMstFrm95::where('UDFSSOID','=',$id)->first();
        ...
        $objMstVoucherType = DB::table("TBL_MST_VOUCHERTYPE")
            ...
            return view('masters.Sales.UDFSSO.mstfrm95attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
    }
   ...
   public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            ...
            {
                $req_data[$i] = [
                    'UDFSSOID' => (isset($request['UDFSSOID_'.$i]) ? $request['UDFSSOID_'.$i] : "0") ,
            ...
            $sp_result = DB::select('EXEC SP_UDFFOR_SSO_INUPDE ?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
     ...
     public function getListing(Request $request){
        ...
        $COL_APP_STATUS =   'STATUS';  //never change value, value must be 'APPROVED_STATUS' as per stored procedure;
            ...
            $BRID_REF      =   Session::get('BRID_REF');
            $FYID_REF      =   Session::get('FYID_REF');       
            $TABLE1        =   "TBL_MST_UDFFOR_SSO";
            $PK_COL        =   "UDFSSOID";
            $SELECT_COL    =   "UDFSSOID,LABEL,VALUETYPE,DESCRIPTIONS,ISMANDATORY,DEACTIVATED,DODEACTIVATED";    
            ...
            if(!empty($request->input('search.value')))
            {
                ...
                if($filtercolumn =='ALL'){
                    ...
                    $WHERE_COL =  " WHERE PARENTID = 0 AND UDFSSOID LIKE  ". $search_text;
            ...
            if(!empty($sp_listing_result))
            {
                foreach ($sp_listing_result as $key=>$reqdataitem)
                {
                    ...
                    $nestedData['NO']           = '<input type="checkbox" id="chkId'.$reqdataitem->UDFSSOID.'"  value="'.$reqdataitem->UDFSSOID.'" class="js-selectall1" data-rcdstatus="'.$app_status.'">';
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            $objUdfResponse = DB::table('TBL_MST_UDFFOR_SSO')                    
                             ->where('TBL_MST_UDFFOR_SSO.UDFSSOID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_SSO.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_SSO.*')
                             ->orderBy('TBL_MST_UDFFOR_SSO.UDFSSOID','ASC')
                             ->get()->toArray();
            ...
            $objRights = DB::table('TBL_MST_USERROLMAP')
                             ...
                             ->select('TBL_MST_USERROLMAP.*', 'TBL_MST_ROLEDETAILS.*')
                             ->first();
           return view('masters.Sales.UDFSSO.mstfrm95edit',compact(['objUdfResponse','objRights','objCount']));
        }
       ...
       public function view($id){
         ...
         if(!is_null($id))
         {
             // $objUdfResponse = TblMstFrm95::where('UDFSSOID','=',$id)->first();
             $objUdfResponse = DB::table('TBL_MST_UDFFOR_SSO')                    
                             ->where('TBL_MST_UDFFOR_SSO.UDFSSOID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_SSO.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_SSO.*')
                             ->orderBy('TBL_MST_UDFFOR_SSO.UDFSSOID','ASC')
                             ->get()->toArray();
             return view('masters.Sales.UDFSSO.mstfrm95view',compact(['objUdfResponse']));
         }
   ...
   public function update(Request $request){
    ...
    // dd($request->VALUETYPE);
        $data = [
            'UDFSSOID' => (isset($request->UDFSSOID_0) ? $request->UDFSSOID_0 : "0"),
            'LABEL'    => strtoupper($request->LABEL_0),
            'VALUETYPE' => $request->VALUETYPE_0,
            'DESCRIPTIONS' => (isset($request->DESCRIPTIONS_0)? $request->DESCRIPTIONS_0 : ""),
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            if((isset($request['UDFSSOID_'.$i])) && (isset($request['LABEL_'.$i])))
            {
                $req_data[$i] = [
                    'UDFSSOID' => (isset($request['UDFSSOID_'.$i]) ? $request['UDFSSOID_'.$i] : "0") ,
                    'LABEL'    => strtoupper($request['LABEL_'.$i]),
   ...
   public function docuploads(Request $request){
    ...
    $destinationPath = storage_path()."/docs/company".$CYID_REF."/udfssomst";
    ...
    if(empty($uploaded_data)){
        return redirect()->route("master",[95,"attachment",$ATTACH_DOCNO])->with("success","Already exists. No file uploaded");
    }
  ...
  //  }
    ...
    if($sp_result[0]->RESULT=="SUCCESS"){
        ...
        return redirect()->route("master",[95,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


    }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
   
        return redirect()->route("master",[95,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);

    }else{

        return redirect()->route("master",[95,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
    }
    ...
    public function checkLabel(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_UDFFOR_SSO')
        ->where('TBL_MST_UDFFOR_SSO.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_UDFFOR_SSO.LABEL','=',$LABEL)
        ->select('TBL_MST_UDFFOR_SSO.UDFSSOID')
        ->first();
