...
//use App\Models\Master\TblMstFrm215;
...
class MstFrm215Controller extends Controller
{
    protected $form_id = 215;
    protected $vtid_ref   = 315;  //voucher type id
    // //validation messages
    protected   $messages = [
    ...
    public function index(){    
        $objRights = DB::table('TBL_MST_USERROLMAP')
                        ...
                        ->first();
        $objCount = DB::table('TBL_MST_UDFFOR_CPV')
                        ->where('TBL_MST_UDFFOR_CPV.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_MST_UDFFOR_CPV.BRID_REF','=',Session::get('BRID_REF'))
                        ->where('TBL_MST_UDFFOR_CPV.FYID_REF','=',Session::get('FYID_REF'))
                        ->where('TBL_MST_UDFFOR_CPV.STATUS', '!=', 'C')
                        ->where('TBL_MST_UDFFOR_CPV.PARENTID','=', 0)
                        ->select('TBL_MST_UDFFOR_CPV.*')
                        ->count();

       

        return view('masters.Accounts.UDFCPV.mstfrm215',compact(['objRights','objCount']));        
    }

    public function add(){       
        return view('masters.Accounts.UDFCPV.mstfrm215add');       
   }
   ...
   public function attachment($id){
    ...
    if(!is_null($id))
    {
        $objResponse = DB::table("TBL_MST_UDFFOR_CPV")->where('UDFCPVID','=',$id)->first();
        ...
        $objMstVoucherType = DB::table("TBL_MST_VOUCHERTYPE")
            ...
            return view('masters.Accounts.UDFCPV.mstfrm215attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
    }
   ...
   public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            ...
            {
                $req_data[$i] = [
                    'UDFCPVID' => (isset($request['UDFCPVID_'.$i]) ? $request['UDFCPVID_'.$i] : "0") ,
            ...
            $sp_result = DB::select('EXEC SP_UDFFOR_CPV_INUPDE ?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
     ...
     public function getListing(Request $request){
        ...
        $COL_APP_STATUS =   'STATUS';  //never change value, value must be 'APPROVED_STATUS' as per stored procedure;
            ...
            $BRID_REF      =   Session::get('BRID_REF');
            $FYID_REF      =   Session::get('FYID_REF');       
            $TABLE1        =   "TBL_MST_UDFFOR_CPV";
            $PK_COL        =   "UDFCPVID";
            $SELECT_COL    =   "UDFCPVID,LABEL,VALUETYPE,DESCRIPTIONS,ISMANDATORY,DEACTIVATED,DODEACTIVATED";    
            ...
            if(!empty($request->input('search.value')))
            {
                ...
                if($filtercolumn =='ALL'){
                    ...
                    $WHERE_COL =  " WHERE PARENTID = 0 AND UDFCPVID LIKE  ". $search_text;
            ...
            if(!empty($sp_listing_result))
            {
                foreach ($sp_listing_result as $key=>$valueitem)
                {
                    ...
                    $nestedData['NO']           = '<input type="checkbox" id="chkId'.$valueitem->UDFCPVID.'"  value="'.$valueitem->UDFCPVID.'" class="js-selectall1" data-rcdstatus="'.$app_status.'">';
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            $objUdfResponse = DB::table('TBL_MST_UDFFOR_CPV')                    
                             ->where('TBL_MST_UDFFOR_CPV.UDFCPVID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_CPV.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_CPV.*')
                             ->orderBy('TBL_MST_UDFFOR_CPV.UDFCPVID','ASC')
                             ->get()->toArray();
            ...
            $objRights = DB::table('TBL_MST_USERROLMAP')
                             ...
                             ->select('TBL_MST_USERROLMAP.*', 'TBL_MST_ROLEDETAILS.*')
                             ->first();
           return view('masters.Accounts.UDFCPV.mstfrm215edit',compact(['objUdfResponse','objRights','objCount']));
        }
       ...
       public function view($id){
            ...
            if(!is_null($id))
            {
                // $objUdfResponse = TblMstFrm215::where('UDFCPVID','=',$id)->first();
                $objUdfResponse = DB::table('TBL_MST_UDFFOR_CPV')                    
                                ->where('TBL_MST_UDFFOR_CPV.UDFCPVID','=',$id)
                                ->orwhere('TBL_MST_UDFFOR_CPV.PARENTID','=',$id)
                                ->select('TBL_MST_UDFFOR_CPV.*')
                                ->orderBy('TBL_MST_UDFFOR_CPV.UDFCPVID','ASC')
                                ->get()->toArray();
                return view('masters.Accounts.UDFCPV.mstfrm215view',compact(['objUdfResponse']));
            }
   ...
   public function update(Request $request){
    ...
    // dd($request->VALUETYPE);
        $data = [
            'UDFCPVID' => (isset($request->UDFCPVID_0) ? $request->UDFCPVID_0 : "0"),
            'LABEL'    => strtoupper($request->LABEL_0),
            'VALUETYPE' => $request->VALUETYPE_0,
            'DESCRIPTIONS' => (isset($request->DESCRIPTIONS_0)? $request->DESCRIPTIONS_0 : ""),
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            if((isset($request['UDFCPVID_'.$i])) && (isset($request['LABEL_'.$i])))
            {
                $req_data[$i] = [
                    'UDFCPVID' => (isset($request['UDFCPVID_'.$i]) ? $request['UDFCPVID_'.$i] : "0") ,
                    'LABEL'    => strtoupper($request['LABEL_'.$i]),
   ...
   //save data
        ...
        $cancelData[0]= ['NT' =>'TBL_MST_UDFFOR_CPV'];
   ...
   public function docuploads(Request $request){
    ...
    $destinationPath = storage_path()."/docs/company".$CYID_REF."/udfcpv"; 
	$image_path         =   "docs/company".$CYID_REF."/udfcpv";     
    ...
    if(empty($uploaded_data)){
        return redirect()->route("master",[215,"attachment",$ATTACH_DOCNO])->with("success","Already exists. No file uploaded");
    }
  ...
  //  }
    ...
    if($sp_result[0]->RESULT=="SUCCESS"){
        ...
        return redirect()->route("master",[215,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


    }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
   
        return redirect()->route("master",[215,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);

    }else{

        return redirect()->route("master",[215,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
    }
    ...
    public function checkLabel(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_UDFFOR_CPV')
        ->where('TBL_MST_UDFFOR_CPV.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_UDFFOR_CPV.BRID_REF','=',Session::get('BRID_REF'))
        ->where('TBL_MST_UDFFOR_CPV.FYID_REF','=',Session::get('FYID_REF'))
        ->where('TBL_MST_UDFFOR_CPV.LABEL','=',$LABEL)
        ->select('TBL_MST_UDFFOR_CPV.UDFCPVID')
        ->first();
