...
use App\Models\Master\TblMstFrm440;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm440Controller extends Controller{

    protected $form_id    = 440;
    protected $vtid_ref   = 510;
    protected $view       = "masters.PreSales.LeadSource.mstfrm";
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_LEAD_SOURCE')
        ->orderBy('ID','desc')
        ->get();
    ...
    public function save(Request $request){
        ...
        $LEAD_SOURCECODE   =   trim($request['LEAD_SOURCECODE']);
        $LEAD_SOURCENAME   =   trim($request['LEAD_SOURCENAME']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        $array_data     = [$LEAD_SOURCECODE, $LEAD_SOURCENAME, $DEACTIVATED, $DODEACTIVATED,$CYID_REF, $BRID_REF, $FYID_REF, $VTID,  $USERID, $UPDATE,$UPTIME, $ACTION, $IPADDRESS  ];
        
        try {

            $sp_result = DB::select('EXEC SP_LEAD_SOURCE_IN ?,?,?,?,?,?, ?,?,?,?,?,?, ?', $array_data);
    ...
    public function showRecord($id,$type,$ActionStatus){
        ...
        if(!is_null($id)){
            ...
            $objResponse = DB::table('TBL_MST_LEAD_SOURCE')
            ->where('ID','=',$id)
            ->first();
    ...
    public function updateRecord($request,$type){
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_LEAD_SOURCE_up ?,?,?,?,?,?, ?,?,?,?,?,?,?', $array_data);
