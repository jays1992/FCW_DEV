...
class MstFrm531Controller extends Controller{

    protected $form_id  =   531;
    protected $vtid_ref =   601;
    protected $view     =   "masters.Sales.ValueCardMaster.mstfrm531";
    ...
    public function index(){  
        ...
        WHERE T1.CYID_REF='$CYID_REF' AND T1.BRID_REF='$BRID_REF' AND T1.FRANCHISE_ID='2' 
    ...
    public function save(Request $request){
        ...
        $sp_result  =   DB::select('EXEC SP_VCM_IN ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?', $log_data);  
    ...
    public function edit($id=NULL){
        ...
        if(!is_null($id)){
            ...
            $DETAILS        =   DB::select("SELECT * FROM TBL_MST_V_MASTER_DETAILS WHERE DOC_ID_REF='$id'"); 

            $objtempUdf     =   $objUdf;
            foreach ($objtempUdf as $index => $udfvalue) {

                $objSavedUDF =  DB::table('TBL_MST_V_MASTER_UDF')
                ->where('VMID_REF','=',$id)
    ...
    public function docuploads(Request $request){
		...
		$image_path         =   "docs/company".$CYID_REF."/ValueCardMaster";     
