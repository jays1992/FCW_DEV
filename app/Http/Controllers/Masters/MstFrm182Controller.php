...
use App\Models\Master\TblMstFrm182;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm182Controller extends Controller
{
   
    protected $form_id = 182;
    protected $vtid_ref   = 205;  //voucher type id

    //validation messages
    protected   $messages = [
                    'DIVCODE.required' => 'Required field',
                    'DIVCODE.unique' => 'Duplicate Code',
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_DIVISON')
            ->where('CYID_REF','=',Auth::user()->CYID_REF)

            ->get();


        return view('masters.Payroll.DivisionMaster.mstfrm182',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        return view('masters.Payroll.DivisionMaster.mstfrm182add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_DIVISON')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
   ...
   public function save(Request $request){
        ...
        $DIVCODE       =   strtoupper(trim($request['DIVCODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_DIVISON_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm182::where('DIVID','=',$id)->first();
            ...
            return view('masters.Payroll.DivisionMaster.mstfrm182edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_DIVISON_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/DivisionMaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[182,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[182,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[182,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[182,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[182,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm182::where('DIVID','=',$id)->first();
            return view('masters.Payroll.DivisionMaster.mstfrm182view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm182::whereIn('DIVID',$ids_data)->get();
        
        return view('masters.Payroll.DivisionMaster.mstfrm182print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.DivisionMaster.mstfrm182attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
