...
use App\Models\Master\TblMstFrm137;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm137Controller extends Controller
{
   
    protected $form_id = 137;
    protected $vtid_ref   = 108;  //voucher type id

    //validation messages
    protected   $messages = [
                    'PSTAGE_CODE.required' => 'Required field',
                    'PSTAGE_CODE.unique' => 'Duplicate Code',
    ...
    public function index(){
        ...
        $objDataList	=	DB::select("SELECT * FROM TBL_MST_PRODUCTIONSTAGES WHERE CYID_REF='$CYID_REF'  ORDER BY PSTAGEID DESC");

        return view('masters.Production.ProductionStages.mstfrm137',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.Production.ProductionStages.mstfrm137add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $BRID_REF = Session::get('BRID_REF');
        $FYID_REF = Session::get('FYID_REF');
        $PSTAGE_CODE =   $request['PSTAGE_CODE'];
        
        $objLabel = DB::table('TBL_MST_PRODUCTIONSTAGES')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
        ->where('BRID_REF','=',Session::get('BRID_REF'))
        ->where('FYID_REF','=',Session::get('FYID_REF'))
        ->where('PSTAGE_CODE','=',$PSTAGE_CODE)
        ->select('PSTAGE_CODE')
        ->first();
   ...
   public function save(Request $request){
        ...
        $PSTAGE_CODE       =   strtoupper(trim($request['PSTAGE_CODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_PRODUCTIONSTAGE_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm137::where('PSTAGEID','=',$id)->first();
            ...
            return view('masters.Production.ProductionStages.mstfrm137edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_PRODUCTIONSTAGE_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
		...
		$destinationPath = storage_path()."/docs/company".$CYID_REF."/ProductionStages";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[137,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[137,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[137,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[137,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[137,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[137,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm137::where('PSTAGEID','=',$id)->first();
            return view('masters.Production.ProductionStages.mstfrm137view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm137::whereIn('PSTAGEID',$ids_data)->get();
        
        return view('masters.Production.ProductionStages.mstfrm137print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Production.ProductionStages.mstfrm137attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
    ...
    public function MultiApprove(Request $request){
        ...
        if(!empty($sp_listing_result))
            ...
            $BRID_REF   =   Session::get('BRID_REF');
            $FYID_REF   =   Session::get('FYID_REF');       
            $TABLE      =   "TBL_MST_PRODUCTIONSTAGES";
            $FIELD      =   "PSTAGEID";
