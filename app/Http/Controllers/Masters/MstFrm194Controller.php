...
use App\Models\Master\TblMstFrm194;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm194Controller extends Controller
{
   
    protected $form_id = 194;
    protected $vtid_ref   = 305;  //voucher type id

    //validation messages
    protected   $messages = [
                    'LEAVE_TYPE_CODE.required' => 'Required field',
                    'LEAVE_TYPE_CODE.unique' => 'Duplicate Code',
                    'LEAVE_TYPE_DESC.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_LEAVE_ENCASHMENT_PROVISION')
            ->where('CYID_REF','=',Auth::user()->CYID_REF)
            ->get();


        return view('masters.Payroll.LeaveEncashmentProvision.mstfrm194',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        return view('masters.Payroll.LeaveEncashmentProvision.mstfrm194add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_LEAVE_ENCASHMENT_PROVISION')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
   ...
   public function save(Request $request){
        ...
        $req_data = [
            ...
            'LEAVE_TYPE_CODE'     =>    $request['LEAVE_TYPE_CODE'],
            'LEAVE_TYPE_DESC' =>   $request['LEAVE_TYPE_DESC'],
        ...
        $LEAVE_TYPE_CODE       =   strtoupper(trim($request['LEAVE_TYPE_CODE']) );
        $LEAVE_TYPE_DESC   =   $request['LEAVE_TYPE_DESC'];  
        $PROVISION   =   $request['PROVISION'];  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        $array_data   = [
                        $LEAVE_TYPE_CODE, $LEAVE_TYPE_DESC,$PROVISION,
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_LEAVE_ENCASHMENT_PROVISION_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm194::where('LEPID','=',$id)->first();
            ...
            return view('masters.Payroll.LeaveEncashmentProvision.mstfrm194edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    //  /   dd($array_data); 
        ...
        $sp_result = DB::select('EXEC SP_LEAVE_ENCASHMENT_PROVISION_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?', $array_data);
        //dd($array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/LeaveEncashmentProvision";
		$image_path         =   "docs/company".$CYID_REF."/LeaveEncashmentProvision";     
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[194,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[194,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[194,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[194,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[194,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm194::where('LEPID','=',$id)->first();
            return view('masters.Payroll.LeaveEncashmentProvision.mstfrm194view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm194::whereIn('LEPID',$ids_data)->get();
        
        return view('masters.Payroll.LeaveEncashmentProvision.mstfrm194print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.LeaveEncashmentProvision.mstfrm194attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
    ...
    public function MultiApprove(Request $request){
        ...
        if(!empty($sp_listing_result))
            ...
            $BRID_REF   =   Session::get('BRID_REF');
            $FYID_REF   =   Session::get('FYID_REF');       
            $TABLE      =   "TBL_MST_LEAVE_ENCASHMENT_PROVISION";
            $FIELD      =   "LEPID";
   ...
   public function cancel(Request $request){
    ...
    $id = $request->{0};    
        ...
        $req_data[0]=[
            'NT'  => 'TBL_MST_LEAVE_ENCASHMENT_PROVISION',
        ];
