...
use App\Models\Master\TblMstFrm17;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm17Controller extends Controller
{
   
    protected $form_id = 17;
    protected $vtid_ref   = 17;  //voucher type id

    //validation messages
    protected   $messages = [
                    'INDSCODE.required' => 'Required field',
                    'INDSCODE.unique' => 'Duplicate Code',
    ...
    public function index(){
        ...
        $objDataList=DB::select("SELECT * FROM TBL_MST_INDUSTRYTYPE ORDER BY INDSID ");

        return view('masters.Sales.INDUSTRYTYPE.mstfrm17',compact(['objRights','objDataList']));
    ...
    public function add(){ 
       ...
       // dump($objDD);
        ...
        return view('masters.Sales.INDUSTRYTYPE.mstfrm17add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $BRID_REF = Session::get('BRID_REF');
        $FYID_REF = Session::get('FYID_REF');
        $INDSCODE =  strtoupper($request['INDSCODE']);
        
        $objLabel = DB::table('TBL_MST_INDUSTRYTYPE')
        ->where('INDSCODE','=',$INDSCODE)
        ->select('INDSCODE')
        ->first();
   ...
   public function save(Request $request){
        ...
        $INDSCODE       =   strtoupper(trim($request['INDSCODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_INDUSTRYTYPE_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm17::where('INDSID','=',$id)->first();
            ...
            return view('masters.Sales.INDUSTRYTYPE.mstfrm17edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_INDUSTRYTYPE_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/industrymst";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[17,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[17,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[17,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[17,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[17,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[17,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm17::where('INDSID','=',$id)->first();
            return view('masters.Sales.INDUSTRYTYPE.mstfrm17view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm17::whereIn('INDSID',$ids_data)->get();
        
        return view('masters.Sales.INDUSTRYTYPE.mstfrm17print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Sales.INDUSTRYTYPE.mstfrm17attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
