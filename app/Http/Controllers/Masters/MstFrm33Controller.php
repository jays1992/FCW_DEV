...
use App\Models\Master\TblMstFrm33;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm33Controller extends Controller
{
   
    protected $form_id = 33;
    protected $vtid_ref   = 33;  //voucher type id

    //validation messages
    protected   $messages = [
                    'PCCODE.required' => 'Required field',
                    'PCCODE.unique' => 'Duplicate Code',
                    'PCDESCRIPTIONS.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $objDataList=DB::select("SELECT * FROM TBL_MST_PROFIT_CENTER WHERE CYID_REF='$CYID_REF' ORDER BY PCID DESC");


        return view('masters.Sales.ProfitCentre.mstfrm33',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.Sales.ProfitCentre.mstfrm33add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_PROFIT_CENTER')
   ...
   public function save(Request $request){
        ...
        $rules = [
            'PCCODE' => 'required',
            'PCDESCRIPTIONS' => 'required',          
        ];

        $req_data = [

            'PCCODE'     =>    $request['PCCODE'],
            'PCDESCRIPTIONS' =>   $request['PCDESCRIPTIONS']
        ]; 
        ...
        $PCCODE       =   strtoupper(trim($request['PCCODE']) );
        $PCDESCRIPTIONS   =   trim($request['PCDESCRIPTIONS']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_PROFIT_CENTER_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm33::where('PCID','=',$id)->first();
            ...
            return view('masters.Sales.ProfitCentre.mstfrm33edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_PROFIT_CENTER_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/ProfitCentreMst";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[33,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[33,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[33,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[33,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[33,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[33,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm33::where('PCID','=',$id)->first();
            return view('masters.Sales.ProfitCentre.mstfrm33view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm33::whereIn('PCID',$ids_data)->get();
        
        return view('masters.Sales.ProfitCentre.mstfrm33print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Sales.ProfitCentre.mstfrm33attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
