...
use App\Models\Master\TblMstFrm183;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm183Controller extends Controller
{
   
    protected $form_id = 183;
    protected $vtid_ref   = 206;  //voucher type id

    //validation messages
    protected   $messages = [
                    'BGCODE.required' => 'Required field',
                    'BGCODE.unique' => 'Duplicate Code',
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_BLOODGROUP')

            ->get();


        return view('masters.Payroll.BloodGroupMaster.mstfrm183',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        return view('masters.Payroll.BloodGroupMaster.mstfrm183add' ,compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $BRID_REF = Session::get('BRID_REF');
        $FYID_REF = Session::get('FYID_REF');
        $BGCODE =   $request['BGCODE'];
        
        $objLabel = DB::table('TBL_MST_BLOODGROUP')
   ...
   public function save(Request $request){
        ...
        $BGCODE       =   strtoupper(trim($request['BGCODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_BLOODGROUP_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm183::where('BGID','=',$id)->first();
            ...
            return view('masters.Payroll.BloodGroupMaster.mstfrm183edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_BLOODGROUP_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/BloodGroupMaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[183,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[183,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[183,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[183,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[183,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm183::where('BGID','=',$id)->first();
            return view('masters.Payroll.BloodGroupMaster.mstfrm183view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm183::whereIn('BGID',$ids_data)->get();
        
        return view('masters.Payroll.BloodGroupMaster.mstfrm183print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.BloodGroupMaster.mstfrm183attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
