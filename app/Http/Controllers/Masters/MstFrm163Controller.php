...
use App\Models\Master\TblMstFrm163;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm163Controller extends Controller
{
   
    protected $form_id = 163;
    protected $vtid_ref   = 238;  //voucher type id

    //validation messages
    protected   $messages = [
                    'SECTION_CODE.required' => 'Required field',
                    'SECTION_CODE.unique' => 'Duplicate Code',
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_SECTION')
        ...
        return view('masters.Accounts.SectionMaster.mstfrm163',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        return view('masters.Accounts.SectionMaster.mstfrm163add');
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_SECTION')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
   ...
   public function save(Request $request){
        ...
        $rules = [
            'SECTION_CODE' => 'required|unique:TBL_MST_SECTION',
        ...
        $SECTION_CODE       =   strtoupper(trim($request['SECTION_CODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_SECTION_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm163::where('SECTIONID','=',$id)->first();
            ...
            return view('masters.Accounts.SectionMaster.mstfrm163edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_SECTION_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
        //dd($array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/SectionMaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[163,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[163,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[163,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[163,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[163,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm163::where('SECTIONID','=',$id)->first();
            return view('masters.Accounts.SectionMaster.mstfrm163view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm163::whereIn('SECTIONID',$ids_data)->get();
        
        return view('masters.Accounts.SectionMaster.mstfrm163print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Accounts.SectionMaster.mstfrm163attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
   ...
   public function cancel(Request $request){
    ...
    $id = $request->{0};    
        ...
        $cancelData[0]= ['NT' =>'TBL_MST_SECTION'];
