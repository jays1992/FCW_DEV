...
use App\Models\Master\TblMstFrm172;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm172Controller extends Controller
{
   
    protected $form_id = 172;
    protected $vtid_ref   = 198;  //voucher type id

    //validation messages
    protected   $messages = [
                    'GRADE_CODE.required' => 'Required field',
                    'GRADE_DESC.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_GRADE')
        ...
        return view('masters.Payroll.GradeMaster.mstfrm172',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        return view('masters.Payroll.GradeMaster.mstfrm172add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_GRADE')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
        ->where('BRID_REF','=',Session::get('BRID_REF'))
        ->where('FYID_REF','=',Session::get('FYID_REF'))
   ...
   public function save(Request $request){
        ...
        $GRADE_CODE       =   strtoupper(trim($request['GRADE_CODE']) );
        $GRADE_DESC   =   trim($request['GRADE_DESC']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_GRADE_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm172::where('GRADEID','=',$id)->first();
            ...
            return view('masters.Payroll.GradeMaster.mstfrm172edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_GRADE_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
        //dd($array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/GradeMaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[172,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[172,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[172,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[172,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[172,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm172::where('GRADEID','=',$id)->first();
            return view('masters.Payroll.GradeMaster.mstfrm172view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm172::whereIn('GRADEID',$ids_data)->get();
        
        return view('masters.Payroll.GradeMaster.mstfrm172print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.GradeMaster.mstfrm172attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
