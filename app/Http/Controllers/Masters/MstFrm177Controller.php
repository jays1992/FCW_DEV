...
use App\Models\Master\TblMstFrm177;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm177Controller extends Controller
{
   
    protected $form_id = 177;
    protected $vtid_ref   = 203;  //voucher type id

    //validation messages
    protected   $messages = [
                    'SACODE.required' => 'Required field',
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_SALUTATION')
            ->where('CYID_REF','=',Auth::user()->CYID_REF)
            ->get();


        return view('masters.Payroll.SalutationMaster.mstfrm177',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        return view('masters.Payroll.SalutationMaster.mstfrm177add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_SALUTATION')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
        ->where('SACODE','=',$SACODE)
        ->select('SACODE')
        ->first();
   ...
   public function save(Request $request){
        ...
        $SACODE       =   strtoupper(trim($request['SACODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_SALUTATION_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm177::where('SAID','=',$id)->first();
            ...
            return view('masters.Payroll.SalutationMaster.mstfrm177edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_SALUTATION_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
        //dd($array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/SalutationMaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[177,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[177,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[177,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[177,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[177,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm177::where('SAID','=',$id)->first();
            return view('masters.Payroll.SalutationMaster.mstfrm177view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm177::whereIn('SAID',$ids_data)->get();
        
        return view('masters.Payroll.SalutationMaster.mstfrm177print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.SalutationMaster.mstfrm177attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
