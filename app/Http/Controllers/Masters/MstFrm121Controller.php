...
use App\Models\Master\TblMstFrm121;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm121Controller extends Controller
{
   
    protected $form_id = 121;
    protected $vtid_ref   = 3;  //voucher type id

    //validation messages
    protected   $messages = [
                    'MTCODE.required' => 'Required field',
                    'MTCODE.unique' => 'Duplicate Code',
                    'MTDESCRIPTION.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $objDataList	=	DB::select("SELECT * FROM TBL_MST_MONTH");

        return view('masters.Common.MonthMaster.mstfrm121',compact(['objRights','objDataList']));

    }

    
    public function add(){ 
        return view('masters.Common.MonthMaster.mstfrm121add');
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_MONTH')
        ->where('MTCODE','=',$MTCODE)
        ->select('MTCODE')
        ->first();
   ...
   public function save(Request $request){
        ...
        $rules = [
            'MTCODE' => 'required|unique:TBL_MST_MONTH',
            'MTDESCRIPTION' => 'required',          
        ];

        $req_data = [

            'MTCODE'     =>    $request['MTCODE'],
            'MTDESCRIPTION' =>   $request['MTDESCRIPTION']
        ]; 
        ...
        $MTCODE       =   strtoupper(trim($request['MTCODE']) );
        $MTDESCRIPTION   =   trim($request['MTDESCRIPTION']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_MT_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm121::where('MTID','=',$id)->first();
            ...
            return view('masters.Common.MonthMaster.mstfrm121edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_MT_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/MonthMas";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[121,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[121,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[121,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[121,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[121,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[121,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm121::where('MTID','=',$id)->first();
            return view('masters.Common.MonthMaster.mstfrm121view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm121::whereIn('MTID',$ids_data)->get();
        
        return view('masters.Common.MonthMaster.mstfrm121print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Common.MonthMaster.mstfrm121attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
