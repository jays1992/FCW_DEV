...
use App\Models\Master\TblMstFrm176;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm176Controller extends Controller
{
   
    protected $form_id = 176;
    protected $vtid_ref   = 202;  //voucher type id

    //validation messages
    protected   $messages = [
                    'PCODE.required' => 'Required field',
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_PROJECT')
            ->where('CYID_REF','=',Auth::user()->CYID_REF)
            ->orderBy('PID','desc')
            ->get();


        return view('masters.Payroll.ProjectMaster.mstfrm176',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        return view('masters.Payroll.ProjectMaster.mstfrm176add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_PROJECT')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
   ...
   public function save(Request $request){
        ...
        $PCODE       =   strtoupper(trim($request['PCODE']) );
        $DESCRIPTIONS   =   trim($request['DESCRIPTIONS']);
        $PROJECT_START_DATE   =   trim($request['PROJECT_START_DATE']);
        $PROJECT_END_DATE   =   trim($request['PROJECT_END_DATE']);
        $PROJECT_LOCATION   =   trim($request['PROJECT_LOCATION']);
        ...
        $array_data   = [
                        $PCODE,                 $DESCRIPTIONS,          $DEACTIVATED, 
                        $DODEACTIVATED,         $CYID_REF,              $BRID_REF,
                        $FYID_REF,              $VTID,                  $USERID,     
                        $UPDATE,                $UPTIME,                $ACTION,            
                        $IPADDRESS,             $PROJECT_START_DATE,    $PROJECT_END_DATE,  
                        $PROJECT_LOCATION,      $PROJECT_STATUS
                    ];

                    
        try {

            $sp_result = DB::select('EXEC SP_PROJECT_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?, ?,?,?,  ?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm176::where('PID','=',$id)->first();
            ...
            return view('masters.Payroll.ProjectMaster.mstfrm176edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_PROJECT_UP ?,?,?, ?,?,?, ?,?,?, ?,?,?, ?,?,?,  ?,?', $array_data);
        //dd($array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/ProjectMaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[176,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[176,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[176,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[176,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[176,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm176::where('PID','=',$id)->first();
            return view('masters.Payroll.ProjectMaster.mstfrm176view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm176::whereIn('PID',$ids_data)->get();
        
        return view('masters.Payroll.ProjectMaster.mstfrm176print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.ProjectMaster.mstfrm176attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
