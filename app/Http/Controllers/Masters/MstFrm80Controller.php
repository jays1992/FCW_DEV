...
use App\Models\Master\TblMstFrm80;
...
class MstFrm80Controller extends Controller
{
    protected $form_id = 80;
    protected $vtid_ref   = 80;  //voucher type id
    // //validation messages
    protected   $messages = [
    ...
    public function index(){    
        $objRights = DB::table('TBL_MST_USERROLMAP')
                        ...
                        ->first();
        $objCount = DB::table('TBL_MST_UDFFOR_MIS')
                        ->where('TBL_MST_UDFFOR_MIS.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_MST_UDFFOR_MIS.STATUS', '!=', 'C')
                        ->where('TBL_MST_UDFFOR_MIS.PARENTID','=', 0)
                        ->select('TBL_MST_UDFFOR_MIS.*')
                        ->count();

       

        return view('masters.inventory.UDFMIS.mstfrm80',compact(['objRights','objCount']));        
    }

    public function add(){       
        return view('masters.inventory.UDFMIS.mstfrm80add');       
   }
   ...
   public function attachment($id){
    ...
    if(!is_null($id))
    {
        $objResponse = TblMstFrm80::where('UDFMISID','=',$id)->first();
        ...
        $objMstVoucherType = DB::table("TBL_MST_VOUCHERTYPE")
            ...
            return view('masters.inventory.UDFMIS.mstfrm80attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
    }
   ...
   public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            ...
            {
                $req_data[$i] = [
                    'UDFMISID' => (isset($request['UDFMISID_'.$i]) ? $request['UDFMISID_'.$i] : "0") ,
            ...
            $sp_result = DB::select('EXEC SP_UDFFOR_MIS_INUPDE ?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
     ...
     public function getListing(Request $request){
        ...
        $COL_APP_STATUS =   'STATUS';  //never change value, value must be 'APPROVED_STATUS' as per stored procedure;
            ...
            $BRID_REF      =   Session::get('BRID_REF');
            $FYID_REF      =   Session::get('FYID_REF');       
            $TABLE1        =   "TBL_MST_UDFFOR_MIS";
            $PK_COL        =   "UDFMISID";
            $SELECT_COL    =   "UDFMISID,LABEL,VALUETYPE,DESCRIPTIONS,ISMANDATORY,DEACTIVATED,DODEACTIVATED";    
            ...
            if(!empty($request->input('search.value')))
            {
                ...
                if($filtercolumn =='ALL'){
                    ...
                    $WHERE_COL =  " WHERE PARENTID = 0 AND UDFMISID LIKE  ". $search_text;
            ...
            if(!empty($sp_listing_result))
            {
                foreach ($sp_listing_result as $key=>$valueitem)
                {
                    ...
                    $nestedData['NO']           = '<input type="checkbox" id="chkId'.$valueitem->UDFMISID.'"  value="'.$valueitem->UDFMISID.'" class="js-selectall1" data-rcdstatus="'.$app_status.'">';
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            $objUdfResponse = DB::table('TBL_MST_UDFFOR_MIS')                    
                             ->where('TBL_MST_UDFFOR_MIS.UDFMISID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_MIS.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_MIS.*')
                             ->orderBy('TBL_MST_UDFFOR_MIS.UDFMISID','ASC')
                             ->get()->toArray();
            ...
            $objRights = DB::table('TBL_MST_USERROLMAP')
                             ...
                             ->select('TBL_MST_USERROLMAP.*', 'TBL_MST_ROLEDETAILS.*')
                             ->first();
           return view('masters.inventory.UDFMIS.mstfrm80edit',compact(['objUdfResponse','objRights','objCount']));
        }
       ...
       public function view($id){
         ...
         if(!is_null($id))
         {
             // $objUdfResponse = TblMstFrm80::where('UDFMISID','=',$id)->first();
             $objUdfResponse = DB::table('TBL_MST_UDFFOR_MIS')                    
                             ->where('TBL_MST_UDFFOR_MIS.UDFMISID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_MIS.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_MIS.*')
                             ->orderBy('TBL_MST_UDFFOR_MIS.UDFMISID','ASC')
                             ->get()->toArray();
             return view('masters.inventory.UDFMIS.mstfrm80view',compact(['objUdfResponse']));
         }
   ...
   public function update(Request $request){
    ...
    // dd($request->VALUETYPE);
        $data = [
            'UDFMISID' => (isset($request->UDFMISID_0) ? $request->UDFMISID_0 : "0"),
            'LABEL'    => strtoupper($request->LABEL_0),
            'VALUETYPE' => $request->VALUETYPE_0,
            'DESCRIPTIONS' => (isset($request->DESCRIPTIONS_0)? $request->DESCRIPTIONS_0 : ""),
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            if((isset($request['UDFMISID_'.$i])) && (isset($request['LABEL_'.$i])))
            {
                $req_data[$i] = [
                    'UDFMISID' => (isset($request['UDFMISID_'.$i]) ? $request['UDFMISID_'.$i] : "0") ,
                    'LABEL'    => strtoupper($request['LABEL_'.$i]),
   ...
   public function docuploads(Request $request){
    ...
    $destinationPath = storage_path()."/docs/company".$CYID_REF."/udfmismst";
    ...
    if(empty($uploaded_data)){
        return redirect()->route("master",[80,"attachment",$ATTACH_DOCNO])->with("success","Already exists. No file uploaded");
    }
  ...
  //  }
    ...
    if($sp_result[0]->RESULT=="SUCCESS"){
        ...
        return redirect()->route("master",[80,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


    }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
   
        return redirect()->route("master",[80,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);

    }else{

        return redirect()->route("master",[80,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
    }
    ...
    public function checkLabel(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_UDFFOR_MIS')
        ->where('TBL_MST_UDFFOR_MIS.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_UDFFOR_MIS.LABEL','=',$LABEL)
        ->select('TBL_MST_UDFFOR_MIS.UDFMISID')
        ->first();
