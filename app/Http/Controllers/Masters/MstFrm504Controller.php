...
use App\Models\Master\TblMstFrm504;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm504Controller extends Controller{

    protected $form_id    = 504;
    protected $vtid_ref   = 574;
    protected $view       = "masters.PreSales.TeamMaster.mstfrm";       
    /**
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_TEAM_MASTER_HRD')
                            ...
                            ->orderBy('TBL_MST_TEAM_MASTER_HRD.TEAMMSTID','desc')
                            ->get();
    ...
    public function save(Request $request){
        ...
        $TEAMMST_NO     =   trim($request['DOC_NO']);
        $TEAMMST_DATE   =   trim($request['DOC_DT']);
        $EMPID_REF      =   trim($request['TEAMHEADID_REF']);
        ...
        $array_data     = [$TEAMMST_NO, $TEAMMST_DATE, $EMPID_REF, $DEACTIVATED, $DODEACTIVATED,$CYID_REF, $BRID_REF, $FYID_REF, $VTID,  $USERID, $UPDATE,$UPTIME, $ACTION, $IPADDRESS];
        
        try {

            $sp_result = DB::select('EXEC SP_TEAM_MASTER_IN ?,?,?,?,?,?,?,?,?,?,?,?,?,?', $array_data);
    ...
    public function showRecord($id,$type,$ActionStatus){
        ...
        if(!is_null($id)){
            ...
            $objResponse = DB::table('TBL_MST_TEAM_MASTER_HRD')
                            ->leftJoin('TBL_MST_EMPLOYEE', 'TBL_MST_TEAM_MASTER_HRD.EMPID_REF','=','TBL_MST_EMPLOYEE.EMPID')
                            ->select('TBL_MST_TEAM_MASTER_HRD.*', 'TBL_MST_EMPLOYEE.EMPID','TBL_MST_EMPLOYEE.EMPCODE','TBL_MST_EMPLOYEE.FNAME')
                            ->where('TBL_MST_TEAM_MASTER_HRD.TEAMMSTID','=',$id)
                            ->first();
    ...
    public function updateRecord($request,$type){
        ...
        $array_data = [$TEAMMST_NO, $TEAMMST_DATE, $EMPID_REF, $DEACTIVATED,  $DODEACTIVATED, $CYID_REF, $BRID_REF, $FYID_REF, $VTID_REF,  $USERID, $UPDATE,$UPTIME, $ACTION, $IPADDRESS ];   

        try {

            $sp_result = DB::select('EXEC SP_TEAM_MASTER_UP ?,?,?,?,?,?,?,?,?,?,?,?,?,?', $array_data);
   ...
   public function docuploads(Request $request){
    ...
    $destinationPath = storage_path()."/docs/company".$CYID_REF."/TeamMaster";
...
/*************************************   Team Head Code    ****************************************************** */
        ...
        public function getTeamHead(Request $request){
