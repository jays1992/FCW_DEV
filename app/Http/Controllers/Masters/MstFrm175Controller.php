...
use App\Models\Master\TblMstFrm175;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm175Controller extends Controller
{
   
    protected $form_id = 175;
    protected $vtid_ref   = 201;  //voucher type id

    //validation messages
    protected   $messages = [
                    'ECODE.required' => 'Required field',
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_EMPLOYEETYPE')
            ->where('CYID_REF','=',Auth::user()->CYID_REF)

            ->get();


        return view('masters.Payroll.EmployeeType.mstfrm175',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        return view('masters.Payroll.EmployeeType.mstfrm175add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_EMPLOYEETYPE')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
   ...
   public function save(Request $request){
        ...
        $ECODE       =   strtoupper(trim($request['ECODE']) );
        ...
        try {
                ...
                $sp_result = DB::select('EXEC SP_EMPLOYEETYPE_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm175::where('ETYPEID','=',$id)->first();
            ...
            return view('masters.Payroll.EmployeeType.mstfrm175edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_EMPLOYEETYPE_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
        //dd($array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/EmployeeType";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[175,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[175,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[175,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[175,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[175,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm175::where('ETYPEID','=',$id)->first();
            return view('masters.Payroll.EmployeeType.mstfrm175view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm175::whereIn('ETYPEID',$ids_data)->get();
        
        return view('masters.Payroll.EmployeeType.mstfrm175print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.EmployeeType.mstfrm175attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
