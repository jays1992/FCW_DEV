...
use App\Models\Master\TblMstFrm434;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm434Controller extends Controller{

    protected $form_id    = 434;
    protected $vtid_ref   = 504;
    protected $view       = "masters.PreSales.TaskType.mstfrm";
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_TASK_TYPE_TYPE')
        ->orderBy('ID','desc')
        ->get();
    ...
    public function save(Request $request){
        ...
        $TASK_TYPECODE   =   trim($request['TASK_TYPECODE']);
        $TASK_TYPENAME   =   trim($request['TASK_TYPENAME']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        $array_data     = [$TASK_TYPECODE, $TASK_TYPENAME, $DEACTIVATED, $DODEACTIVATED,$CYID_REF, $BRID_REF, $FYID_REF, $VTID,  $USERID, $UPDATE,$UPTIME, $ACTION, $IPADDRESS  ];
        
        try {

            $sp_result = DB::select('EXEC SP_TASK_TYPE_IN ?,?,?,?,?,?, ?,?,?,?,?,?, ?', $array_data);
    ...
    public function showRecord($id,$type,$ActionStatus){
        ...
        if(!is_null($id)){
            ...
            $objResponse = DB::table('TBL_MST_TASK_TYPE_TYPE')
            ->where('ID','=',$id)
            ->first();
    ...
    public function updateRecord($request,$type){
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_TASK_TYPE_UP ?,?,?,?,?,?, ?,?,?,?,?,?,?', $array_data);
