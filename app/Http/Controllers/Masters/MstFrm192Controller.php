...
use App\Models\Master\TblMstFrm192;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm192Controller extends Controller
{
   
    protected $form_id = 192;
    protected $vtid_ref   = 233;  //voucher type id

    //validation messages
    protected   $messages = [
                    'LOANTYPE_CODE.required' => 'Required field',
                    'LOANTYPE_CODE.unique' => 'Duplicate Code',
                    'LOANTYPE_DESC.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_LOANTYPE')
            ->where('CYID_REF','=',Auth::user()->CYID_REF)
            ->get();


        return view('masters.Payroll.LoanType.mstfrm192',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        return view('masters.Payroll.LoanType.mstfrm192add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $BRID_REF = Session::get('BRID_REF');
        $FYID_REF = Session::get('FYID_REF');
        $LOANTYPE_CODE =   $request['LOANTYPE_CODE'];
        
        $objLabel = DB::table('TBL_MST_LOANTYPE')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
   ...
   public function save(Request $request){
        ...
        $rules = [
            'LOANTYPE_CODE' => 'required',
            'LOANTYPE_DESC' => 'required',          
        ];

        $req_data = [

            'LOANTYPE_CODE'     =>    $request['LOANTYPE_CODE'],
            'LOANTYPE_DESC' =>   $request['LOANTYPE_DESC']
        ]; 
        ...
        $LOANTYPE_CODE       =   strtoupper(trim($request['LOANTYPE_CODE']) );
        $LOANTYPE_DESC   =   trim($request['LOANTYPE_DESC']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_LOANTYPE_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm192::where('LOANTYPEID','=',$id)->first();
            ...
            return view('masters.Payroll.LoanType.mstfrm192edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_LOANTYPE_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
        //dd($array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/LoanType";
		$image_path         =   "docs/company".$CYID_REF."/LoanType";     
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[192,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[192,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[192,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[192,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[192,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm192::where('LOANTYPEID','=',$id)->first();
            return view('masters.Payroll.LoanType.mstfrm192view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm192::whereIn('LOANTYPEID',$ids_data)->get();
        
        return view('masters.Payroll.LoanType.mstfrm192print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.LoanType.mstfrm192attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
