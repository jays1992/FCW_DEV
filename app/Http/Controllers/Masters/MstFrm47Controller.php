...
use App\Models\Master\TblMstFrm47;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm47Controller extends Controller
{
   
    protected $form_id = 47;
    protected $vtid_ref   = 47;  //voucher type id

    //validation messages
    protected   $messages = [
                    'VGCODE.required' => 'Required field',
                    'VGCODE.unique' => 'Duplicate Code',
    ...
    public function index(){
        ...
        $objDataList	=	DB::select("SELECT * FROM TBL_MST_VENDORGROUP WHERE CYID_REF='$CYID_REF' ORDER BY VGID DESC");

        return view('masters.Purchase.VendorGroup.mstfrm47',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.Purchase.VendorGroup.mstfrm47add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $BRID_REF = Session::get('BRID_REF');
        $FYID_REF = Session::get('FYID_REF');
        $VGCODE =   $request['VGCODE'];
        
        $objLabel = DB::table('TBL_MST_VENDORGROUP')
   ...
   public function save(Request $request){
        ...
        $VGCODE       =   strtoupper(trim($request['VGCODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_VENDORGROUP_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm47::where('VGID','=',$id)->first();
            ...
            return view('masters.Purchase.VendorGroup.mstfrm47edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_VENDORGROUP_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/VendorGroupMaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[47,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[47,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[47,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[47,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[47,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[47,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm47::where('VGID','=',$id)->first();
            return view('masters.Purchase.VendorGroup.mstfrm47view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm47::whereIn('VGID',$ids_data)->get();
        
        return view('masters.Purchase.VendorGroup.mstfrm47print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Purchase.VendorGroup.mstfrm47attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
