...
use App\Models\Master\TblMstFrm134;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm134Controller extends Controller
{
   
    protected $form_id = 134;
    protected $vtid_ref   = 155;  //voucher type id

    //validation messages
    protected   $messages = [
                    'GSTCODE.required' => 'Required field',
                    'GSTCODE.unique' => 'Duplicate Code',
    ...
    public function index(){
       ...
       // ->where('TBL_MST_USERROLMAP.FYID_REF','=',Session::get('FYID_REF'))
        ...
        $objDataList	=	DB::select("SELECT * FROM TBL_MST_GSTTYPE");

        return view('masters.Accounts.GSTTYPE.mstfrm134',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        return view('masters.Accounts.GSTTYPE.mstfrm134add');
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_GSTTYPE')
   ...
   public function save(Request $request){
        ...
        $rules = [
            'GSTCODE' => 'required|unique:TBL_MST_GSTTYPE',
        ...
        $GSTCODE       =   strtoupper(trim($request['GSTCODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_GSTTYPE_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm134::where('GSTID','=',$id)->first();
            ...
            return view('masters.Accounts.GSTTYPE.mstfrm134edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_GSTTYPE_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/gsttypemst";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[134,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[134,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[134,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[134,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[134,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[134,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm134::where('GSTID','=',$id)->first();
            return view('masters.Accounts.GSTTYPE.mstfrm134view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm134::whereIn('GSTID',$ids_data)->get();
        
        return view('masters.Accounts.GSTTYPE.mstfrm134print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Accounts.GSTTYPE.mstfrm134attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
