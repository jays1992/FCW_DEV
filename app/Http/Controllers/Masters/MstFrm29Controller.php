...
use App\Models\Master\TblMstFrm29;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm29Controller extends Controller
{
   
    protected $form_id = 29;
    protected $vtid_ref   = 29;  //voucher type id

    //validation messages
    protected   $messages = [
                    'PLCCODE.required' => 'Required field',
                    'PLCCODE.unique' => 'Duplicate Code',
                    'PLCDESCRIPTIONS.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $objDataList=DB::select("SELECT * FROM TBL_MST_PRICELIST_CAT WHERE CYID_REF='$CYID_REF' ORDER BY PLCID DESC");


        return view('masters.Sales.PriceListCategory.mstfrm29',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.Sales.PriceListCategory.mstfrm29add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_PRICELIST_CAT')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
        ->where('PLCCODE','=',$PLCCODE)
        ->select('PLCCODE')
        ->first();
   ...
   public function save(Request $request){
        ...
        $rules = [
            'PLCCODE' => 'required',
            'PLCDESCRIPTIONS' => 'required',          
        ];

        $req_data = [

            'PLCCODE'     =>   $request['PLCCODE'],
            'PLCDESCRIPTIONS' =>  $request['PLCDESCRIPTIONS']
        ]; 
        ...
        $PLCCODE       =   strtoupper(trim($request['PLCCODE']) );
        $PLCDESCRIPTIONS   =   trim($request['PLCDESCRIPTIONS']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_PRICELIST_CAT_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm29::where('PLCID','=',$id)->first();
            ...
            return view('masters.Sales.PriceListCategory.mstfrm29edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_PRICELIST_CAT_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/PriceListCategoryMst";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[29,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[29,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[29,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[29,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[29,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[29,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm29::where('PLCID','=',$id)->first();
            return view('masters.Sales.PriceListCategory.mstfrm29view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm29::whereIn('PLCID',$ids_data)->get();
        
        return view('masters.Sales.PriceListCategory.mstfrm29print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Sales.PriceListCategory.mstfrm29attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
