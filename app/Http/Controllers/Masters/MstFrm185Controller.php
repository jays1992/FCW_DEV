...
use App\Models\Master\TblMstFrm185;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm185Controller extends Controller
{
   
    protected $form_id = 185;
    protected $vtid_ref   = 208;  //voucher type id

    //validation messages
    protected   $messages = [
                    'EARNING_TYPECODE.required' => 'Required field',
                    'EARNING_TYPECODE.unique' => 'Duplicate Code',
                    'EARNING_TYPE_DESC.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_EARNING_HEAD_TYPE')
        ->where('TBL_MST_EARNING_HEAD_TYPE.CYID_REF','=',Auth::user()->CYID_REF)
            
            ->get();


        return view('masters.Payroll.EarningHeadType.mstfrm185',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        return view('masters.Payroll.EarningHeadType.mstfrm185add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $EARNING_TYPECODE =   $request['EARNING_TYPECODE'];
        
        $objLabel = DB::table('TBL_MST_EARNING_HEAD_TYPE')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
        ->where('BRID_REF','=',Session::get('BRID_REF'))
        ->where('FYID_REF','=',Session::get('FYID_REF'))
        ->where('EARNING_TYPECODE','=',$EARNING_TYPECODE)
        ->select('EARNING_TYPECODE')
        ->first();
   ...
   public function save(Request $request){
        ...
        $rules = [
            'EARNING_TYPECODE' => 'required',
            'EARNING_TYPE_DESC' => 'required',          
        ];

        $req_data = [

            'EARNING_TYPECODE'     =>    $request['EARNING_TYPECODE'],
            'EARNING_TYPE_DESC' =>   $request['EARNING_TYPE_DESC']
        ]; 
        ...
        $EARNING_TYPECODE       =   strtoupper(trim($request['EARNING_TYPECODE']) );
        $EARNING_TYPE_DESC   =   trim($request['EARNING_TYPE_DESC']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        $array_data   = [
                        $EARNING_TYPECODE, $EARNING_TYPE_DESC,
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_EARNING_HEAD_TYPE_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm185::where('EARNING_TYPEID','=',$id)->first();
            ...
            return view('masters.Payroll.EarningHeadType.mstfrm185edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_EARNING_HEAD_TYPE_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
        //dd($array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/EarningHeadType";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[185,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[185,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[185,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[185,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[185,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm185::where('EARNING_TYPEID','=',$id)->first();
            return view('masters.Payroll.EarningHeadType.mstfrm185view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm185::whereIn('EARNING_TYPEID',$ids_data)->get();
        
        return view('masters.Payroll.EarningHeadType.mstfrm185print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.EarningHeadType.mstfrm185attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
    ...
    public function MultiApprove(Request $request){
        ...
        if(!empty($sp_listing_result))
            ...
            $BRID_REF   =   Session::get('BRID_REF');
            $FYID_REF   =   Session::get('FYID_REF');       
            $TABLE      =   "TBL_MST_EARNING_HEAD_TYPE";
            $FIELD      =   "EARNING_TYPEID";
