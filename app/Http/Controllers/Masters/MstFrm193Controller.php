...
use App\Models\Master\TblMstFrm193;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm193Controller extends Controller
{
   
    protected $form_id = 193;
    protected $vtid_ref   = 240;  //voucher type id

    //validation messages
    protected   $messages = [
                    'OT_CODE.required' => 'Required field',
                    'OT_CODE.unique' => 'Duplicate Code',
                    'OT_DESC.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_OVERTIME')
            ->where('CYID_REF','=',Auth::user()->CYID_REF)
            ->get();


        return view('masters.Payroll.OverTime.mstfrm193',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        return view('masters.Payroll.OverTime.mstfrm193add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_OVERTIME')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
   ...
   public function save(Request $request){
        ...
        $OT_CODE       =   strtoupper(trim($request['OT_CODE']) );
        $OT_DESC       =   $request['OT_DESC'];  
        $OT_RATE       =   trim($request['OT_RATE']);  
        $OT_FACTOR     =   $request['OT_FACTOR'];  
        $DEACTIVATED   =   NULL;  
        $DODEACTIVATED =   NULL;  
        ...
        $array_data   = [
                        $OT_CODE, $OT_DESC,$OT_RATE,$OT_FACTOR,
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_OVERTIME_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm193::where('OTID','=',$id)->first();
            ...
            return view('masters.Payroll.OverTime.mstfrm193edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_OVERTIME_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,?', $array_data);
        //dd($array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/OverTime";
		$image_path         =   "docs/company".$CYID_REF."/OverTime";     
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[193,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[193,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[193,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[193,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[193,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm193::where('OTID','=',$id)->first();
            return view('masters.Payroll.OverTime.mstfrm193view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm193::whereIn('OTID',$ids_data)->get();
        
        return view('masters.Payroll.OverTime.mstfrm193print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.OverTime.mstfrm193attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
