...
use App\Models\Master\TblMstFrm433;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm433Controller extends Controller{

    protected $form_id    = 433;
    protected $vtid_ref   = 503;
    protected $view       = "masters.PreSales.ResponseType.mstfrm";
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_RESPONSE_TYPE')
        ->orderBy('ID','desc')
        ->get();
    ...
    public function save(Request $request){
        ...
        $RESPONSECODE   =   trim($request['RESPONSECODE']);
        $RESPONSENAME   =   trim($request['RESPONSENAME']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        $array_data     = [$RESPONSECODE, $RESPONSENAME, $DEACTIVATED, $DODEACTIVATED,$CYID_REF, $BRID_REF, $FYID_REF, $VTID,  $USERID, $UPDATE,$UPTIME, $ACTION, $IPADDRESS  ];
        
        try {

            $sp_result = DB::select('EXEC SP_RESPONSE_TYPE_IN ?,?,?,?,?,?, ?,?,?,?,?,?, ?', $array_data);
    ...
    public function showRecord($id,$type,$ActionStatus){
        ...
        if(!is_null($id)){
            ...
            $objResponse = DB::table('TBL_MST_RESPONSE_TYPE')
            ->where('ID','=',$id)
            ->first();
    ...
    public function updateRecord($request,$type){
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_RESPONSE_TYPE_UP ?,?,?,?,?,?, ?,?,?,?,?,?,?', $array_data);
