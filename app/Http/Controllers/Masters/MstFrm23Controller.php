...
use App\Models\Master\TblMstFrm23;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm23Controller extends Controller
{
   
    protected $form_id = 23;
    protected $vtid_ref   = 23;  //voucher type id

    //validation messages
    protected   $messages = [
                    'COUPONTCODE.required' => 'Required field',
                    'COUPONTCODE.unique' => 'Duplicate Code',
                    'CTDESCRIPTIONS.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $objDataList=DB::select("SELECT * FROM TBL_MST_COUPON_TYPE WHERE CYID_REF='$CYID_REF' ORDER BY COUPONTID DESC");


        return view('masters.Sales.CouponType.mstfrm23',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.Sales.CouponType.mstfrm23add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $BRID_REF = Session::get('BRID_REF');
        $FYID_REF = Session::get('FYID_REF');
        $COUPONTCODE =   $request['COUPONTCODE'];
        
        $objLabel = DB::table('TBL_MST_COUPON_TYPE')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
        ->where('BRID_REF','=',Session::get('BRID_REF'))
        ->where('FYID_REF','=',Session::get('FYID_REF'))
        ->where('COUPONTCODE','=',$COUPONTCODE)
        ->select('COUPONTCODE')
        ->first();
   ...
   public function save(Request $request){
        ...
        $rules = [
            'COUPONTCODE' => 'required',
            'CTDESCRIPTIONS' => 'required',          
        ];

        $req_data = [

            'COUPONTCODE'     =>   $request['COUPONTCODE'],
            'CTDESCRIPTIONS' =>   $request['CTDESCRIPTIONS']
        ]; 
        ...
        $COUPONTCODE       =   strtoupper(trim($request['COUPONTCODE']) );
        $CTDESCRIPTIONS   =   trim($request['CTDESCRIPTIONS']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_COUPON_TYPE_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm23::where('COUPONTID','=',$id)->first();
            ...
            return view('masters.Sales.CouponType.mstfrm23edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_COUPON_TYPE_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/CouponTypeMst";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[23,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[23,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[23,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[23,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[23,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[23,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm23::where('COUPONTID','=',$id)->first();
            return view('masters.Sales.CouponType.mstfrm23view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm23::whereIn('COUPONTID',$ids_data)->get();
        
        return view('masters.Sales.CouponType.mstfrm23print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Sales.CouponType.mstfrm23attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
