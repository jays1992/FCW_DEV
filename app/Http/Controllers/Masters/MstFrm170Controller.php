...
use App\Models\Master\TblMstFrm170;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm170Controller extends Controller
{
   
    protected $form_id = 170;
    protected $vtid_ref   = 196;  //voucher type id

    //validation messages
    protected   $messages = [
                    'PLCODE.required' => 'Required field',
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_POSTLEVEL')
            ->where('CYID_REF','=',Auth::user()->CYID_REF)
            ->get();


        return view('masters.Payroll.PostLevel.mstfrm170',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        return view('masters.Payroll.PostLevel.mstfrm170add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_POSTLEVEL')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
   ...
   public function save(Request $request){
        ...
        $PLCODE       =   strtoupper(trim($request['PLCODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_POSTLEVEL_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm170::where('PLID','=',$id)->first();
            ...
            return view('masters.Payroll.PostLevel.mstfrm170edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_POSTLEVEL_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
        //dd($array_data);
    ...
    public function docuploads(Request $request){
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[170,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[170,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[170,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[170,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[170,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm170::where('PLID','=',$id)->first();
            return view('masters.Payroll.PostLevel.mstfrm170view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm170::whereIn('PLID',$ids_data)->get();
        
        return view('masters.Payroll.PostLevel.mstfrm170print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.PostLevel.mstfrm170attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
