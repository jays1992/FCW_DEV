...
use App\Models\Master\TblMstFrm184;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm184Controller extends Controller
{
   
    protected $form_id = 184;
    protected $vtid_ref   = 207;  //voucher type id

    //validation messages
    protected   $messages = [
                    'RSCODE.required' => 'Required field',
                    'RSCODE.unique' => 'Duplicate Code',
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_RELATIONSHIP')

            ->get();


        return view('masters.Payroll.RelationshipMaster.mstfrm184',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        return view('masters.Payroll.RelationshipMaster.mstfrm184add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_RELATIONSHIP')
        ->where('RSCODE','=',$RSCODE)
        ->select('RSCODE')
        ->first();
   ...
   public function save(Request $request){
        ...
        $RSCODE       =   strtoupper(trim($request['RSCODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_RELATIONSHIP_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm184::where('RSID','=',$id)->first();
            ...
            return view('masters.Payroll.RelationshipMaster.mstfrm184edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_RELATIONSHIP_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/RelationshipMaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[184,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[184,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[184,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[184,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[184,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm184::where('RSID','=',$id)->first();
            return view('masters.Payroll.RelationshipMaster.mstfrm184view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm184::whereIn('RSID',$ids_data)->get();
        
        return view('masters.Payroll.RelationshipMaster.mstfrm184print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.RelationshipMaster.mstfrm184attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
