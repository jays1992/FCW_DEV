...
use App\Models\Master\TblMstFrm195;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm195Controller extends Controller
{
   
    protected $form_id = 195;
    protected $vtid_ref   = 241;  //voucher type id

    //validation messages
    protected   $messages = [
                    'BONUS_CODE.required' => 'Required field',
                    'BONUS_CODE.unique' => 'Duplicate Code',
                    'BONUS_DESC.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_BONUS')
            ->where('CYID_REF','=',Auth::user()->CYID_REF)
            ->get();


        return view('masters.Payroll.BonusMaster.mstfrm195',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        return view('masters.Payroll.BonusMaster.mstfrm195add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_BONUS')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
   ...
   public function save(Request $request){
        ...
        $BONUS_CODE    =   strtoupper(trim($request['BONUS_CODE']) );
        $BONUS_DESC    =   $request['BONUS_DESC'];  
        ...
        $array_data   = [
                        $BONUS_CODE, $BONUS_DESC,$BONUS_RATE,$BASIC_SALARY,$MAX_BONUS,$FLAT_BONUS,$BONUS_TYPE,
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_BONUS_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?,?,?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm195::where('BONUSID','=',$id)->first();
            ...
            return view('masters.Payroll.BonusMaster.mstfrm195edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    $BONUS_TYPE    =   $request['BonusType'];
      ...
      // dd($array_data);
        ...
        $sp_result = DB::select('EXEC SP_BONUS_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,?,?,?,?', $array_data);
        //dd($array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/BonusMaster";
		$image_path         =   "docs/company".$CYID_REF."/BonusMaster";     
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[195,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[195,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[195,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[195,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[195,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm195::where('BONUSID','=',$id)->first();
            return view('masters.Payroll.BonusMaster.mstfrm195view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm195::whereIn('BONUSID',$ids_data)->get();
        
        return view('masters.Payroll.BonusMaster.mstfrm195print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.BonusMaster.mstfrm195attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
