...
//use App\Models\Master\TblMstFrm211;
...
class MstFrm211Controller extends Controller
{
    protected $form_id = 211;
    protected $vtid_ref   = 311;  //voucher type id
    // //validation messages
    protected   $messages = [
    ...
    public function index(){    
        $objRights = DB::table('TBL_MST_USERROLMAP')
                        ...
                        ->first();
        $objCount = DB::table('TBL_MST_UDFFOR_SL')
                        ->where('TBL_MST_UDFFOR_SL.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_MST_UDFFOR_SL.BRID_REF','=',Session::get('BRID_REF'))
                        ->where('TBL_MST_UDFFOR_SL.FYID_REF','=',Session::get('FYID_REF'))
                        ->where('TBL_MST_UDFFOR_SL.STATUS', '!=', 'C')
                        ->where('TBL_MST_UDFFOR_SL.PARENTID','=', 0)
                        ->select('TBL_MST_UDFFOR_SL.*')
                        ->count();

       

        return view('masters.Accounts.UDFSL.mstfrm211',compact(['objRights','objCount']));        
    }

    public function add(){       
        return view('masters.Accounts.UDFSL.mstfrm211add');       
   }
   ...
   public function attachment($id){
    ...
    if(!is_null($id))
    {
        $objResponse = DB::table("TBL_MST_UDFFOR_SL")->where('UDFSLID','=',$id)->first();
        ...
        $objMstVoucherType = DB::table("TBL_MST_VOUCHERTYPE")
            ...
            return view('masters.Accounts.UDFSL.mstfrm211attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
    }
   ...
   public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            ...
            {
                $req_data[$i] = [
                    'UDFSLID' => (isset($request['UDFSLID_'.$i]) ? $request['UDFSLID_'.$i] : "0") ,
            ...
            $sp_result = DB::select('EXEC SP_UDFFOR_SL_INUPDE ?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
     ...
     public function getListing(Request $request){
        ...
        $COL_APP_STATUS =   'STATUS';  //never change value, value must be 'APPROVED_STATUS' as per stored procedure;
            ...
            $BRID_REF      =   Session::get('BRID_REF');
            $FYID_REF      =   Session::get('FYID_REF');       
            $TABLE1        =   "TBL_MST_UDFFOR_SL";
            $PK_COL        =   "UDFSLID";
            $SELECT_COL    =   "UDFSLID,LABEL,VALUETYPE,DESCRIPTIONS,ISMANDATORY,DEACTIVATED,DODEACTIVATED";    
            ...
            if(!empty($request->input('search.value')))
            {
                ...
                if($filtercolumn =='ALL'){
                    ...
                    $WHERE_COL =  " WHERE PARENTID = 0 AND UDFSLID LIKE  ". $search_text;
            ...
            if(!empty($sp_listing_result))
            {
                foreach ($sp_listing_result as $key=>$valueitem)
                {
                    ...
                    $nestedData['NO']           = '<input type="checkbox" id="chkId'.$valueitem->UDFSLID.'"  value="'.$valueitem->UDFSLID.'" class="js-selectall1" data-rcdstatus="'.$app_status.'">';
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            $objUdfResponse = DB::table('TBL_MST_UDFFOR_SL')                    
                             ->where('TBL_MST_UDFFOR_SL.UDFSLID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_SL.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_SL.*')
                             ->orderBy('TBL_MST_UDFFOR_SL.UDFSLID','ASC')
                             ->get()->toArray();
            ...
            $objRights = DB::table('TBL_MST_USERROLMAP')
                             ...
                             ->select('TBL_MST_USERROLMAP.*', 'TBL_MST_ROLEDETAILS.*')
                             ->first();
           return view('masters.Accounts.UDFSL.mstfrm211edit',compact(['objUdfResponse','objRights','objCount']));
        }
       ...
       public function view($id){
            ...
            if(!is_null($id))
            {
                // $objUdfResponse = TblMstFrm211::where('UDFSLID','=',$id)->first();
                $objUdfResponse = DB::table('TBL_MST_UDFFOR_SL')                    
                                ->where('TBL_MST_UDFFOR_SL.UDFSLID','=',$id)
                                ->orwhere('TBL_MST_UDFFOR_SL.PARENTID','=',$id)
                                ->select('TBL_MST_UDFFOR_SL.*')
                                ->orderBy('TBL_MST_UDFFOR_SL.UDFSLID','ASC')
                                ->get()->toArray();
                return view('masters.Accounts.UDFSL.mstfrm211view',compact(['objUdfResponse']));
            }
   ...
   public function update(Request $request){
    ...
    // dd($request->VALUETYPE);
        $data = [
            'UDFSLID' => (isset($request->UDFSLID_0) ? $request->UDFSLID_0 : "0"),
            'LABEL'    => strtoupper($request->LABEL_0),
            'VALUETYPE' => $request->VALUETYPE_0,
            'DESCRIPTIONS' => (isset($request->DESCRIPTIONS_0)? $request->DESCRIPTIONS_0 : ""),
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            if((isset($request['UDFSLID_'.$i])) && (isset($request['LABEL_'.$i])))
            {
                $req_data[$i] = [
                    'UDFSLID' => (isset($request['UDFSLID_'.$i]) ? $request['UDFSLID_'.$i] : "0") ,
                    'LABEL'    => strtoupper($request['LABEL_'.$i]),
   ...
   //save data
        ...
        $cancelData[0]= ['NT' =>'TBL_MST_UDFFOR_SL'];
   ...
   public function docuploads(Request $request){
    ...
    $destinationPath = storage_path()."/docs/company".$CYID_REF."/udfsl";
	$image_path         =   "docs/company".$CYID_REF."/udfsl";     
    ...
    if(empty($uploaded_data)){
        return redirect()->route("master",[211,"attachment",$ATTACH_DOCNO])->with("success","Already exists. No file uploaded");
    }
  ...
  //  }
    ...
    if($sp_result[0]->RESULT=="SUCCESS"){
        ...
        return redirect()->route("master",[211,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


    }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
   
        return redirect()->route("master",[211,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);

    }else{

        return redirect()->route("master",[211,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
    }
    ...
    public function checkLabel(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_UDFFOR_SL')
        ->where('TBL_MST_UDFFOR_SL.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_UDFFOR_SL.BRID_REF','=',Session::get('BRID_REF'))
        ->where('TBL_MST_UDFFOR_SL.FYID_REF','=',Session::get('FYID_REF'))
        ->where('TBL_MST_UDFFOR_SL.LABEL','=',$LABEL)
        ->select('TBL_MST_UDFFOR_SL.UDFSLID')
        ->first();
