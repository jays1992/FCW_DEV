...
use App\Models\Master\TblMstFrm77;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm77Controller extends Controller
{
   
    protected $form_id = 77;
    protected $vtid_ref   = 77;  //voucher type id

    //validation messages
    protected   $messages = [
                    'PTCODE.required' => 'Required field',
                    'PTCODE.unique' => 'Duplicate Code',
                    'PTNAME.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $objDataList	=	DB::select("SELECT * FROM TBL_MST_PACKAGINGTYPE WHERE CYID_REF='$CYID_REF'  ORDER BY PTID DESC");


        return view('masters.inventory.PackagingTypeMaster.mstfrm77',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.inventory.PackagingTypeMaster.mstfrm77add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_PACKAGINGTYPE')
   ...
   public function save(Request $request){
        ...
        $PTCODE       =   strtoupper(trim($request['PTCODE']) );
        $PTNAME   =   trim($request['PTNAME']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_PACKAGINGTYPE_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm77::where('PTID','=',$id)->first();
            ...
            return view('masters.inventory.PackagingTypeMaster.mstfrm77edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_PACKAGINGTYPE_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/packagingtypemaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[77,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[77,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[77,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[77,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[77,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[77,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm77::where('PTID','=',$id)->first();
            return view('masters.inventory.PackagingTypeMaster.mstfrm77view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm77::whereIn('PTID',$ids_data)->get();
        
        return view('masters.inventory.PackagingTypeMaster.mstfrm77print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.inventory.PackagingTypeMaster.mstfrm77attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
