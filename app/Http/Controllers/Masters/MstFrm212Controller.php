...
//use App\Models\Master\TblMstFrm212;
...
class MstFrm212Controller extends Controller
{
    protected $form_id = 212;
    protected $vtid_ref   = 312;  //voucher type id
    // //validation messages
    protected   $messages = [
    ...
    public function index(){    
        $objRights = DB::table('TBL_MST_USERROLMAP')
                        ...
                        ->first();
        $objCount = DB::table('TBL_MST_UDFFOR_BRV')
                        ->where('TBL_MST_UDFFOR_BRV.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_MST_UDFFOR_BRV.BRID_REF','=',Session::get('BRID_REF'))
                        ->where('TBL_MST_UDFFOR_BRV.FYID_REF','=',Session::get('FYID_REF'))
                        ->where('TBL_MST_UDFFOR_BRV.STATUS', '!=', 'C')
                        ->where('TBL_MST_UDFFOR_BRV.PARENTID','=', 0)
                        ->select('TBL_MST_UDFFOR_BRV.*')
                        ->count();

       

        return view('masters.Accounts.UDFBRV.mstfrm212',compact(['objRights','objCount']));        
    }

    public function add(){       
        return view('masters.Accounts.UDFBRV.mstfrm212add');       
   }
   ...
   public function attachment($id){
    ...
    if(!is_null($id))
    {
        $objResponse = DB::table("TBL_MST_UDFFOR_BRV")->where('UDFBRVID','=',$id)->first();
        ...
        $objMstVoucherType = DB::table("TBL_MST_VOUCHERTYPE")
            ...
            return view('masters.Accounts.UDFBRV.mstfrm212attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
    }
   ...
   public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            ...
            {
                $req_data[$i] = [
                    'UDFBRVID' => (isset($request['UDFBRVID_'.$i]) ? $request['UDFBRVID_'.$i] : "0") ,
            ...
            $sp_result = DB::select('EXEC SP_UDFFOR_BRV_INUPDE ?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
     ...
     public function getListing(Request $request){
        ...
        $COL_APP_STATUS =   'STATUS';  //never change value, value must be 'APPROVED_STATUS' as per stored procedure;
            ...
            $BRID_REF      =   Session::get('BRID_REF');
            $FYID_REF      =   Session::get('FYID_REF');       
            $TABLE1        =   "TBL_MST_UDFFOR_BRV";
            $PK_COL        =   "UDFBRVID";
            $SELECT_COL    =   "UDFBRVID,LABEL,VALUETYPE,DESCRIPTIONS,ISMANDATORY,DEACTIVATED,DODEACTIVATED";    
            ...
            if(!empty($request->input('search.value')))
            {
                ...
                if($filtercolumn =='ALL'){
                    ...
                    $WHERE_COL =  " WHERE PARENTID = 0 AND UDFBRVID LIKE  ". $search_text;
            ...
            if(!empty($sp_listing_result))
            {
                foreach ($sp_listing_result as $key=>$valueitem)
                {
                    ...
                    $nestedData['NO']           = '<input type="checkbox" id="chkId'.$valueitem->UDFBRVID.'"  value="'.$valueitem->UDFBRVID.'" class="js-selectall1" data-rcdstatus="'.$app_status.'">';
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            $objUdfResponse = DB::table('TBL_MST_UDFFOR_BRV')                    
                             ->where('TBL_MST_UDFFOR_BRV.UDFBRVID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_BRV.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_BRV.*')
                             ->orderBy('TBL_MST_UDFFOR_BRV.UDFBRVID','ASC')
                             ->get()->toArray();
            ...
            $objRights = DB::table('TBL_MST_USERROLMAP')
                             ...
                             ->select('TBL_MST_USERROLMAP.*', 'TBL_MST_ROLEDETAILS.*')
                             ->first();
           return view('masters.Accounts.UDFBRV.mstfrm212edit',compact(['objUdfResponse','objRights','objCount']));
        }
       ...
       public function view($id){
            ...
            if(!is_null($id))
            {
                // $objUdfResponse = TblMstFrm212::where('UDFBRVID','=',$id)->first();
                $objUdfResponse = DB::table('TBL_MST_UDFFOR_BRV')                    
                                ->where('TBL_MST_UDFFOR_BRV.UDFBRVID','=',$id)
                                ->orwhere('TBL_MST_UDFFOR_BRV.PARENTID','=',$id)
                                ->select('TBL_MST_UDFFOR_BRV.*')
                                ->orderBy('TBL_MST_UDFFOR_BRV.UDFBRVID','ASC')
                                ->get()->toArray();
                return view('masters.Accounts.UDFBRV.mstfrm212view',compact(['objUdfResponse']));
            }
   ...
   public function update(Request $request){
    ...
    // dd($request->VALUETYPE);
        $data = [
            'UDFBRVID' => (isset($request->UDFBRVID_0) ? $request->UDFBRVID_0 : "0"),
            'LABEL'    => strtoupper($request->LABEL_0),
            'VALUETYPE' => $request->VALUETYPE_0,
            'DESCRIPTIONS' => (isset($request->DESCRIPTIONS_0)? $request->DESCRIPTIONS_0 : ""),
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            if((isset($request['UDFBRVID_'.$i])) && (isset($request['LABEL_'.$i])))
            {
                $req_data[$i] = [
                    'UDFBRVID' => (isset($request['UDFBRVID_'.$i]) ? $request['UDFBRVID_'.$i] : "0") ,
                    'LABEL'    => strtoupper($request['LABEL_'.$i]),
   ...
   //save data
        ...
        $cancelData[0]= ['NT' =>'TBL_MST_UDFFOR_BRV'];
   ...
   public function docuploads(Request $request){
    ...
    $destinationPath = storage_path()."/docs/company".$CYID_REF."/udfbrv"; 
	$image_path         =   "docs/company".$CYID_REF."/udfbrv";     
    ...
    if(empty($uploaded_data)){
        return redirect()->route("master",[212,"attachment",$ATTACH_DOCNO])->with("success","Already exists. No file uploaded");
    }
  ...
  //  }
    ...
    if($sp_result[0]->RESULT=="SUCCESS"){
        ...
        return redirect()->route("master",[212,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


    }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
   
        return redirect()->route("master",[212,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);

    }else{

        return redirect()->route("master",[212,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
    }
    ...
    public function checkLabel(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_UDFFOR_BRV')
        ->where('TBL_MST_UDFFOR_BRV.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_UDFFOR_BRV.BRID_REF','=',Session::get('BRID_REF'))
        ->where('TBL_MST_UDFFOR_BRV.FYID_REF','=',Session::get('FYID_REF'))
        ->where('TBL_MST_UDFFOR_BRV.LABEL','=',$LABEL)
        ->select('TBL_MST_UDFFOR_BRV.UDFBRVID')
        ->first();
