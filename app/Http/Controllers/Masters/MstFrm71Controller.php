...
use App\Models\Master\TblMstFrm71;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm71Controller extends Controller
{
   
    protected $form_id = 71;
    protected $vtid_ref   = 71;  //voucher type id

    //validation messages
    protected   $messages = [
                    'ATTCODE.required' => 'Required field',
                    'ATTCODE.unique' => 'Duplicate Code',
    ...
    public function index(){
        ...
        $objDataList	=	DB::select("SELECT * FROM TBL_MST_ATTRIBUTE WHERE CYID_REF='$CYID_REF'  ORDER BY ATTID DESC");


        return view('masters.inventory.AttributeMaster.mstfrm71',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.inventory.AttributeMaster.mstfrm71add',compact(['objDD','objDOCNO']));
    }
   ...
   public function save(Request $request){
        ...
        for ($i=0; $i<=$r_count; $i++){
            ...
            if((isset($request['VALUE_'.$i]))){
                $data[$i] = [
                    'VALUE' => !(is_null($request['VALUE_'.$i]))=="true"? strtoupper(trim($request['VALUE_'.$i])) : NULL
                ];

                $existData[$i]=strtoupper(trim($request['VALUE_'.$i]));
            }
        }
        ...
        $wrapped_links["ATTVALUE"] = $data; 
        $xml = ArrayToXml::convert($wrapped_links);

        $ATTCODE      =   strtoupper(trim($request['ATTCODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_ATTRIBUTE_IN ?,?, ?,?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm71::where('ATTID','=',$id)->first();
            ...
            $objDataResponse = DB::table('TBL_MST_ATTRIBUTE_VAL')                    
                             ->where('TBL_MST_ATTRIBUTE_VAL.ATTID_REF','=',$id)
                             ->select('TBL_MST_ATTRIBUTE_VAL.*')
                             ->orderBy('TBL_MST_ATTRIBUTE_VAL.ATTVID','ASC')
                             ->get()->toArray();

            $objCount = count($objDataResponse);

            return view('masters.inventory.AttributeMaster.mstfrm71edit',compact(['objResponse','user_approval_level','objRights','objDataResponse','objCount']));
        }
      ...
      // dd($request->all());
        ...
        for ($i=0; $i<=$r_count; $i++){
            ...
            if((isset($request['VALUE_'.$i]))){
                $data[$i] = [
                    'VALUE' => !(is_null($request['VALUE_'.$i]))=="true"? strtoupper(trim($request['VALUE_'.$i])) : NULL,
                    'ATTVID' => !(is_null($request['ATTVID_'.$i]))=="true"? trim($request['ATTVID_'.$i]) : NULL 
                ];
        ...
        $sp_result = DB::select('EXEC SP_ATTRIBUTE_UP ?,?, ?,?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
		...
		$destinationPath = storage_path()."/docs/company".$CYID_REF."/AttributeMaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[71,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[71,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[71,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[71,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[71,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[71,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.inventory.AttributeMaster.mstfrm71view',compact(['objResponse','objDataResponse','objCount']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm71::whereIn('ATTID',$ids_data)->get();
        
        return view('masters.inventory.AttributeMaster.mstfrm71print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.inventory.AttributeMaster.mstfrm71attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
   ...
   //save data
        ...
        $cancelData[0]= ['NT' =>'TBL_MST_ATTRIBUTE_VAL'];
