...
use App\Models\Master\TblMstFrm112;
...
class MstFrm112Controller extends Controller
{
    protected $form_id = 112;
    protected $vtid_ref   = 289;  //voucher type id
    // //validation messages
    protected   $messages = [
    ...
    public function index(){    
        $objRights = DB::table('TBL_MST_USERROLMAP')
                        ...
                        ->first();
        $objCount = DB::table('TBL_MST_UDFFOR_RPF')
                        ->where('TBL_MST_UDFFOR_RPF.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_MST_UDFFOR_RPF.STATUS', '!=', 'C')
                        ->where('TBL_MST_UDFFOR_RPF.PARENTID','=', 0)
                        ->select('TBL_MST_UDFFOR_RPF.*')
                        ->count();

       

        return view('masters.Production.UDFRPF.mstfrm112',compact(['objRights','objCount']));        
    }

    public function add(){       
        return view('masters.Production.UDFRPF.mstfrm112add');       
   }
   ...
   public function attachment($id){
    ...
    if(!is_null($id))
    {
        $objResponse = TblMstFrm112::where('UDFRPFID','=',$id)->first();
        ...
        $objMstVoucherType = DB::table("TBL_MST_VOUCHERTYPE")
            ...
            return view('masters.Production.UDFRPF.mstfrm112attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
    }
   ...
   public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            ...
            {
                $req_data[$i] = [
                    'UDFRPFID' => (isset($request['UDFRPFID_'.$i]) ? $request['UDFRPFID_'.$i] : "0") ,
            ...
            $sp_result = DB::select('EXEC SP_UDFFOR_RPF_INUPDE ?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
     ...
     public function getListing(Request $request){
        ...
        $COL_APP_STATUS =   'STATUS';  //never change value, value must be 'APPROVED_STATUS' as per stored procedure;
            ...
            $BRID_REF      =   Session::get('BRID_REF');
            $FYID_REF      =   Session::get('FYID_REF');       
            $TABLE1        =   "TBL_MST_UDFFOR_RPF";
            $PK_COL        =   "UDFRPFID";
            $SELECT_COL    =   "UDFRPFID,LABEL,VALUETYPE,DESCRIPTIONS,ISMANDATORY,DEACTIVATED,DODEACTIVATED";    
            ...
            if(!empty($request->input('search.value')))
            {
                ...
                if($filtercolumn =='ALL'){
                    ...
                    $WHERE_COL =  " WHERE PARENTID = 0 AND UDFRPFID LIKE  ". $search_text;
            ...
            if(!empty($sp_listing_result))
            {
                foreach ($sp_listing_result as $key=>$valueitem)
                {
                    ...
                    $nestedData['NO']           = '<input type="checkbox" id="chkId'.$valueitem->UDFRPFID.'"  value="'.$valueitem->UDFRPFID.'" class="js-selectall1" data-rcdstatus="'.$app_status.'">';
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            $objUdfResponse = DB::table('TBL_MST_UDFFOR_RPF')                    
                             ->where('TBL_MST_UDFFOR_RPF.UDFRPFID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_RPF.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_RPF.*')
                             ->orderBy('TBL_MST_UDFFOR_RPF.UDFRPFID','ASC')
                             ->get()->toArray();
            ...
            $objRights = DB::table('TBL_MST_USERROLMAP')
                             ...
                             ->select('TBL_MST_USERROLMAP.*', 'TBL_MST_ROLEDETAILS.*')
                             ->first();
           return view('masters.Production.UDFRPF.mstfrm112edit',compact(['objUdfResponse','objRights','objCount']));
        }
       ...
       public function view($id){
         ...
         if(!is_null($id))
         {
             // $objUdfResponse = TblMstFrm112::where('UDFRPFID','=',$id)->first();
             $objUdfResponse = DB::table('TBL_MST_UDFFOR_RPF')                    
                             ->where('TBL_MST_UDFFOR_RPF.UDFRPFID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_RPF.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_RPF.*')
                             ->orderBy('TBL_MST_UDFFOR_RPF.UDFRPFID','ASC')
                             ->get()->toArray();
             return view('masters.Production.UDFRPF.mstfrm112view',compact(['objUdfResponse']));
         }
   ...
   public function update(Request $request){
    ...
    // dd($request->VALUETYPE);
        $data = [
            'UDFRPFID' => (isset($request->UDFRPFID_0) ? $request->UDFRPFID_0 : "0"),
            'LABEL'    => strtoupper($request->LABEL_0),
            'VALUETYPE' => $request->VALUETYPE_0,
            'DESCRIPTIONS' => (isset($request->DESCRIPTIONS_0)? $request->DESCRIPTIONS_0 : ""),
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            if((isset($request['UDFRPFID_'.$i])) && (isset($request['LABEL_'.$i])))
            {
                $req_data[$i] = [
                    'UDFRPFID' => (isset($request['UDFRPFID_'.$i]) ? $request['UDFRPFID_'.$i] : "0") ,
                    'LABEL'    => strtoupper($request['LABEL_'.$i]),
   ...
   public function docuploads(Request $request){
    ...
    $destinationPath = storage_path()."/docs/company".$CYID_REF."/udfrpfmst";
    ...
    if(empty($uploaded_data)){
        return redirect()->route("master",[112,"attachment",$ATTACH_DOCNO])->with("success","Already exists. No file uploaded");
    }
  ...
  //  }
    ...
    if($sp_result[0]->RESULT=="SUCCESS"){
        ...
        return redirect()->route("master",[112,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


    }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
   
        return redirect()->route("master",[112,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);

    }else{

        return redirect()->route("master",[112,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
    }
    ...
    public function checkLabel(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_UDFFOR_RPF')
        ->where('TBL_MST_UDFFOR_RPF.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_UDFFOR_RPF.LABEL','=',$LABEL)
        ->select('TBL_MST_UDFFOR_RPF.UDFRPFID')
        ->first();
