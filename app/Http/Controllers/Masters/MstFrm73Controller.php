...
use App\Models\Master\TblMstFrm73;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm73Controller extends Controller
{
   
    protected $form_id = 73;
    protected $vtid_ref   = 73;  //voucher type id

    //validation messages
    protected   $messages = [
                    'UOMCODE.required' => 'Required field',
                    'UOMCODE.unique' => 'Duplicate Code',
    ...
    public function index(){
        ...
        $objDataList	=	DB::select("SELECT * FROM TBL_MST_UOM WHERE CYID_REF='$CYID_REF'  ORDER BY UOMID DESC ");

        return view('masters.inventory.UnitOfMeasurement.mstfrm73',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.inventory.UnitOfMeasurement.mstfrm73add',compact(['objDD','objDOCNO']));
    }
   ...
   public function save(Request $request){
        ...
        $UOMCODE       =   strtoupper(trim($request['UOMCODE']) );
       ...
       // dump($array_data);
        try {
            ...
            $sp_result = DB::select('EXEC SP_UOM_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm73::where('UOMID','=',$id)->first();
            ...
            return view('masters.inventory.UnitOfMeasurement.mstfrm73edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_UOM_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/UnitOfMeasurementMst";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[73,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[73,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[73,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[73,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[73,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[73,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm73::where('UOMID','=',$id)->first();
            return view('masters.inventory.UnitOfMeasurement.mstfrm73view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm73::whereIn('UOMID',$ids_data)->get();
        
        return view('masters.inventory.UnitOfMeasurement.mstfrm73print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.inventory.UnitOfMeasurement.mstfrm73attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
