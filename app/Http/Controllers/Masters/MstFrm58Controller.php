...
use App\Models\Master\TblMstFrm58;
...
class MstFrm58Controller extends Controller
{
    protected $form_id = 58;
    protected $vtid_ref   = 58;  //voucher type id
    // //validation messages
    protected   $messages = [
    ...
    public function index(){    
        $objRights = DB::table('TBL_MST_USERROLMAP')
                        ...
                        ->first();
        $objCount = DB::table('TBL_MST_UDFFOR_VENDOR')
                        ->where('TBL_MST_UDFFOR_VENDOR.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_MST_UDFFOR_VENDOR.STATUS', '!=', 'C')
                        ->where('TBL_MST_UDFFOR_VENDOR.PARENTID','=', 0)
                        ->select('TBL_MST_UDFFOR_VENDOR.*')
                        ->count();

       

        return view('masters.Purchase.UDFVM.mstfrm58',compact(['objRights','objCount']));        
    }

    public function add(){       
        return view('masters.Purchase.UDFVM.mstfrm58add');       
   }
   ...
   public function attachment($id){
    ...
    if(!is_null($id))
    {
        $objResponse = TblMstFrm58::where('UDFVID','=',$id)->first();
        ...
        $objMstVoucherType = DB::table("TBL_MST_VOUCHERTYPE")
            ...
            return view('masters.Purchase.UDFVM.mstfrm58attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
    }
   ...
   public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            ...
            {
                $req_data[$i] = [
                    'UDFVID' => (isset($request['UDFVID_'.$i]) ? $request['UDFVID_'.$i] : "0") ,
            ...
            $sp_result = DB::select('EXEC SP_UDFFOR_VENDOR_INUPDE ?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
     ...
     public function getListing(Request $request){
        ...
        $COL_APP_STATUS =   'STATUS';  //never change value, value must be 'APPROVED_STATUS' as per stored procedure;
            ...
            $BRID_REF      =   Session::get('BRID_REF');
            $FYID_REF      =   Session::get('FYID_REF');       
            $TABLE1        =   "TBL_MST_UDFFOR_VENDOR";
            $PK_COL        =   "UDFVID";
            $SELECT_COL    =   "UDFVID,LABEL,VALUETYPE,DESCRIPTIONS,ISMANDATORY,DEACTIVATED,DODEACTIVATED";    
            ...
            if(!empty($request->input('search.value')))
            {
                ...
                if($filtercolumn =='ALL'){
                    ...
                    $WHERE_COL =  " WHERE PARENTID = 0 AND UDFVID LIKE  ". $search_text;
            ...
            if(!empty($sp_listing_result))
            {
                foreach ($sp_listing_result as $key=>$valueitem)
                {
                    ...
                    $nestedData['NO']           = '<input type="checkbox" id="chkId'.$valueitem->UDFVID.'"  value="'.$valueitem->UDFVID.'" class="js-selectall1" data-rcdstatus="'.$app_status.'">';
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            $objUdfResponse = DB::table('TBL_MST_UDFFOR_VENDOR')                    
                             ->where('TBL_MST_UDFFOR_VENDOR.UDFVID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_VENDOR.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_VENDOR.*')
                             ->orderBy('TBL_MST_UDFFOR_VENDOR.UDFVID','ASC')
                             ->get()->toArray();
            ...
            $objRights = DB::table('TBL_MST_USERROLMAP')
                             ...
                             ->select('TBL_MST_USERROLMAP.*', 'TBL_MST_ROLEDETAILS.*')
                             ->first();
           return view('masters.Purchase.UDFVM.mstfrm58edit',compact(['objUdfResponse','objRights','objCount']));
        }
       ...
       public function view($id){
         ...
         if(!is_null($id))
         {
             // $objUdfResponse = TblMstFrm58::where('UDFVID','=',$id)->first();
             $objUdfResponse = DB::table('TBL_MST_UDFFOR_VENDOR')                    
                             ->where('TBL_MST_UDFFOR_VENDOR.UDFVID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_VENDOR.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_VENDOR.*')
                             ->orderBy('TBL_MST_UDFFOR_VENDOR.UDFVID','ASC')
                             ->get()->toArray();
             return view('masters.Purchase.UDFVM.mstfrm58view',compact(['objUdfResponse']));
         }
   ...
   public function update(Request $request){
    ...
    // dd($request->VALUETYPE);
        $data = [
            'UDFVID' => (isset($request->UDFVID_0) ? $request->UDFVID_0 : "0"),
            'LABEL'    => strtoupper($request->LABEL_0),
            'VALUETYPE' => $request->VALUETYPE_0,
            'DESCRIPTIONS' => (isset($request->DESCRIPTIONS_0)? $request->DESCRIPTIONS_0 : ""),
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            if((isset($request['UDFVID_'.$i])) && (isset($request['LABEL_'.$i])))
            {
                $req_data[$i] = [
                    'UDFVID' => (isset($request['UDFVID_'.$i]) ? $request['UDFVID_'.$i] : "0") ,
                    'LABEL'    => strtoupper($request['LABEL_'.$i]),
   ...
   public function docuploads(Request $request){
    ...
    $destinationPath = storage_path()."/docs/company".$CYID_REF."/udfvendormst";
    ...
    if(empty($uploaded_data)){
        return redirect()->route("master",[58,"attachment",$ATTACH_DOCNO])->with("success","Already exists. No file uploaded");
    }
  ...
  //  }
    ...
    if($sp_result[0]->RESULT=="SUCCESS"){
        ...
        return redirect()->route("master",[58,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


    }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
   
        return redirect()->route("master",[58,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);

    }else{

        return redirect()->route("master",[58,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
    }
    ...
    public function checkLabel(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_UDFFOR_VENDOR')
        ->where('TBL_MST_UDFFOR_VENDOR.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_UDFFOR_VENDOR.LABEL','=',$LABEL)
        ->select('TBL_MST_UDFFOR_VENDOR.UDFVID')
        ->first();
