...
use App\Models\Master\TblMstFrm108;
...
class MstFrm108Controller extends Controller
{
    protected $form_id = 108;
    protected $vtid_ref   = 285;  //voucher type id
    // //validation messages
    protected   $messages = [
    ...
    public function index(){    
        $objRights = DB::table('TBL_MST_USERROLMAP')
                        ...
                        ->first();
        $objCount = DB::table('TBL_MST_UDFFOR_PRO')
                        ->where('TBL_MST_UDFFOR_PRO.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_MST_UDFFOR_PRO.STATUS', '!=', 'C')
                        ->where('TBL_MST_UDFFOR_PRO.PARENTID','=', 0)
                        ->select('TBL_MST_UDFFOR_PRO.*')
                        ->count();

       

        return view('masters.Production.UDFPRO.mstfrm108',compact(['objRights','objCount']));        
    }

    public function add(){       
        return view('masters.Production.UDFPRO.mstfrm108add');       
   }
   ...
   public function attachment($id){
    ...
    if(!is_null($id))
    {
        $objResponse = TblMstFrm108::where('UDFPROID','=',$id)->first();
        ...
        $objMstVoucherType = DB::table("TBL_MST_VOUCHERTYPE")
            ...
            return view('masters.Production.UDFPRO.mstfrm108attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
    }
   ...
   public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            ...
            {
                $req_data[$i] = [
                    'UDFPROID' => (isset($request['UDFPROID_'.$i]) ? $request['UDFPROID_'.$i] : "0") ,
            ...
            $sp_result = DB::select('EXEC SP_UDFFOR_PRO_INUPDE ?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
     ...
     public function getListing(Request $request){
        ...
        $COL_APP_STATUS =   'STATUS';  //never change value, value must be 'APPROVED_STATUS' as per stored procedure;
            ...
            $BRID_REF      =   Session::get('BRID_REF');
            $FYID_REF      =   Session::get('FYID_REF');       
            $TABLE1        =   "TBL_MST_UDFFOR_PRO";
            $PK_COL        =   "UDFPROID";
            $SELECT_COL    =   "UDFPROID,LABEL,VALUETYPE,DESCRIPTIONS,ISMANDATORY,DEACTIVATED,DODEACTIVATED";    
            ...
            if(!empty($request->input('search.value')))
            {
                ...
                if($filtercolumn =='ALL'){
                    ...
                    $WHERE_COL =  " WHERE PARENTID = 0 AND UDFPROID LIKE  ". $search_text;
            ...
            if(!empty($sp_listing_result))
            {
                foreach ($sp_listing_result as $key=>$valueitem)
                {
                    ...
                    $nestedData['NO']           = '<input type="checkbox" id="chkId'.$valueitem->UDFPROID.'"  value="'.$valueitem->UDFPROID.'" class="js-selectall1" data-rcdstatus="'.$app_status.'">';
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            $objUdfResponse = DB::table('TBL_MST_UDFFOR_PRO')                    
                             ->where('TBL_MST_UDFFOR_PRO.UDFPROID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_PRO.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_PRO.*')
                             ->orderBy('TBL_MST_UDFFOR_PRO.UDFPROID','ASC')
                             ->get()->toArray();
            ...
            $objRights = DB::table('TBL_MST_USERROLMAP')
                             ...
                             ->select('TBL_MST_USERROLMAP.*', 'TBL_MST_ROLEDETAILS.*')
                             ->first();
           return view('masters.Production.UDFPRO.mstfrm108edit',compact(['objUdfResponse','objRights','objCount']));
        }
       ...
       public function view($id){
         ...
         if(!is_null($id))
         {
             // $objUdfResponse = TblMstFrm108::where('UDFPROID','=',$id)->first();
             $objUdfResponse = DB::table('TBL_MST_UDFFOR_PRO')                    
                             ->where('TBL_MST_UDFFOR_PRO.UDFPROID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_PRO.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_PRO.*')
                             ->orderBy('TBL_MST_UDFFOR_PRO.UDFPROID','ASC')
                             ->get()->toArray();
             return view('masters.Production.UDFPRO.mstfrm108view',compact(['objUdfResponse']));
         }
   ...
   public function update(Request $request){
    ...
    // dd($request->VALUETYPE);
        $data = [
            'UDFPROID' => (isset($request->UDFPROID_0) ? $request->UDFPROID_0 : "0"),
            'LABEL'    => strtoupper($request->LABEL_0),
            'VALUETYPE' => $request->VALUETYPE_0,
            'DESCRIPTIONS' => (isset($request->DESCRIPTIONS_0)? $request->DESCRIPTIONS_0 : ""),
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            if((isset($request['UDFPROID_'.$i])) && (isset($request['LABEL_'.$i])))
            {
                $req_data[$i] = [
                    'UDFPROID' => (isset($request['UDFPROID_'.$i]) ? $request['UDFPROID_'.$i] : "0") ,
                    'LABEL'    => strtoupper($request['LABEL_'.$i]),
   ...
   public function docuploads(Request $request){
    ...
    $destinationPath = storage_path()."/docs/company".$CYID_REF."/udfpromst";
    ...
    if(empty($uploaded_data)){
        return redirect()->route("master",[108,"attachment",$ATTACH_DOCNO])->with("success","Already exists. No file uploaded");
    }
  ...
  //  }
    ...
    if($sp_result[0]->RESULT=="SUCCESS"){
        ...
        return redirect()->route("master",[108,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


    }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
   
        return redirect()->route("master",[108,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);

    }else{

        return redirect()->route("master",[108,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
    }
    ...
    public function checkLabel(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_UDFFOR_PRO')
        ->where('TBL_MST_UDFFOR_PRO.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_UDFFOR_PRO.LABEL','=',$LABEL)
        ->select('TBL_MST_UDFFOR_PRO.UDFPROID')
        ->first();
