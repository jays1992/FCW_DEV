...
use App\Models\Master\TblMstFrm8;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm8Controller extends Controller
{
   
    protected $form_id = 8;
    protected $vtid_ref   = 296;  //voucher type id

    //validation messages
    protected   $messages = [
                    'BUCODE.required' => 'Required field',
                    'BUCODE.unique' => 'Duplicate Code',
                    'BUNAME.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $objDataList=DB::select("SELECT * FROM TBL_MST_BUSINESSUNIT WHERE CYID_REF='$CYID_REF' ORDER BY BUID DESC");


        return view('masters.Sales.BusinessUnit.mstfrm8',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.Sales.BusinessUnit.mstfrm8add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_BUSINESSUNIT')
   ...
   public function save(Request $request){
        ...
        $BUCODE       =   strtoupper(trim($request['BUCODE']) );
        $BUNAME   =   trim($request['BUNAME']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_BUSINESSUNIT_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm8::where('BUID','=',$id)->first();
            ...
            return view('masters.Sales.BusinessUnit.mstfrm8edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_BUSINESSUNIT_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/businessunitmst";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[8,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[8,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[8,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[8,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[8,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[8,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm8::where('BUID','=',$id)->first();
            return view('masters.Sales.BusinessUnit.mstfrm8view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm8::whereIn('BUID',$ids_data)->get();
        
        return view('masters.Sales.BusinessUnit.mstfrm8print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Sales.BusinessUnit.mstfrm8attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
