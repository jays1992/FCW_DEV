...
use App\Models\Master\TblMstFrm164;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm164Controller extends Controller
{
   
    protected $form_id = 164;
    protected $vtid_ref   = 264;  //voucher type id
    ...
    public function index(){
        ...
        $objDataList	=	DB::select("SELECT * FROM TBL_MST_NATUAREOF_ASSESSEE WHERE CYID_REF='$CYID_REF'  ORDER BY NOAID DESC");

        return view('masters.Accounts.NatureOfAssessee.mstfrm164',compact(['objRights','objDataList']));

    }

    
    public function add(){ 
        return view('masters.Accounts.NatureOfAssessee.mstfrm164add');
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_MODULE')
   ...
   public function save(Request $request){
        ...
        $rules = [
            'NOA_CODE' => 'required|unique:TBL_MST_NATUAREOF_ASSESSEE',
            'NOA_NAME' => 'required',          
        ];
        ...
        $NOA_CODE       =   strtoupper(trim($request['NOA_CODE']) );
        $NOA_NAME   =   trim($request['NOA_NAME']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_NATUAREOF_ASSESSEE_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm164::where('NOAID','=',$id)->first();
            ...
            return view('masters.Accounts.NatureOfAssessee.mstfrm164edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_NATUAREOF_ASSESSEE_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
        //dd($array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/company".$CYID_REF."/NatureOfAssessee";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[164,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[164,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[164,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[164,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[164,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm164::where('NOAID','=',$id)->first();
            return view('masters.Accounts.NatureOfAssessee.mstfrm164view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm164::whereIn('NOAID',$ids_data)->get();
        
        return view('masters.Accounts.NatureOfAssessee.mstfrm164print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Accounts.NatureOfAssessee.mstfrm164attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
    ...
    public function MultiApprove(Request $request){
        ...
        if(!empty($sp_listing_result))
            ...
            $BRID_REF   =   Session::get('BRID_REF');
            $FYID_REF   =   Session::get('FYID_REF');       
            $TABLE      =   "TBL_MST_NATUAREOF_ASSESSEE";
            $FIELD      =   "NOAID";
   ...
   public function cancel(Request $request){
    ...
    $id = $request->{0};    
        ...
        $cancelData[0]= ['NT' =>'TBL_MST_NATUAREOF_ASSESSEE'];
