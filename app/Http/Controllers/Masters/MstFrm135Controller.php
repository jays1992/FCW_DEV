...
use App\Models\Master\TblMstFrm135;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm135Controller extends Controller
{
   
    protected $form_id = 135;
    protected $vtid_ref   = 97;  //voucher type id

    //validation messages
    protected   $messages = [
                    'QCP_CODE.required' => 'Required field',
                    'QCP_CODE.unique' => 'Duplicate Code',
                    'QCP_DESC.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        return view('masters.Quality.QCParameterMaster.mstfrm135',compact(['objRights']));
    ...
    public function getlisting(Request $request){
        ...
        $TABLE1     =   "TBL_MST_QCP";
        $PK_COL     =   "QCPID";
        $SELECT_COL =   "QCPID,QCP_CODE,QCP_DESC";    
        ...
        if(!empty($request->input('search.value')))
        {
            ...
            if($filtercolumn =='ALL'){
                ...
                $WHERE_COL =  " WHERE QCP_CODE LIKE  ". $search_text;
                $WHERE_COL =  $WHERE_COL.  " OR  QCP_DESC LIKE  ". $search_text;
        ...
        if(!empty($sp_listing_result))
        {
            foreach ($sp_listing_result as $key=>$valueitem)
            {
                ...
                $nestedData['NO']           = '<input type="checkbox" id="chkId'.$valueitem->QCPID.'"  value="'.$valueitem->QCPID.'" class="js-selectall1" data-rcdstatus="'.$app_status.'">';
                $nestedData['QCP_CODE']         = $valueitem->QCP_CODE;
                $nestedData['QCP_DESC']     = $valueitem->QCP_DESC;
    ...
    public function add(){ 
        ...
        return view('masters.Quality.QCParameterMaster.mstfrm135add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_QCP')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
        //->where('BRID_REF','=',Session::get('BRID_REF'))
        //->where('FYID_REF','=',Session::get('FYID_REF'))
        ->where('QCP_CODE','=',$QCP_CODE)
        ->select('QCP_CODE')
        ->first();
   ...
   public function save(Request $request){
        ...
        $QCP_CODE       =   strtoupper(trim($request['QCP_CODE']) );
        $QCP_DESC   =   trim($request['QCP_DESC']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_QCP_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm135::where('QCPID','=',$id)->first();
            ...
            return view('masters.Quality.QCParameterMaster.mstfrm135edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_QCP_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/qcparametermaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[135,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[135,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[135,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[135,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[135,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[135,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm135::where('QCPID','=',$id)->first();
            return view('masters.Quality.QCParameterMaster.mstfrm135view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm135::whereIn('QCPID',$ids_data)->get();
        
        return view('masters.Quality.QCParameterMaster.mstfrm135print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Quality.QCParameterMaster.mstfrm135attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
