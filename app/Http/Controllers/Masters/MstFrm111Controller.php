...
use App\Models\Master\TblMstFrm111;
...
class MstFrm111Controller extends Controller
{
    protected $form_id = 111;
    protected $vtid_ref   = 288;  //voucher type id
    // //validation messages
    protected   $messages = [
    ...
    public function index(){    
        $objRights = DB::table('TBL_MST_USERROLMAP')
                        ...
                        ->first();
        $objCount = DB::table('TBL_MST_UDFFOR_PNM')
                        ->where('TBL_MST_UDFFOR_PNM.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_MST_UDFFOR_PNM.STATUS', '!=', 'C')
                        ->where('TBL_MST_UDFFOR_PNM.PARENTID','=', 0)
                        ->select('TBL_MST_UDFFOR_PNM.*')
                        ->count();

       

        return view('masters.Production.UDFPNM.mstfrm111',compact(['objRights','objCount']));        
    }

    public function add(){       
        return view('masters.Production.UDFPNM.mstfrm111add');       
   }
   ...
   public function attachment($id){
    ...
    if(!is_null($id))
    {
        $objResponse = TblMstFrm111::where('UDFPNMID','=',$id)->first();
        ...
        $objMstVoucherType = DB::table("TBL_MST_VOUCHERTYPE")
            ...
            return view('masters.Production.UDFPNM.mstfrm111attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
    }
   ...
   public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            ...
            {
                $req_data[$i] = [
                    'UDFPNMID' => (isset($request['UDFPNMID_'.$i]) ? $request['UDFPNMID_'.$i] : "0") ,
            ...
            $sp_result = DB::select('EXEC SP_UDFFOR_PNM_INUPDE ?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
     ...
     public function getListing(Request $request){
        ...
        $COL_APP_STATUS =   'STATUS';  //never change value, value must be 'APPROVED_STATUS' as per stored procedure;
            ...
            $BRID_REF      =   Session::get('BRID_REF');
            $FYID_REF      =   Session::get('FYID_REF');       
            $TABLE1        =   "TBL_MST_UDFFOR_PNM";
            $PK_COL        =   "UDFPNMID";
            $SELECT_COL    =   "UDFPNMID,LABEL,VALUETYPE,DESCRIPTIONS,ISMANDATORY,DEACTIVATED,DODEACTIVATED";    
            ...
            if(!empty($request->input('search.value')))
            {
                ...
                if($filtercolumn =='ALL'){
                    ...
                    $WHERE_COL =  " WHERE PARENTID = 0 AND UDFPNMID LIKE  ". $search_text;
            ...
            if(!empty($sp_listing_result))
            {
                foreach ($sp_listing_result as $key=>$valueitem)
                {
                    ...
                    $nestedData['NO']           = '<input type="checkbox" id="chkId'.$valueitem->UDFPNMID.'"  value="'.$valueitem->UDFPNMID.'" class="js-selectall1" data-rcdstatus="'.$app_status.'">';
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            $objUdfResponse = DB::table('TBL_MST_UDFFOR_PNM')                    
                             ->where('TBL_MST_UDFFOR_PNM.UDFPNMID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_PNM.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_PNM.*')
                             ->orderBy('TBL_MST_UDFFOR_PNM.UDFPNMID','ASC')
                             ->get()->toArray();
            ...
            $objRights = DB::table('TBL_MST_USERROLMAP')
                             ...
                             ->select('TBL_MST_USERROLMAP.*', 'TBL_MST_ROLEDETAILS.*')
                             ->first();
           return view('masters.Production.UDFPNM.mstfrm111edit',compact(['objUdfResponse','objRights','objCount']));
        }
       ...
       public function view($id){
         ...
         if(!is_null($id))
         {
             // $objUdfResponse = TblMstFrm111::where('UDFPNMID','=',$id)->first();
             $objUdfResponse = DB::table('TBL_MST_UDFFOR_PNM')                    
                             ->where('TBL_MST_UDFFOR_PNM.UDFPNMID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_PNM.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_PNM.*')
                             ->orderBy('TBL_MST_UDFFOR_PNM.UDFPNMID','ASC')
                             ->get()->toArray();
             return view('masters.Production.UDFPNM.mstfrm111view',compact(['objUdfResponse']));
         }
   ...
   public function update(Request $request){
    ...
    // dd($request->VALUETYPE);
        $data = [
            'UDFPNMID' => (isset($request->UDFPNMID_0) ? $request->UDFPNMID_0 : "0"),
            'LABEL'    => strtoupper($request->LABEL_0),
            'VALUETYPE' => $request->VALUETYPE_0,
            'DESCRIPTIONS' => (isset($request->DESCRIPTIONS_0)? $request->DESCRIPTIONS_0 : ""),
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            if((isset($request['UDFPNMID_'.$i])) && (isset($request['LABEL_'.$i])))
            {
                $req_data[$i] = [
                    'UDFPNMID' => (isset($request['UDFPNMID_'.$i]) ? $request['UDFPNMID_'.$i] : "0") ,
                    'LABEL'    => strtoupper($request['LABEL_'.$i]),
   ...
   public function docuploads(Request $request){
    ...
    $destinationPath = storage_path()."/docs/company".$CYID_REF."/udfpnmmst";
    ...
    if(empty($uploaded_data)){
        return redirect()->route("master",[111,"attachment",$ATTACH_DOCNO])->with("success","Already exists. No file uploaded");
    }
  ...
  //  }
    ...
    if($sp_result[0]->RESULT=="SUCCESS"){
        ...
        return redirect()->route("master",[111,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


    }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
   
        return redirect()->route("master",[111,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);

    }else{

        return redirect()->route("master",[111,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
    }
    ...
    public function checkLabel(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_UDFFOR_PNM')
        ->where('TBL_MST_UDFFOR_PNM.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_UDFFOR_PNM.LABEL','=',$LABEL)
        ->select('TBL_MST_UDFFOR_PNM.UDFPNMID')
        ->first();
