...
use App\Models\Master\TblMstFrm7;
...
class MstFrm7Controller extends Controller
{
    protected $form_id = 7;
    protected $vtid_ref   = 7;  //voucher type id
    // //validation messages
    protected   $messages = [
    ...
    public function index(){    
        $objRights = DB::table('TBL_MST_USERROLMAP')
                        ...
                        ->first();
        $objCount = DB::table('TBL_MST_UDFFOR_CM')
                        ->where('TBL_MST_UDFFOR_CM.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_MST_UDFFOR_CM.STATUS', '!=', 'C')
                        ->where('TBL_MST_UDFFOR_CM.PARENTID','=', 0)
                        ->select('TBL_MST_UDFFOR_CM.*')
                        ->count();

       

        return view('masters.Sales.UDFCM.mstfrm7',compact(['objRights','objCount']));        
    }

    public function add(){       
        return view('masters.Sales.UDFCM.mstfrm7add');       
   }
   ...
   public function attachment($id){
    ...
    if(!is_null($id))
    {
        $objResponse = TblMstFrm7::where('UDFCMID','=',$id)->first();
        ...
        $objMstVoucherType = DB::table("TBL_MST_VOUCHERTYPE")
            ...
            return view('masters.Sales.UDFCM.mstfrm7attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
    }
   ...
   public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            ...
            {
                $req_data[$i] = [
                    'UDFCMID' => (isset($request['UDFCMID_'.$i]) ? $request['UDFCMID_'.$i] : "0") ,
            ...
            $sp_result = DB::select('EXEC SP_UDFFOR_CM_INUPDE ?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
     ...
     public function getListing(Request $request){
        ...
        $COL_APP_STATUS =   'STATUS';  //never change value, value must be 'APPROVED_STATUS' as per stored procedure;
            ...
            $BRID_REF      =   Session::get('BRID_REF');
            $FYID_REF      =   Session::get('FYID_REF');       
            $TABLE1        =   "TBL_MST_UDFFOR_CM";
            $PK_COL        =   "UDFCMID";
            $SELECT_COL    =   "UDFCMID,LABEL,VALUETYPE,DESCRIPTIONS,ISMANDATORY,DEACTIVATED,DODEACTIVATED";    
            ...
            if(!empty($request->input('search.value')))
            {
                ...
                if($filtercolumn =='ALL'){
                    ...
                    $WHERE_COL =  " WHERE PARENTID = 0 AND UDFCMID LIKE  ". $search_text;
            ...
            if(!empty($sp_listing_result))
            {
                foreach ($sp_listing_result as $key=>$valueitem)
                {
                    ...
                    $nestedData['NO']           = '<input type="checkbox" id="chkId'.$valueitem->UDFCMID.'"  value="'.$valueitem->UDFCMID.'" class="js-selectall1" data-rcdstatus="'.$app_status.'">';
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            $objUdfResponse = DB::table('TBL_MST_UDFFOR_CM')                    
                             ->where('TBL_MST_UDFFOR_CM.UDFCMID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_CM.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_CM.*')
                             ->orderBy('TBL_MST_UDFFOR_CM.UDFCMID','ASC')
                             ->get()->toArray();
            ...
            $objRights = DB::table('TBL_MST_USERROLMAP')
                             ...
                             ->select('TBL_MST_USERROLMAP.*', 'TBL_MST_ROLEDETAILS.*')
                             ->first();
           return view('masters.Sales.UDFCM.mstfrm7edit',compact(['objUdfResponse','objRights','objCount']));
        }
       ...
       public function view($id){
         ...
         if(!is_null($id))
         {
             // $objUdfResponse = TblMstFrm7::where('UDFCMID','=',$id)->first();
             $objUdfResponse = DB::table('TBL_MST_UDFFOR_CM')                    
                             ->where('TBL_MST_UDFFOR_CM.UDFCMID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_CM.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_CM.*')
                             ->orderBy('TBL_MST_UDFFOR_CM.UDFCMID','ASC')
                             ->get()->toArray();
             return view('masters.Sales.UDFCM.mstfrm7view',compact(['objUdfResponse']));
         }
   ...
   public function update(Request $request){
    ...
    // dd($request->VALUETYPE);
        $data = [
            'UDFCMID' => (isset($request->UDFCMID_0) ? $request->UDFCMID_0 : "0"),
            'LABEL'    => strtoupper($request->LABEL_0),
            'VALUETYPE' => $request->VALUETYPE_0,
            'DESCRIPTIONS' => (isset($request->DESCRIPTIONS_0)? $request->DESCRIPTIONS_0 : ""),
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            if((isset($request['UDFCMID_'.$i])) && (isset($request['LABEL_'.$i])))
            {
                $req_data[$i] = [
                    'UDFCMID' => (isset($request['UDFCMID_'.$i]) ? $request['UDFCMID_'.$i] : "0") ,
                    'LABEL'    => strtoupper($request['LABEL_'.$i]),
   ...
   public function docuploads(Request $request){
    ...
    $destinationPath = storage_path()."/docs/company".$CYID_REF."/udfcmmst";
    ...
    if(empty($uploaded_data)){
        return redirect()->route("master",[7,"attachment",$ATTACH_DOCNO])->with("success","Already exists. No file uploaded");
    }
  ...
  //  }
    ...
    if($sp_result[0]->RESULT=="SUCCESS"){
        ...
        return redirect()->route("master",[7,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


    }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
   
        return redirect()->route("master",[7,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);

    }else{

        return redirect()->route("master",[7,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
    }
    ...
    public function checkLabel(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_UDFFOR_CM')
        ->where('TBL_MST_UDFFOR_CM.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_UDFFOR_CM.LABEL','=',$LABEL)
        ->select('TBL_MST_UDFFOR_CM.UDFCMID')
        ->first();
