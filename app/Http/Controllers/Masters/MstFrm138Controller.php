...
use App\Models\Master\TblMstFrm138;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm138Controller extends Controller
{
   
    protected $form_id = 138;
    protected $vtid_ref   = 148;  //voucher type id

    //validation messages
    protected   $messages = [
                    'CCOMPONENT_CODE.required' => 'Required field',
                    'CCOMPONENT_CODE.unique' => 'Duplicate Code',
    ...
    public function index(){
        ...
        $objDataList	=	DB::select("SELECT * FROM TBL_MST_COSTCOMPONENT WHERE CYID_REF='$CYID_REF'  ORDER BY CCOMPONENTID DESC");


        return view('masters.Production.CostComponent.mstfrm138',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.Production.CostComponent.mstfrm138add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $BRID_REF = Session::get('BRID_REF');
        $FYID_REF = Session::get('FYID_REF');
        $CCOMPONENT_CODE =   $request['CCOMPONENT_CODE'];
        
        $objLabel = DB::table('TBL_MST_COSTCOMPONENT')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
        ->where('BRID_REF','=',Session::get('BRID_REF'))
        ->where('FYID_REF','=',Session::get('FYID_REF'))
        ->where('CCOMPONENT_CODE','=',$CCOMPONENT_CODE)
        ->select('CCOMPONENT_CODE')
        ->first();
   ...
   public function save(Request $request){
        ...
        $CCOMPONENT_CODE       =   strtoupper(trim($request['CCOMPONENT_CODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_COSTCOMPONENT_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm138::where('CCOMPONENTID','=',$id)->first();
            ...
            return view('masters.Production.CostComponent.mstfrm138edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_COSTCOMPONENT_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
		...
		$destinationPath = storage_path()."/docs/company".$CYID_REF."/CostComponent";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[138,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[138,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[138,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[138,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[138,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[138,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm138::where('CCOMPONENTID','=',$id)->first();
            return view('masters.Production.CostComponent.mstfrm138view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm138::whereIn('CCOMPONENTID',$ids_data)->get();
        
        return view('masters.Production.CostComponent.mstfrm138print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Production.CostComponent.mstfrm138attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
