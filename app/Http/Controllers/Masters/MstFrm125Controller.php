...
use App\Models\Master\TblMstFrm125;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm125Controller extends Controller
{
   
    protected $form_id = 125;
    protected $vtid_ref   = 116;  //voucher type id

    //validation messages
    protected   $messages = [
                    'CITYCODE.required' => 'Required field',
                    'CITYCODE.unique' => 'Duplicate Code',
    ...
    public function index(){
        ...
        $objDataList=DB::select("SELECT CITYID,CITYCODE,NAME,DEACTIVATED,DODEACTIVATED,INDATE,STATUS FROM TBL_MST_CITY");

        return view('masters.Common.City.mstfrm125',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.Common.City.mstfrm125add',compact(['objCountryList','objDD','objDOCNO']));
    }
    ...
    public function getCountryWiseState(Request $request){
        ...
        if(!empty($objStateList)){
            foreach($objStateList as $index=>$state){
            
                echo '<tr >
                <td width="12%" class="ROW1"> <input type="checkbox" name="SELECT_STID_REF[]" id="stidref_'.$state->STID.'" class="cls_stidref" value="'.$state->STID.'" ></td>
                <td width="40%" class="ROW2">'.$state->STCODE.'
                <input type="hidden" id="txtstidref_'.$state->STID.'" data-desc="'.$state->STCODE.'" data-descname="'.$state->NAME.'" value="'.$state->STID.'" />
                </td>
                <td width="40%" class="ROW3">'.$state->NAME.'</td>
                </tr>';
            }
        }
        else{
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_CITY')
   ...
   public function save(Request $request){
        ...
        $CITYCODE       =   strtoupper(trim($request['CITYCODE']) );
        ...
        $array_data   = [
                        $CITYCODE, $NAME,$CTRYID_REF, $STID_REF,
       ...
       // dd( $array_data) ;  
        try {
            ...
            $sp_result = DB::select('EXEC SP_CITY_IN ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm125::where('CITYID','=',$id)->first();
            ...
            return view('masters.Common.City.mstfrm125edit',compact(['objResponse','user_approval_level','objRights','objCountryList','objCountryName','objStateName']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_CITY_UP ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/CityMaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[125,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[125,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[125,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[125,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[125,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[125,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Common.City.mstfrm125view',compact(['objResponse','objCountryName','objStateName']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm125::whereIn('CITYID',$ids_data)->get();
        
        return view('masters.Common.City.mstfrm125print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Common.City.mstfrm125attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
