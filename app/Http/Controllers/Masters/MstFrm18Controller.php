...
use App\Models\Master\TblMstFrm18;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm18Controller extends Controller
{
   
    protected $form_id = 18;
    protected $vtid_ref   = 18;  //voucher type id

    //validation messages
    protected   $messages = [
                    'INDSVCODE.required' => 'Required field',
                    'INDSVCODE.unique' => 'Duplicate Code',
    ...
    public function index(){
        ...
        $objDataList=DB::select("SELECT * FROM TBL_MST_INDUSTRYVERTICAL ORDER BY INDSVID");

        return view('masters.Sales.INDUSTRYVERTICAL.mstfrm18',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.Sales.INDUSTRYVERTICAL.mstfrm18add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $BRID_REF = Session::get('BRID_REF');
        $FYID_REF = Session::get('FYID_REF');
        $INDSVCODE =   $request['INDSVCODE'];
        
        $objLabel = DB::table('TBL_MST_INDUSTRYVERTICAL')
        ->where('INDSVCODE','=',$INDSVCODE)
        ->select('INDSVCODE')
        ->first();
   ...
   public function save(Request $request){
        ...
        $INDSVCODE       =   strtoupper(trim($request['INDSVCODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_INDUSTRYVERTICAL_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm18::where('INDSVID','=',$id)->first();
            ...
            return view('masters.Sales.INDUSTRYVERTICAL.mstfrm18edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_INDUSTRYVERTICAL_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/industryverticalmst";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[18,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[18,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[18,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[18,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[18,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[18,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm18::where('INDSVID','=',$id)->first();
            return view('masters.Sales.INDUSTRYVERTICAL.mstfrm18view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm18::whereIn('INDSVID',$ids_data)->get();
        
        return view('masters.Sales.INDUSTRYVERTICAL.mstfrm18print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Sales.INDUSTRYVERTICAL.mstfrm18attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
    ...
    public function MultiApprove(Request $request){
        ...
        if(!empty($sp_listing_result))
            ...
            $BRID_REF   =   Session::get('BRID_REF');
            $FYID_REF   =   Session::get('FYID_REF');       
            $TABLE      =   "TBL_MST_INDUSTRYVERTICAL";
            $FIELD      =   "INDSVID";
