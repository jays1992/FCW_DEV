...
use App\Models\Master\TblMstFrm142;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm142Controller extends Controller
{
   
    protected $form_id = 142;
    protected $vtid_ref   = 1;  //voucher type id

    //validation messages
    protected   $messages = [
                    'MODULECODE.required' => 'Required field',
                    'MODULECODE.unique' => 'Duplicate Code',
                    'MODULENAME.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $objDataList=DB::select("SELECT * FROM TBL_MST_MODULE ORDER BY MODULENAME");  

        return view('masters.Module.ModuleMaster.mstfrm142',compact(['objRights','objDataList']));

    }
    
    public function add(){ 
        return view('masters.Module.ModuleMaster.mstfrm142add');
    }
   ...
   public function save(Request $request){
        ...
        $rules = [
            'MODULECODE' => 'required|unique:TBL_MST_MODULE',
            'MODULENAME' => 'required',          
        ];
        ...
        $MODULECODE       =   strtoupper(trim($request['MODULECODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_MODULE_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm142::where('MODULEID','=',$id)->first();
            ...
            return view('masters.Module.ModuleMaster.mstfrm142edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_MODULE_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/ModuleMaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[142,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[142,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[142,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[142,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[142,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[142,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm142::where('MODULEID','=',$id)->first();
            return view('masters.Module.ModuleMaster.mstfrm142view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm142::whereIn('MODULEID',$ids_data)->get();
        
        return view('masters.Module.ModuleMaster.mstfrm142print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Module.ModuleMaster.mstfrm142attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
