...
use App\Models\Master\TblMstFrm174;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm174Controller extends Controller
{
   
    protected $form_id = 174;
    protected $vtid_ref   = 200;  //voucher type id

    //validation messages
    protected   $messages = [
                    'CATCODE.required' => 'Required field',
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_EMPCATEGORY')
            ->where('CYID_REF','=',Auth::user()->CYID_REF)

            ->get();


        return view('masters.Payroll.EmployeeCategory.mstfrm174',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        return view('masters.Payroll.EmployeeCategory.mstfrm174add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_EMPCATEGORY')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
   ...
   public function save(Request $request){
        ...
        $CATCODE       =   strtoupper(trim($request['CATCODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_EMPCATEGORY_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm174::where('CATID','=',$id)->first();
            ...
            return view('masters.Payroll.EmployeeCategory.mstfrm174edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_EMPCATEGORY_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
        //dd($array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/EmployeeCategory";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[174,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[174,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[174,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[174,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[174,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm174::where('CATID','=',$id)->first();
            return view('masters.Payroll.EmployeeCategory.mstfrm174view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm174::whereIn('CATID',$ids_data)->get();
        
        return view('masters.Payroll.EmployeeCategory.mstfrm174print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.EmployeeCategory.mstfrm174attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
