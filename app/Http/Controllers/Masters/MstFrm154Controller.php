...
use App\Models\Master\TblMstFrm154;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm154Controller extends Controller{
   
    protected $form_id  = 154;
    protected $vtid_ref = 121;
    protected $view     = "masters.Common.VendorCodingDefinition.mstfrm";
    ...
    public function index(){
        ...
        $objDataList    =   DB::table('TBL_MST_VENDORCODE')
    ...
    public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_VENDORCODE')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
        ->where('STATUS','!=','C')
        ->where('DEACTIVATE','!=',1)
        ->count();   
        ...
        if($objLabel > 0){  
            ...
            return Response::json(['exists' =>true,'msg' => 'Vendor coding definition already created.']);
    ...
    public function save(Request $request){
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_VENDORCODE_DEFINITION_IN ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id)){
            ...
            $objResponse = DB::table('TBL_MST_VENDORCODE')->where('VCODEDEFIID','=',$id)->first();
    ...
    public function update(Request $request){
        ...
        $VCODEDEFIID        =   trim($request['VCODEDEFIID']); 
        ...
        $array_data   = [
            $VCODEDEFIID,$MANUAL_SR, $SYSTEM_GRSR, $MANUAL_MAXLENGTH,$MAX_DIGIT, $NO_START,
            $DEACTIVATED, $DODEACTIVATED, $CYID_REF, $BRID_REF,$FYID_REF, 
            $VTID, $USERID, $UPDATE,$UPTIME, $ACTION, 
            $IPADDRESS,$PREFIX
        ];
        ...
        $sp_result = DB::select('EXEC SP_VENDORCODE_DEFINITION_UP ?,?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?', $array_data);
    ...
    public function docuploads(Request $request){
		...
		$destinationPath = storage_path()."/docs/company".$CYID_REF."/VendorCodingDefinition";
