...
use App\Models\Master\TblMstFrm224;
use DB;
use Response;
use Auth;
USE Session;
...
class MstFrm224Controller extends Controller
{
   
    protected $form_id = 224;
    protected $vtid_ref   = 180;  //voucher type id

    //validation messages
    protected   $messages = [
                    'MP_CODE.required' => 'Required field',
                    'MP_DESC.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_MAINTENANCE_PARAMETER')
            ->where('CYID_REF','=',Auth::user()->CYID_REF)
            ->get();


        return view('masters.PlantMaintenance.MaintenanceParameter.mstfrm224',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        return view('masters.PlantMaintenance.MaintenanceParameter.mstfrm224add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $BRID_REF = Session::get('BRID_REF');
        $FYID_REF = Session::get('FYID_REF');
        $MP_CODE =   $request['MP_CODE'];
        
        $objLabel = DB::table('TBL_MST_MAINTENANCE_PARAMETER')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
   ...
   public function save(Request $request){
        ...
        $MP_CODE       =   strtoupper(trim($request['MP_CODE']) );
        $MP_DESC   =   trim($request['MP_DESC']);  
        $DEACTIVATED    =   0;  
        $DODEACTIVATED  =   NULL;  
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_MAINTENANCE_PARAMETER_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?, ?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm224::where('MPID','=',$id)->first();
            ...
            return view('masters.PlantMaintenance.MaintenanceParameter.mstfrm224edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $MP_CODE       =   strtoupper(trim($request['MP_CODE']) );
        $DESCRIPTIONS   =   trim($request['MP_DESC']); 
        ...
        $sp_result = DB::select('EXEC SP_MAINTENANCE_PARAMETER_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
        //dd($array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/MaintenanceParameter";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[224,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[224,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[224,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[224,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[224,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm224::where('MPID','=',$id)->first();
            return view('masters.PlantMaintenance.MaintenanceParameter.mstfrm224view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm224::whereIn('MPID',$ids_data)->get();
        
        return view('masters.PlantMaintenance.MaintenanceParameter.mstfrm224print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.PlantMaintenance.MaintenanceParameter.mstfrm224attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
    ...
    public function MultiApprove(Request $request){
        ...
        if(!empty($sp_listing_result))
            ...
            $BRID_REF   =   Session::get('BRID_REF');
            $FYID_REF   =   Session::get('FYID_REF');       
            $TABLE      =   "TBL_MST_MAINTENANCE_PARAMETER";
            $FIELD      =   "MPID";
   ...
   public function cancel(Request $request){
    ...
    $id = $request->{0};    
        ...
        $req_data[0]=[
            'NT'  => 'TBL_MST_MAINTENANCE_PARAMETER',
        ];
