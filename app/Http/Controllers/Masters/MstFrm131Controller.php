...
use App\Models\Master\TblMstFrm131;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm131Controller extends Controller
{
   
    protected $form_id = 131;
    protected $vtid_ref   = 107;  //voucher type id

    //validation messages
    protected   $messages = [
                    'CLASS_CODE.required' => 'Required field',
                    'CLASS_CODE.unique' => 'Duplicate Code',
                    'CLASS_DESC.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $objDataList=DB::select("SELECT * FROM TBL_MST_INVENTORY_CLASS  ORDER BY CLASSID DESC");


        return view('masters.inventory.InventoryClass.mstfrm131',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.inventory.InventoryClass.mstfrm131add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_INVENTORY_CLASS')
   ...
   public function save(Request $request){
        ...
        $CLASS_DESC   =   trim($request['CLASS_DESC']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_INVENTORY_CLASS_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm131::where('CLASSID','=',$id)->first();
            ...
            return view('masters.inventory.InventoryClass.mstfrm131edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        //$sp_result = DB::select('EXEC SP_INVENTORY_CLASS_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/InventoryClassMaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[131,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[131,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[131,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[131,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[131,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[131,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm131::where('CLASSID','=',$id)->first();
            return view('masters.inventory.InventoryClass.mstfrm131view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm131::whereIn('CLASSID',$ids_data)->get();
        
        return view('masters.inventory.InventoryClass.mstfrm131print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.inventory.InventoryClass.mstfrm131attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
