...
use App\Models\Master\TblMstFrm181;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm181Controller extends Controller
{
   
    protected $form_id = 181;
    protected $vtid_ref   = 204;  //voucher type id

    //validation messages
    protected   $messages = [
                    'DCODE.required' => 'Required field',                    
    ...
    public function index(){
        ...
        $FormId         =   $this->form_id;
        $objDataList    =   DB::table('TBL_MST_DEPARTMENT')

            ->get();


        return view('masters.Payroll.DepartmentMaster.mstfrm181',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        return view('masters.Payroll.DepartmentMaster.mstfrm181add' ,compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_DEPARTMENT')
        ->where('DCODE','=',$DCODE)
        ->select('DCODE')
        ->first();
   ...
   public function save(Request $request){
        ...
        $DCODE       =   strtoupper(trim($request['DCODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_DEPARTMENT_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm181::where('DEPID','=',$id)->first();
            ...
            return view('masters.Payroll.DepartmentMaster.mstfrm181edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_DEPARTMENT_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/DepartmentMaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[181,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[181,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[181,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[181,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[181,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm181::where('DEPID','=',$id)->first();
            return view('masters.Payroll.DepartmentMaster.mstfrm181view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm181::whereIn('DEPID',$ids_data)->get();
        
        return view('masters.Payroll.DepartmentMaster.mstfrm181print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.DepartmentMaster.mstfrm181attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
