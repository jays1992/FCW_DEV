...
use App\Models\Master\TblMstFrm22;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm22Controller extends Controller
{
   
    protected $form_id = 22;
    protected $vtid_ref   = 22;  //voucher type id

    //validation messages
    protected   $messages = [
                    'COUPONCCODE.required' => 'Required field',
                    'COUPONCCODE.unique' => 'Duplicate Code',
                    'CCDESCRIPTIONS.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $objDataList=DB::select("SELECT * FROM TBL_MST_COUPON_CAT WHERE CYID_REF='$CYID_REF' ORDER BY COUPONCID DESC");


        return view('masters.Sales.CouponCategory.mstfrm22',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.Sales.CouponCategory.mstfrm22add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_COUPON_CAT')
        ->where('CYID_REF','=',Auth::user()->CYID_REF)
        ->where('BRID_REF','=',Session::get('BRID_REF'))
        ->where('FYID_REF','=',Session::get('FYID_REF'))
        ->where('COUPONCCODE','=',$COUPONCCODE)
        ->select('COUPONCCODE')
        ->first();
   ...
   public function save(Request $request){
        ...
        $rules = [
            'COUPONCCODE' => 'required',
            'CCDESCRIPTIONS' => 'required',          
        ];

        $req_data = [

            'COUPONCCODE'     =>  $request['COUPONCCODE'],
            'CCDESCRIPTIONS' =>  $request['CCDESCRIPTIONS']
        ]; 
        ...
        $COUPONCCODE       =   strtoupper(trim($request['COUPONCCODE']) );
        $CCDESCRIPTIONS   =   trim($request['CCDESCRIPTIONS']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_COUPON_CAT_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm22::where('COUPONCID','=',$id)->first();
            ...
            return view('masters.Sales.CouponCategory.mstfrm22edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_COUPON_CAT_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/CouponCategoryMst";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[22,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[22,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[22,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[22,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[22,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[22,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm22::where('COUPONCID','=',$id)->first();
            return view('masters.Sales.CouponCategory.mstfrm22view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm22::whereIn('COUPONCID',$ids_data)->get();
        
        return view('masters.Sales.CouponCategory.mstfrm22print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Sales.CouponCategory.mstfrm22attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
