...
use App\Models\Master\TblMstFrm102;
...
class MstFrm102Controller extends Controller
{
    protected $form_id = 102;
    protected $vtid_ref   = 279;  //voucher type id
    // //validation messages
    protected   $messages = [
    ...
    public function index(){    
        $objRights = DB::table('TBL_MST_USERROLMAP')
                        ...
                        ->first();
        $objCount = DB::table('TBL_MST_UDFFOR_GEJ')
                        ->where('TBL_MST_UDFFOR_GEJ.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_MST_UDFFOR_GEJ.STATUS', '!=', 'C')
                        ->where('TBL_MST_UDFFOR_GEJ.PARENTID','=', 0)
                        ->select('TBL_MST_UDFFOR_GEJ.*')
                        ->count();

       

        return view('masters.JobWork.UDFGEJ.mstfrm102',compact(['objRights','objCount']));        
    }

    public function add(){       
        return view('masters.JobWork.UDFGEJ.mstfrm102add');       
   }
   ...
   public function attachment($id){
    ...
    if(!is_null($id))
    {
        $objResponse = TblMstFrm102::where('UDFGEJID','=',$id)->first();
        ...
        $objMstVoucherType = DB::table("TBL_MST_VOUCHERTYPE")
            ...
            return view('masters.JobWork.UDFGEJ.mstfrm102attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
    }
   ...
   public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            ...
            {
                $req_data[$i] = [
                    'UDFGEJID' => (isset($request['UDFGEJID_'.$i]) ? $request['UDFGEJID_'.$i] : "0") ,
            ...
            $sp_result = DB::select('EXEC SP_UDFFOR_GEJ_INUPDE ?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
     ...
     public function getListing(Request $request){
        ...
        $COL_APP_STATUS =   'STATUS';  //never change value, value must be 'APPROVED_STATUS' as per stored procedure;
            ...
            $BRID_REF      =   Session::get('BRID_REF');
            $FYID_REF      =   Session::get('FYID_REF');       
            $TABLE1        =   "TBL_MST_UDFFOR_GEJ";
            $PK_COL        =   "UDFGEJID";
            $SELECT_COL    =   "UDFGEJID,LABEL,VALUETYPE,DESCRIPTIONS,ISMANDATORY,DEACTIVATED,DODEACTIVATED";    
            ...
            if(!empty($request->input('search.value')))
            {
                ...
                if($filtercolumn =='ALL'){
                    ...
                    $WHERE_COL =  " WHERE PARENTID = 0 AND UDFGEJID LIKE  ". $search_text;
            ...
            if(!empty($sp_listing_result))
            {
                foreach ($sp_listing_result as $key=>$valueitem)
                {
                    ...
                    $nestedData['NO']           = '<input type="checkbox" id="chkId'.$valueitem->UDFGEJID.'"  value="'.$valueitem->UDFGEJID.'" class="js-selectall1" data-rcdstatus="'.$app_status.'">';
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            $objUdfResponse = DB::table('TBL_MST_UDFFOR_GEJ')                    
                             ->where('TBL_MST_UDFFOR_GEJ.UDFGEJID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_GEJ.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_GEJ.*')
                             ->orderBy('TBL_MST_UDFFOR_GEJ.UDFGEJID','ASC')
                             ->get()->toArray();
            ...
            $objRights = DB::table('TBL_MST_USERROLMAP')
                             ...
                             ->select('TBL_MST_USERROLMAP.*', 'TBL_MST_ROLEDETAILS.*')
                             ->first();
           return view('masters.JobWork.UDFGEJ.mstfrm102edit',compact(['objUdfResponse','objRights','objCount']));
        }
       ...
       public function view($id){
         ...
         if(!is_null($id))
         {
             // $objUdfResponse = TblMstFrm102::where('UDFGEJID','=',$id)->first();
             $objUdfResponse = DB::table('TBL_MST_UDFFOR_GEJ')                    
                             ->where('TBL_MST_UDFFOR_GEJ.UDFGEJID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_GEJ.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_GEJ.*')
                             ->orderBy('TBL_MST_UDFFOR_GEJ.UDFGEJID','ASC')
                             ->get()->toArray();
             return view('masters.JobWork.UDFGEJ.mstfrm102view',compact(['objUdfResponse']));
         }
   ...
   public function update(Request $request){
    ...
    // dd($request->VALUETYPE);
        $data = [
            'UDFGEJID' => (isset($request->UDFGEJID_0) ? $request->UDFGEJID_0 : "0"),
            'LABEL'    => strtoupper($request->LABEL_0),
            'VALUETYPE' => $request->VALUETYPE_0,
            'DESCRIPTIONS' => (isset($request->DESCRIPTIONS_0)? $request->DESCRIPTIONS_0 : ""),
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            if((isset($request['UDFGEJID_'.$i])) && (isset($request['LABEL_'.$i])))
            {
                $req_data[$i] = [
                    'UDFGEJID' => (isset($request['UDFGEJID_'.$i]) ? $request['UDFGEJID_'.$i] : "0") ,
                    'LABEL'    => strtoupper($request['LABEL_'.$i]),
   ...
   //save data
        ...
        $canceldata[0]=[
            'NT'  => 'TBL_MST_UDFFOR_GEJ',
       ];        
       $links["TABLES"] = $canceldata; 
       $cancelxml = ArrayToXml::convert($links);
        
        $udf_cancel_data = [ $USERID_REF, $VTID_REF, $TABLE, $FIELD, $ID, $CYID_REF, $BRID_REF,$FYID_REF,$UPDATE,$UPTIME, $IPADDRESS,$canceldata ];
   ...
   public function docuploads(Request $request){
    ...
    $destinationPath = storage_path()."/docs/company".$CYID_REF."/udfgejmst";
    ...
    if(empty($uploaded_data)){
        return redirect()->route("master",[102,"attachment",$ATTACH_DOCNO])->with("success","Already exists. No file uploaded");
    }
  ...
  //  }
    ...
    if($sp_result[0]->RESULT=="SUCCESS"){
        ...
        return redirect()->route("master",[102,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


    }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
   
        return redirect()->route("master",[102,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);

    }else{

        return redirect()->route("master",[102,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
    }
    ...
    public function checkLabel(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_UDFFOR_GEJ')
        ->where('TBL_MST_UDFFOR_GEJ.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_UDFFOR_GEJ.LABEL','=',$LABEL)
        ->select('TBL_MST_UDFFOR_GEJ.UDFGEJID')
        ->first();
