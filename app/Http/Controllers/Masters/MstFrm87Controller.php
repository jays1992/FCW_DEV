...
class MstFrm87Controller extends Controller
{
   ...
   public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            ...
            {
                $req_data[$i] = [
                    'UDFSRNID' => (isset($request['UDFSRNID_'.$i]) ? $request['UDFSRNID_'.$i] : "0") ,
            ...
            $sp_result = DB::select('EXEC SP_UDFFOR_SRN_INUPDE ?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
     ...
     public function getListing(Request $request){
        ...
        $COL_APP_STATUS =   'STATUS';  //never change value, value must be 'APPROVED_STATUS' as per stored procedure;
            ...
            $BRID_REF      =   Session::get('BRID_REF');
            $FYID_REF      =   Session::get('FYID_REF');       
            $TABLE1        =   "TBL_MST_UDFFOR_SRN";
            $PK_COL        =   "UDFSRNID";
            $SELECT_COL    =   "UDFSRNID,LABEL,VALUETYPE,DESCRIPTIONS,ISMANDATORY,DEACTIVATED,DODEACTIVATED";    
            ...
            if(!empty($request->input('search.value')))
            {
                ...
                if($filtercolumn =='ALL'){
                    ...
                    $WHERE_COL =  " WHERE PARENTID = 0 AND UDFSRNID LIKE  ". $search_text;
            ...
            if(!empty($sp_listing_result))
            {
                foreach ($sp_listing_result as $key=>$valueitem)
                {
                    ...
                    $nestedData['NO']           = '<input type="checkbox" id="chkId'.$valueitem->UDFSRNID.'"  value="'.$valueitem->UDFSRNID.'" class="js-selectall1" data-rcdstatus="'.$app_status.'">';
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            $objUdfResponse = DB::table('TBL_MST_UDFFOR_SRN')                    
                             ->where('TBL_MST_UDFFOR_SRN.UDFSRNID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_SRN.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_SRN.*')
                             ->orderBy('TBL_MST_UDFFOR_SRN.UDFSRNID','ASC')
                             ->get()->toArray();
            ...
            $objRights = DB::table('TBL_MST_USERROLMAP')
                             ...
                             ->select('TBL_MST_USERROLMAP.*', 'TBL_MST_ROLEDETAILS.*')
                             ->first();
           return view('masters.inventory.UDFSRN.mstfrm87edit',compact(['objUdfResponse','objRights','objCount']));
        }
       ...
       public function view($id){
         ...
         if(!is_null($id))
         {
             // $objUdfResponse = TblMstFrm87::where('UDFSRNID','=',$id)->first();
             $objUdfResponse = DB::table('TBL_MST_UDFFOR_SRN')                    
                             ->where('TBL_MST_UDFFOR_SRN.UDFSRNID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_SRN.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_SRN.*')
                             ->orderBy('TBL_MST_UDFFOR_SRN.UDFSRNID','ASC')
                             ->get()->toArray();
             return view('masters.inventory.UDFSRN.mstfrm87view',compact(['objUdfResponse']));
         }
   ...
   public function update(Request $request){
    ...
    // dd($request->VALUETYPE);
        $data = [
            'UDFSRNID' => (isset($request->UDFSRNID_0) ? $request->UDFSRNID_0 : "0"),
            'LABEL'    => strtoupper($request->LABEL_0),
            'VALUETYPE' => $request->VALUETYPE_0,
            'DESCRIPTIONS' => (isset($request->DESCRIPTIONS_0)? $request->DESCRIPTIONS_0 : ""),
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            if((isset($request['UDFSRNID_'.$i])) && (isset($request['LABEL_'.$i])))
            {
                $req_data[$i] = [
                    'UDFSRNID' => (isset($request['UDFSRNID_'.$i]) ? $request['UDFSRNID_'.$i] : "0") ,
                    'LABEL'    => strtoupper($request['LABEL_'.$i]),
   ...
   public function docuploads(Request $request){
    ...
    $destinationPath = "E:/company".$CYID_REF."/udfsrnmst";
    ...
    if(empty($uploaded_data)){
        return redirect()->route("master",[87,"attachment",$ATTACH_DOCNO])->with("success","Already exists. No file uploaded");
    }
  ...
  //  }
    ...
    if($sp_result[0]->RESULT=="SUCCESS"){
        ...
        return redirect()->route("master",[87,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


    }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
   
        return redirect()->route("master",[87,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);

    }else{

        return redirect()->route("master",[87,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
    }
    ...
    public function checkLabel(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_UDFFOR_SRN')
        ->where('TBL_MST_UDFFOR_SRN.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_UDFFOR_SRN.LABEL','=',$LABEL)
        ->select('TBL_MST_UDFFOR_SRN.UDFSRNID')
        ->first();
