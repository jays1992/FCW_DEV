...
use App\Models\Master\TblMstFrm136;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm136Controller extends Controller
{
   
    protected $form_id = 136;
    protected $vtid_ref   = 99;  //voucher type id

    //validation messages
    protected   $messages = [
                    'RR_CODE.required' => 'Required field',
                    'RR_CODE.unique' => 'Duplicate Code',
                    'RR_DESC.required' => 'Required field'
                ];
    ...
    public function index(){
        ...
        $objDataList	=	DB::select("SELECT * FROM TBL_MST_REJECTION_REASON WHERE CYID_REF='$CYID_REF'");

        return view('masters.Quality.RejectionReasonMaster.mstfrm136',compact(['objRights','objDataList']));
    ...
    public function add(){ 
        ...
        return view('masters.Quality.RejectionReasonMaster.mstfrm136add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_REJECTION_REASON')
   ...
   public function save(Request $request){
        ...
        $RR_CODE       =   strtoupper(trim($request['RR_CODE']) );
        $RR_DESC   =   trim($request['RR_DESC']);  
        $DEACTIVATED    =   NULL;  
        $DODEACTIVATED  =   NULL;  
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_REJECTION_REASON_IN ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm136::where('RRID','=',$id)->first();
            ...
            return view('masters.Quality.RejectionReasonMaster.mstfrm136edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_REJECTION_REASON_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/RejectionReasonMaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[136,"attachment",$ATTACH_DOCNO])->with("success","No file uploaded");
        }
     //  dd($uploaded_data);
      ...
      //  } catch (\Throwable $th) {
        ...
        //    return redirect()->route("master",[136,"attachment",$ATTACH_DOCNO])->with("error","There is some error. Please try after sometime");
      ...
      //  }
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[136,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[136,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[136,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[136,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm136::where('RRID','=',$id)->first();
            return view('masters.Quality.RejectionReasonMaster.mstfrm136view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm136::whereIn('RRID',$ids_data)->get();
        
        return view('masters.Quality.RejectionReasonMaster.mstfrm136print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Quality.RejectionReasonMaster.mstfrm136attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
    ...
    public function MultiApprove(Request $request){
        ...
        if(!empty($sp_listing_result))
            ...
            $BRID_REF   =   Session::get('BRID_REF');
            $FYID_REF   =   Session::get('FYID_REF');       
            $TABLE      =   "TBL_MST_REJECTION_REASON";
            $FIELD      =   "RRID";
