...
use App\Models\Master\TblMstFrm56;
...
class MstFrm56Controller extends Controller
{
    protected $form_id = 56;
    protected $vtid_ref   = 56;  //voucher type id
    // //validation messages
    protected   $messages = [
    ...
    public function index(){    
        $objRights = DB::table('TBL_MST_USERROLMAP')
                        ...
                        ->first();
        $objCount = DB::table('TBL_MST_UDFFOR_SCH_ABLPO')
                        ->where('TBL_MST_UDFFOR_SCH_ABLPO.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_MST_UDFFOR_SCH_ABLPO.STATUS', '!=', 'C')
                        ->where('TBL_MST_UDFFOR_SCH_ABLPO.PARENTID','=', 0)
                        ->select('TBL_MST_UDFFOR_SCH_ABLPO.*')
                        ->count();

       

        return view('masters.Purchase.UDFSCHABLPO.mstfrm56',compact(['objRights','objCount']));        
    }

    public function add(){       
        return view('masters.Purchase.UDFSCHABLPO.mstfrm56add');       
   }
   ...
   public function attachment($id){
    ...
    if(!is_null($id))
    {
        $objResponse = TblMstFrm56::where('SCHBLPOID','=',$id)->first();
        ...
        $objMstVoucherType = DB::table("TBL_MST_VOUCHERTYPE")
            ...
            return view('masters.Purchase.UDFSCHABLPO.mstfrm56attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
    }
   ...
   public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            ...
            {
                $req_data[$i] = [
                    'SCHBLPOID' => (isset($request['SCHBLPOID_'.$i]) ? $request['SCHBLPOID_'.$i] : "0") ,
            ...
            $sp_result = DB::select('EXEC SP_UDFFOR_SCH_ABLPO_INUPDE ?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
     ...
     public function getListing(Request $request){
        ...
        $COL_APP_STATUS =   'STATUS';  //never change value, value must be 'APPROVED_STATUS' as per stored procedure;
            ...
            $BRID_REF      =   Session::get('BRID_REF');
            $FYID_REF      =   Session::get('FYID_REF');       
            $TABLE1        =   "TBL_MST_UDFFOR_SCH_ABLPO";
            $PK_COL        =   "SCHBLPOID";
            $SELECT_COL    =   "SCHBLPOID,LABEL,VALUETYPE,DESCRIPTIONS,ISMANDATORY,DEACTIVATED,DODEACTIVATED";    
            ...
            if(!empty($request->input('search.value')))
            {
                ...
                if($filtercolumn =='ALL'){
                    ...
                    $WHERE_COL =  " WHERE PARENTID = 0 AND SCHBLPOID LIKE  ". $search_text;
            ...
            if(!empty($sp_listing_result))
            {
                foreach ($sp_listing_result as $key=>$valueitem)
                {
                    ...
                    $nestedData['NO']           = '<input type="checkbox" id="chkId'.$valueitem->SCHBLPOID.'"  value="'.$valueitem->SCHBLPOID.'" class="js-selectall1" data-rcdstatus="'.$app_status.'">';
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            $objUdfResponse = DB::table('TBL_MST_UDFFOR_SCH_ABLPO')                    
                             ->where('TBL_MST_UDFFOR_SCH_ABLPO.SCHBLPOID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_SCH_ABLPO.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_SCH_ABLPO.*')
                             ->orderBy('TBL_MST_UDFFOR_SCH_ABLPO.SCHBLPOID','ASC')
                             ->get()->toArray();
            ...
            $objRights = DB::table('TBL_MST_USERROLMAP')
                             ...
                             ->select('TBL_MST_USERROLMAP.*', 'TBL_MST_ROLEDETAILS.*')
                             ->first();
           return view('masters.Purchase.UDFSCHABLPO.mstfrm56edit',compact(['objUdfResponse','objRights','objCount']));
        }
       ...
       public function view($id){
         ...
         if(!is_null($id))
         {
             // $objUdfResponse = TblMstFrm56::where('SCHBLPOID','=',$id)->first();
             $objUdfResponse = DB::table('TBL_MST_UDFFOR_SCH_ABLPO')                    
                             ->where('TBL_MST_UDFFOR_SCH_ABLPO.SCHBLPOID','=',$id)
                             ->orwhere('TBL_MST_UDFFOR_SCH_ABLPO.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFOR_SCH_ABLPO.*')
                             ->orderBy('TBL_MST_UDFFOR_SCH_ABLPO.SCHBLPOID','ASC')
                             ->get()->toArray();
             return view('masters.Purchase.UDFSCHABLPO.mstfrm56view',compact(['objUdfResponse']));
         }
   ...
   public function update(Request $request){
    ...
    // dd($request->VALUETYPE);
        $data = [
            'SCHBLPOID' => (isset($request->SCHBLPOID_0) ? $request->SCHBLPOID_0 : "0"),
            'LABEL'    => strtoupper($request->LABEL_0),
            'VALUETYPE' => $request->VALUETYPE_0,
            'DESCRIPTIONS' => (isset($request->DESCRIPTIONS_0)? $request->DESCRIPTIONS_0 : ""),
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            if((isset($request['SCHBLPOID_'.$i])) && (isset($request['LABEL_'.$i])))
            {
                $req_data[$i] = [
                    'SCHBLPOID' => (isset($request['SCHBLPOID_'.$i]) ? $request['SCHBLPOID_'.$i] : "0") ,
                    'LABEL'    => strtoupper($request['LABEL_'.$i]),
   ...
   public function Approve(Request $request){
    ...
    public function MultiApprove(Request $request){
            ...
            if(!empty($sp_listing_result))
                ...
                $BRID_REF   =   Session::get('BRID_REF');
                $FYID_REF   =   Session::get('FYID_REF');       
                $TABLE      =   "TBL_MST_UDFFOR_SCH_ABLPO";
                $FIELD      =   "SCHBLPOID";
                $ACTIONNAME     = $Approvallevel;
                $UPDATE     =   Date('Y-m-d');
                $UPTIME     =   Date('h:i:s.u');
   ...
   public function docuploads(Request $request){
    ...
    $destinationPath = storage_path()."/docs/company".$CYID_REF."/udfschablpomst";
    ...
    if(empty($uploaded_data)){
        return redirect()->route("master",[56,"attachment",$ATTACH_DOCNO])->with("success","Already exists. No file uploaded");
    }
  ...
  //  }
    ...
    if($sp_result[0]->RESULT=="SUCCESS"){
        ...
        return redirect()->route("master",[56,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


    }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
   
        return redirect()->route("master",[56,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);

    }else{

        return redirect()->route("master",[56,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
    }
    ...
    public function checkLabel(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_UDFFOR_SCH_ABLPO')
        ->where('TBL_MST_UDFFOR_SCH_ABLPO.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_UDFFOR_SCH_ABLPO.LABEL','=',$LABEL)
        ->select('TBL_MST_UDFFOR_SCH_ABLPO.SCHBLPOID')
        ->first();
