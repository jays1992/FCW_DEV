...
use App\Models\Master\TblMstFrm90;
...
class MstFrm90Controller extends Controller
{
    protected $form_id = 90;
    protected $vtid_ref   = 8;  //voucher type id
    // //validation messages
    protected   $messages = [
    ...
    public function index(){    
        $objRights = DB::table('TBL_MST_USERROLMAP')
                        ...
                        ->first();
        $objCount = DB::table('TBL_MST_UDFFORSE')
                        ->where('TBL_MST_UDFFORSE.CYID_REF','=',Auth::user()->CYID_REF)
                        ->where('TBL_MST_UDFFORSE.STATUS', '!=', 'C')
                        ->where('TBL_MST_UDFFORSE.PARENTID','=', 0)
                        ->select('TBL_MST_UDFFORSE.*')
                        ->count();
        return view('masters.Sales.UDFSE.mstfrm90',compact(['objRights','objCount']));        
    }

    public function add(){       
        return view('masters.Sales.UDFSE.mstfrm90add');       
   }
   ...
   public function attachment($id){
    ...
    if(!is_null($id))
    {
        $objSalesenquiry = TblMstFrm90::where('UDFSEID','=',$id)->first();
        ...
        $objMstVoucherType = DB::table("TBL_MST_VOUCHERTYPE")
            ...
            return view('masters.Sales.UDFSE.mstfrm90attachment',compact(['objSalesenquiry','objMstVoucherType','objAttachments']));
    }
   ...
   public function save(Request $request) {
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            ...
            {
                $req_data[$i] = [
                    'UDFSEID' => (isset($request['UDFSEID_'.$i]) ? $request['UDFSEID_'.$i] : "0") ,
                    'LABEL'    => $request['LABEL_'.$i],
            ...
            $wrapped_links["UDFFORSE"] = $req_data; 
            ...
            $sp_result = DB::select('EXEC SP_UDFFORSE_INUPDE ?,?,?,?,?,?,?,?,?,?,?',  $log_data);       
     ...
     public function getsalesenquiry(Request $request){
        ...
        $COL_APP_STATUS =   'STATUS';  //never change value, value must be 'APPROVED_STATUS' as per stored procedure;
            ...
            $BRID_REF      =   Session::get('BRID_REF');
            $FYID_REF      =   Session::get('FYID_REF');       
            $TABLE1        =   "TBL_MST_UDFFORSE";
            $PK_COL        =   "UDFSEID";
            $SELECT_COL    =   "UDFSEID,LABEL,VALUETYPE,DESCRIPTIONS,ISMANDATORY,DEACTIVATED,DODEACTIVATED";    
            ...
            if(!empty($request->input('search.value')))
            {
                ...
                if($filtercolumn =='ALL'){
                    ...
                    $WHERE_COL =  " WHERE PARENTID = 0 AND UDFSEID LIKE  ". $search_text;
            ...
            if(!empty($sp_listing_result))
            {
                ...
                {
                    $totalRows      = $salesenquiryitem->TotalRows;
                    $totalFiltered  = $salesenquiryitem->FilteredRows;

                    if (!Empty($salesenquiryitem->STATUS) && $salesenquiryitem->STATUS=="Approved") 
                    { $app_status = 1 ;} 
                    elseif($salesenquiryitem->STATUS=="Cancel")
                    { $app_status = 2 ;}
                    else{ $app_status = 0 ;}

                    if (!Empty($salesenquiryitem->ISMANDATORY) && $salesenquiryitem->ISMANDATORY=="1") 
                    { $ISMANDATORY = "Yes" ;} 
                    else{ $ISMANDATORY = "No" ;}
                    if (!Empty($salesenquiryitem->DEACTIVATED) && $salesenquiryitem->DEACTIVATED=="1") 
                    { $DEACTIVATED = "Yes" ;} 
                    else{ $DEACTIVATED = "No" ;}

                    $nestedData['NO']           = '<input type="checkbox" id="chkId'.$salesenquiryitem->UDFSEID.'"  value="'.$salesenquiryitem->UDFSEID.'" class="js-selectall1" data-rcdstatus="'.$app_status.'">';
                    $nestedData['LABEL']         = strtoupper($salesenquiryitem->LABEL);
                    $nestedData['VALUETYPE']     = $salesenquiryitem->VALUETYPE;
                    $nestedData['DESCRIPTIONS']      = $salesenquiryitem->DESCRIPTIONS;
                    $nestedData['ISMANDATORY']         = $ISMANDATORY;
                    $nestedData['DEACTIVATED']  = $DEACTIVATED;
                    $nestedData['DODEACTIVATED']      = $salesenquiryitem->DODEACTIVATED =="1900-01-01"?"":$salesenquiryitem->DODEACTIVATED;
                    $nestedData['STATUS']       = $salesenquiryitem->STATUS;
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            $objUdfForSE = DB::table('TBL_MST_UDFFORSE')                    
                             ->where('TBL_MST_UDFFORSE.UDFSEID','=',$id)
                             ->orwhere('TBL_MST_UDFFORSE.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFORSE.*')
                             ->orderBy('TBL_MST_UDFFORSE.UDFSEID','ASC')
                             ->get()->toArray();
            $objCount = count($objUdfForSE);
            if(strtoupper($objUdfForSE[0]->STATUS)!="N"){
            ...
            $objRights = DB::table('TBL_MST_USERROLMAP')
                             ...
                             ->select('TBL_MST_USERROLMAP.*', 'TBL_MST_ROLEDETAILS.*')
                             ->first();
           return view('masters.Sales.UDFSE.mstfrm90edit',compact(['objUdfForSE','objRights','objCount']));
        }
       ...
       public function view($id){
         ...
         if(!is_null($id))
         {
             // $objUdfForSE = TblMstFrm90::where('UDFSEID','=',$id)->first();
             $objUdfForSE = DB::table('TBL_MST_UDFFORSE')                    
                             ->where('TBL_MST_UDFFORSE.UDFSEID','=',$id)
                             ->orwhere('TBL_MST_UDFFORSE.PARENTID','=',$id)
                             ->select('TBL_MST_UDFFORSE.*')
                             ...
                             ->get()->toArray();
             $objCount = count($objUdfForSE);
             return view('masters.Sales.UDFSE.mstfrm90view',compact(['objUdfForSE','objCount']));
         }
   ...
   public function update(Request $request){
       ...
       $r_count = $request['Row_Count'];
        $data = [
            'UDFSEID' => (isset($request->UDFSEID_0) ? $request->UDFSEID_0 : "0"),
            'LABEL'    => strtoupper($request->LABEL_0),
            'VALUETYPE' => $request->VALUETYPE_0,
            'DESCRIPTIONS' => (isset($request->DESCRIPTIONS_0)? $request->DESCRIPTIONS_0 : ""),
        ...
        for ($i=0; $i<=$r_count; $i++)
        {
            if((isset($request['LABEL_'.$i])) && $request['LABEL_'.$i]!='')
            {
                $req_data[$i] = [
                    'UDFSEID' => (isset($request['UDFSEID_'.$i]) ? $request['UDFSEID_'.$i] : "0") ,
                    'LABEL'    => $request['LABEL_'.$i],
        ...
        $log_data = [ 
            ...
            if($sp_result[0]->RESULT=="SUCCESS"){
                ...
                return Response::json(['duplicate'=>true,'msg' => 'Duplicate record.','salesenquiry'=>'duplicate']);
   ...
   public function Approve(Request $request){
        ...
        if(!empty($sp_listing_result))
            ...
            for ($i=0; $i<=$r_count; $i++)
            {
                if((isset($request['UDFSEID_'.$i])) && (isset($request['LABEL_'.$i])))
                {
                    $req_data[$i] = [
                        'UDFSEID' => (isset($request['UDFSEID_'.$i]) ? $request['UDFSEID_'.$i] : "0") ,
                        'LABEL'    => $request['LABEL_'.$i],
   ...
   public function docuploads(Request $request){
    ...
    $destinationPath = storage_path()."/docs/company".$CYID_REF."/countrymst";
    ...
    if(empty($uploaded_data)){
        return redirect()->route("master",[90,"attachment",$ATTACH_DOCNO])->with("success","Already exists. No file uploaded");
    }
  ...
  //  }
    ...
    if($sp_result[0]->RESULT=="SUCCESS"){
        ...
        return redirect()->route("master",[90,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


    }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
   
        return redirect()->route("master",[90,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);

    }else{

        return redirect()->route("master",[90,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
    }
    ...
    public function checkLabel(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_UDFFORSE')
        ->where('TBL_MST_UDFFORSE.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_UDFFORSE.LABEL','=',$LABEL)
        ->select('TBL_MST_UDFFORSE.UDFSEID')
        ->first();
