...
use App\Models\Master\TblMstFrm173;
use DB;
use Response;
use Session;
use Auth;
...
class MstFrm173Controller extends Controller
{
   
    protected $form_id = 173;
    protected $vtid_ref   = 199;  //voucher type id

    //validation messages
    protected   $messages = [
                    'GCODE.required' => 'Required field',
    ...
    public function index(){
        ...
        return view('masters.Payroll.GenderMaster.mstfrm173',compact(['objRights','objDataList','FormId']));
    ...
    public function add(){ 
        ...
        return view('masters.Payroll.GenderMaster.mstfrm173add',compact(['objDD','objDOCNO']));
    }
   ...
   public function codeduplicate(Request $request){
        ...
        $objLabel = DB::table('TBL_MST_GENDER')
        ->where('GCODE','=',$GCODE)
        ->select('GCODE')
        ->first();
   ...
   public function save(Request $request){
        ...
        $GCODE       =   strtoupper(trim($request['GCODE']) );
        ...
        try {
            ...
            $sp_result = DB::select('EXEC SP_GENDER_IN ?,?,?, ?,?,?, ?,?,?, ?,?,?,?', $array_data);
    ...
    public function edit($id){
        ...
        if(!is_null($id))
        {
            ...
            $objResponse = TblMstFrm173::where('GID','=',$id)->first();
            ...
            return view('masters.Payroll.GenderMaster.mstfrm173edit',compact(['objResponse','user_approval_level','objRights']));
        }
    ...
    public function update(Request $request)
    {
      ...
      // dd($request->all());
        ...
        $sp_result = DB::select('EXEC SP_GENDER_UP ?,?, ?,?,?,?, ?,?,?,?, ?,?,?', $array_data);
        //dd($array_data);
    ...
    public function docuploads(Request $request){
        ...
        $destinationPath = storage_path()."/docs/company".$CYID_REF."/GenderMaster";
        ...
        if(empty($uploaded_data)){
            return redirect()->route("master",[173,"attachment",$ATTACH_DOCNO])->with("success","The file is already exist");
        }
     //  dd($uploaded_data);
        ...
        if($sp_result[0]->RESULT=="SUCCESS"){
            ...
            return redirect()->route("master",[173,"attachment",$ATTACH_DOCNO])->with("success","Files successfully attached. ".$duplicate_files.$invlid_files);


        }        elseif($sp_result[0]->RESULT=="Duplicate file for same records"){
       
            return redirect()->route("master",[173,"attachment",$ATTACH_DOCNO])->with("success","Duplicate file name. ".$invlid_files);
    
        }else{

            //return redirect()->route("master",[173,"attachment",$ATTACH_DOCNO])->with("error","There is some data error. Please try after sometime.  ");
            return redirect()->route("master",[173,"attachment",$ATTACH_DOCNO])->with($sp_result[0]->RESULT);
        }
    ...
    public function view($id){
        ...
        {
            $objResponse = TblMstFrm173::where('GID','=',$id)->first();
            return view('masters.Payroll.GenderMaster.mstfrm173view',compact(['objResponse']));
        }
    ...
    public function printdata(Request $request){
        ...
        $objResponse = TblMstFrm173::whereIn('GID',$ids_data)->get();
        
        return view('masters.Payroll.GenderMaster.mstfrm173print',compact(['objResponse']));
   }//print
    ...
    public function attachment($id){
        ...
        if(!is_null($id))
        {
            ...
            return view('masters.Payroll.GenderMaster.mstfrm173attachment',compact(['objResponse','objMstVoucherType','objAttachments']));
        }
